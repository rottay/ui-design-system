...
test("reassign updates an active coordinator without rewriting prior progress", () => {
  const workOrder = {
    id: "WO-CRA-23",
    status: "in-progress",
    claimedBy: "previous-dt",
    notes: "Previous model.",
    progressLog: [{ at: "2026-08-30 12:00", by: "previous-dt", note: "Existing evidence." }],
  };

  const previous = reassignInProgressWorkOrder(workOrder, {
    by: "kimi-k3-dt",
    notes: "Kimi K3 coordinates; independent auditors remain separate.",
    at: "2026-08-31 00:00",
  });

  assert.equal(previous, "previous-dt");
  assert.equal(workOrder.claimedBy, "kimi-k3-dt");
  assert.equal(workOrder.notes, "Kimi K3 coordinates; independent auditors remain separate.");
  assert.equal(workOrder.progressLog.length, 2);
  assert.equal(workOrder.progressLog[0].note, "Existing evidence.");
  assert.match(workOrder.progressLog[1].note, /reassigned from previous-dt to kimi-k3-dt/);
});

test("reassign refuses inactive work or missing current authority", () => {
  assert.throws(
    () => reassignInProgressWorkOrder({ id: "WO-X", status: "todo" }, { by: "kimi", notes: "Current." }),
    /reassign requires in-progress/,
  );
  assert.throws(
    () => reassignInProgressWorkOrder({ id: "WO-X", status: "in-progress" }, { notes: "Current." }),
    /requires --by/,
  );
  assert.throws(
    () => reassignInProgressWorkOrder({ id: "WO-X", status: "in-progress" }, { by: "kimi" }),
    /requires --notes/,
  );
});
...
test("decisions-lit indicator publishes the probe's own headline, never its own", () => {
  ...
  // invented here. `0` is what the probe measures since the conn2 lot
  // (28ab609b5) connected the last two underived new decisions (shape.nesting,
  // shape.control-height), taking the new-decision denominator 2 -> 0; the
  // earlier 8 -> 2 transition was the CC-01 connection lot plus WO-DER-03's
  // palette half.
  assert.match(indicator.headline, /^decisions lit = \d+\/22 \(\+\d+\/0 new\)$/);
  assert.match(
