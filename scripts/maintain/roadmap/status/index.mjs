...
// Ascending marker search, not a hardcoded `..` chain: this file is three
// levels below scripts/ (scripts/maintain/roadmap/status/index.mjs), so a fixed
// depth breaks the moment any level changes. Fail-closed, mirrors
// packages/core/scripts/libraries/repo-root/index.mjs's repoRoot() without
...
function generateStatus() {
  ...
  const dsProgramState = reg.traceability?.[DS_IMPROVEMENTS_KEY];
  if (dsProgramState?.phaseControls) {
    const phaseState = (phase) => dsProgramState.phaseControls[phase] || {};
    const openPhases = DS_IMPROVEMENTS_PHASES.filter((phase) => phaseState(phase).claimState === "open");
    const lockedPhases = DS_IMPROVEMENTS_PHASES.filter((phase) => phaseState(phase).claimState === "locked");
    const lockedPendingGo = lockedPhases.filter((phase) => phaseState(phase).ownerGo?.decision === "pending");
    const traceItems = Array.isArray(dsProgramState.items) ? dsProgramState.items : [];
    const deferredCount = traceItems.filter((item) => item?.disposition === "deferred").length;
    const openLabel = openPhases.length ? `Phase ${openPhases.join(", ")} open` : "no phase open";
    const lockedLabel = lockedPhases.length
      ? `Phases ${lockedPhases.join(", ")} locked${lockedPendingGo.length === lockedPhases.length ? ", ownerGo pending" : lockedPendingGo.length ? ` (ownerGo pending: ${lockedPendingGo.join(", ")})` : ""}`
      : "no phases locked";
    lines.push("");
...
export function reassignInProgressWorkOrder(workOrder, {
  by,
  notes,
  at = localDateTime(),
} = {}) {
  if (workOrder?.status !== "in-progress") {
    throw new Error(`${workOrder?.id || "work order"} is ${workOrder?.status || "unknown"}; reassign requires in-progress`);
  }
  if (!isNonEmptyString(by)) throw new Error("reassign requires --by <name>");
  if (!isNonEmptyString(notes)) throw new Error("reassign requires --notes <current operating model>");

  const previous = workOrder.claimedBy || "unclaimed";
  workOrder.claimedBy = by;
  workOrder.notes = notes;
  workOrder.progressLog = workOrder.progressLog || [];
  workOrder.progressLog.push({
    at,
    by,
    note: `Active coordinator reassigned from ${previous} to ${by}; the work-order summary now reflects the current operating model.`,
  });
  return previous;
}
...
switch (cmd) {
  ...
  case "progress": {
    ...
    saveRegistry(reg); generateStatus();
    console.log(`${w.id} progress logged (entry ${w.progressLog.length}). Successors resume via: node scripts/maintain/roadmap/status/index.mjs show ${w.id}`);
    break;
  }
  case "reassign": {
    const reg = loadRegistry();
    ...
    const w = requireWo(reg, arg);
    try {
      const previous = reassignInProgressWorkOrder(w, {
        by: flag(args, "--by"),
        notes: flag(args, "--notes"),
      });
      saveRegistry(reg); generateStatus();
      console.log(`${w.id} reassigned from ${previous} to ${w.claimedBy}.`);
    } catch (error) {
