#!/usr/bin/env node
// Copied from ui-design-system scripts/roadmap-status.mjs (2026-07-07). Do not diverge without a WO; record divergences below this line.
// Orchestrator for the isolated commercial-surfaces program in roadmap/commercial/, extracted from the main
// ui-design-system roadmap by owner decision 2026-07-07. Byte-identical to scripts/roadmap-status.mjs (whose
// own lineage is a copy of app-bithire@c12901962) EXCEPT for exactly TWO functional divergences, recorded per
// the provenance convention:
// - ROADMAP: resolves to `roadmap/commercial` — the const near the top of this file.
// - LANES: ['showroom'] (not ['engine-modern','craft','gates','tokens','architecture','showroom']) — the single commercial (showroom) lane.
// The `progress` command + progressLog and all other orchestration logic are inherited unchanged.
/**
...
// levels below scripts/ (scripts/maintain/roadmap/commercial-status/index.mjs), so a
// fixed depth breaks the moment any level changes. Fail-closed, mirrors
...
switch (cmd) {
  ...
  default:
    console.log("Usage: node scripts/maintain/roadmap/commercial-status/index.mjs <status|check|next|show WO-ID|delegate WO-ID|claim WO-ID [--by name]|progress WO-ID --note \"...\" [--by name]|done WO-ID --evidence \"...\"|reopen WO-ID [--note ...]>");
