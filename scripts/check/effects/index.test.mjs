...
import { CI_GATES } from '../../../packages/core/scripts/check/automation/gates/manifest/index.mjs';

// Ascending marker search, not a hardcoded `..` chain: this file is two
// levels below scripts/ (scripts/check/effects/index.test.mjs).
...
const canonicalRoot = resolve(repoRoot, 'packages/core/governance/effects');
...
function mutateLedger(root, mutate) {
  const path = resolve(root, 'sources/index.json');
...
test('provenance fails closed on revision or license-byte drift', () => {
  ...
  withFixture((root) => {
    const license = resolve(root, 'licenses/magicui/index.md');
...
test('provenance rejects ungoverned archived licenses', () => {
  withFixture((root) => {
    mkdirSync(resolve(root, 'licenses/unknown'), { recursive: true });
    writeFileSync(resolve(root, 'licenses/unknown/index.md'), 'unknown\n');
    assert.throws(
      () => auditEffectProvenance(root),
...
test('the first-party archived license is byte-exact', () => {
  withFixture((root) => {
    const license = resolve(root, 'licenses/rottay-ui-design-system/index.txt');
    writeFileSync(license, `${readFileSync(license, 'utf8')}tampered\n`);
    assert.throws(
      () => auditEffectProvenance(root),
...
test('effect provenance remains a first-class local and CI release gate', () => {
  ...
  assert.equal(coreManifest.scripts['gates:ci'], 'node scripts/check/automation/runner/index.mjs');
  assertBlockingGate(
    CI_GATES,
    'effects:provenance',
  ...
  assert.match(coreManifest.scripts['test:scripts'], /check\/effects\/index\.test\.mjs/);
  assert.ok(workflow.includes('scripts/check/effects/index(\\.test)?\\.mjs$'));
