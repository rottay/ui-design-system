...
} from '../../../packages/core/src/entrypoints/suppliers/cli/index.mjs';
...
test('app manifest scanning accepts computed data access and still collects literal runtime suppliers', () => {
  ...
  try {
    mkdirSync(resolve(fixtureRoot, 'src'), { recursive: true });
    mkdirSync(resolve(fixtureRoot, 'scripts/src/entrypoints/suppliers/cli'), { recursive: true });
    writeFileSync(
    ...
    writeFileSync(
      resolve(fixtureRoot, 'scripts/src/entrypoints/suppliers/cli/index.mjs'),
      readFileSync(resolve(coreRoot, 'src/entrypoints/suppliers/cli/index.mjs'), 'utf8'),
    );
...
test('runtime export inventory includes the public CLI import and every CJS condition', () => {
  ...
  assert.ok(conditions.some((entry) => (
    entry.subpath === './supplier-honesty-cli' && entry.mode === 'import' && entry.target.endsWith('.js')
  )));
  const fixtures = runtimeExportFixtures(manifest);
  assert.equal(fixtures.import.length, conditions.filter((entry) => entry.mode === 'import').length);
  assert.equal(fixtures.require.length, conditions.filter((entry) => entry.mode === 'require').length);
  ...
  for (const assetSubpath of [
    '/icons/full',
    '/icons/presets/bithire',
    '/marks/brand',
    '/marks/cloud',
  ]) {
    assert.ok(fixtures.import.some((entry) => entry.specifier.endsWith(assetSubpath)), assetSubpath);
    assert.ok(fixtures.require.some((entry) => entry.specifier.endsWith(assetSubpath)), assetSubpath);
  }
