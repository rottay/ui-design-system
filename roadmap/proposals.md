...
- **Ask** — give `build-vertical-css.mjs` the same `--check` mode `build-vertical-artifacts.mjs` has, and chain it into `lint` and `pretest`. Ship it with a drill: hand-edit one byte of `artifacts/generated/css/verticals/rottay/index.css` and watch `pnpm lint` reject it.
- **Status** — OPEN.
...
- **`personality.css` IS EQUALLY EXPOSED, and one of its dead rules was mistaken for a live one** — the accent-bar rule (`.alert, .ant-alert, .rottay-alert, [data-engine] .ds-alert { border-left-width: var(--ds-personality-accent-bar-thickness, 4px) }`) sits in `rottay-personality`, which loses to preflight exactly as `rottay-engines` does. It does not render. Measured on the shipped modern Alert: the border is a **uniform 1px on all four sides**, painted by DaisyUI's own `.alert { border-width: var(--border) }` from a layer above preflight; `--ds-personality-accent-bar-thickness` resolves to 2px and reaches nothing. WO-SKIN-03 documented a "hard runtime constraint" claiming a border longhand would let that accent bar re-materialize. **That mechanism is false** — the bar cannot re-materialize. The shipped pixels are correct (the batch was certified byte-exact), and the practical advice (use the `border` shorthand) still holds for a different reason: a longhand setting only `border-color` leaves DaisyUI's `var(--border)` in charge of the WIDTH. The doc has been corrected. The lesson generalizes: a suppression law inferred from reading the CSS, without measuring the live cascade, can be right about the outcome and wrong about the cause — and the wrong cause is what gets copied into the next contract.
- **Unconditionally dead by layer math alone** (nothing inline needed to contest them): `padding` + `margin` declarations — modern 45, rustic 33, classic 66 = **144**.
...
### P-88 — `<Input>`/`<Button>` own `data-part`; Input's caller-style landing differs by engine
- **Finding (CK-C, source- and DOM-probed).** Caller `data-part` is replaced by the primitive's own
  anatomy (`root` for Input, `trigger` for Button). Caller `className` is not lost: Button merges it
  onto the painted element in both engines; Input co-locates `className` and `style` on an outer
  wrapper in modern and on the painted shell in rustic.
- **Consequence.** A consumer selector keyed to its requested Input/Button `data-part` never matches.
  A single fallback descendant selector is also unsafe for Input: `.rottay-input` targets the native
  shell in modern, while the caller paint it replaces originally lived on the wrapper. That silently
  transposes paint between elements even though the counter reaches zero.
- **CK-C workaround (byte-exact):** pass a private BEM class through `className`; target the modern
  wrapper guarded by `:has(> .rottay-input--modern[data-part='root'])`, and separately target the
  rustic BEM shell at specificity above every Input state. Contract tests pin both shapes. Do not add
  a wrapper or use a broad descendant part selector.
- **The real fix (DS architecture):** formalize primitive ownership and composition separately:
  primitives retain their own `data-part`, while consumers receive a documented owner/slot-class API
  whose landing node is stable across engines. Audit every primitive for `className`/`style` landing
  parity before promising it as a public skin hook.
- **Status** — OPEN (architecture); CK-C's workaround is implemented and regression-tested.
