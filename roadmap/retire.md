...
- **Owner amendment (2026-09-20, Fable-reviewed)** — The approved reusable-style packets in `proposals.md` do not authorize a parallel tree rewrite. If WO-DER-09 introduces the bounded `foundation/presets/styles/` data exception, include it with the vertical preset data in this WO's single final move; agree the final data owner/dependency direction and preserve the one-authority/source-truth gates. New style contracts use the target `contracts/` grammar. Do not duplicate authored documents and family fragments, widen identity baselines, or leave permanent internal forwarding wrappers. Coordinate the style registry/generator reservation; until the three additions are registered, they are not executable WOs.
