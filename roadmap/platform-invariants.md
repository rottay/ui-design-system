...
- **POST-CLOSE DEFECT (RUNTIME-02, audit/103-day-direction-2026-09-12/runtime.md, 2026-09-12)** — `token-eased-loop` reads the computed easing once when starting WAAPI and re-runs only on ref/enabled/duration/token-name/direction/keyframes; `GradientBackground`/`GlowEffect` pass constant inputs, so switching `motion.character` (organic→playful) with constant duration leaves mounted ambient effects on the previous curve while CSS consumers update (strong hook-dependency inference; focal test owed). **Obligation:** a CSS-owned animation consuming the variable directly, or timing updated from the shared artifact/theme change signal after CSS commit; no second character map, no DOM polling. **Focal acceptance:** a rerender changing ONLY the computed easing (duration/keyframes/token name constant) asserts timing changes or old animation cancelled+replaced; the reduced-motion cancellation assertion stays. Status kept `done` (reopen would cascade per DAG validation); the correction lot routes here.
...
- **AMENDMENT (R4, 2026-09-08) — effective closure scope, and the acceptance-cycle it breaks** — The acceptance gate above required the `adapt-slot` gate green for seven families "once their cuts land", while every one of those cuts depends on this WO being done. The registry carries only the forward edge, so its cycle detector cannot see the back-edge; the cycle is real and is in the prose. **Retained verbatim** as this WO's effective closure scope, unchanged: "SSR test: phone request renders the cards presentation with the three declared columns; a container 320 px wide flips `data-posture=\"compact\"` without a viewport change; posture names identical across families (`grep` census = 1 vocabulary)" — plus the reference implementation on `PatternDataTable` and the `adapt-slot` gate existing with its families declared. **Removed** from this WO's effective scope: the "once their cuts land" clause, which is not this WO's work. It is rerouted, not deleted: per-cut, into each family cut's own acceptance ("a layout-sensitive family that does not expose `adapt` and stamp `data-posture` is not done"); fleet-wide, into `WO-EVI-02`'s per-family `adapt` adoption evidence. **Dependency (owner-approved 2026-09-10)**: the registry edge `WO-INV-07 → WO-INV-04` is approved and recorded; this WO consumes `WO-INV-04`'s named `@container` contract, and the container half must not start before `WO-INV-04`'s named containers exist. The edge is a full dependency of this WO: no half of it is enabled early.
- **POST-CLOSE DEFECT (RUNTIME-03, audit/103-day-direction-2026-09-12/runtime.md, 2026-09-12)** — `data-table` `presentation="list"` rows render as `Box role="listitem"` with `onClick` but no focusable control, `tabIndex` or keyboard handler for `onRowClick`; plain-text columns with only `onRowClick` become pointer-only when an `adapt` slot chooses `list` (strong code/semantics inference; focal test owed). **Obligation:** keep list semantics and provide a focusable, semantically appropriate row-opening control with Enter/Space activation and nested action/selection isolation, using the existing interaction contract; coordinate with the data-table family cut. **Focal acceptance:** list presentation with plain text columns, `onRowClick`, no `actions`; tab reaches the row-opening control; keyboard activation fires exactly one row-open callback; nested selection/action controls never open the row. Status kept `done` (reopen would cascade per DAG validation); the correction lot routes here.
