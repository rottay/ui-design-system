...
- **Steps** —
  1. Grayscale ramp tokens (spec section 2): define the commercial ramp ONCE — ~12 perceptually-even steps
     ...
     variables in a dedicated `packages/core/src/components/patterns/commercial/foundation/tokens/commercial-ramp.css`, and mirror atomic
     references in a TS token file (`packages/core/src/components/patterns/commercial/foundation/tokens/index.ts`) so styled/TS consumers use
  ...
  2. Build the kit components under `packages/core/src/components/patterns/commercial/` (one folder per component with an
...
- **Files** — `packages/core/src/components/patterns/commercial/**` (component folders + `tokens/`), `packages/core/src/commercial.ts`
...
- **Acceptance gate** — `pnpm --filter @rottay/design-system run build` emits `dist/commercial.js`,
  ...
  `packages/core/src/components/patterns/commercial/**` proves ZERO non-grayscale color literals except inside `ProductWindow`
...
- **Delegation prompt** — In `/Users/daniel/Developer/Rottay/ui-design-system`, build the shared commercial
  ...
  first). Under `packages/core/src/components/patterns/commercial/` build monochrome-only, domain-agnostic components (one folder
...
- **Why** — Audit top offenders #1/#2/#4 and section 3.2 (the largest gap). "The single most damaging fact
  ...
  Verified: `packages/showroom/src/components/layout/navigation/config/index.ts` carries nine accent hex literals — the eight
  ...
  (`packages/showroom/src/components/runtime/fingerprint/index.tsx`) is printed up to four times per page
...
- **Steps** —
  1. Kill the accents: in `packages/showroom/src/components/layout/navigation/config/index.ts`, remove the per-section accent
  ...
  4. Dedupe the runtime readout to ONE placement: pick a single deliberate home (a compact header slot or an
     ...
     `packages/showroom/src/components/runtime/fingerprint/index.tsx` (sidebar launchpad, sidebar footer,
     live-preview card).
...
- **Files** — `packages/showroom/src/components/layout/navigation/config/index.ts`,
  ...
  `packages/showroom/src/components/runtime/fingerprint/index.tsx` (single-home + remove redundant callers),
...
- **Delegation prompt** — In `/Users/daniel/Developer/Rottay/ui-design-system`, carry the landing's monochrome
  ...
  `packages/showroom/src/components/layout/navigation/config/index.ts` delete the eight section accent hex (config.ts:26-74) +
  ...
  readout (`packages/showroom/src/components/runtime/fingerprint/index.tsx`) to ONE placement; delete the other three callers.
