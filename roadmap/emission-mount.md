...
- **AMENDMENT (checkpoint 99, 2026-09-11; obligations preserved, not deleted)** — Close scope is split by owner. EMI-02 closes locally: F-19 (mount law + scope proof), F-20 (first paint — the SSR responsive snapshot; this is the first-paint finding, not F-12), F-112 (recipe profile in the artifact with a runtime reader), F-89, and its own F-12 arm (`recipeProfile` travels in the artifact only, D-26). **F-46's final closure routes to WO-INV-04 verbatim** — INV-04 already lists F-46 in its own `Closes` and depends on EMI-02, so EMI-02's done evidence must show the mount-side inputs (SSR hint, snapshot via `useSyncExternalStore`, root attributes before first paint) and must NOT claim the fleet responsive close. The remaining F-12 arms (adapters declaring `invariant`, the accent-bar `null`, per-family personality reach) stay with the derivation/family lanes that own them.
- **POST-CLOSE DEFECT (RUNTIME-01, audit/103-day-direction-2026-09-12/runtime.md, 2026-09-12)** — The build runs `tsc && vite build` BEFORE `build:vertical-css`; the generator consumes `dist` and writes the runtime metadata module under `src` after Vite already bundled the previous module, so one build after a profile change can ship stale JS with new CSS (strong build-order inference; focal reproduction owed by the correction lot). **Obligation:** generation completes before the final distributable JS bundle and the stamp; a bootstrap compiler build, if needed, is a distinct stage; no second authored-theme authority. **Focal acceptance:** in the lot worktree, change one valid profile selection, run the supported build ONCE, verify the public package mounts the same profile/runtime/CSS, and that a second run produces no further changes. Resolve before widening DER-06 profiles. Status kept `done` (a reopen would cascade over done dependents per DAG validation); the close evidence stands; the correction lot routes here.
