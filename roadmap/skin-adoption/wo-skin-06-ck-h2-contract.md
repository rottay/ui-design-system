...
## MIGRATION CERTIFICATION (2026-07-14)

CK-H2 is certified byte-exact at `c229859c`.

- Nine unlayered skins are wired append-only in both canonical entrypoints. The five tracked vertical
  bundles were regenerated by the production core build.
- Counters moved from `35/29/30/28/22/31/11/10/20` to
  `3/0/0/0/0/0/0/0/0`: **213 of 216** counted sites moved. The only residuals are file-manager
  modern's three deliberate caller-precedence defaults (`background`, `borderRadius`, `boxShadow`).
- `skins.parseErrors=0`, `skins.unwired=0`, `skins.exemptionsBreached=0`, and
  `skins.deadParts=0`; the full engine audit passes without changing a baseline or floor.
- The H2 anatomy contract is 25/25 and the five focused component/contract files are 115/115. Core
  typecheck, core production build, showroom production build, and `git diff --check` are green.
- All 30 committed screenshots pass through seven Playwright cases against the pre-step production
  baselines, then pass a second stability run without updating snapshots (`7/7 + 7/7`).
- P-78 remained intact: `panelCardStyle`, `pillBadgeSmStyle`, `spinnerStyle()` and the shared modern
  style module were not migrated. Static SVG presentation attributes and the two existing spinner
  keyframe references remain on their original channels. TokenInspector's literal hex palette was
  transcribed verbatim rather than tokenized.
- Trap 2 was measured, not guessed. On the pre-migration production build, modern `tr.active` did not
  distinguish selected from unselected rows for either rottay or bithire; `a.link` was un-underlined
  at rest and underlined on hover for rottay, and underlined in both states for bithire. The migration
  retains `active` and `link link-hover`, adds no invented row/link paint, and the post-migration pixel
  matrix proves those computed outcomes remain visually unchanged.
