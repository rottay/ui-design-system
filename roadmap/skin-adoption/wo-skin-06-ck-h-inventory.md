...
against `node scripts/check/engine-token-audit.mjs | grep fleet.inlinePaint.runtime/patterns/misc`
...
`scripts/libraries/inline-paint-counter.mjs`) before classification, not estimated.
...
(the exact `ARC09_PAINT_KEY_RE` from `scripts/libraries/inline-paint-counter.mjs`),
