...
`node scripts/check/engine-token-audit.mjs` emits `fleet.inlinePaint.<path>: N` per file. A
"site" is:
...
1. `cd packages/core && node scripts/check/engine-token-audit.mjs | grep -E "<your file paths>"`
