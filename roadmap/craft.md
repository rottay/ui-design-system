...
- **Execution** — Calibrate one active control end-to-end; then close each family once against the stabilized applicability matrix; then reconcile the frozen tree. R7 remains documented as disabled future work and may not execute without new owner authorization.
- **Acceptance** — Source-bound static/DB parity, intended computed deltas, negative controls, exact restore, applicable resilience stress and final sighted acceptance by the DT (Kimi K3) with the Fable 5 independent closure audit. Unknown applicability blocks closure.
