...
### WO-CRA-23 Modern Rescue — canonical family customization closure
- **Source IDs / phase** — Owner-authorized standalone `modern-rescue` programme; R0–R6 only.
- **Depends on** — WO-GAT-05, WO-GAT-06.
- **Outcome** — Close the active canonical Modern family inventory against the active operational public controls (both counts are derived in the programme contracts, never pinned here) so a small coherent product surface produces broad, causal, static/DB-equivalent and exactly restorable visual change across primitives, patterns, structures and surfaces.
- **Single human authority** — Start at `packages/core/scripts/quality-evidence/programs/modern-rescue/README.md`. It owns the laws, resume algorithm and execution order. Its segmented `manifest/index.json` owns current control/family assessment state.
- **Machine status** — `roadmap/registry.json` indexes this work order; it does not copy the execution plan. Historical evidence under `packages/core/test-artifacts/quality-evidence/wo-cra-23/` is immutable provenance, never a competing roadmap.
- **Scope** — The active canonical family inventory and the operational control set recorded in the programme contracts. Static reach, file visitation and old ledger states are prioritization only. Owner amendment `ROADMAP-OWNER-AMENDMENT-2026-08-28` (`docs/ROADMAP-EJECUCION-2026-08-19.md` §12, items 21-34): the DS and its reference lab are the build and visual-certification target; BitHire enters only as final consumer-readiness (install/exports/build/typecheck/smoke — no lots on its current UI); app-platform and app-evnto are DEFERRED with guarantees intact; dark mode gets non-regression only; no-push is owner policy; the cohort release gate is not programme closure.
- **Execution** — After representative control calibration, execute conflict-free cross-layer product slices in parallel: data/workflow, shell/navigation and forms/surfaces, with one singleton compiler/theme/recipe integrator. Opus writes product source, Sonnet scouts and performs exact mechanical work, Kimi K3 integrates and commits locally, Fable audits each lot and Codex audits each slice/shared-authority close. The detailed plan lives only in the programme README and machine contracts. R7 remains disabled.
- **Acceptance** — A visible complete journey; source-bound static/DB parity; intended computed deltas and fixed negative controls; exact restore; applicable responsive, RTL, accessibility and motion stress; every brandable hardcode in the adopted write-set retired or adjudicated; final sighted acceptance by Kimi K3; Fable acceptance per integrated lot and Codex acceptance per product-slice or compiler/manifest/public-control/architecture close. Unknown applicability blocks the affected family closure, not unrelated source work.
- **Fences** — Never push. Local commits follow audited packets. Classic/Rustic are read-only. No parallel token/compiler/tenant system, tenant branches, family-minted public `--ds-*`, baseline weakening or destructive Git.
- **Size** — XL, multi-cohort.
