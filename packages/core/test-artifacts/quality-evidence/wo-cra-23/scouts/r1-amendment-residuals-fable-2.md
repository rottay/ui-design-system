...
- **Perímetro verificado del diff:** exactamente 7 archivos, 15 inserciones /
  ...
  pasada previa son `quality-rubric.json` (−1 línea) y
  `visual-craft-contract.json` (−1 línea) — D1; `rounds.json` pasó de 1 a 3
  líneas tocadas y `tenant-art-direction.json` de 2 a 5 — D2. Nada más entró
  al write-set.
...
El diff remueve exactamente una línea de cada lista: `visual-craft-contract.json`
(hunk sobre la vieja `:348`) y `quality-rubric.json` (hunk sobre la vieja
...
vetoes" (`rounds.json:120`) ya no es satisfacible únicamente ejecutando la
...
`tenant-art-direction.json:116` ("recognizable current destination" — sentido
distinto: wayfinding de navegación, dentro de la documentación histórica
diferida), `rounds.json:422` (la nota de retiro misma) y
`visual-craft-contract.json:425` (observación E.1 portada, §E).

## C. Guardia de regresión: ACCEPT previo + seis restos intactos ✓

Contra disco, punto por punto:

- **Los seis restos:** `program.json:100` (invariante de fixtures con la coda
  de retiro) ✓; `tenant-art-direction.json:11` (purpose "Historical record…
  binds nothing in the current stage") y `:30` (ley RETIRED, fixtures como
  controlled baselines) ✓; `stageStatus` `:4-9` intacto ✓; `rounds.json:108`
...
- **Mi ACCEPT de la enmienda:** `rounds.json:103` (fixtures de prueba con
  ...
  `:469` — presentes; campos machine-checked de `tenant-art-direction.json`
...
1. **`r1-craft-prep.md:145` — corregir "de los 25" → "de los 24".** La línea
   dice "Cero vetos duros aplicables de los 25 (`visual-craft-contract.json`
...
1. **`visual-craft-contract.json:425`** (antes `:426`) — checkpoint artifact
...
2. **`customization-model.json:155`** — `grayscaleAndNeutralizedPrimaryRequired`
...
python3 -c "import json; vc=json.load(open('$P/visual-craft-contract.json')); \
  qr=json.load(open('$P/quality-rubric.json')); c=vc['hardVisualVetoes']; r=qr['hardVetoes']; \
...
R=packages/core/test-artifacts/quality-evidence/wo-cra-23/F4B
...
# D2
python3 -c "import json; dc=json.load(open('$P/tenant-art-direction.json'))['divergenceContract']; \
  print(sorted(dc.keys())); print('threeSecondGrayscaleRecognitionRequired' in dc)"   # ... False
grep -n "observable fixture divergence under controlled\|fixture divergence matrix" $P/rounds.json  # 368, 422
grep -rln -i -e "three.second" -e "three seconds" -e "recognizable by tenant" \
  --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=test-artifacts .
# => solo probe/ds-reference/{page.tsx,chrome/index.tsx,ground/stamp.ts} (comentarios)

# Staging D.1
grep -n "de los 25" packages/core/test-artifacts/quality-evidence/wo-cra-23/scouts/r1-craft-prep.md  # 145
```
