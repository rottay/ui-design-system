...
> aceptación de R1 (ver `rounds.json` R1 + `tenant-art-direction.json.stageStatus`).
...
- **Marco:** el owner declaró AESTHETIC NOT_ACCEPTED sobre F4C (`F4C/README.md:461-463`) y ordenó el lote R1 CRAFT CALIBRATION: tres unidades completas punta a punta. La secuencia ya estaba escrita en la ley del round: `rounds.json` R1 `objectives` — *"attack Button then PatternDataTable then AppShell as the first-failing grammar sequence before the remaining canaries"*.

---

## 1. Identidad canónica

Ninguna de las tres unidades mapea 1:1 a una sola familia canónica; cada una es un **canary de R1** (`rounds.json` R1 `scope.families`, 12 canaries) que cubre un conjunto acotado de familias del inventario activo (255 filas, `family-inventory.json`). Estado de adjudicación global hoy: `adjudication.unreviewed = 255` (README del programa, tabla derivada, líneas 290-294) — **ninguna familia tiene review**; todos los manifests tienen `review.verdict: null` y las 21 celdas `themeControls` en `UNKNOWN`.
...
Ley aplicable: "A family is DONE" (`README.md:192-205`), fórmula de elegibilidad (`quality-rubric.json` `eligibility.formula`: binaryContractsPass && hardVetoCount===0 && craftScore>=layerThreshold && criticalDimensionsMin>=4 && resilience===1 && canonClosure===1 && evidenceFresh && DT sighted), ley de checkpoint (`visual-craft-contract.json` `checkpointPolicy`: 3 familias de calibración, DT GO explícito antes de propagar) y piso de estrés (`quality-rubric.json` `stressMatrix`). `calibrationFamilyCount: 3` — **estas tres unidades SON la cohorte de calibración**: nadie propaga nada hasta su DT GO.

### Checklist común a las tres unidades

- [ ] Identidad y ownership resueltos a family id(s) canónicos (§1) y write-set declarado por lane (`rounds.json` R1 `writeDomains`: family-source / reference-lab / observable-tests / i18n-catalogs / round-evidence).
...
- **Constitución íntegra:** `node packages/core/scripts/quality-evidence/programs/modern-rescue/program-check.mjs` y `program-check.test.mjs` (hoy rojo solo por receipts F4B stale pre-existentes — `checkpoint.intent.json:4`; no introducir nuevos rojos).
...
- **Receipts de checkpoint R1** (`evidence-contract.json` `checkpointRequiredFields`): `checkpoint.json`, `visual-craft.json`, pares before/after por familia representativa en ambos tenants, una captura mobile/estrecha por familia, receipts de CSS ownership y causalidad static-DB, reporte de vetos/pisos, y el **DT GO o lista de corrección** — layout en `R1/` con `manifest.json` + `SHA256SUMS` (precedente congelado: `R1/manifest.json`).
...
4. **Dirección de tenant same-tree:** BitHire = Professional Network Hiring OS, The Management = Monochrome Executive Ledger (`rounds.json` R1 `objectives`; `tenant-art-direction.json` — "The current BitHire and The Management palettes are migration inputs, not accepted target palettes", y veto `current-tenant-palette-retained-without-r1-sighted-readjudication`). El R1 exige divergencia observable en ≥8 ejes (≥6 no-color) — eso se diseña, no se deriva.
...
6. **Bloqueos asentados que NO se reabren en el lote:** `control.size ↔ density.mode` (T-1), `--ds-input-md-line-height` de bithire (owner, recomendación DT preservar 1.5385) — `checkpoint.intent.json:4`.
...
**Button → PatternDataTable → AppShell**, que es además el orden ya legislado (`rounds.json` R1 `objectives`: "attack Button then PatternDataTable then AppShell as the first-failing grammar sequence"). Razones medidas:
...
python3 - <<'EOF'
import json
inv = json.load(open('packages/core/scripts/quality-evidence/programs/modern-rescue/family-inventory.json'))
