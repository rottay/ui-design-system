...
automatically illegal. Sources read in full: modern-rescue `README.md`, `program.json`,
`customization-model.json`, `manifest/schema.json`, `manifest/rules.mjs`, `evidence-contract.json`,
`quality-rubric.json`, `tenant-art-direction.json`, `visual-craft-contract.json`,
...
Operational (capability registry is the only authority; `customization-model.json#standard.current`,
...
unbounded in count but bounded in ownership. Evidence: `program.json#currentImpactTruth` counts
...
- `--_ds-*` — private/provisional. Legal landing zone for evaluation: component-private values,
  ...
  per `quality-rubric.json#cssOwnershipContract.retirementLaw` (attribution, consumer census,
