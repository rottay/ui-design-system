...
- **Objeto:** el diff sin commitear de la enmienda de dirección R1 (4 archivos:
  `rounds/index.json`, `art-direction/index.json`, `README.md` del programa,
...
- `rounds/index.json:103` — objetivo R1 reemplazado: *"use the BitHire and The
...
- `art-direction/index.json:4-8` — campo nuevo `stageStatus` con
...
- `rounds/index.json:119` — el exit viejo *"art-direction contract passes all
  ...
  acceptance)"*.
- `rounds/index.json:253` — el `writeDomains.why` de los tenant sources ya no dice
  *"R1 must implement Professional Network Hiring OS… and Monochrome
  Executive Ledger…"*.
- `art-direction/index.json:7` — *"the R1 acceptance obligation to implement
  them is retired from rounds/index.json"*.
- `README.md:459-460` — *"The R1 acceptance obligation to implement the
  recorded directions is retired"*, con puntero cruzado a `rounds/index.json` R1 y
  `art-direction/index.json.stageStatus`.
- **Verificación negativa** (barrido repo-wide, `--exclude-dir=node_modules`):
  ...
  `art-direction/index.json:7,37,164` — documentación conservada por orden
...
- `rounds/index.json:103` — *"by static BrandTheme and by DB TenantTheme with
  parity between both paths"*.
- `rounds/index.json:107` — el objetivo pre-existente de divergencia same-tree
  (≥8 ejes, ≥6 no-color) se CONSERVA sin tocar, coherente con "diferencias
  reales, previsibles y consistentes".
- `rounds/index.json:119` — exit: *"the vertical fixtures prove the public-control
  cascade across all required scenes at 320 390 768 and 1440, by static
  BrandTheme and by DB TenantTheme with parity"*.
- `art-direction/index.json:8` (última oración del `ownerOrder`) — *"the two
...
- `rounds/index.json:253` — *"the cascade proof needs write access for bounded
...
- `rounds/index.json:105` — objetivo nuevo: *"every cohort changes real source —
...
- El orden ya estaba legislado antes de la enmienda y no fue tocado:
  `rounds/index.json:110` — *"attack Button then PatternDataTable then AppShell as
...
- Hardcodes del write-set: `rounds/index.json:104` — *"each cohort retires or
  adjudicates every brandable hardcode in its write-set, with monotonic
  census reduction per lot"*; exit en `rounds/index.json:121` — *"the brandable
...
- Cascada observable + paridad + tests: `rounds/index.json:106` — *"every cohort
  ...
  tests"*.
- No lotes solo-documentación: `rounds/index.json:105` y `README.md:461-463`.
- Calidad visual/composición/estados/responsive: pre-existente y conservada —
  objetivos `rounds/index.json:100-102` (defecto dominante por canary; no overlap /
  clipping / per-character wrap / compresión desktop; 320/390/768/1440 y
  contenedores estrechos) y exit `rounds/index.json:115-118` (contratos binarios,
...
- `rounds/index.json:104` — *"PatternDataTable closes or adjudicates its 52 class-B
...
- `rounds/index.json:104` — *"drain the brandable hardcode census as an
...
- `README.md:464-468` — ley global del programa (sección Fences, vinculante
  ...
  lot."*
- **Nota:** rounds/index.json asienta el drenaje sólo dentro de R1; los bloques
...
- `rounds/index.json:105` — *"at least 80% of the effort goes to source and visible
...
- Los asientos están en `orchestration/index.json` desde antes de la
...
- **117 fallos exactos**, los 117 con el mismo patrón *"source digest is
  ...
  `artifacts/quality/programs/modern-rescue/cascade-proofs/controls/` (conteo por
...
- **Pre-existencia en HEAD probada sin mutación git:** los fallos stale son
  ...
  previo del programa (`checkpoint/index.json:4`, citado en
...
- `git status --short`: exactamente los 3 modificados del write-set
  (`README.md`, `rounds/index.json`, `art-direction/index.json` del programa) + 3
...
- La cabecera además importa las reglas nuevas del lote (80/20, no
  ...
  byte-a-byte en sustancia con `rounds/index.json:104-106,121` y
  `README.md:461-472`.
...
2. **El drenaje del censo no está re-asentado en los bloques R2-R6 de
   rounds/index.json**; rige por la ley global del README (§A.8). Asiento por-ronda
...
grep -rl 'rounds\.json\|tenant-art-direction\|modern-rescue/README' \
  packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls \
