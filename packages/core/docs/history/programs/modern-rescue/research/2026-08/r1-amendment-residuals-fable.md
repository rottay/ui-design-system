...
- **Objeto:** el diff sin commitear de la remediación de los SEIS restos que el
  owner mandó cerrar (5 archivos, 11 líneas: `program/index.json`, `rounds/index.json`,
  `art-direction/index.json`, `program-check.mjs`, `r1-craft-prep.md` §6.4) +
...
- **D2** — la prueba de reconocimiento retirada del objetivo R1 sigue REQUERIDA
  ...
  + dos acceptanceQuestions de identidad en `art-direction/index.json` (campo
  que el propio resto 2 ordenó congelar), y verbatim en los objetivos R4 y R6
  de `rounds/index.json`.
...
### A.1 — `program/index.json`: invariante reemplazada ✓

- `program/index.json:100` — la invariante vieja ("BitHire and The Management comply
...
- **Ninguna otra invariante re-impone la dirección diferida:** las 25
  ...
  (`program/index.json:12`) es el puntero de registro `artDirectionContract` al
  archivo — un path, no una imposición.

### A.2 — `art-direction/index.json`: diferido en su prosa; campos congelados intactos ✓ (pero ver D2)

- `art-direction/index.json:11` — `authority.purpose` ya no dice "Binding
...
- `art-direction/index.json:30` — la ley "R1 replaces and sightedly
...
- `art-direction/index.json:4-9` — `stageStatus` (`DEFERRED_AS_R1_TARGETS`,
...
- **PERO** el congelamiento de `divergenceContract` preserva dentro del campo
  ...
  fuente viva de los mínimos ≥8/≥6 que `rounds/index.json:107` mantiene y que
  program-check valida. Detalle y disposición en **D2**.

### A.3 — `rounds/index.json` R1: objetivo reemplazado ✓

- `rounds/index.json:108` — "prove three-second tenant recognition in grayscale and
...
- `r1-craft-prep.md:174` — el ítem 4 ya no presenta la dirección vieja como
  ...
  `rounds/index.json:107` como propiedad de la prueba. La cabecera de corrección de
...
- `rounds/index.json:103` (fixtures de prueba, paridad static/DB), `:104` (drenaje
...
- `art-direction/index.json:4-9` `stageStatus` intacto.
...
- `visual-craft/index.json:348` — `hardVisualVetoes` (25 entradas) incluye
  `current-tenant-palette-retained-without-r1-sighted-readjudication`.
- `quality-rubric/index.json:310` — el mismo ID en `hardVetoes` (índice 41 de 52; la
  lista arranca en `:268`), que es elegibilidad de rubric.
- Vinculación actual, no futura: `rounds/index.json:111` (objetivo R1 "close the
  twelve reproduced visual-craft incidents and every generalized
  hardVisualVeto") + `rounds/index.json:120` (exit R1 "zero hard vetoes") +
...
estado que la enmienda sanciona (`art-direction/index.json:30`: "the fixtures
...
- `art-direction/index.json:289` —
  `"threeSecondGrayscaleRecognitionRequired": true` dentro de
  `divergenceContract`.
- `art-direction/index.json:307-308` — `acceptanceQuestions` 1 y 2: "Can an
...
- `rounds/index.json:368` — objetivo R4: "make each complete screen recognizable by
  tenant in grayscale through chrome type geometry material rhythm and
  responsive composition".
- `rounds/index.json:422` — objetivo R6: "tenant-art-direction scene matrix and
...
`rounds/index.json:107` conserva como objetivo R1 y que
...
y que reescriba o difiera explícitamente `rounds/index.json:368` y `:422` con la
...
1. **`visual-craft/index.json:426`** — checkpoint artifact "tenant
...
2. **`customization-model/index.json:155`** —
...
4. **Scout concurrente `scouts/r1-button-opus-grammar.md`** (untracked, creado
   ...
   cabecera declara haber leído el `rounds/index.json` post-enmienda (donde ese
...
grep -rln "current-tenant-palette-retained-without-r1-sighted-readjudication" \
  --exclude-dir=node_modules --exclude-dir=.git .   # quality-rubric/index.json + visual-craft/index.json
grep -n '"zero hard vetoes"' $P/rounds/index.json          # 120 (R1), 429, 536
python3 -c "import json; vc=json.load(open('$P/visual-craft/index.json')); \
  qr=json.load(open('$P/quality-rubric/index.json')); \
...
grep -n "threeSecondGrayscaleRecognitionRequired\|three seconds\|temporarily neutralized" \
  $P/art-direction/index.json                        # 289, 307, 308
grep -n -i "threeSecond" $P/program-check.mjs $P/program-check.test.mjs    # 0 matches

# A.6 — censo
git check-ignore packages/core/docs/history/programs/modern-rescue/research/2026-08/hardcode-census-top5.md
# exit 1 => no ignorado; git add plano lo trackea
```

Veredicto final: **DEFECTS-2** (D1 veto de paleta en
`visual-craft/index.json:348` + `quality-rubric/index.json:310`; D2 ley de
reconocimiento en `art-direction/index.json:289,307-308` +
`rounds/index.json:368,422`). Los seis restos enumerados: remediados. El guard de
regresión: intacto.
