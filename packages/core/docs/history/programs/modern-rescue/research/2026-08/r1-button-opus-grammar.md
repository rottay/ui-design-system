...
> la enmienda del owner y su cierre de restos (`rounds/index.json` ya no la contiene;
...
- **Árbol:** 5 tracked modificados ajenos (`program-check.mjs`, `program/index.json`, `rounds/index.json`,
  `art-direction/index.json`, `scouts/r1-craft-prep.md` — la enmienda en curso) + untracked
  ajenos (`docs/reauditoria-cloud/`, `scouts/hardcode-census-top5.md`). Leí `rounds/index.json` del
...
vive en `packages/core/src/foundation/tokens/**`, que `rounds/index.json` R1 asigna al dominio
...
| 15 | `probe/ds-reference/sections/control/index.tsx` (+ chrome si hace falta) | Ampliación de escena de §3. **Sin importar el torture harness** (ley del ground) y con readiness `data-testid` propia. | `rounds/index.json` R1 `scope.referenceLabLaw` | — |
...
1. **Ningún canal `--ds-*` público nuevo minteado por estas familias** (README del programa :446;
   `customization-model/index.json:9,19`). Sólo sockets privados provisionales, **registrados completos**
...
2. **El lote es multi-lane por construcción** (§7.4-iii). Si se lanza como un solo
   `family-writer`, chocará contra los dominios de `rounds/index.json` exactamente como pasó en la
   Cohorte 1.
