...
> (`scripts/tooling/quality/evidence/scorecard.example.json`) is deliberately pending.
...
> 252 families through `scripts/check/evidence/framework/`, where binary eligibility
...
- `packages/core/scripts/check/engine-baseline/index.json` still records the
