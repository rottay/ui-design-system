...
> Generado por `scripts/generate/tokens/customization/controls/index.mjs --write`. NO editar a mano.
...
|---|---|---|---|---|---|---|---|---|
| `palette.seeds` | Brand palette seeds | color-set | vertical baseline palette | 5 ch | `palette.{primaryColor,secondaryColor,accentColor,backgroundColor}` | `appearance.general.palette.{primary,secondary,accent,background}` | 1930 | `button.css` |
...
| `token-overrides` | Bounded raw channel overrides | token-map | none; closed allowlist, max 200 entries, fails closed | 3 ch | `tokenOverrides` | `visualFoundation.advanced.tokenOverrides.{--ds-color-error,--ds-color-bg-overlay}` | 835 | `index.ts` |
