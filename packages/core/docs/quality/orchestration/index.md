...
`scripts/check/modern-rescue/family-inventory/index.json`, the same regenerated
...
`artifacts/quality/programs/modern-rescue/archive/ledgers/family-source-visitation/252-family-era/index.json` is a **sealed R0 receipt** and is an
...
node packages/core/scripts/check/orchestration/public/write-set-intersection/index.mjs \
     --plan packages/core/scripts/check/orchestration/plans/examples/index.json
```
...
```bash
# against a plan
node packages/core/scripts/check/orchestration/public/containment/index.mjs \
...
```bash
node packages/core/scripts/check/orchestration/public/work-order/index.mjs \
     --work-order packages/core/scripts/check/orchestration/public/work-order/examples/index.json
```

`public/work-order/schema/index.json` states the shape. The validator states what a shape cannot:

| Rule | What it refuses |
|---|---|
| `W1-model-reason` | a padded or boilerplate reason; a delegated lane with no pre-pass, or one whose cited evidence is not on disk |
...
| `W4-build-free` | a verification needing the build; a work order that names no acceptance gate — the admissible set is READ from `scripts/check/automation/gates/manifest/index.mjs`, so a gate CI retires stops being quotable here in the same commit |
...
**Synthetic ownership rows** for the non-family waves live in `public/work-order/synthetic-ownership/index.json`,
...
node packages/core/scripts/check/orchestration/public/program-state/index.mjs --check --intent <intent.json>
```
...
```bash
node packages/core/scripts/check/orchestration/tests/drills/index.mjs
```
...
```jsonc
{
  ...
  "lanes": [
    { "id": "avatar", "row": "primitive/display/avatar", "model": "implementation" },
    { "id": "base",   "row": "layer:base", "model": "review",
      "claimsSharedFiles": ["packages/core/src/foundation/tokens/css/foundation/themes/default/index.css"] }
  ]
}
```
