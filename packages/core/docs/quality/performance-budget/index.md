...
Enforced in CI via `scripts/check/automation/bundle/index.mjs`. Any violation fails the build.
...
| Flagship component bundle | Component-specific gzip ceilings in `scripts/check/automation/bundle/index.mjs`; all nine fixtures currently exceed them, so this lane is **failing** rather than silently rebaselined |
...
| BitHire selected `Icon` preset closure | ≤ 75,275/81,828 B gzip ESM/CJS; 74,701/81,710 B measured for the exact productive 104-role inventory, renderer supplier included, React external |
| Explicit full-corpus `Icon` compatibility closure | < 203/226 KB gzip ESM/CJS compatibility ratchet; 200,611/225,008 B measured for all 282 roles, never presented as a route-sized < 40 KB payload |
...
| Charts experience entry (`dist/charts.js` / `.cjs`) | < 1.7/1.7 KB raw (1,517/1,521 B measured; eight public renderers, runtime governed separately) |
...
| Chart renderers entry (`dist/chart-renderers.js` / `.cjs`) | < 1.8/1.8 KB raw (1,611/1,615 B measured; eight renderers plus datum-key facade, runtime governed separately) |
...
| Named Bar renderer | < 13.9 KB gzipped (12,596 B measured + >10%; accessible interaction/insight runtime, D3/React external) |
...
| Named Funnel renderer | < 12.2 KB gzipped (11,003 B measured + >10%) |
| Named Gauge renderer | < 12.5 KB gzipped (11,311 B measured + >10%) |
| Named Radar renderer | < 12.4 KB gzipped (11,220 B measured + >10%) |
| Named Scatter renderer | < 13.0 KB gzipped (11,782 B measured + >10%) |
| Deduplicated renderer family | < 29.7 KB gzipped (26,967 B measured + >10%; all eight public renderers, shared runtime paid once) |
...
| Aggregate JS package footprint | < 9.1 MB raw; 11,715,861 B measured on 2026-08-31 — **failing** (consumer routes and asset-retention scenarios are independently gated) |

The governed semantic-icon corpus currently contains 282 roles. The executable
asset-retention gate owns the per-scenario measurements; this document does not
repeat an older corpus count as though it were current.

## CSS Budget

| Metric | Limit |
|--------|-------|
| Generic styles (`dist/styles.css`) | < 460 KB gzip; 1,062,511 B measured on 2026-08-31 — **failing** |
| Rottay styles (`dist/rottay.css`) | < 425 KB gzip; 1,043,070 B measured on 2026-08-31 — **failing** |
| BitHire styles (`dist/bithire.css`) | < 430 KB gzip; 1,043,475 B measured on 2026-08-31 — **failing** |
| Evnto styles (`dist/evnto.css`) | < 410 KB gzip; 1,034,017 B measured on 2026-08-31 — **failing** |
| Modern engine (`dist/modern-engine.css`) | < 30 KB gzip; 2,326 B measured on 2026-08-31 — passing |
...
or mix built artifacts with source CSS. The four bundle overruns above are open
delivery debt; their limits were not raised to disguise the regression.
...
3. The semantic motion gate bundles the pure policy and `MotionProvider` separately through the
   ...
   peers external. `node scripts/check/automation/bundle/index.mjs --motion` runs only this focused gate.
4. The EffectRegistry gate bundles all nine runtime exports from `./effects`, rejects suppliers,
   ...
   `require()` closure and `.d.ts` graph. `node scripts/check/automation/bundle/index.mjs --effects` runs only
   this focused gate.
5. The VIZ-03A chart-spec gate bundles every runtime export from `./charts/spec`, rejects React,
   ...
   CJS `require()` closure and `.d.ts` graph. `node scripts/check/automation/bundle/index.mjs --chart-spec` runs
...
6. The VIZ-03C chart-access gate bundles the complete `./charts/access` runtime inventory and an
   ...
   both raw facade ceilings. `node scripts/check/automation/bundle/index.mjs --chart-access` runs only this gate.
7. `node scripts/check/automation/bundle/index.mjs --chart-renderers` runs only that focused renderer gate;
...
8. The SPATIAL-01 gate uses `export *` fixtures and compares both the built-facade and bundled
   ...
   also enforces the four raw facade ceilings. `node scripts/check/automation/bundle/index.mjs --spatial` runs
   only this focused gate.
9. The semantic-asset retention gate selects a generated role subpath, the exact
   ...
   the existing 40/30/4 KB ceilings; the complete 282-role facade has a separate measured +10%
   ...
   `node scripts/check/automation/bundle/index.mjs --asset-retention` runs only this focused gate.
...
1. Update the limits in both this document and `scripts/check/automation/bundle/index.mjs`.
