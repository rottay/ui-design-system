...
Engines are code-split. Only the active engine downloads when a component first renders. If the user switches engines at runtime, the new engine lazy-loads on demand. See [engine splitting](../../architecture/engine-splitting/index.md) for details.
