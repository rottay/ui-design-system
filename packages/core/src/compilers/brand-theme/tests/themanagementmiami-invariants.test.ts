 ...
 *    legacy `tokens/css/legacy/themanagementmiami/index.css` shipped a BLUE
...
import { themanagementmiamiBrandTheme, bithireBrandTheme } from '../../../tokens/ts/brand-themes';
import { isBundledTenant, isKnownTenant, BUNDLED_TENANT_SLUGS } from '../../../runtime/tenant/registry';
import { hexToRgb, isHexColor } from '../../_shared/color-math';
import { contrastRatio, rgbToHsl } from '../../../_internal/a11y/contrast';
...
// cssVariables output (verified against packages/core/src/compilers/brand-theme/index.ts);
...
const TIER1_DIVERGENCE_CHANNELS: readonly DivergenceChannel[] = [
  ...
  { name: '1. typography: heading font family', extract: (c) => c.cssVariables['--ds-font-family-heading'] },
  { name: '1. typography: heading letter-spacing (serif tracking discipline)', extract: (c) => c.cssVariables['--ds-letter-spacing-heading'] },
  // 2. Radius — deco-crisp 4/6/8/12 against bithire's 6/8/10/14.
  { name: '2. radius: sm step', extract: (c) => c.cssVariables['--ds-radius-sm'] },
  { name: '2. radius: md step', extract: (c) => c.cssVariables['--ds-radius-md'] },
  { name: '2. radius: lg step', extract: (c) => c.cssVariables['--ds-radius-lg'] },
  { name: '2. radius: xl step', extract: (c) => c.cssVariables['--ds-radius-xl'] },
  // 3. Motion temperament — spring-settled slide-up against bithire's linear fade.
  { name: '3. motion: entrance type', extract: (c) => c.personality.animation?.entrance },
  { name: '3. motion: entrance duration (--ds-motion-calm)', extract: (c) => c.cssVariables['--ds-motion-calm'] },
  { name: '3. motion: spring usage', extract: (c) => c.personality.animation?.useSpring },
  { name: '3. motion: hover lift', extract: (c) => c.personality.animation?.hoverLift },
  // 4. Separation strategy — shadow-led + warm temperature against bithire's
  //    border-led + cool temperature.
  { name: '4. separation strategy: card border', extract: (c) => c.cssVariables['--ds-card-border'] },
  { name: '4. separation strategy: card shadow', extract: (c) => c.cssVariables['--ds-card-shadow'] },
  { name: '4. separation strategy: sidebar border', extract: (c) => c.cssVariables['--ds-sidebar-border'] },
  { name: '4. separation strategy: card showBorder posture', extract: (c) => c.personality.card?.showBorder },
  // 5. Accent grammar — top gradient bar + square badges against bithire's
  //    left solid bar + pill badges.
  { name: '5. accent grammar: bar position', extract: (c) => c.personality.accent?.barPosition },
  { name: '5. accent grammar: bar style', extract: (c) => c.personality.accent?.barStyle },
  { name: '5. accent grammar: badge shape', extract: (c) => c.personality.accent?.badgeShape },
  { name: '5. accent grammar: divider style', extract: (c) => c.personality.accent?.dividerStyle },
  // 6. Effects posture — nonzero effect-intensity against bithire's flat zero.
  { name: '6. effects posture: effect intensity', extract: (c) => c.cssVariables['--ds-effect-intensity'] },
  // 7. Palette, last.
  { name: '7. palette: primary color', extract: (c) => c.cssVariables['--ds-color-primary'] },
  { name: '7. palette: secondary color', extract: (c) => c.cssVariables['--ds-color-secondary'] },
  { name: '7. palette: accent color', extract: (c) => c.cssVariables['--ds-color-accent'] },
  // chrome.table — white + hairline against bithire's tinted paper-band header.
  { name: 'chrome.table: header background', extract: (c) => c.cssVariables['--ds-table-header-bg'] },
  { name: 'chrome.table: border', extract: (c) => c.cssVariables['--ds-table-border'] },
];
...
describe('themanagementmiami diverges from bithire on every Tier-1 channel', () => {
  it.each(TIER1_DIVERGENCE_CHANNELS)('$name', ({ extract }) => {
    ...
    expect(tmmValue, 'themanagementmiami must emit a defined value for this channel').not.toBeUndefined();
    expect(
      tmmValue,
      `themanagementmiami and bithire must differ on this channel (both resolved to ${JSON.stringify(tmmValue)})`,
...
// implementation: hexToRgb (compilers/_shared/color-math) and rgbToHsl +
// contrastRatio (_internal/a11y/contrast, the WCAG validator already used to
...
function hexHue(hex: string): number {
  if (!isHexColor(hex)) {
    throw new Error(`semantic-collision check received a non-hex color: ${hex}`);
  }
  const rgb = hexToRgb(hex);
  if (!rgb) {
    throw new Error(`semantic-collision check could not parse hex color: ${hex}`);
  }
...
function isDistinguishable(a: string, b: string): boolean {
  return circularHueDistance(hexHue(a), hexHue(b)) >= MIN_HUE_SEPARATION_DEG || contrastRatio(a, b) >= MIN_CONTRAST_SEPARATION;
}

describe('semantic-color-collision invariant (owner decision 2026-07-09)', () => {
  const palette = themanagementmiamiBrandTheme.palette;
  if (!palette) {
    throw new Error('themanagementmiamiBrandTheme.palette is required for the semantic-collision invariant');
  }

  const colors: Record<'primary' | 'success' | 'warning' | 'error' | 'info', string | undefined> = {
  ...
  it.each(Object.entries(colors))('%s color is a real hex value', (_name, value) => {
    expect(value, 'every semantic + primary color must be a literal hex value for a measured separation check').toBeDefined();
    expect(isHexColor(value as string)).toBe(true);
  });

  const REQUIRED_PAIRS: ReadonlyArray<readonly [keyof typeof colors, keyof typeof colors]> = [
    ['success', 'primary'],
    ['warning', 'primary'],
    ['error', 'primary'],
    ['info', 'primary'],
  ...
  it.each(REQUIRED_PAIRS)('%s is distinguishable from %s', (a, b) => {
    ...
    expect(
      ...
      `${a} (${colorA}) vs ${b} (${colorB}): hueDist=${hueDist.toFixed(1)}deg (need >=${MIN_HUE_SEPARATION_DEG}) and ` +
        `contrast=${contrast.toFixed(2)} (need >=${MIN_CONTRAST_SEPARATION}) -- neither threshold met`,
    ).toBe(true);
  });
});
...
describe('BUNDLED_TENANT_SLUGS promise holds for every member', () => {
  ...
  it.each([...BUNDLED_TENANT_SLUGS])('%s has a bundled CSS artifact on disk', (slug) => {
    const artifactPath = resolve(TEST_DIR, '../../../tokens/css/artifacts', slug, 'index.css');
