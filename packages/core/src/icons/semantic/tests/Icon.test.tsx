...
describe('semantic Icon accessibility and rendering', () => {
  ...
  it('keeps public props free of supplier types and CSS carries accessibility fallbacks', () => {
    const publicTypes = readFileSync(resolve(process.cwd(), 'src/icons/semantic/types.ts'), 'utf8');
    const publicBarrel = readFileSync(resolve(process.cwd(), 'src/icon-entry.ts'), 'utf8');
    const iconCss = readFileSync(
      resolve(process.cwd(), 'src/tokens/css/components/semantic-icon.css'),
      'utf8',
    );
