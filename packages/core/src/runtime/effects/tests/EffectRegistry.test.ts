...
import { EFFECT_IDS, type EffectId } from '../../../contracts/effects';
import {
  EFFECT_DEFINITIONS,
  EFFECT_REGISTRY,
...
import { resolveEffect, resolveEffectDefinition } from '../resolver';
import { isEffectDefinition, isEffectId } from '../validation';
...
function measuredBudget(loop: 'none' | 'finite' | 'while-live') {
  return {
    ...
    evidence: 'packages/core/src/runtime/effects/tests/EffectRegistry.test.ts',
  };
}
