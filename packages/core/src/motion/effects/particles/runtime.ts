...
import { resolveCssColor } from '@/_internal/color/css/resolve-css-color';
...
type LeaseOwner = symbol;
type LeaseAvailableCallback = () => void;
...
export function acquireParticleAnimationLease(
  ...
  if (activeAnimationLease === owner) return true;
  if (activeAnimationLease === null) {
    animationLeaseWaiters.delete(owner);
    activeAnimationLease = owner;
    return true;
  }
  animationLeaseWaiters.set(owner, onAvailable);
  return false;
}

export function releaseParticleAnimationLease(owner: LeaseOwner): void {
  animationLeaseWaiters.delete(owner);
  if (activeAnimationLease !== owner) return;
  activeAnimationLease = null;

  const next = animationLeaseWaiters.entries().next();
  if (next.done) return;
  const [nextOwner, notify] = next.value;
  animationLeaseWaiters.delete(nextOwner);
  notify();
}

export function getParticleAnimationLeaseCount(): 0 | 1 {
  return activeAnimationLease === null ? 0 : 1;
}

/** Test-only cleanup for the module-local lease; not re-exported publicly. */
export function resetParticleAnimationLeaseForTests(): void {
  activeAnimationLease = null;
  animationLeaseWaiters.clear();
}
