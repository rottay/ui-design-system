...
const DEFAULT_THEME = join(CSS_ROOT, "foundation/themes/default.css");
...
describe("R5 chain repair -- retired names stay retired", () => {
  ...
  it("keeps the popover and tooltip inverse ink on a declared role", () => {
    const popover = readFileSync(join(CSS_ROOT, "runtime/engines/modern/skin/popover.css"), "utf8");
    const tooltip = readFileSync(join(CSS_ROOT, "runtime/engines/modern/skin/tooltip.css"), "utf8");
  ...
  it("keeps the avatar-group ring on a tenant-causal role instead of dropping the border", () => {
    const avatar = readFileSync(
      join(CSS_ROOT, "presentation/components/skin/avatar-compounds.css"),
      "utf8",
    );
