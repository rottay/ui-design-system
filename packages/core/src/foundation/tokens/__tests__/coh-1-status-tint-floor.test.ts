/**
 * COH-1 (WO-CRA-23) — status-tint floor acceptance spec, static path.
 *
 * Formula under test, ADJUDICATED by the Opus architectural review
 * (`test-artifacts/quality-evidence/wo-cra-23/scouts/coh-1-opus-formula-review.md`,
 * section A.1-A.4), which overrides the earlier scout/prep draft:
 *
 ...
 * `withoutKeypath` pattern in `mass-c3-bithire-drain.test.ts`, generalized to
...
// and, as of COH-1, no longer authors any `*BgColor`/`*BorderColor` literal
...
describe("COH-1 — floor fires when the seed exists and the field is unauthored", () => {
...
describe("COH-1 — an authored literal still outranks the floor, per channel not per family", () => {
...
describe("COH-1 — an absent seed emits nothing", () => {
 ...
 */
describe("COH-1 — STATUS_SEED_SHADOWING_FIELDS is exactly what the floor emits", () => {
 ...
 */
describe("COH-1 — foundation :root is untouched (T4)", () => {
  ...
  it("alpha block (:197-203) is byte-identical to pre-COH-1", () => {
  ...
  it("bg/border block (:252-259) is byte-identical to pre-COH-1", () => {
    expect(defaultCss).toContain(
...
// ── Contrast: ink over the corrected well (T5) ──────────────────────────────
 ...
 * (`tinted-well-tone-ink`, untouched by COH-1), which mixes the seed 60% into
 ...
 */
describe("COH-1 — ink/well contrast holds after the hue correction (T5)", () => {
