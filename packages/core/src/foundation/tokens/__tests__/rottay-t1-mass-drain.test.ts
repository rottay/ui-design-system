/**
 * ROTTAY-T1 MASS -- the rottay artifact extension drained from 973 to 698,
 * then to 682 by the T1 P0 repair.
 *
 * -- Two legs, one file, stated up front -------------------------------------
 *
 * This file grades TWO chained source states of the same stylesheet:
 *
 *   leg 1  ROTTAY-T1 drain   973 -> 698   (-275)  HISTORICAL
 *   leg 2  T1 P0 repair      698 -> 682   (-16)   CURRENT, on disk today
 *
 ...
 * -- Why leg 2 exists --------------------------------------------------------
 *
 * The C3-BITHIRE-ALL wave authored `chrome.controls.select` on the rottay
 ...
 * T1's roster never included the select family, so those rows survived and the
 ...
 *
 * -- T2 preimage disclosure --------------------------------------------------
 *
 * T2 opens against the file leg 2 leaves behind, not against the file T1 left
 ...
 * counted a second time in T2's roster. The 302/318 figures are T2 plan
 ...
 * file must equal neither the pre-drain state nor the T1 state.
 */
const POST_T1_DIGEST =
  "f36959c1270082bacd43efbb6610e528e0331ac3db00a855f7bb93c5743b2f7e";
/**
 * The digest of leg 2's output -- 682 declarations, the whole T2 control
...
/**
 * The digest of leg 3's output -- leg 2 minus the 302 declarations ROTTAY-T2
 * MASS drained. This file does not own the T2 ruling; it owns the fact that
 * its own ledger still reconciles after T2 moved the floor under it, which is
 * why only the digest/census chain is extended here and no T1 causality is
...
/**
 * The current file's digest -- leg 3 minus the 380 declarations ROTTAY-T3 MASS
 ...
 * read as a pass. Same rationale as the T2 leg -- the T3 ruling is adjudicated
 * in `rottay-t3-mass-drain.test.ts`; only the chain is extended here.
 */
...
/**
 * Leg 2 -- the T1 P0 repair. Separate ledger, because CENSUS above is the
 * signed T1 ruling and must keep meaning what it meant when it was signed.
...
const REPAIR = {
  ...
  perModeBefore: 349,
  perModeAfter: 341,
  /** T2 plan quantities; only their difference is provable from source. */
  t2PreimageBefore: 318,
  t2PreimageAfter: 302,
} as const;

/**
 * Leg 3 -- ROTTAY-T2 MASS. A third ledger, for the same reason leg 2 got its
 ...
 * battery live in `rottay-t2-mass-drain.test.ts`. What is asserted here is only
 * that leg 3 consumed exactly leg 2's disclosed T2 preimage and that this
...
/**
 * Leg 4 -- ROTTAY-T3 MASS. The terminal leg: it takes the whole 380-declaration
 ...
 * `rottay-t3-mass-drain.test.ts`; asserted here only as chain arithmetic --
 ...
 * `facade/entrypoints/styles.css` for the rottay root element in each mode.
 *
...
describe("ROTTAY-T1 MASS - census", () => {
  ...
  it("chains leg 3 onto leg 2 and consumes exactly the disclosed T2 preimage", () => {
    ...
    // Leg 2 disclosed how big the T2 preimage would be once it had taken its
  ...
  it("chains leg 2 onto leg 1 instead of restating a bare total", () => {
    ...
    // The T2 disclosure: its preimage shrinks by exactly what leg 2 consumed.
...
// EXCISED (SEV-2): describe "ROTTAY-T1 MASS - drain" — 4 tests, all reading the
...
describe("ROTTAY-T1 MASS - the common lowering emits every migrated channel", () => {
...
/**
 * T1 authority: a migrated name must lower identically through BOTH transports
 ...
 *
 * Scope note: this grades the names ROTTAY-T1 migrated. Eight
 ...
 */
describe("ROTTAY-T1 MASS - both transports lower migrated names identically", () => {
  /**
...
describe("ROTTAY-T1 MASS - mode authority", () => {
  /**
...
describe("ROTTAY-T1 MASS - one lowering, both transports", () => {
...
describe("ROTTAY-T1 MASS - initial is a nine-field exception, not a policy", () => {
...
describe("ROTTAY-T1 MASS - why the drain had to author dark at the root", () => {
  /**
   * `foundation/themes/default.css` carries a dark block that redeclares 72
...
describe("ROTTAY-T1 MASS - the derivations are common, not rottay-shaped", () => {
  /**
...
/**
 * Leg 2 -- the T1 P0 repair. Everything here grades the eight select channels
 * removed from both mode blocks, and the eighteen that stay.
 */
// EXCISED (SEV-2): describe "T1 P0 REPAIR - the extension no longer restates a
 ...
 * not the whole gate.
 */
describe("T1 P0 REPAIR - causality", () => {
