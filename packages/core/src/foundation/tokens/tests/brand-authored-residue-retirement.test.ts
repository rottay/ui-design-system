 ...
 * owner GO independent audit+Turing, those 112 declarations (49 rottay × dark+light +
...
// enforcement. Its successor is `scripts/check/verticals/single-author/index.mjs`,
...
// `scripts/check/verticals/single-author/index.mjs`: there is no second source to
...
describe("compileBrandTheme full roster parity (channel=value hash)", () => {
  it("R35 default (dark) matches the signed effective-value hash", () => {
    ...
    expect(hash).toBe("9dbf727ae73a39c02448df7d3a9d008b5d1a3b5891ee3d71055abdf69e863e59");
  });
...
// `scripts/check/verticals/single-author/index.mjs` (G1 roster exactness, G2
// resurrection scan, G3 API/marker scan, G4 delegated ink causality), drilled
// by `scripts/check/verticals/single-author/index.test.mjs` and wired blocking in
// `scripts/check/automation/gates/manifest/index.mjs`.
...
describe("zero productive consumers in Core + Showroom", () => {
  ...
  it("scanner positive controls find known live consumers", () => {
    const css = readFileSync(join(ROOT, "src/foundation/tokens/css/foundation/themes/default/index.css"), "utf8");
...
describe("planted mutants turn red", () => {
  ...
  // `scripts/check/verticals/single-author/index.mjs`. The SCANNER mutants below
