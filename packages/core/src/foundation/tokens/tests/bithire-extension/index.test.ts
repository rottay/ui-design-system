/**
 * BITHIRE EXTENSION DRAIN — the BitHire artifact extension drained to zero.
 *
...
const LEDGER = JSON.parse(
  readFileSync(
    join(process.cwd(), "governance/tokens/decisions/writers/unused/system/index.json"),
    "utf8"
  )
) as {
...
// EXCISED (SEV-2): "BITHIRE EXTENSION DRAIN · the extension declares nothing" —
...
// unconditionally by `scripts/check/verticals/single-author/index.mjs` law G2: an
...
describe("BITHIRE EXTENSION DRAIN · the table is the file that was drained", () => {
...
describe("BITHIRE EXTENSION DRAIN · the shell geometry deltas are graded, not silent", () => {
...
describe("BITHIRE EXTENSION DRAIN · the typed owner is the author", () => {
 ...
 * of the family.
 */
describe("BITHIRE EXTENSION DRAIN · every first-party theme authors the new families", () => {
  const THEMES = {
   ...
   *
   * ROTTAY EXTENSION CONTROL-FAMILY DRAIN migrated eighteen further select channels out of rottay's
  ...
  it("controls.select is one shared vocabulary, with rottay a declared superset", () => {
    ...
    // is present in every vertical including the one control-family tranche extended.
    ...
    // Rottay's surplus is exactly the ROTTAY EXTENSION CONTROL-FAMILY DRAIN migration, named — so a
   ...
   *
   * ROTTAY EXTENSION FOUNDATION DRAIN later migrated six further channels out of rottay's extension
  ...
  it("chrome.surface is one shared vocabulary across all three verticals", () => {
    ...
    // The C3 core is exactly what the untouched verticals still carry, and it
    // is present in every vertical including the one foundation tranche extended.
    expect(byId.bithire).toEqual(byId.evnto);
    ...
    // Rottay's surplus is exactly the ROTTAY EXTENSION FOUNDATION DRAIN migration, named — so a future
