...
const DEFAULT_THEME_CSS = join(SRC, 'foundation/tokens/css/foundation/themes/default/index.css');
 ...
 *
 * Every entry sits in `foundation/themes/default/index.css`, and that is the whole
...
describe('MODERN-TENANT-VALUE-FREE · the engine declares no tenant color', () => {
  ...
  it('drill · a planted tenant hex in a copied Modern skin turns the floor red', () => {
    const donor = modernFiles.find((file) => file.endsWith('skin/card/index.css'));
