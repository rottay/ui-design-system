/**
 * ROTTAY EXTENSION COMPONENT-FAMILY DRAIN -- the rottay artifact extension drained from 380 to ZERO by
 ...
 * `artifacts/rottay/_source/extension.css`; `ROTTAY EXTENSION CONTROL-FAMILY DRAIN` took 302 of them
 ...
 *
 * The control-family tranche families all hung off `chrome.controls`. These twenty-six are
 ...
 * vertical/generated staleness is not an acceptance signal for component-family tranche.
 */
...
/**
 * The partition hash recipes, identical to foundation tranche/control-family tranche so the three tranches are
 * comparable by hand:
 *
 ...
 *
 * `preImage` is the sha256 of the whole stylesheet as control-family tranche left it; `terminal`
...
/** Project one mode authority down to the component-family tranche leaves it owns. */
function project(
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the signed partition", () => {
...
// EXCISED (SEV-2): describe "ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - what the file on disk now says"
...
//      that, and `scripts/check/verticals/single-author/index.mjs` law G2 proves no
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the pre-image is literal, so the before-paint is not simulated", () => {
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the paint did not move", () => {
  ...
  it("emits each channel exactly once, at the compiled value", () => {
    for (const row of T3_ROSTER) {
      expect(resolveValue(row.mode, EMITTED[row.mode][row.name]), `${row.mode} ${row.name}`).toBe(
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the dark floor layer is a measured collision, not a hope", () => {
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the typed shape is closed and total", () => {
  ...
  it("authors the union of body and light exactly, with no `initial`", () => {
    for (const row of T3_ROSTER) {
      ...
      expect(resolveValue(row.mode, authored), `${row.mode} ${row.name}`).toBe(
        resolveValue(row.mode, emittedToday(row))
      );
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - one lowering, both transports", () => {
  const PROBE = "#010203";
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the DB documents of the twenty-six families", () => {
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - the channel map is exact, not prefix matching", () => {
...
describe("ROTTAY EXTENSION COMPONENT-FAMILY DRAIN - causality", () => {
  ...
  // `scripts/check/verticals/single-author/index.mjs` law G2. The wrong-block and
