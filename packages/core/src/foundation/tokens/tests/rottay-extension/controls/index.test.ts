/**
 * ROTTAY EXTENSION CONTROL-FAMILY DRAIN -- the rottay artifact extension drained from 682 to 380 by
 * moving the thirteen form-control families into typed Theme owners.
 *
 * -- What this tranche is ----------------------------------------------------
 *
 * `ROTTAY EXTENSION FOUNDATION DRAIN` left 682 custom-property declarations in
 ...
 * vertical/generated staleness is not an acceptance signal for control-family tranche.
 */
 ...
 * re-cablo a una raiz DESPUES del drenaje. Misma ley que en component-family tranche: el roster es
 ...
 * after we signed it": the component-family tranche pins are provably NOT this value.
 */
const EMPTY_LIST_HASH =
  "01ba4719c80b6fe911b091a7c05124b64eeece964e09c058ef8f9805daca546b";

/**
 * What this tranche left behind, as measured here, before `ROTTAY EXTENSION COMPONENT-FAMILY DRAIN`
 * took it. The rows themselves now live in `rottay-extension/components/index.test.ts`,
...
/**
 * The eight select channels the foundation tranche P0 repair already removed, kept here so the
 * reinsert mutant can prove that leg 2's boundary is still defended after leg
 * 3 drained the other eighteen. These are NOT part of the control-family tranche roster.
 */
...
// measured was ALREADY empty (ROTTAY-component-family tranche drained the last 380 rows), so nothing
// below loses a non-vacuous claim — but the emptiness was measured then and is
// structural now: `scripts/check/verticals/single-author/index.mjs` law G2 fails if
...
function authoredComponentCssFiles(directory: string): string[] {
  const files: string[] = [];
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    const path = join(directory, entry.name);
    if (entry.isDirectory()) files.push(...authoredComponentCssFiles(path));
    else if (entry.isFile() && entry.name === "index.css") files.push(path);
  }
  return files.sort();
}

for (const file of authoredComponentCssFiles(COMPONENTS_DIR)) {
  const vars = rootVariables(readFileSync(file, "utf8"));
  const relativeFile = file.slice(COMPONENTS_DIR.length + 1).replaceAll("\\", "/");
  for (const [name, value] of Object.entries(vars)) {
    if (name in COMPONENT_FLOORS) continue;
    COMPONENT_FLOORS[name] = value;
    COMPONENT_FLOOR_SOURCE[name] = `components/${relativeFile}`;
  }
}
 ...
 * survivor half reached zero when ROTTAY-component-family tranche drained the remaining 380, and
 ...
 * roster this tranche signed, which is the only half control-family tranche ever owned.
 */
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the signed partition", () => {
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the component-family tranche remainder has since been drained", () => {
  /**
   * When this file was written the 380-declaration remainder was still on
   * disk, and these assertions read it there. `ROTTAY EXTENSION COMPONENT-FAMILY DRAIN` has since taken
   * all 380, so the remainder is no longer a survivor set this file can weigh:
   * `rottay-extension/components/index.test.ts` now carries those 380 rows as ITS signed
   * roster and re-proves the same two hashes from them.
   *
   * What stays here is the half of the claim control-family tranche still owns -- the ARITHMETIC
   * of the chain and the fact that the remainder it measured was real -- plus
   * a disk read that confirms the handover actually happened. The component-family tranche pins are
   * kept verbatim so the two files agree by hash and not by narrative.
   */
  it("hands the 380 rows to the component-family tranche tranche, keeping the signed pins", () => {
  ...
  it("closes the chain: 682 = 302 drained here + 380 drained by component-family tranche", () => {
  ...
  // EXCISED (SEV-2): "shares no channel with the control-family tranche roster". It was already
  // vacuous on disk and was kept as the tripwire that would fire first if a component-family tranche
...
// EXCISED (SEV-2): describe "ROTTAY EXTENSION CONTROL-FAMILY DRAIN - what the file on disk now says"
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the paint did not move", () => {
  /**
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the typed shape is closed and total", () => {
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the DB documents of the thirteen families", () => {
  ...
  /** Project a mode authority down to the control-family tranche leaves it owns. */
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - the select vocabulary opened by exactly eighteen names", () => {
  const selectFields =
  ...
  it("records which core select fields a DB document may not carry", () => {
    // Not a control-family tranche regression and not fixed here. This pin records WHICH select
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - causality", () => {
  /**
  ...
  // EXCISED (SEV-2): three reinsertion mutants — "a reinserted control-family tranche row is caught
  // even though it restores a byte-equal value", "a reinserted foundation tranche-repaired
  ...
  // by `scripts/check/verticals/single-author/index.mjs` law G2. The roster-hash and
...
describe("ROTTAY EXTENSION CONTROL-FAMILY DRAIN - rate is a stylesheet neighbour, not a controls member", () => {
