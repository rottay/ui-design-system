...
export default function ModernTenantPreview(props: TenantPreviewProps) {
  ...
  /* Informational data-tenant marker; no CSS keys on it. Removed on unmount
     so a detached root never advertises a tenant (customization anatomy lifecycle contract). */
  useEffect(() => {
  ...
  return (
    <div
      ...
      /* `buildPreviewCss` names the axes it could not paint instead of dropping
         ...
         "nothing to report" cannot be confused with "never asked". */
      {...(preview.unsupportedAxes.length > 0
        ? { 'data-ds-tenant-preview-unsupported': preview.unsupportedAxes.join(' ') }
        : {})}
      style={{
        ...style,
      }}
