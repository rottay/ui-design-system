...
const DOMAIN_VOCABULARY =
  /candidate|interview|recruiter|\btenant\b|venue|ticket|evnto|bithire/i;
...
describe("tenant-theme preview fixtures render without a product concept", () => {
  ...
  it("renders the Modern visual-excellence stress fixture", async () => {
    const { container } = render(
      <Harness>
        <VisualExcellencePreviewFixture />
      </Harness>
    );
    expect(await screen.findByText("Recent journey")).toBeInTheDocument();
    expect(screen.getByText("Decision comparison")).toBeInTheDocument();
    expect(screen.getByText("Prepare plan · 1.6k tokens")).toBeInTheDocument();
    expect(container.textContent ?? "").not.toMatch(DOMAIN_VOCABULARY);
  });
...
describe("PatternBrandStudio tenant-theme preview composition", () => {
  it("renders the galleries inside the sanitized CMP-02 preview scope for a valid document", async () => {
    ...
    expect(
      within(scope as HTMLElement).getAllByText("Records").length
    ).toBeGreaterThan(0);
