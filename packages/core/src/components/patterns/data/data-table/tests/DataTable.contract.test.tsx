...
describe("DataTable data-part contract (desktop)", () => {
  ...
  it.each(["modern", "rustic"] as const)(
    ...
    async (engine) => {
      ...
      const pinnedFlags = Array.from(headerCells).map((el) =>
      ...
      expect(
        Array.from(bodyRows).map((el) => el.getAttribute("data-row-key"))
      ).toEqual(["r1", "r2", "r3"]);
      if (engine === "modern") {
        expect(
          Array.from(bodyRows).map((el) => el.getAttribute("data-selected"))
        ).toEqual(["true", "false", "false"]);
        expect(
          Array.from(bodyRows).map((el) => el.getAttribute("data-striped"))
        ).toEqual(["false", "true", "false"]);
      } else {
        // Rustic consolidates the same 3-way into one data-state enum per
        // independent audit decision 3 (row data-state + td data-pinned, ONE rule instead
        // of the 3x-duplicated pinned-td ternary).
        expect(
          Array.from(bodyRows).map((el) => el.getAttribute("data-state"))
        ).toEqual(["selected", "striped", "default"]);
      }
      ...
      const pinnedDataCells = Array.from(dataCells).filter(
      ...
      if (engine === "modern") {
        expect(
          container.querySelectorAll('[data-part="pagination-page-button"]')
        ).toHaveLength(3);
        expect(
          Array.from(
            container.querySelectorAll('[data-part="pagination-page-button"]')
          ).map((el) => el.getAttribute("data-current"))
        ).toEqual(["false", "true", "false"]);
      } else {
      ...
      const bulkButtons = container.querySelectorAll(
        '[data-part="bulk-bar-action"]'
      );
      expect(bulkButtons).toHaveLength(3);
      expect(
        Array.from(bulkButtons).map((el) => el.getAttribute("data-variant"))
      ).toEqual(["danger", "primary", "default"]);
    }
  );

  it.each(["modern", "rustic"] as const)(
    "projects active root states under the %s engine",
    async (engine) => {
      const { container } = renderWithEngine(
        <PatternDataTable<Row>
          engine={engine}
  ...
  it.each(["classic", "modern", "rustic"] as const)(
    "stamps engine-neutral action column and content parts under the %s engine",
    async (engine) => {
      const { container } = renderWithEngine(
        <PatternDataTable<Row>
          engine={engine}
      ...
      const header = container.querySelector(
        '[data-part="header-cell"][data-cell-kind="actions"]'
      );
      const cells = container.querySelectorAll('[data-part="actions-cell"]');
      const contents = container.querySelectorAll(
        '[data-part="actions-content"]'
      );
      ...
      expect(
        Array.from(contents).every(
          (content) => content.parentElement?.dataset.part === "actions-cell"
        )
      ).toBe(true);
    }
  );
  ...
  it("stamps group-header-row/-chevron/-count-pill and toggles data-collapsed on click (modern only -- rustic has no groupBy support)", async () => {
    ...
    expect(
      Array.from(container.querySelectorAll('[data-part="body-row"]')).map(
        (el) => el.getAttribute("data-row-key")
      )
    ).toEqual(["r1", "r3", "r2"]);

    const chevronsBefore = container.querySelectorAll(
      '[data-part="group-header-chevron"]'
    );
    expect(
      Array.from(chevronsBefore).map((el) => el.getAttribute("data-collapsed"))
    ).toEqual(["false", "false"]);

    const countPills = container.querySelectorAll(
      '[data-part="group-header-count-pill"]'
    );
    expect(Array.from(countPills).map((el) => el.textContent)).toEqual([
      "2",
      "1",
    ]); // active: r1,r3; inactive: r2

    fireEvent.click(groupRows[0]);

    await waitFor(() => {
      const chevronsAfter = container.querySelectorAll(
        '[data-part="group-header-chevron"]'
      );
  ...
  it("stamps the inline cell editor's data-invalid from its local validation error (modern only -- rustic has no inline-edit parity)", async () => {
    ...
    const editableCell = container.querySelector(
...
describe("DataTable data-part contract (mobile branch)", () => {
  ...
  it("stamps the mobile card tree (Stack scope pair, Card className modifier, title/summary parts) and the bulk-actions scope pair", async () => {
    ...
    // querySelectorAll returns a static NodeList. Re-query after the independent
    // Flex lazy boundary resolves instead of asserting against an earlier
    // Stack-only snapshot.
    expect(
      Array.from(
    ...
    await waitFor(() => {
      ...
      expect(
        container.querySelectorAll(".ds-data-table__mobile-card")
      ).toHaveLength(3);
      expect(
        container.querySelectorAll(".ds-data-table__mobile-card--selected")
      ).toHaveLength(1);
      expect(
        container.querySelectorAll('[data-part="mobile-card-title"]')
      ).toHaveLength(3);
      // 2 summary columns per card (amount, status) x 3 cards.
      expect(
        container.querySelectorAll('[data-part="mobile-card-summary-label"]')
      ).toHaveLength(6);
      expect(
        container.querySelectorAll('[data-part="mobile-card-summary-value"]')
      ).toHaveLength(6);
    });
  });
});
