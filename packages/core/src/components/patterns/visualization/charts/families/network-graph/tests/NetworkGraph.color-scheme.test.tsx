...
describe('NetworkGraph governed colorScheme input', () => {
  ...
  it('materializes twelve slots in either mode', () => {
    ...
    try {
      const decision = resolveChartPaint({ family: 'network-graph', scheme: 'accessible' });
 ...
 * `colorScheme` coexist: a node colour is applied by the draw procedure after
 * the palette is chosen, and `colors` replaces the scheme-resolved palette
 * outright rather than merging with it.
 */
describe('NetworkGraph colors/colorScheme precedence', () => {
  ...
  it('keeps the overridden flag on the decision while colors wins', () => {
    expect(
      resolveChartPaint({ family: 'network-graph', scheme: 'monochrome', override: OVERRIDE })
        .overridden,
    ).toBe(true);
    expect(
      resolveChartPaint({ family: 'network-graph', scheme: 'monochrome' }).overridden,
    ).toBe(false);
  });
});
...
const refusedScheme = <NetworkGraph nodes={NODES} links={LINKS} colorScheme="rainbow" />;

/* The input is the governed union, not an open string. */
// @ts-expect-error an arbitrary string is not a governed scheme
const refusedWidening = <NetworkGraph nodes={NODES} links={LINKS} colorScheme={'default' as string} />;
