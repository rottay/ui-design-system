...
const STAGES = Array.from({ length: CHART_CATEGORICAL_SIZE + 1 }, (_, index) => ({
  label: `Stage ${index + 1}`,
  value: 120 - index * 6,
}));
...
describe('FunnelChart governed colorScheme input', () => {
  ...
  it('materializes twelve slots in either mode', () => {
    ...
    try {
      const decision = resolveChartPaint({ family: 'funnel-chart', scheme: 'accessible' });
 ...
 * `colorScheme` coexist: a datum colour is applied by the geometry builder
 * after the palette is chosen, and `colors` replaces the scheme-resolved
 * palette outright rather than merging with it.
 */
describe('FunnelChart colors/colorScheme precedence', () => {
  const OVERRIDE = ['#111111', '#222222', '#333333'];

  it('lets an explicit stage colour beat both colors and colorScheme', () => {
    const data = STAGES.map((stage, index) => (
      index === 1 ? { ...stage, color: '#abcdef' } : stage
    ));
  ...
  it('orders the three tiers datum > colors > scheme', () => {
    const data = STAGES.map((stage, index) => (
      index === 0 ? { ...stage, color: '#abcdef' } : stage
    ));
    const { container } = renderSurface(
      <FunnelChart
        data={data}
  ...
  it('keeps the overridden flag on the decision while colors wins', () => {
    expect(
      resolveChartPaint({ family: 'funnel-chart', scheme: 'monochrome', override: OVERRIDE })
        .overridden,
    ).toBe(true);
    expect(
      resolveChartPaint({ family: 'funnel-chart', scheme: 'monochrome' }).overridden,
    ).toBe(false);
  });
});
...
const refusedScheme = <FunnelChart data={STAGES} colorScheme="rainbow" />;

/* The input is the governed union, not an open string. */
// @ts-expect-error an arbitrary string is not a governed scheme
const refusedWidening = <FunnelChart data={STAGES} colorScheme={'default' as string} />;
