...
const TASKS = Array.from({ length: CHART_CATEGORICAL_SIZE + 1 }, (_, index) => ({
  id: `task-${index}`,
  name: `Task ${index + 1}`,
...
describe('GanttChart governed colorScheme input', () => {
  ...
  it('materializes twelve slots in either mode', () => {
    ...
    try {
      const decision = resolveChartPaint({ family: 'gantt-chart', scheme: 'accessible' });
 ...
 * `colorScheme` coexist: a task colour is applied by the geometry builder after
 * the palette is chosen, and `colors` replaces the scheme-resolved palette
 * outright rather than merging with it.
 */
describe('GanttChart colors/colorScheme precedence', () => {
  const OVERRIDE = ['#111111', '#222222', '#333333'];

  it('lets an explicit task colour beat both colors and colorScheme', () => {
    const tasks = TASKS.map((task, index) => (
      index === 1 ? { ...task, color: '#abcdef' } : task
    ));
    const { container } = renderWithScheme('vibrant', {
      tasks,
      colors: [...OVERRIDE],
  ...
  it('orders the three tiers datum > colors > scheme', () => {
    const tasks = TASKS.map((task, index) => (
      index === 0 ? { ...task, color: '#abcdef' } : task
    ));
    const { container } = renderWithScheme('monochrome', {
      tasks,
      colors: [...OVERRIDE],
  ...
  it('keeps the overridden flag on the decision while colors wins', () => {
    expect(
      resolveChartPaint({ family: 'gantt-chart', scheme: 'monochrome', override: OVERRIDE })
        .overridden,
    ).toBe(true);
    expect(
      resolveChartPaint({ family: 'gantt-chart', scheme: 'monochrome' }).overridden,
    ).toBe(false);
  });
});
...
const refusedScheme = <GanttChart tasks={TASKS} colorScheme="rainbow" />;

/* The input is the governed union, not an open string. */
// @ts-expect-error an arbitrary string is not a governed scheme
const refusedWidening = <GanttChart tasks={TASKS} colorScheme={'default' as string} />;
