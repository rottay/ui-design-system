...
/**
 * @fileoverview WorkspaceViewsMenu pattern -- saved-views dropdown for
 ...
 * horizontal scrollable bar of view chips. WorkspaceViewsMenu is a
...
export type WorkspaceSavedViewKind = 'system' | 'persona' | 'custom';
 ...
 *  any richer SavedView<T> via structural subtyping. */
export interface WorkspaceSavedView {
  key: string;
  label: string;
  kind?: WorkspaceSavedViewKind;
  isSystem?: boolean;
  isDefault?: boolean;
  state: {
    query?: string;
    scope?: string;
    filters?: WorkspaceSavedViewFilter[];
...
function getViewKind(view: WorkspaceSavedView): WorkspaceSavedViewKind {
  if (view.kind) return view.kind;
  return view.isSystem ? 'system' : 'custom';
}

function getViewKindLabel(view: WorkspaceSavedView): string {
...
function cloneViewState(view: WorkspaceSavedView): WorkspaceSavedView['state'] {
  return {
    ...view.state,
...
function buildShareSnapshot(view: WorkspaceSavedView): string {
  const lines = [
...
export function WorkspaceViewsMenu({
  views,
  activeViewKey,
  onViewSelect,
  onViewDelete,
...
function describeView(view: WorkspaceSavedView): string {
