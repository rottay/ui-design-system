...
// customization skin ownership migration contract. These tests exercise the hooks consumed
...
describe('BrandingPreviewSandbox customization anatomy createElement anatomy', () => {
  it('preserves instance scoping while exposing the full and compact state trees', async () => {
    ...
    await waitFor(() => {
      expect(queryAll(root, '.ds-button--modern')).toHaveLength(4);
    });
    ...
    await waitFor(() => {
      expect(queryAll(compactRoot, '.ds-button--modern')).toHaveLength(4);
    });
...
describe('PatternBrandStudio customization anatomy anatomy and behavior', () => {
  it.each(['modern', 'rustic', 'classic'] as const)(
    ...
    async (engine) => {
      ...
      await screen.findByText(`customization anatomy ${engine} host probe`);
  ...
  it('exposes editor, both preview grounds, contrast and action states while callbacks remain live', async () => {
    ...
    await screen.findByText('customization anatomy Brand Studio');
    const root = container.querySelector('.ds-pattern-brand-studio') as HTMLElement;
