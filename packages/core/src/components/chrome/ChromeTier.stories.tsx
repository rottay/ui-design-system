/**
 * @fileoverview Chrome tier Storybook landing page.
 *
...
const FAMILIES = [
  ...
  { name: 'RecordSummaryStrip / RecordFieldGrid / RecordField / RecordActionBar / RecordPanel', folder: 'record-chrome', desc: 'Record-page building blocks' },
  ...
  { name: 'Metrics/Activity variants', folder: 'dashboard-insight-variants', desc: '8 dashboard insight widget variants + useVariant hook' },
];

function ChromeTierIndex() {
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 800, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>Chrome Tier</h1>
      <p style={{ color: '#666', marginBottom: 24, lineHeight: 1.6 }}>
        Page-chrome families that sit between <strong>patterns</strong> (task-level compositions)
        ...
        <tbody>
          {FAMILIES.map((f) => (
            <tr key={f.folder} style={{ borderBottom: '1px solid #f0f0f0' }}>
              ...
              <td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: 12, color: '#888' }}>chrome/{f.folder}/</td>
      ...
      <p style={{ marginTop: 24, fontSize: 13, color: '#999' }}>
        See <code>docs/chrome-tier.md</code> for the full guide including the
...
const meta: Meta = {
  title: 'Chrome/Overview',
  component: ChromeTierIndex,
  parameters: {
    layout: 'padded',
    docs: { description: { component: 'Index page listing all chrome-tier families.' } },
  },
};
