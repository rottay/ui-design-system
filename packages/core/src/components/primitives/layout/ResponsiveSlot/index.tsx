 ...
 * content at different breakpoints. Uses Show components internally so everything
 * works without JavaScript via server-rendered CSS media queries.
 *
 * @remarks
 ...
 *
 * Renders different React nodes at different viewport sizes using Show
 * components internally. All variants are server-rendered with CSS media
...
function renderDeviceSlots(props: ResponsiveSlotProps): React.ReactElement {
  ...
  if (phoneContent === tabletContent && tabletContent !== desktopContent) {
    ...
    return (
      <>
        <Show below="desktop">{phoneContent}</Show>
        <Show from="desktop">{desktopContent}</Show>
      </>
    );
  }
  ...
  if (tabletContent === desktopContent && phoneContent !== tabletContent) {
    ...
    return (
      <>
        <Show on="phone">{phoneContent}</Show>
        <Show from="tablet">{tabletContent}</Show>
      </>
    );
  }
  ...
  return (
    <>
      <Show on="phone">{phoneContent}</Show>
      <Show on="tablet">{tabletContent}</Show>
      <Show on="desktop">{desktopContent}</Show>
    </>
  );
}
    ...
    <>
      {validGroups.map((group, idx) => {
        ...
        if (fromProp && belowProp) {
          return (
            <Show key={idx} from={fromProp}>
              <Show below={belowProp}>{group.content}</Show>
            </Show>
          );
        }
