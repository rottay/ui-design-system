    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      {(['titan', 'hermes', 'apollo'] as const).map((engine) => (
        <div key={engine}>
          ...
          <Layout engine={engine} style={{ minHeight: '200px' }}>
            <Layout.Header engine={engine} style={headerStyle}>Header</Layout.Header>
            <Layout.Content engine={engine} style={contentStyle}>Content</Layout.Content>
            <Layout.Footer engine={engine} style={footerStyle}>Footer</Layout.Footer>
          </Layout>
        </div>
      ))}
    </div>
