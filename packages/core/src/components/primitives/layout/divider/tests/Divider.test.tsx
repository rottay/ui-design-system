      ...
      const MockDivider = React.forwardRef<HTMLDivElement, any>(
        (
          ...
          const resolvedTextPosition =
            textPosition || orientationMargin || "center";
          ...
          return React.createElement(
            "div",
            {
              ...
              "data-text-position": hasChildren
...
describe("Divider", () => {
  ...
  describe("Text Position", () => {
    ...
    it.each(["left", "center", "right"] as const)(
      ...
      (position) => {
        render(
          <Divider textPosition={position} data-testid="divider">
            Text
          </Divider>
        );
        expect(screen.getByTestId("divider")).toHaveAttribute(
    ...
    it("supports orientationMargin as alias for textPosition", () => {
      render(
        <Divider orientationMargin="left" data-testid="divider">
          Text
        </Divider>
      );
      expect(screen.getByTestId("divider")).toHaveAttribute(
  ...
  describe("Plain Mode", () => {
    ...
    it("sets plain mode when prop is true", () => {
      render(
        <Divider plain data-testid="divider">
          Text
        </Divider>
      );
      expect(screen.getByTestId("divider")).toHaveAttribute(
...
describe("Divider Types and Constants", () => {
  ...
  describe("THICKNESS_MAP", () => {
    it("has correct thickness values", () => {
      expect(THICKNESS_MAP.thin).toBe("var(--ds-divider-thickness-thin, 1px)");
      expect(THICKNESS_MAP.medium).toBe(
        "var(--ds-divider-thickness-medium, 2px)"
      );
      expect(THICKNESS_MAP.thick).toBe(
        "var(--ds-divider-thickness-thick, 3px)"
      );
    });
  });
  ...
  describe("getThicknessValue", () => {
    it("returns thin thickness by default", () => {
      expect(getThicknessValue(undefined)).toBe(
        "var(--ds-divider-thickness-thin, 1px)"
      );
    });

    it("returns correct preset values", () => {
      expect(getThicknessValue("thin")).toBe(
        "var(--ds-divider-thickness-thin, 1px)"
      );
      expect(getThicknessValue("medium")).toBe(
        "var(--ds-divider-thickness-medium, 2px)"
      );
      expect(getThicknessValue("thick")).toBe(
        "var(--ds-divider-thickness-thick, 3px)"
      );
    });
