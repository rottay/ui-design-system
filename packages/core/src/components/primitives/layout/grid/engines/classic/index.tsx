 ...
 * hooks. Responsive templates are projected through private CSS parameters;
 * the shared layout stylesheet owns the media-query cascade.
 *
 ...
 */
const ClassicGrid = forwardRef<HTMLElement, GridProps>((props, ref) => {
  const {
  ...
  const baseStyle = buildGridStyles(props, needsResponsiveCSS);

  const responsiveGrid = needsResponsiveCSS
    ? projectResponsiveGrid(
        resolveColumns,
        hasResponsiveColumns ? columns : undefined,
        hasResponsiveRows ? rows : undefined
      )
    : {
        columns: {},
        rows: {},
        columnBreakpoints: undefined,
        rowBreakpoints: undefined,
      };
  const computedStyle: ResponsiveGridStyle = {
    ...baseStyle,
    "--_ds-grid-columns-xs": responsiveGrid.columns.xs,
    "--_ds-grid-columns-sm": responsiveGrid.columns.sm,
    "--_ds-grid-columns-md": responsiveGrid.columns.md,
    "--_ds-grid-columns-lg": responsiveGrid.columns.lg,
    "--_ds-grid-columns-xl": responsiveGrid.columns.xl,
    "--_ds-grid-columns-2xl": responsiveGrid.columns["2xl"],
    "--_ds-grid-rows-xs": responsiveGrid.rows.xs,
    "--_ds-grid-rows-sm": responsiveGrid.rows.sm,
    "--_ds-grid-rows-md": responsiveGrid.rows.md,
    "--_ds-grid-rows-lg": responsiveGrid.rows.lg,
    "--_ds-grid-rows-xl": responsiveGrid.rows.xl,
    "--_ds-grid-rows-2xl": responsiveGrid.rows["2xl"],
  };
  ...
  return React.createElement(
    ElementType,
    {
      ...
      "data-grid-columns": responsiveGrid.columnBreakpoints,
      "data-grid-rows": responsiveGrid.rowBreakpoints,
    },
    children
  );
});
 ...
 */
const ClassicGridItem = forwardRef<HTMLElement, GridItemProps>((props, ref) => {
  const {
  ...
  return React.createElement(
    ElementType,
    {
      ref: ref as Ref<HTMLElement>,
      className:
        `rottay-grid-item rottay-grid-item--classic ${className}`.trim(),
      style: computedStyle,
      id,
      ...semanticAttributes,
