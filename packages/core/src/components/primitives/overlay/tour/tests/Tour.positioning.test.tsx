...
const MODERN_SURFACE = ".ds-tour--modern [data-part='surface']";
const RUSTIC_SURFACE = ".rottay-tour--rustic [data-part='surface']";

/** Modern carries the measured rect as family channels; rustic writes it inline. */
function spotlightBox(spotlight: HTMLElement, engine: 'modern' | 'rustic') {
  if (engine === 'rustic') return spotlight.style;
  const read = (edge: string) => spotlight.style.getPropertyValue(`--ds-tour-spotlight-${edge}`);
...
describe.each([
...
] as const)('Tour %s engine positioning', (engine, Engine, surfaceSelector) => {
  ...
  it('covers the target with the padded spotlight cutout and places the surface through the shared runtime', () => {
    ...
    const box = spotlightBox(spotlight, engine);
    expect(box.top).toBe('32px');
    expect(box.left).toBe('52px');
    expect(box.width).toBe('196px');
  ...
  it('re-measures cutout and surface together on scroll', () => {
    ...
    expect(spotlightBox(spotlight, engine).top).toBe('132px');
  ...
  it('centers in the viewport when the step has no target', () => {
    ...
    expect(surface.getAttribute('data-ds-position-strategy')).toBe('js');
    if (engine === 'modern') {
      // The skin centers an unanchored surface; nothing is claimed inline.
