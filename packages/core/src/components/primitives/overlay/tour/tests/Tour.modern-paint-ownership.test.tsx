...
const SKIN = readFileSync(
  join(here, '../../../../../foundation/tokens/css/runtime/engines/modern/skin/tour/index.css'),
  'utf8'
).replace(/\/\*[\s\S]*?\*\//g, '');
...
describe('Tour modern engine — the skin owns the drained chrome', () => {
  ...
  it('pins the leveled Pass-2 interaction contract (the retired zero-hover is gone)', () => {
    ...
    expect(SKIN).toMatch(
      /min-block-size:\s*max\(\s*var\(\s*--ds-touch-target-min\s*,\s*44px\s*\)\s*,\s*var\(\s*--ds-tour-touch-target-min\s*,\s*2rem\s*\)\s*\)/
    );
