/**
 * The confirm-dialog family in a real browser: every decision its paint consumes
...
function dialogMarkup(): string {
  const view = render(
    <div>
      <ModernConfirmDialog open title="Archive project?" description="It leaves the board." variant="info" onConfirm={() => {}} onCancel={() => {}} />
    </div>,
  );
...
describe('confirm-dialog direction, language, loading and accessibility', () => {
  ...
  it('names its alert dialog from the title and its actions from the active catalog', () => {
    render(
      <I18nProvider locale="es" fallbackLocale="en">
        <ModernConfirmDialog open title="Archivar proyecto" description="Sale del tablero." onConfirm={() => {}} onCancel={() => {}} />
      </I18nProvider>,
    );
    const dialog = screen.getByRole('alertdialog', { name: 'Archivar proyecto' });
    expect(dialog).toHaveAccessibleDescription('Sale del tablero.');
    expect(document.querySelector("[data-action='confirm']")).toHaveTextContent('Confirmar');
  ...
  it('builds its loading state from its own anatomy', () => {
    render(<ModernConfirmDialog open title="Archive project?" description="It leaves the board." variant="info" onConfirm={() => {}} onCancel={() => {}} />);
