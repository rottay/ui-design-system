...
function CompactItem({ item, index }: { item: ActivityProps["items"][0]; index: number }) {
  ...
  return (
    <Box className="activity-compact-item-v3" data-part="item" data-type={item.type} style={{ padding: "10px 14px", position: "relative", overflow: "hidden", cursor: "pointer" }}>
      <Box data-part="accent-bar" data-type={item.type} style={{ position: "absolute", left: 0, top: 0, bottom: 0, transition: "width 0.3s ease" }} />
      <Flex align="center" gap={12} data-part="item-row" style={{ position: "relative", paddingLeft: 4 }}>
        ...
        <Stack spacing="none" style={{ flex: 1, minWidth: 0 }}>
          <Text size="xs" data-part="item-text" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 500 }}>{item.text}</Text>
...
export function ActivityCompact({ items, schedule: _schedule = [], viewAllHref, viewAllLabel = "View all" }: ActivityProps) {
  return (
    <Box className="ds-activity-compact" data-part="root" style={{ height: 415, padding: "16px", position: "relative", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <Flex align="center" justify="between" data-part="header" style={{ paddingBottom: 12, marginBottom: 12, position: "relative" }}>
        <Flex align="center" gap={10}>
          ...
          <Stack spacing="none">
            <Text weight="bold" data-part="title">Recent Activity</Text>
            <Text size="xs" data-part="update-count" style={{ fontFamily: "monospace", fontSize: 9 }}>{items.length} UPDATES</Text>
          </Stack>
        </Flex>
        {viewAllHref ? (
