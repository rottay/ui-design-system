...
export function ActivityTicker({ items, schedule: _schedule = [], viewAllHref, viewAllLabel = "View all" }: ActivityProps) {
  ...
  return (
    <Box className="ds-activity-ticker" data-part="root" style={{ height: 415, padding: "16px", background: "var(--ds-color-bg-secondary)", border: "1px solid var(--ds-color-border-secondary)", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <Flex align="center" justify="between" data-part="header" style={{ paddingBottom: 12, marginBottom: 12, borderBottom: "1px solid var(--ds-color-border-secondary)", position: "relative" }}>
        <Flex align="center" gap={10}>
          <Box data-part="bell-box" style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, var(--ds-color-primary-100), var(--ds-color-secondary-100))", border: "1px solid var(--ds-color-primary-200)", position: "relative" }}>
          ...
          <Stack spacing="none">
            <Text weight="bold" data-part="title" style={{ color: "var(--ds-color-text-primary)" }}>Live Updates</Text>
      ...
      <Box style={{ flex: 1, display: "flex", flexDirection: "column" }}>
        <Box className="ticker-container" data-part="ticker-body" style={{ flex: 1, padding: "20px", background: "linear-gradient(180deg, var(--ds-color-bg-primary) 0%, var(--ds-color-neutral-50) 100%)", border: "1px solid var(--ds-color-border-secondary)", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          <Box className={"ticker-content-v3 " + (isAnimating ? (direction === "next" ? "ticker-out-left" : "ticker-out-right") : "ticker-in")} style={{ textAlign: "center" }}>
            <Flex align="center" justify="center" style={{ marginBottom: 16 }}>
              <Box data-part="item-icon-box" data-type={currentItem.type} style={{ width: 52, height: 52, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--ds-color-" + currentItem.type + "-100)", border: "2px solid var(--ds-color-" + currentItem.type + ")", position: "relative" }}>
                <IconComponent style={{ width: 24, height: 24, color: "var(--ds-color-" + currentItem.type + ")" }} />
              </Box>
            </Flex>
            <Text size="sm" weight="semibold" data-part="item-text" style={{ color: "var(--ds-color-text-primary)", marginBottom: 10, lineHeight: 1.6, fontSize: 15 }}>{currentItem.text}</Text>
            <Flex align="center" justify="center" gap={8}>
              <Box data-part="item-type-dot" data-type={currentItem.type} style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--ds-color-" + currentItem.type + ")" }} />
              <Text size="xs" data-part="item-time" style={{ color: "var(--ds-color-text-muted)", fontFamily: "monospace", fontSize: 11 }}>{currentItem.time} ago</Text>
            </Flex>
          </Box>

          {items.length > 1 && (
            <Flex align="center" justify="center" gap={16} style={{ marginTop: 24 }}>
              <Box onClick={goToPrev} className="nav-button" data-part="nav-button" style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--ds-color-bg-secondary)", border: "1px solid var(--ds-color-border-secondary)", cursor: "pointer", position: "relative" }}>
                <ChevronLeft style={{ width: 16, height: 16, color: "var(--ds-color-text-muted)" }} />
              </Box>
              <Flex align="center" gap={8}>
                {items.map((item: ActivityItem, i: number) => (
                  <Box key={i} onClick={() => { if (i !== currentIndex) { setDirection(i > currentIndex ? "next" : "prev"); setIsAnimating(true); setTimeout(() => { setCurrentIndex(i); setIsAnimating(false); }, 300); } }} className="ticker-dot" data-part="ticker-dot" data-active={i === currentIndex} data-type={item.type} style={{ width: i === currentIndex ? 24 : 10, height: 10, background: i === currentIndex ? (TYPE_CONFIG[item.type as keyof typeof TYPE_CONFIG]?.gradient || "var(--ds-color-" + item.type + ")") : "var(--ds-color-neutral-200)", cursor: "pointer", transition: "all 0.3s ease", position: "relative" }} />
                ))}
              </Flex>
              <Text size="xs" data-part="ticker-counter" style={{ color: "var(--ds-color-text-muted)", fontFamily: "monospace", fontSize: 10 }}>{currentIndex + 1}/{items.length}</Text>
              <Box onClick={goToNext} className="nav-button" data-part="nav-button" style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--ds-color-bg-secondary)", border: "1px solid var(--ds-color-border-secondary)", cursor: "pointer", position: "relative" }}>
          ...
          {items.length > 1 && (
            <Box data-part="progress-track" style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 3, background: "var(--ds-color-bg-tertiary)", overflow: "hidden" }}>
              <Box className="ticker-progress" data-part="progress-fill" data-type={currentItem.type} key={currentIndex} style={{ height: "100%", background: config.gradient }} />
            </Box>
          )}
        </Box>
      </Box>
