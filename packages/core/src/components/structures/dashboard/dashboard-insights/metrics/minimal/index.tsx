  ...
  return (
    ...
    >
      ...
      <Flex align="center" gap={16} style={{ position: "relative" }}>
        <Box data-part="metric-icon-box" style={{ width: 48, height: 48, display: "flex", alignItems: "center", justifyContent: "center", background: METRIC_CARD_ICON_BG, border: `1px solid ${METRIC_CARD_ICON_BORDER}`, flexShrink: 0, transition: "transform 0.3s ease", transform: isHovered ? "scale(1.05)" : "scale(1)" }}>
          <metric.icon data-part="metric-icon" style={{ width: 22, height: 22, color: iconColor }} />
        </Box>

        <Box style={{ flex: 1, minWidth: 0 }}>
          <Flex align="center" justify="between" style={{ marginBottom: 8 }}>
            <Stack spacing="none">
              <Text size="sm" weight="medium" data-part="metric-label" style={{ color: METRIC_CARD_LABEL_COLOR }}>{metric.label}</Text>
              <Flex align="center" gap={6}>
                {metric.positive ? <TrendingUp data-part="trend-icon" style={{ width: 12, height: 12, color: trendColor }} /> : <TrendingDown data-part="trend-icon" style={{ width: 12, height: 12, color: trendColor }} />}
                <Text size="xs" data-part="metric-change" style={{ color: trendColor, fontFamily: METRIC_MONO_FONT }}>{metric.change}</Text>
              </Flex>
            </Stack>
            <Text data-part="metric-value" style={{ fontSize: 28, fontWeight: 800, color: isHovered ? METRIC_CARD_VALUE_HOVER_COLOR : METRIC_CARD_VALUE_COLOR, fontFamily: METRIC_MONO_FONT, letterSpacing: 0, minWidth: METRIC_CARD_NUMBER_MIN_WIDTH, textAlign: "right", fontVariantNumeric: METRIC_CARD_NUMBER_FONT_VARIANT, transition: "color 0.2s ease" }}>{animatedValue}{suffix}</Text>
          </Flex>
        ...
        <ChevronRight data-part="metric-chevron" style={{ width: 16, height: 16, color: METRIC_CARD_ICON_COLOR, opacity: isHovered ? 1 : 0, transform: isHovered ? "translateX(0)" : "translateX(-8px)", transition: "all 0.3s ease", flexShrink: 0 }} />
      </Flex>
    </Box>
  );
}
...
export function MetricsMinimal({ metrics }: MetricsProps) {
  ...
  return (
    <Box className="ds-metrics-minimal" data-part="root" style={{ height: 415, padding: "16px", background: METRIC_PANEL_BG, border: `1px solid ${METRIC_PANEL_BORDER}`, borderRadius: METRIC_PANEL_RADIUS, boxShadow: METRIC_PANEL_SHADOW, position: "relative", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <Flex align="center" justify="between" data-part="header" style={{ paddingBottom: 12, marginBottom: 12, borderBottom: `1px solid ${METRIC_PANEL_BORDER}`, position: "relative" }}>
        <Flex align="center" gap={10}>
          <Box data-part="panel-icon-box" style={{ width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: METRIC_PANEL_ICON_BG, border: `1px solid ${METRIC_PANEL_ICON_BORDER}` }}>
          ...
          <Stack spacing="none">
            ...
            <Text size="xs" data-part="subtitle" style={{ color: METRIC_PANEL_BODY_COLOR, fontFamily: METRIC_MONO_FONT, fontSize: 9 }}>REAL-TIME DATA</Text>
          </Stack>
        </Flex>
        <Box data-part="live-badge" style={{ padding: "6px 12px", background: METRIC_PANEL_BADGE_BG, border: `1px solid ${METRIC_PANEL_BADGE_BORDER}` }}>
          <Flex align="center" gap={6}>
            <Box className="live-dot" data-part="live-dot" style={{ width: 8, height: 8, borderRadius: "50%", background: METRIC_PANEL_BADGE_COLOR }} />
