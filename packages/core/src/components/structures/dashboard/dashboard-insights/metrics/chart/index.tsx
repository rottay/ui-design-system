...
function ChartRow({ metric, index, maxValue }: { metric: MetricsProps["metrics"][0]; index: number; maxValue: number }) {
  ...
  return (
    <Box
      ...
      style={{ padding: METRIC_CARD_PADDING, position: "relative", overflow: "hidden" }}
    >
      ...
      <Flex align="center" justify="between" style={{ marginBottom: 8, position: "relative" }}>
        <Flex align="center" gap={10}>
          <Box data-part="metric-icon-box" style={{ width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            <metric.icon data-part="metric-icon" style={{ width: 14, height: 14 }} />
          </Box>
          <Stack spacing="none">
            <Text size="sm" weight="medium" data-part="metric-label" style={{ fontSize: 13 }}>{metric.label}</Text>
            <Flex align="center" gap={4}>
              {metric.positive ? <TrendingUp data-part="trend-icon" style={{ width: 10, height: 10 }} /> : <TrendingDown data-part="trend-icon" style={{ width: 10, height: 10 }} />}
              <Text size="xs" data-part="metric-change" style={{ fontFamily: METRIC_MONO_FONT, fontSize: 9 }}>{metric.change}</Text>
            </Flex>
          </Stack>
        </Flex>
        <Text data-part="metric-value" style={{ fontSize: 22, fontWeight: 800, fontFamily: METRIC_MONO_FONT, letterSpacing: 0, minWidth: METRIC_CARD_NUMBER_MIN_WIDTH, textAlign: "right", fontVariantNumeric: METRIC_CARD_NUMBER_FONT_VARIANT, transition: "color 0.2s ease" }}>{animatedValue}{suffix}</Text>
      </Flex>
      ...
      <Flex justify="between" align="center" style={{ marginTop: 4 }}>
        <Text size="xs" data-part="progress-label" style={{ fontFamily: METRIC_MONO_FONT, fontSize: 9 }}>Progress</Text>
        <Text size="xs" weight="bold" data-part="progress-percent" style={{ fontFamily: METRIC_MONO_FONT, fontSize: 9 }}>{Math.round(percentage)}%</Text>
      </Flex>
    </Box>
  );
}
...
export function MetricsChart({ metrics }: MetricsProps) {
  ...
  return (
    <Box className="ds-metrics-chart" data-part="root" style={{ height: 415, padding: "16px", position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <Flex align="center" justify="between" data-part="header" style={{ paddingBottom: 12, marginBottom: 12, position: "relative" }}>
        <Flex align="center" gap={8}>
          <Box data-part="panel-icon-box" style={{ width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Activity data-part="panel-icon" style={{ width: 14, height: 14 }} />
          </Box>
          <Text weight="bold" size="sm" data-part="title">Performance</Text>
        </Flex>
        <Flex align="center" gap={4} data-part="live-badge" style={{ padding: "4px 8px" }}>
          ...
          <Text size="xs" weight="bold" data-part="live-label" style={{ fontFamily: METRIC_MONO_FONT, fontSize: 9 }}>LIVE</Text>
        </Flex>
      </Flex>
