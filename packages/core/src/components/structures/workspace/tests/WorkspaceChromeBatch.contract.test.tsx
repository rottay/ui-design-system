...
// ---------------------------------------------------------------------------
// workspace-chrome anatomy -- the workspace-chrome family (list-toolbar,
...
//
// P-79 EXTENDS TO Input (see communication-pattern anatomy's Card finding for the precedent). Modern
// forwards a caller-owned `data-part` to the painted shell while retaining
// its internal control/affix anatomy. Rustic remains frozen in this tranche,
// so engine-conditional assertions below preserve its existing vocabulary.
//
...
// workspace-chrome anatomy-RUSTIC-PARITY: 9 parameterized cases below now run `['modern'] only.
// Their rustic iteration is ADJUDICATED debt of the separate Classic/Rustic
// parity tranche (engine policy 2026-07-25, independent code audit: Classic/Rustic read-only
...
describe("list-toolbar -- data-part contract (workspace-chrome anatomy)", () => {
  it("stamps root/title/search-icon/filter-trigger/segmented-control/filter-chips-strip (modern)", async () => {
    ...
    expect(q(container, '[data-part="title"]').length).toBeGreaterThanOrEqual(
      1
    );
    expect(
      q(container, '[data-part="search-icon"]').length
    ).toBeGreaterThanOrEqual(1);
    expect(
      q(container, '[data-part="count-badge"]').length
    ).toBeGreaterThanOrEqual(1);
    // one active filter pill, one inactive
    expect(
      q(container, '[data-part="filter-trigger"][data-active="true"]')
    ).toHaveLength(1);
    expect(
      q(container, '[data-part="filter-trigger"][data-active="false"]')
    ).toHaveLength(1);
  ...
  it("opens a filter dropdown and stamps filter-dropdown-item/filter-checkmark (modern)", async () => {
    ...
    await waitFor(() => {
      expect(
        q(document.body, '[data-part="filter-dropdown-item"]').length
      ).toBeGreaterThanOrEqual(1);
    });
    expect(
      q(
        document.body,
        '[data-part="filter-dropdown-item"][data-selected="true"]'
      ).length
    ).toBeGreaterThanOrEqual(1);
    expect(
      q(document.body, '[data-part="filter-checkmark"]').length
    ).toBeGreaterThanOrEqual(1);
  });
});
...
describe("saved-views -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(ENGINES)(
    "stamps root/pill/default-star/menu-trigger + opens menu-panel/menu-item (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
      ...
      expect(
        q(container, `[data-part="${pillPart}"][data-active="true"]`)
      ).toHaveLength(1);
      expect(
      ...
      if (engine === "modern") {
        expect(
          q(container, '[data-part="unsaved-dot"][data-dirty="true"]')
        ).toHaveLength(1);
      } else {
      ...
      const menuRoot =
        engine === "modern" ? document.documentElement : container;
      const surfacePart = engine === "modern" ? "surface" : "menu-panel";
      const itemPart = engine === "modern" ? "item" : "menu-item";
      await waitForPart(menuRoot, surfacePart);
      expect(
        q(menuRoot, `[data-part="${surfacePart}"]`)[0].getAttribute("data-open")
      ).toBe("true");
      expect(
        q(menuRoot, `[data-part="${itemPart}"]:not([data-danger="true"])`).length
      ).toBeGreaterThanOrEqual(1);
      expect(
        q(
          menuRoot,
          engine === "modern"
            ? `[data-part="${itemPart}"][data-tone="danger"]`
            : `[data-part="${itemPart}"][data-danger="true"]`
        ).length
      ).toBeGreaterThanOrEqual(1);
      // Modern composes Dropdown and therefore consumes its canonical
      // anatomy; Rustic keeps the frozen pattern-local vocabulary.
      expect(
        q(menuRoot, '[data-part="divider"]').length
      ).toBeGreaterThanOrEqual(1);
    }
  );
  ...
  it.each(ENGINES)(
    "stamps the loading branch as its own root (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <PatternSavedViewsBar
          views={[]}
...
// ===========================================================================
describe("status-filter-pills -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(["modern"] as const)(
    "stamps root/pill/pill-label/count-badge, all keyed on data-selected (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <StatusFilterPills
          options={[
      ...
      expect(
        q(container, '[data-part="pill"][data-selected="true"]')
      ).toHaveLength(1);
      expect(
        q(container, '[data-part="pill"][data-selected="false"]')
      ).toHaveLength(1);
      expect(q(container, ".ds-status-filter-pills__pill")).toHaveLength(2);
      expect(q(container, '[data-part="pill-label"]')).toHaveLength(2);
      expect(
        q(container, '[data-part="count-badge"][data-selected="true"]')
      ).toHaveLength(1);
...
// ===========================================================================
// column-menu -- canonical Popover owns the portal/focus/dismissal contract
// ===========================================================================
describe("column-menu -- canonical Popover composition contract", () => {
  it.each(["modern"] as const)(
    "trigger carries structure ownership; canonical surface carries panel ownership (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ColumnMenu
          columns={[
      ...
      // The public Popover portals a governed surface. ColumnMenu contributes
      // only its content anatomy; it must not recreate a backdrop or portal.
      await waitFor(() => {
        expect(
          document.querySelector('.ds-column-menu-panel[data-part="surface"]')
        ).not.toBeNull();
        expect(
          document.querySelector(
            '.ds-column-menu-panel[data-part="surface"] [data-part="panel"]'
          )
        ).not.toBeNull();
      });
      const surface = document.querySelector(
        '.ds-column-menu-panel[data-part="surface"]'
      ) as HTMLElement;
      const panel = surface.querySelector('[data-part="panel"]') as HTMLElement;
      ...
      expect(surface.getAttribute("aria-label")).toBe("Table columns");
      ...
      expect(
        document.querySelector('[data-part="row"][data-visible="true"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="row"][data-visible="false"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="width-badge"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="drag-handle"]')
      ).not.toBeNull();
      ...
      fireEvent.click(
        document.querySelector('[data-part="width-badge"]') as Element
      );
      await waitFor(() => {
        expect(
          document.querySelector('[data-part="width-input"]')
        ).not.toBeNull();
      });
    }
  );
  ...
  it.each(["modern"] as const)(
    "R2: trigger is a dialog disclosure, the panel is a labelled dialog, Escape closes and refocuses (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ColumnMenu
          columns={[
      ...
      await waitFor(() => {
        ...
        expect(document.querySelector('[data-part="row"]')).not.toBeNull();
      });
  ...
  it.each(["modern"] as const)(
    "groups render group-toggle/group-toggle-label + action rows render action-row/action-section (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ColumnMenu
          columns={[
      ...
      await waitFor(() => {
        expect(
          document.querySelector('[data-part="group-toggle"]')
        ).not.toBeNull();
      });
      expect(
        document.querySelector('[data-part="group-toggle-label"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="action-section"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="action-section-label"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="action-row"][data-visible="true"]')
      ).not.toBeNull();
    }
  );
});
...
// ===========================================================================
describe("saved-views-menu -- data-part contract (workspace-chrome anatomy), portal-panel standalone class", () => {
  it.each(["modern"] as const)(
    "trigger carries ds-structure ds-saved-views-menu; opened panel carries ds-structure ds-saved-views-menu-panel STANDALONE (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SavedViewsMenu
          views={[
      ...
      expect(
        document.querySelector('[data-part="active-card"]')
      ).not.toBeNull();
      expect(
        document.querySelectorAll(
          '[data-part="status-pill"][data-tone="primary"]'
        ).length
      ).toBeGreaterThanOrEqual(1);
      expect(
        document.querySelectorAll(
          '[data-part="status-pill"][data-tone="neutral"]'
        ).length
      ).toBeGreaterThanOrEqual(1);
      expect(
        document.querySelectorAll('[data-part="view-item"][data-active="true"]')
      ).toHaveLength(1);
      expect(
        document.querySelectorAll(
          '[data-part="view-item"][data-active="false"]'
        )
      ).toHaveLength(1);
      expect(
        document.querySelector('[data-part="checkmark"][data-active="true"]')
      ).not.toBeNull();
      expect(document.querySelector('[data-part="delete"]')).not.toBeNull();
      expect(
        document.querySelectorAll('[data-part="glyph"]').length
      ).toBeGreaterThanOrEqual(2);
      expect(
        document.querySelectorAll('[data-part="count-pill"]').length
      ).toBeGreaterThanOrEqual(3);
      expect(
        document.querySelectorAll('[data-part="action-button"]').length
      ).toBeGreaterThanOrEqual(1);
      expect(
        document.querySelectorAll('[data-part="section-header"]').length
      ).toBeGreaterThanOrEqual(1);
    }
  );

  it.each(["modern"] as const)(
    "R2: trigger is a dialog disclosure, the panel is a labelled dialog with header-title, Escape closes (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SavedViewsMenu
          views={[
      ...
      expect(
        document.querySelector('[data-part="header-title"]')?.textContent
      ).toBe("Saved views");
      expect(
        document.querySelector('[data-part="active-card-label"]')?.textContent
      ).toBe("All");
  ...
  it.each(["modern"] as const)(
    "stamps empty-state when there are no views (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SavedViewsMenu
          views={[]}
      ...
      await waitFor(() => {
        expect(
          document.querySelectorAll('[data-part="empty-state"]').length
        ).toBeGreaterThanOrEqual(1);
      });
      expect(
        document.querySelector('[data-part="empty-state-icon"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="empty-state-description"]')
      ).not.toBeNull();
    }
  );
});
...
// ===========================================================================
describe("export-button -- data-part contract (workspace-chrome anatomy), portal-panel standalone class + data-export-item preserved", () => {
  it.each(["modern"] as const)(
    "root carries ds-structure ds-export-button; opened panel carries ds-structure ds-export-button-panel STANDALONE; data-export-item + data-part coexist (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ExportButton
          data={[{ a: 1 }]}
      ...
      items.forEach((item) =>
        expect(item.hasAttribute("data-export-item")).toBe(true)
      );
      expect(
        document.querySelector('[data-part="menu-item"] [data-part="icon"]')
      ).not.toBeNull();
      expect(
        document.querySelector('[data-part="menu-item"] [data-part="label"]')
      ).not.toBeNull();
    }
  );

  it.each(["modern"] as const)(
    "R2: menu labels resolve to the English i18n floor and item typography is skin-owned (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ExportButton
          data={[{ a: 1 }]}
      ...
      const labels = [
        ...document.querySelectorAll(
          '[data-part="menu-item"] [data-part="label"]'
        ),
      ].map((n) => n.textContent);
...
// active-filters-bar
// ===========================================================================
describe("active-filters-bar -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(["modern"] as const)(
    "stamps root/pill/chip/chip-label/chip-value/chip-remove/clear-all/add-filter (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ActiveFiltersBar
          activeFilters={[
      ...
      for (const part of [
        ...
        expect(
          q(container, `[data-part="${part}"]`).length,
          `must stamp "${part}"`
        ).toBeGreaterThanOrEqual(1);
      }
    }
  ...
  it.each(["modern"] as const)(
    "R2: root is a labelled region, strings resolve to the English i18n floor, and typography is skin-owned (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ActiveFiltersBar
          activeFilters={[
      ...
      expect(q(container, '[data-part="pill"]')[0].textContent).toContain(
        "1 active"
      );
      expect(q(container, '[data-part="clear-all"]')[0].textContent).toContain(
        "Clear all"
      );
      expect(q(container, '[data-part="add-filter"]')[0].textContent).toContain(
        "Add filter"
      );
      expect(
        q(container, '[data-part="close"]')[0].getAttribute("aria-label")
      ).toBe("Remove filter Status");
  ...
  it.each(["modern"] as const)(
    "returns null (no root) when there are no active filters (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ActiveFiltersBar
          activeFilters={[]}
...
describe("scope-switcher -- data-part contract (workspace-chrome anatomy)", () => {
  ...
  it.each(["modern"] as const)(
    ...
    async (engine) => {
      ...
      expect(
        q(container, '[data-part="option"][aria-checked="false"]')
      ).toHaveLength(1);
      const option = q(container, '[data-part="option"]')[0] as HTMLElement;
...
// view-mode-switcher
// ===========================================================================
describe("view-mode-switcher -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(["modern"] as const)(
    "stamps root/button, data-selected + data-disabled (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ViewModeSwitcher
          modes={[
      ...
      expect(
        q(container, `[data-part="${optionPart}"][data-selected="true"]`)
      ).toHaveLength(1);
      expect(
        q(container, `[data-part="${optionPart}"][data-selected="false"]`)
      ).toHaveLength(1);
      expect(
        q(container, `[data-part="${optionPart}"][data-disabled="true"]`)
      ).toHaveLength(1);
      // aria-checked stays the untouched a11y attribute (DRAFTER NOTE 8 -- do
      // not couple CSS to it, but do not remove it either).
      const active = q(
        container,
        `[data-part="${optionPart}"][data-selected="true"]`
      )[0];
  ...
  it.each(["modern"] as const)(
    "R2: radiogroup label hits the English i18n floor; disabled dim + motion are skin-owned (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <ViewModeSwitcher
          modes={[
...
// table-toolbar
// ===========================================================================
describe("table-toolbar -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(["modern"] as const)(
    "stamps root/divider/search-icon/search-input (%s)",
    async (engine) => {
      mockMatchMedia(1280);
      const { container } = renderWithEngine(
        <TableToolbar
      ...
      const searchInputPaintNode = container.querySelector(
        engine === "modern"
          ? '[data-part="search-input"]'
          : ".ds-table-toolbar__search-input"
      ) as HTMLElement;
      ...
      if (engine === "modern") {
        ...
        expect(
          searchInputPaintNode.matches(
            '.rottay-input.rottay-input--rustic[data-part="root"]'
          )
        ).toBe(true);
      }
  ...
  it.each(["modern"] as const)(
    "R2: the search placeholder resolves to the English i18n floor and the icon uses logical positioning (%s)",
    async (engine) => {
      mockMatchMedia(1280);
      const { container } = renderWithEngine(
        <TableToolbar search="" onSearchChange={() => undefined} />,
        engine
      );
      await waitForPart(container, "root");
      await waitFor(() =>
...
// ===========================================================================
describe("search-command-bar -- data-part contract (workspace-chrome anatomy)", () => {
  it.each(["modern"] as const)(
    "stamps root/suggestion-chip/search-shell/search-icon/input/status/suggestions/actions-slot (%s)",
    async (engine) => {
      mockMatchMedia(1280);
      const { container } = renderWithEngine(
        <SearchCommandBar
      ...
      const commandInputPaintNode = container.querySelector(
        '[data-part="input"]'
      ) as HTMLElement;
  ...
  it.each(["modern"] as const)(
    "stamps suggestion-chip/suggestions/suggestions-label when suggestions are provided (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SearchCommandBar
          command={{
  ...
  it.each(["modern"] as const)(
    "R2: Smart-refine label hits the English i18n floor; input + chip typography are skin-owned; icon uses logical positioning (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SearchCommandBar
          command={{
      ...
      await waitForPart(container, "root");
      expect(
        q(container, '[data-part="suggestions-label"]')[0].textContent
      ).toBe("Smart refine");
      const label = q(
        container,
      ...
      const inputPaintNode = container.querySelector(
        ".ds-search-command-bar__input"
      ) as HTMLElement;
  ...
  it.each(ENGINES)(
    "stamps clear when there is a value (%s)",
    async (engine) => {
      const { container } = renderWithEngine(
        <SearchCommandBar
          command={{
...
function rawColorLiterals(css: string): string[] {
  ...
  const withoutVarFallbacks = withoutComments.replace(/var\([^)]*\)/g, "");
...
describe("workspace chrome skins — R2 structural contract", () => {
  it.each(ALL_BATCH_SKINS)(
    "%s carries zero raw color literals outside documented var() escape hatches",
    (name) => {
      expect(rawColorLiterals(readSkin(name))).toEqual([]);
    }
  );

  it.each(MOTION_SKINS)(
    "%s routes motion through --ds-motion-* channels and guards prefers-reduced-motion",
    (name) => {
  ...
  it.each([
    ...
    (name) => {
      ...
      expect(css).toMatch(/COMPOSITION LAW/);
    }
  );
