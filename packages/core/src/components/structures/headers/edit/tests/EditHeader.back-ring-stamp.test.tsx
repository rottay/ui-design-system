...
const header = () => (
  <EditHeader icon={Icon} title="Edit" backHref="/back" onSave={noop} onCancel={noop} />
);

afterEach(cleanup);

describe('edit-header back-chip focus ring stamp', () => {
  it('edit-header renders the anchor and the chip this suite reads', async () => {
  ...
  it('edit-header leaves the chip silent at rest', async () => {
    const { container } = renderWithEngine(header(), 'modern');
    await waitFor(() => expect(container.querySelector('[data-part="back-button"]')).not.toBeNull());
  ...
  it('edit-header forwards the anchor keyboard focus onto the chip', async () => {
    const { container } = renderWithEngine(header(), 'modern');
    await waitFor(() => expect(container.querySelector('[data-part="back-button"]')).not.toBeNull());
  ...
  it('edit-header withholds the ring from a pointer-driven anchor focus', async () => {
    const { container } = renderWithEngine(header(), 'modern');
    await waitFor(() => expect(container.querySelector('[data-part="back-button"]')).not.toBeNull());
  ...
  it('edit-header writes the stamp onto the anchor itself, so a host Link receives it', async () => {
    const { container } = renderWithEngine(header(), 'modern');
    await waitFor(() => expect(container.querySelector('[data-part="back-button"]')).not.toBeNull());
  ...
  it('edit-header keeps the chip hover/press on the chip, not on the anchor', async () => {
    const { container } = renderWithEngine(header(), 'modern');
    await waitFor(() => expect(container.querySelector('[data-part="back-button"]')).not.toBeNull());
