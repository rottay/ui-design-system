...
/**
 * SprintBoard - By Sprint Preset
 * Notion-style task board grouped by sprint with progress bars
 */
...
export const BySprintSprintBoard = createPreset<SprintBoardProps>({
  name: 'SprintBoard.BySprint',
  render: ({ primitives, props, tokens, engine }: PresetContext<SprintBoardProps>) => {
    ...
    const defaultColumns = columns ?? [
      ...
      { key: 'assignee', label: 'Assign', icon: '👤', width: 120 },
      { key: 'due', label: 'Due', icon: '📅', width: 140 },
      { key: 'sprint', label: 'Sprint', icon: '⚡', width: 120 },
      { key: 'project', label: 'Project', icon: '🚀', width: 160 },
    ];
    ...
    const renderCellValue = (task: SprintTask, colKey: string) => {
      switch (colKey) {
        ...
        case 'assignee': return task.assignee ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: tokens.spacing[1], fontSize: tokens.typography.fontSize.sm }}>
            {task.assignee.avatar ? (
              ...
              <span style={{ width: tokens.spacing[5], height: tokens.spacing[5], borderRadius: tokens.borderRadius.full, backgroundColor: tokens.colors.warningScale[400], color: tokens.colors.common.white, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: tokens.typography.fontSize.xs }}>
        ...
        case 'sprint': return task.sprint ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: tokens.spacing[1], fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[600] }}>⚡ {task.sprint}</span>
        ) : <span style={{ color: tokens.colors.neutral[400] }}>—</span>;
        case 'project': return task.project ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: tokens.spacing[1], fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[600] }}>🚀 {task.project}</span>
        ) : <span style={{ color: tokens.colors.neutral[400] }}>—</span>;
        default: return null;
      }
    };
    ...
    return (
      <Box className={className} style={{
        ...
        {/* Left Sidebar */}
        <Box style={{ width: 220, borderRight: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[200]}`, display: 'flex', flexDirection: 'column', backgroundColor: tokens.colors.neutral[50], flexShrink: 0 }}>
          <Box style={{ padding: `${tokens.spacing[3]}px ${tokens.spacing[3]}px`, borderBottom: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[200]}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[2], fontWeight: tokens.typography.fontWeight.semibold, fontSize: tokens.typography.fontSize.sm }}>
              <span style={{ width: tokens.spacing[5], height: tokens.spacing[5], borderRadius: tokens.borderRadius.sm, backgroundColor: tokens.colors.secondaryScale[600], color: tokens.colors.common.white, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: tokens.typography.fontSize.xs }}>H</span>
              Hooli
              <span style={{ marginLeft: 'auto', color: tokens.colors.neutral[400] }}>⌄</span>
              <span style={{ color: tokens.colors.neutral[400], fontSize: tokens.typography.fontSize.sm }}>«</span>
            </div>
          </Box>
          <Box style={{ padding: tokens.spacing[2], flex: 1 }}>
            <Stack direction="vertical" spacing="none">
              {['Search', 'Updates', 'All teamspaces', 'Settings & members'].map((item) => (
                <div key={item} style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[2]}px`, borderRadius: tokens.borderRadius.sm, fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[600], cursor: 'pointer' }}>{item}</div>
              ))}
              <div style={{ fontSize: tokens.typography.fontSize.xs, fontWeight: tokens.typography.fontWeight.semibold, color: tokens.colors.neutral[400], textTransform: 'uppercase', padding: `${tokens.spacing[3]}px ${tokens.spacing[2]}px ${tokens.spacing[1]}px`, letterSpacing: '0.5px' }}>Teamspaces</div>
              {[
                { label: 'General', bold: true },
                { label: 'Sprints', indent: true },
                { label: 'Projects', indent: true },
                { label: 'Tasks', indent: true, active: true },
                { label: 'Meetings', indent: true },
                { label: 'Docs', indent: true },
              ].map((item) => (
                <div key={item.label} style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[2]}px`, paddingLeft: item.indent ? tokens.spacing[6] : tokens.spacing[2], borderRadius: tokens.borderRadius.sm, fontSize: tokens.typography.fontSize.sm, color: item.active ? tokens.colors.neutral[900] : tokens.colors.neutral[600], backgroundColor: item.active ? tokens.colors.neutral[200] : 'transparent', fontWeight: item.bold || item.active ? tokens.typography.fontWeight.semibold : tokens.typography.fontWeight.normal, cursor: 'pointer' }}>{item.label}</div>
              ))}
              <div style={{ fontSize: tokens.typography.fontSize.xs, fontWeight: tokens.typography.fontWeight.semibold, color: tokens.colors.neutral[400], textTransform: 'uppercase', padding: `${tokens.spacing[3]}px ${tokens.spacing[2]}px ${tokens.spacing[1]}px`, letterSpacing: '0.5px' }}>Private</div>
              {['Getting Started', 'Templates', 'Import', 'Trash'].map((item) => (
                <div key={item} style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[2]}px`, borderRadius: tokens.borderRadius.sm, fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[500], cursor: 'pointer' }}>{item}</div>
              ))}
            </Stack>
          </Box>
        </Box>
        ...
        <Box style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <Box style={{ padding: `${tokens.spacing[3]}px ${tokens.spacing[6]}px ${tokens.spacing[0]}px`, borderBottom: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[200]}` }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: tokens.spacing[3] }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[2] }}>
                {headerActions}
                <button style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[4]}px`, borderRadius: tokens.borderRadius.md, border: 'none', backgroundColor: tokens.colors.primaryScale[600], color: tokens.colors.common.white, fontSize: tokens.typography.fontSize.sm, fontWeight: tokens.typography.fontWeight.semibold, cursor: 'pointer' }}>
                  New <span style={{ fontSize: tokens.typography.fontSize.xs }}>▼</span>
                </button>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[0] }}>
              {tabs.map((tab) => (
              ...
              <button style={{ padding: `${tokens.spacing[2]}px ${tokens.spacing[2]}px`, border: 'none', backgroundColor: 'transparent', color: tokens.colors.neutral[400], cursor: 'pointer' }}>+</button>
            </div>
          </Box>
          ...
          <Box style={{ padding: `${tokens.spacing[2]}px ${tokens.spacing[6]}px`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[100]}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[1] }}>
              {filters.map((filter) => (
                <button key={filter.key} onClick={() => onFilterChange?.(filter.key, '')} style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[2]}px`, borderRadius: tokens.borderRadius.sm, border: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[200]}`, backgroundColor: tokens.colors.common.white, fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[600], cursor: 'pointer', display: 'flex', alignItems: 'center', gap: tokens.spacing[1] }}>
                  {filter.label} <span style={{ fontSize: tokens.typography.fontSize.xs }}>▼</span>
                </button>
              ))}
              <button onClick={onAddFilter} style={{ padding: `${tokens.spacing[1]}px ${tokens.spacing[2]}px`, border: 'none', backgroundColor: 'transparent', fontSize: tokens.typography.fontSize.sm, color: tokens.colors.neutral[400], cursor: 'pointer' }}>+ Add filter</button>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[2] }}>
              ...
              {searchable && <span style={{ cursor: 'pointer' }}>🔍</span>}
              <span style={{ cursor: 'pointer', color: tokens.colors.neutral[500] }}>⋯</span>
            </div>
          </Box>
            ...
            {loading ? (
              ...
              <Stack direction="vertical" spacing="md">
                {groups.map((group) => {
                  ...
                  const progress = group.tasks.length > 0 ? (doneCount / group.tasks.length) * 100 : 0;
                  ...
                  return (
                    <Box key={group.id} style={{ marginTop: tokens.spacing[4] }}>
                      ...
                      <div onClick={() => toggleGroup(group.id)} style={{ display: 'flex', alignItems: 'center', gap: tokens.spacing[2], padding: `${tokens.spacing[2]}px ${tokens.spacing[0]}px`, cursor: 'pointer', userSelect: 'none' }}>
                        ...
                        <span style={{ marginLeft: tokens.spacing[1], color: tokens.colors.neutral[400], cursor: 'pointer' }}>⋯</span>
                        <span style={{ color: tokens.colors.neutral[400], cursor: 'pointer' }}>+</span>
                        {/* Progress bar */}
                        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: tokens.spacing[2] }}>
                          <div style={{ width: 100, height: tokens.spacing[1], backgroundColor: tokens.colors.neutral[200], borderRadius: tokens.borderRadius.sm, overflow: 'hidden' }}>
                            <div style={{ width: `${progress}%`, height: '100%', backgroundColor: tokens.colors.successScale[400], borderRadius: tokens.borderRadius.sm, transition: 'width 0.3s' }} />
                          </div>
                          <span style={{ fontSize: tokens.typography.fontSize.xs, color: tokens.colors.neutral[500] }}>{Math.round(progress)}%</span>
                        </div>
                      </div>
                      ...
                      {!isCollapsed && (
                        <>
                          ...
                          {/* New task row */}
                          <div onClick={() => onNewTask?.(group.id)} style={{ padding: `${tokens.spacing[2]}px ${tokens.spacing[2]}px`, color: tokens.colors.neutral[400], fontSize: tokens.typography.fontSize.sm, cursor: 'pointer', borderBottom: `${tokens.surface.borderWidth} ${tokens.surface.borderStyle} ${tokens.colors.neutral[100]}` }}>
                            + New
                          </div>
