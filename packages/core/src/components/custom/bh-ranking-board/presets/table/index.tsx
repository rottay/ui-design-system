...
export const TableBhRankingBoard = createPreset<BhRankingBoardProps>({
  ...
  render: ({ primitives, props, tokens: t }: PresetContext<BhRankingBoardProps>) => {
    ...
    const toggleSort = (col: RankingSortBy) => {
    ...
    const SortIcon = ({ col }: { col: RankingSortBy }) => (
      <span style={{ color: sortBy === col ? t.colors.primaryScale[600] : t.colors.neutral[300], display: 'inline-flex', marginLeft: 2 }}>
    ...
    return (
      <Box className={className} style={{
        ...
        <Box style={{
          ...
          <Box>
            <Text style={{ fontSize: t.typography.fontSize.lg, fontWeight: t.typography.fontWeight.semibold, color: t.colors.neutral[900] }}>{jobName} - Rankings</Text>
            <Text style={{ fontSize: t.typography.fontSize.xs, color: t.colors.neutral[500], marginTop: 2 }}>{filtered.length} candidates ranked</Text>
          </Box>
        ...
        {/* Distribution chart */}
        <Box style={{ padding: `${t.spacing[4]}px ${t.spacing[6]}px`, borderBottom: `1px solid ${t.colors.neutral[100]}`, backgroundColor: t.colors.neutral[50] }}>
          <Text style={{ fontSize: t.typography.fontSize.xs, fontWeight: t.typography.fontWeight.semibold, color: t.colors.neutral[500], marginBottom: t.spacing[2] }}>Score Distribution</Text>
          <Box style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height: 40 }}>
        ...
        <Box style={{ padding: `${t.spacing[3]}px ${t.spacing[6]}px`, borderBottom: `1px solid ${t.colors.neutral[100]}` }}>
          <Box style={{ position: 'relative', maxWidth: 320 }}>
            ...
            <input type="text" placeholder="Search candidates..." value={search}
              onChange={e => { onFilterChange?.({ ...fp, searchQuery: e.target.value }); setISearch(e.target.value); }}
              style={{
        ...
        <Box style={{ flex: 1, overflow: 'auto' }}>
          ...
          <Box style={{
            ...
            <Box />
            {[
              { label: 'Rank', col: 'rank' as RankingSortBy, w: undefined },
              ...
              <Box key={i} onClick={() => h.col && toggleSort(h.col)}
                style={{
          ...
          {filtered.map(c => {
            ...
            return (
              <Box key={c.id}>
                <Box style={{
                ...
                }} onClick={() => { onCandidateExpand?.(isExp ? null : c.id); if (!ecp) setIExpanded(isExp ? null : c.id); }}>
                  {/* Select */}
                  <Box style={{ display: 'flex', alignItems: 'center' }}>
                    <input type="checkbox" checked={isSel} onChange={() => toggleSelect(c.id)} onClick={e => e.stopPropagation()}
                    ...
                    <Box>
                      ...
                      {c.hasKnockout && (
                        <Box style={{ display: 'flex', alignItems: 'center', gap: 2, marginTop: 1 }}>
                          ...
                          <Text style={{ fontSize: 10, color: t.colors.errorScale[600] }}>Knockout</Text>
                        </Box>
                      )}
                    </Box>
                  </Box>
                  ...
                  <Box style={{ display: 'flex', alignItems: 'center', gap: t.spacing[1] }}>
                    ...
                    <Text style={{ fontSize: 10, color: t.colors.neutral[500], fontWeight: t.typography.fontWeight.medium }}>{c.completionPercent}%</Text>
                  </Box>
                  {/* Decision */}
                  <Box style={{ display: 'flex', alignItems: 'center' }}>
                    <Box style={{
                ...
                {isExp && (
                  <Box style={{
                    ...
                    <Box style={{ flex: 1 }}>
                      ...
                      {c.strengths.map((s, i) => (
                        <Text key={i} style={{ fontSize: t.typography.fontSize.xs, color: t.colors.neutral[600], marginBottom: 2 }}>- {s}</Text>
                      ))}
                    </Box>
                    <Box style={{ flex: 1 }}>
                      ...
                      {c.weaknesses.map((w, i) => (
                        <Text key={i} style={{ fontSize: t.typography.fontSize.xs, color: t.colors.neutral[600], marginBottom: 2 }}>- {w}</Text>
                      ))}
                    </Box>
                    <Box style={{ display: 'flex', gap: t.spacing[2], alignItems: 'flex-start' }}>
