...
function getItemReviewedBy(item: ProctoringEventListItem): string | undefined { return item.reviewedBy ?? item.event?.reviewedBy ?? undefined; }
function getItemReviewNotes(item: ProctoringEventListItem): string | undefined { return item.reviewNotes ?? item.event?.reviewNotes ?? undefined; }
                    ...
                    <Box>
                      {itemReviewed ? (
                        <Box style={{ display: 'flex', flexDirection: 'column' as const, gap: t.spacing[1] }}>
                          ...
                          {getItemReviewedBy(item) && (
                            <Text style={{ fontSize: t.typography.fontSize.xs, color: t.colors.neutral[400] }}>
                              by {getItemReviewedBy(item)}
                            </Text>
                          )}
                        </Box>
                      ) : (
