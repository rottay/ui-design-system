/**
 * The form-surface family in a real browser: the ruled alert the skin paints
 ...
 * does not emit `--ds-form-surface-*`: every probe below targets paint that
...
import { deriveFormSurfaceChannels } from '@/infrastructure/compilers/runtime/theme/runtime/lowering/runtime/derivation/chrome/form-surface';
import {
  AXE_SCOPES,
...
const TENANT: TenantConfig = {
  slug: 'form-surface-causality',
  name: 'Form surface causality',
  theme: 'base',
  locale: 'en',
  fallbackLocale: 'en',
  plan: 'enterprise',
  features: [],
  branding: { companyName: 'Form surface causality' },
};
...
const ROOT = "#page .ds-form-surface[data-part='root']";
const DESCRIPTION = "#page .ds-form-surface [data-part='description']";
const BANNER = "#page .ds-form-surface [data-part='error-banner']";
const ACTIONS = "#page .ds-form-surface [data-part='actions']";
...
describe('form-surface causality surface', () => {
  it('serves the anatomy every probe reads', () => {
    for (const part of ['root', 'description', 'error-banner', 'actions']) {
      expect(rendered, part).toContain(`data-part="${part}"`);
    }
    // The ruled alert replaced the retired error card frame.
    expect(rendered).not.toContain('ds-form-surface__error-card');
  });
