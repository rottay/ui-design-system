/**
 * The detail-form-surface family in a real browser: the ruled alert the skin
 * paints on the stamped anatomy moves with the decision planes that already
 * reach it, the alert's stated geometry holds its negative control, and axe
 * holds on the server-rendered page.
 *
 * The family's own chrome deriver is deliberately NOT registered yet (the DT
 * integrates `derivation/index.ts` after this lot), so the productive compile
 * does not emit `--ds-detail-form-surface-*`: every probe below targets paint
 * that resolves through already-registered planes (`--ds-color-error`,
 * `--ds-edge-emphasis-width`, the spacing ramp). The family-channel probe
 * lands with DT registration; until then the deriver's resting value is
...
import { deriveDetailFormSurfaceChannels } from '@/infrastructure/compilers/runtime/theme/runtime/lowering/runtime/derivation/chrome/detail-form-surface';
import {
  AXE_SCOPES,
...
const TENANT: TenantConfig = {
  slug: 'detail-form-surface-causality',
  name: 'Detail form surface causality',
  theme: 'base',
  locale: 'en',
  fallbackLocale: 'en',
  plan: 'enterprise',
  features: [],
  branding: { companyName: 'Detail form surface causality' },
};
      ...
      <DetailFormSurface
        config={{
          ...
          presentation: {
            ...
            description: 'Adjust the workspace settings.',
...
const BANNER = "#page .ds-detail-form [data-part='error-banner']";
...
describe('detail-form-surface causality surface', () => {
  it('serves the anatomy every probe reads', () => {
    for (const part of ['root', 'error-banner']) {
      expect(rendered, part).toContain(`data-part="${part}"`);
    }
    // The ruled alert replaced the retired error card frame.
    expect(rendered).not.toContain('ds-detail-form__error-card');
  });
  ...
  it('rests every family channel at the single value the skin reads it with', () => {
    ...
    expect(deriveDetailFormSurfaceChannels()).toEqual({
      '--ds-detail-form-surface-error-banner-padding':
