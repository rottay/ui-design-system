...
import { ToastProvider, useToastContext } from '../runtime/state/provider';
import { toast, useToast } from '../runtime/state/provider/use-toast';
import { clearToastMethods, getToastMethods, setToastMethods } from '../runtime/state/method-registry';
...
describe('Toast utilities advanced coverage', () => {
  ...
  it('resolves animation helpers while the legacy injector remains a no-op', async () => {
    vi.resetModules();
    const { getAnimationName, getToastAnimationStyle, injectToastStyles } = await import('../runtime/animation');
