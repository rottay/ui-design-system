...
describe("Box runtime engines", () => {
  ...
  it.each(Object.entries(ENGINE_COMPONENTS))(
    ...
    (engine, Component) => {
      const { element, ref } = renderMaximalBox(
        Component,
        `box-full-${engine}`
      );
  ...
  it.each([
    ...
    "preserves caller-owned radius/shadow precedence in the %s skin contract",
    (engine, Component) => {
      render(
        <>
          <Component
  ...
  it.each(Object.entries(ENGINE_COMPONENTS))(
    "projects responsive height constraints and overflow without leaking objects in the %s engine",
    (engine, Component) => {
      const { container } = render(
        <Component
          data-testid={`box-responsive-geometry-${engine}`}
          height={{ xs: "10rem", lg: "20rem" }}
          minHeight={{ xs: 0, md: 240 }}
          maxHeight={{ xs: 480, xl: 720 }}
          overflow={{ xs: "auto", lg: "visible" }}
          overflowX={{ xs: "hidden", lg: "clip" }}
