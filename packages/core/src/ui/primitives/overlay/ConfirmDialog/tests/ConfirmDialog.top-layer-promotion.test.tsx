/**
 * @fileoverview ConfirmDialog modern -- top-layer PROMOTION contract.
 *
...
describe('ConfirmDialog modern -- top-layer promotion', () => {
  it('promotes the dialog on the first commit, despite Portal mounting it a commit late', () => {
    const { calls } = trackPromotions();

    render(<ModernConfirmDialog open title="Archive project?" onCancel={vi.fn()} onConfirm={vi.fn()} />);
  ...
  it('promotes when it opens after mount (closed -> open)', () => {
    ...
    const { rerender } = render(
      <ModernConfirmDialog open={false} title="Archive project?" onCancel={vi.fn()} onConfirm={vi.fn()} />,
    );
    expect(document.querySelector('dialog')).toBeNull();
    expect(calls).toHaveLength(0);

    rerender(
      <ModernConfirmDialog open title="Archive project?" onCancel={vi.fn()} onConfirm={vi.fn()} />,
    );
  ...
  it('does not re-promote on unrelated re-renders while it stays open', () => {
    ...
    const { rerender } = render(
      <ModernConfirmDialog open title="a" onCancel={vi.fn()} onConfirm={vi.fn()} />,
    );
    expect(calls).toHaveLength(1);

    rerender(<ModernConfirmDialog open title="b" onCancel={vi.fn()} onConfirm={vi.fn()} />);
    rerender(<ModernConfirmDialog open title="c" loading onCancel={vi.fn()} onConfirm={vi.fn()} />);
  ...
  it('survives a rapid open -> closed -> open cycle with one promotion per open', async () => {
    ...
    const dialog = (open: boolean) => (
      <ModernConfirmDialog open={open} title="Archive project?" onCancel={vi.fn()} onConfirm={vi.fn()} />
    );
