...
function presentationCssPath(...segments: string[]): string {
  return join(__dirname, "../presentation", ...segments);
}

const RAMP_CSS = readFileSync(join(__dirname, "../foundation/tokens/commercial-ramp.css"), "utf8");
const ASCII_DIAGRAM_CSS = readFileSync(
  presentationCssPath("content/ascii-diagram/AsciiDiagram.css"),
  "utf8",
);
...
const ASCII_DIAGRAM_SOURCE = readFileSync(
  presentationCssPath("content/ascii-diagram/index.tsx"),
  "utf8",
);
const ASCII_FRAME_CSS = readFileSync(presentationCssPath("framing/ascii-frame/AsciiFrame.css"), "utf8");
const INVERT_SECTION_CSS = readFileSync(
  presentationCssPath("framing/invert-section/InvertSection.css"),
  "utf8",
);
const SECTION_FRAME_CSS = readFileSync(
  presentationCssPath("framing/section-frame/SectionFrame.css"),
  "utf8",
);
const TERMINAL_BLOCK_CSS = readFileSync(
  presentationCssPath("content/terminal-block/TerminalBlock.css"),
  "utf8",
);
const TREE_VIEW_CSS = readFileSync(presentationCssPath("content/tree-view/TreeView.css"), "utf8");

/** Resolves through the ramp stylesheet, following aliases, so the contrast math below cannot
 *  drift from the source of truth the way a hand-copied hex table would. */
function rampHex(name: string): string {
  const literal = RAMP_CSS.match(new RegExp(`--ds-commercial-${name}:\\s*(#[0-9a-fA-F]{6})`));
  if (literal) return literal[1]!;
  const alias = RAMP_CSS.match(new RegExp(`--ds-commercial-${name}:\\s*var\\(--ds-commercial-([a-z0-9-]+)\\)`));
  if (alias) return rampHex(alias[1]!);
  throw new Error(`ramp token --ds-commercial-${name} not found (or not resolvable) in commercial-ramp.css`);
}

const INK = rampHex("ink");
const PAPER = rampHex("paper");
...
/* Walked, not enumerated: the defect this guards was a guard whose file list was narrower than
   the kit it claimed to cover. */
function collectKitCssFiles(dir: string): string[] {
  return readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    const full = join(dir, entry.name);
    if (entry.isDirectory()) return collectKitCssFiles(full);
...
describe("Defect 1 -- AsciiDiagram answers a narrow measure without changing the grid a consumer re-derives", () => {
  ...
  describe("overflow is made perceivable and operable, never relayed out or cropped", () => {
    ...
    it("holds natural metrics and touches nothing when a consumer grants the diagram its natural width (the app-platform-shaped chain)", () => {
      mockViewport(true);
    ...
    it("gives the scroll state an affordance that survives a monochrome ink surface", () => {
      ...
      expect(ASCII_DIAGRAM_CSS).toMatch(
        /::-webkit-scrollbar-thumb\s*\{[^}]*var\(--ds-commercial-hairline-strong\)/,
      );
...
describe("Defect 2/3 -- InvertSection re-keys generic DS text/border roles for composed primitives", () => {
  it("re-keys the generic DS text roles on the ink (default) scope, using ramp var() references, never a hardcoded hex or !important", () => {
    ...
    for (const role of [
      ...
      expect(inkBody).toMatch(new RegExp(`${role}:\\s*var\\(--ds-commercial-`));
    }
  ...
  it("re-keys the muted role to a DIFFERENT, surface-appropriate ramp step on paper than on ink (a single gray cannot clear 4.5:1 against both near-black ink and white paper)", () => {
    ...
    expect(paperBody).toMatch(/--ds-color-text-secondary:\s*var\(--ds-commercial-/);
  ...
  it("the chosen ink-scope muted ramp step clears WCAG AA (4.5:1) body-text contrast against the actual ink background", () => {
    const inkBody = ruleBody(INVERT_SECTION_CSS, ".rt-invert-section");
    const ref = inkBody.match(/--ds-color-text-secondary:\s*var\(--ds-commercial-(gray-\d+)\)/)?.[1];
  ...
  it("the chosen paper-scope muted ramp step clears WCAG AA (4.5:1) body-text contrast against the actual paper background", () => {
    const paperBody = ruleBody(INVERT_SECTION_CSS, '.rt-invert-section[data-surface="paper"]');
    const ref = paperBody.match(/--ds-color-text-secondary:\s*var\(--ds-commercial-(gray-\d+)\)/)?.[1];
    expect(ref).toBeTruthy();
  ...
  it("re-keys a generic DS border/divider role a composed primitive might read, to a ramp step that clears at least 3:1 (meaningful non-text chrome) against ink", () => {
    const inkBody = ruleBody(INVERT_SECTION_CSS, ".rt-invert-section");
    const ref = inkBody.match(/--ds-color-border:\s*var\(--ds-commercial-([a-z0-9-]+)\)/)?.[1];
    expect(ref).toBeTruthy();
  ...
  describe("live cascade proof (real CSS manually re-injected, mirroring the ChartC.skin-migration.test.tsx precedent)", () => {
    ...
    it('a composed primitive reading the generic --ds-color-text-secondary role resolves to the EXACT color .rt-invert-section itself re-keys that role to, inside <InvertSection surface="ink"> (not just SOME non-empty value)', () => {
      ...
      const inkBody = ruleBody(INVERT_SECTION_CSS, ".rt-invert-section");
      const ref = inkBody.match(/--ds-color-text-secondary:\s*var\(--ds-commercial-([a-z0-9-]+)\)/)?.[1];
      expect(ref).toBeTruthy();
...
describe("Defect 2 -- kit color declarations resolve without a .ds-commercial/InvertSection ancestor (honest fallback)", () => {
  it("carries a literal ramp fallback on every var(--ds-commercial-fg) / var(--ds-commercial-bg) reference in AsciiFrame, AsciiDiagram and SectionFrame -- none is a bare, unresolvable reference", () => {
    for (const [name, css] of [
      ["AsciiFrame.css", ASCII_FRAME_CSS],
      ["AsciiDiagram.css", ASCII_DIAGRAM_CSS],
      ["SectionFrame.css", SECTION_FRAME_CSS],
    ] as const) {
      const bareRefs = css.match(/var\(--ds-commercial-(?:fg|bg)\)/g) ?? [];
      expect(bareRefs, `${name} has an un-fallbacked var(--ds-commercial-fg/bg)`).toHaveLength(0);
      ...
      expect(css).toMatch(/var\(--ds-commercial-(?:fg|bg),\s*(?:currentColor|transparent)\)/);
      ...
      expect(css, `${name} falls back to an absolute ramp color instead of degrading safely`).not.toMatch(
        /var\(--ds-commercial-fg,\s*var\(--ds-commercial-gray-1000\)\)/,
      );
    }
  });

  it("carries a fallback on every var() inside every focus-ring declaration in the KIT, not just in the three stylesheets the leg above happens to name", () => {
    /* An unresolvable var() invalidates the shorthand, so `outline-style` falls back to its
       `none` initial and the ring vanishes; `--ds-commercial-fg` is never declared at `:root`. */
    const cssFiles = collectKitCssFiles(join(__dirname, "..")).map(
      (file) => [file, readFileSync(file, "utf8")] as const,
    );
    // Anti-vacuity: the kit really does ship stylesheets, and some really do declare an outline.
  ...
  describe("live cascade proof (real CSS manually re-injected, no InvertSection/.ds-commercial ancestor present)", () => {
    const injected: HTMLStyleElement[] = [];
    ...
    it("AsciiFrame degrades its foreground to the inherited page color when mounted standalone -- never to the kit's absolute white, which would vanish on a light page", () => {
      injectRealCss(RAMP_CSS, injected);
      ...
      expect(parseRgb(resolved)).not.toEqual(hexToRgbTuple(rampHex("gray-1000")));
    });
  });
});
...
describe("P3 -- SectionFrame ordinal has an accessible text equivalent, without double-announcing", () => {
  ...
  it("stays server-safe: the family imports no React hook and declares no client boundary", () => {
    const source = readFileSync(
      presentationCssPath("framing/section-frame/index.tsx"),
      "utf8",
    );
...
describe("Commercial source vetoes -- CLS reservations survive their own scaling", () => {
  ...
  it("gives Typewriter an in-flow reservation sharing one grid cell with the animated layer", () => {
    const css = readFileSync(presentationCssPath("content/typewriter/Typewriter.css"), "utf8");
...
describe("Defect B (refinement) -- section-frame meta is real text and was measured at 4.42:1 on ink, just under the 4.5 body-copy floor", () => {
  it("routes meta's color through the canon-approved re-keyed role instead of the raw ramp step that measured under AA", () => {
    const body = ruleBody(SECTION_FRAME_CSS, ".rt-section-frame__meta");
    expect(body).toMatch(/color:\s*var\(--ds-color-text-secondary,\s*var\(--ds-commercial-gray-500\)\)/);
  });
  ...
  it("leaves the aria-hidden index/dash markers on their existing step (they are decorative chrome, held to the 3:1 non-text floor, which gray-500 already clears)", () => {
    ...
    expect(indexBody).toMatch(/color:\s*var\(--ds-commercial-gray-500\)/);
    expect(dashBody).toMatch(/color:\s*var\(--ds-commercial-gray-500\)/);
  });
...
describe("InvertSection -- text-disabled role (canon-permitted addition)", () => {
  it("re-keys --ds-color-text-disabled to a ramp step, distinct from the ambient theme's own disabled value", () => {
    const inkBody = ruleBody(INVERT_SECTION_CSS, ".rt-invert-section");
    expect(inkBody).toMatch(/--ds-color-text-disabled:\s*var\(--ds-commercial-/);
  });
});

describe("InvertSection -- paper-branch primary/inverse aliasing (the part canon flagged as most likely to be missed)", () => {
  it("does NOT need its own --ds-color-text-primary/-inverse override on the paper branch, because both alias --ds-commercial-fg/-bg, which the paper branch already overrides -- proven via live cascade, not just source inspection", () => {
    const injected: HTMLStyleElement[] = [];
    injectRealCss(RAMP_CSS, injected);
    injectRealCss(INVERT_SECTION_CSS, injected);
    ...
    expect(parseRgb(resolved)).toEqual(hexToRgbTuple(rampHex("gray-0")));
