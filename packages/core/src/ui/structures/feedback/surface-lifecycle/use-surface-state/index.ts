...
/**
 * @fileoverview useSurfaceState -- the surface lifecycle state machine.
 *
 ...
 * states (loading skeleton, empty state, error state, stale banner, offline
 ...
 *
 * It sits inside the lifecycle family rather than on a surfaces support path
 * because it has no surface vocabulary of its own: it reads an `unknown`
 * error, counts an array, and renders the five siblings next to it. Owning the
 * machine and the states it drives in one place is what keeps the state union
 * and the component set from drifting apart.
 *
...
export interface SurfaceStateRenderConfig {
  ...
  /**
   * Call-to-action in the empty state. Deliberately narrower than the
   * `SurfaceAction` the component takes: a render config is configuration, so
   * it asks for the two fields a caller actually has here and the hook builds
   * the action.
   */
  emptyAction?: { label: string; onClick: () => void };
  /** Title for the error state. */
  errorTitle?: string;
  /** Safe copy shown instead of the normalized error message. */
...
/** Stable id for the action the hook builds from `emptyAction`. */
const EMPTY_STATE_ACTION_ID = 'surface-empty-action';
  ...
  const renderState = useMemo(() => {
    return (overrides?: Partial<SurfaceStateRenderConfig>): ReactNode | null => {
      switch (state) {
        ...
        case 'empty':
          return React.createElement(SurfaceEmptyState, {
            icon: overrides?.emptyIcon,
            title: overrides?.emptyTitle,
            description: overrides?.emptyDescription,
            action: overrides?.emptyAction
              ? {
                  id: EMPTY_STATE_ACTION_ID,
                  label: overrides.emptyAction.label,
                  onClick: overrides.emptyAction.onClick,
                }
              : undefined,
          });

        case 'error':
          return React.createElement(SurfaceErrorState, {
            error,
