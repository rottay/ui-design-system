/**
 * The container vocabulary at rest: every channel equals the one fallback the
...
const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/runtime/engines/modern/skin/container/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/container", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === containerChromeDeriver)).toHaveLength(1);
    expect(containerChromeDeriver.family).toBe("container");
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(containerChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: containerChromeDeriver.produces,
