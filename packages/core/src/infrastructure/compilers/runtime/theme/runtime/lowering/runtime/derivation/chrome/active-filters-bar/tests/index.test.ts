/**
 * The active-filters-bar family under the shared contract battery: generated from the
...
import { activeFiltersBarChromeDeriver } from "..";
...
describeFamilyContract(activeFiltersBarChromeDeriver, FIXTURES);

describe("chrome/active-filters-bar", () => {
  it("is registered once, at rank derived", () => {
    expect(
      FAMILY_DERIVERS.filter((deriver) => deriver === activeFiltersBarChromeDeriver)
    ).toHaveLength(1);
    expect(activeFiltersBarChromeDeriver.family).toBe("active-filters-bar");
    expect(activeFiltersBarChromeDeriver.rank).toBe("derived");
  });
});
