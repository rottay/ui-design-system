/**
 * The `workbench-header` vocabulary at rest: every channel equals the one fallback its
...
describe("chrome/workbench-header", () => {
  it("is registered once, at rank derived", () => {
    expect(
      FAMILY_DERIVERS.filter((deriver) => deriver === workbenchHeaderChromeDeriver)
    ).toHaveLength(1);
    expect(workbenchHeaderChromeDeriver.family).toBe("workbench-header");
    expect(workbenchHeaderChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = workbenchHeaderChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...workbenchHeaderChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace", () => {
    for (const channel of workbenchHeaderChromeDeriver.produces) {
      expect(channel.startsWith("--ds-workbench-header-")).toBe(true);
    }
  });

  it("leaves the retired skeleton radius unproduced: the shared renderer owns its own bones", () => {
    expect([...workbenchHeaderChromeDeriver.produces]).not.toContain(
      "--ds-workbench-header-skeleton-radius"
    );
    expect(SKIN).not.toContain("--ds-workbench-header-skeleton-radius");
  });

  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(workbenchHeaderChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: workbenchHeaderChromeDeriver.produces,
