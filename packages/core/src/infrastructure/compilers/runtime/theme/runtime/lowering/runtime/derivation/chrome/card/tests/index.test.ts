...
/* ---- the depth causality arm: `surfaces.borderStyle` reaches card's keyline ---- */

const BORDER_ROLE = "--ds-edge-hairline-width";
const KEYLINE = "--ds-card-border-width";
const ROLE_CHAIN = `var(${BORDER_ROLE})`;
const COMPONENT_CSS = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/card/index.css"),
  "utf8"
);
...
describe("chrome/card depth keyline", () => {
  it("states the keyline through the governed edge role, never a width of its own", () => {
    const derived = cardChromeDeriver.derive(keylineContext(MINIMAL_THEME), {});
  ...
  it("declares the border decision it now consumes", () => {
    expect(cardChromeDeriver.consumes).toContain("surfaces.borderStyle");
  });
});
