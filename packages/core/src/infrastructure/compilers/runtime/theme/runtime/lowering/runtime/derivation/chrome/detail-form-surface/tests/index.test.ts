/**
 * The `detail-form-surface` vocabulary at rest: every channel equals the one fallback its Modern
...
import { detailFormSurfaceChromeDeriver } from "..";

const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/skin/detail-form-surface/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describeFamilyContract(detailFormSurfaceChromeDeriver, FIXTURES);

const context = () => buildLoweringContext({ theme: firstPartyFixture("bithire") });

describe("chrome/detail-form-surface", () => {
  it("is the detail-form-surface family at rank derived", () => {
    expect(detailFormSurfaceChromeDeriver.family).toBe("detail-form-surface");
    expect(detailFormSurfaceChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = detailFormSurfaceChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...detailFormSurfaceChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace", () => {
    for (const channel of detailFormSurfaceChromeDeriver.produces) {
      expect(channel.startsWith("--ds-detail-form-surface-")).toBe(true);
    }
  });

  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(detailFormSurfaceChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: detailFormSurfaceChromeDeriver.produces,
        derive: () => Object.fromEntries(channels.map((channel) => [channel, rank])),
      };
      const result = runDerivation(context(), [detailFormSurfaceChromeDeriver, stated]);
