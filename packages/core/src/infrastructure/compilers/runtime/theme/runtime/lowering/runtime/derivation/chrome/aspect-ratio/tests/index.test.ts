/**
 * The aspect-ratio vocabulary at rest: every channel equals the one fallback
 * the Modern skin reads it with (or is itself the resting declaration where
 * the skin reads it bare), and any higher-ranked statement still wins.
 */
...
import { aspectRatioChromeDeriver } from "..";

const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/runtime/engines/modern/skin/aspect-ratio/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/aspect-ratio", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === aspectRatioChromeDeriver)).toHaveLength(1);
    expect(aspectRatioChromeDeriver.family).toBe("aspect-ratio");
    expect(aspectRatioChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = aspectRatioChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...aspectRatioChromeDeriver.produces].sort());
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(aspectRatioChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: aspectRatioChromeDeriver.produces,
