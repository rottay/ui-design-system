/**
 * The file-manager family under the shared contract battery: generated from the
...
import { fileManagerChromeDeriver } from "..";
...
describeFamilyContract(fileManagerChromeDeriver, FIXTURES);

describe("chrome/file-manager", () => {
  it("is registered once, at rank derived", () => {
    expect(
      FAMILY_DERIVERS.filter((deriver) => deriver === fileManagerChromeDeriver)
    ).toHaveLength(1);
    expect(fileManagerChromeDeriver.family).toBe("file-manager");
    expect(fileManagerChromeDeriver.rank).toBe("derived");
  });
});
