/**
 * The `section-frame` vocabulary at rest: every channel equals the one fallback its Modern
...
import { sectionFrameChromeDeriver } from "..";

const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/skin/section-frame/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/section-frame", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === sectionFrameChromeDeriver)).toHaveLength(1);
    expect(sectionFrameChromeDeriver.family).toBe("section-frame");
    expect(sectionFrameChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = sectionFrameChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...sectionFrameChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace", () => {
    for (const channel of sectionFrameChromeDeriver.produces) {
      expect(channel.startsWith("--ds-section-frame-")).toBe(true);
    }
  });

  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(sectionFrameChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: sectionFrameChromeDeriver.produces,
