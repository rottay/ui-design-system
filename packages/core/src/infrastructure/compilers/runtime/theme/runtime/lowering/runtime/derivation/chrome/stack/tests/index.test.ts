/**
 * The stack vocabulary at rest: every channel equals the one fallback the
...
import { stackChromeDeriver } from "..";

const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/runtime/engines/modern/skin/stack/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/stack", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === stackChromeDeriver)).toHaveLength(1);
    expect(stackChromeDeriver.family).toBe("stack");
    expect(stackChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = stackChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...stackChromeDeriver.produces].sort());
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(stackChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: stackChromeDeriver.produces,
