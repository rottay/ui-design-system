/**
 * The `wizard-surface` vocabulary at rest: every channel equals the one fallback its Modern
...
import { wizardSurfaceChromeDeriver } from "..";

const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/skin/wizard-surface/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describeFamilyContract(wizardSurfaceChromeDeriver, FIXTURES);

const context = () => buildLoweringContext({ theme: firstPartyFixture("bithire") });

describe("chrome/wizard-surface", () => {
  it("is the wizard-surface family at rank derived", () => {
    expect(wizardSurfaceChromeDeriver.family).toBe("wizard-surface");
    expect(wizardSurfaceChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = wizardSurfaceChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...wizardSurfaceChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace", () => {
    for (const channel of wizardSurfaceChromeDeriver.produces) {
      expect(channel.startsWith("--ds-wizard-surface-")).toBe(true);
    }
  });

  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(wizardSurfaceChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: wizardSurfaceChromeDeriver.produces,
        derive: () => Object.fromEntries(channels.map((channel) => [channel, rank])),
      };
      const result = runDerivation(context(), [wizardSurfaceChromeDeriver, stated]);
