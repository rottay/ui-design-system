/**
 * The space vocabulary at rest: every channel equals the one fallback the
...
const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/runtime/engines/modern/skin/space/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/space", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === spaceChromeDeriver)).toHaveLength(1);
    expect(spaceChromeDeriver.family).toBe("space");
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(spaceChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: spaceChromeDeriver.produces,
