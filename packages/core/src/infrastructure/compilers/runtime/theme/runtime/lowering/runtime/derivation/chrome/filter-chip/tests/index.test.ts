/**
 * The filter-chip vocabulary at rest: every channel equals the one fallback the Modern
...
import { filterChipChromeDeriver } from "..";
...
describe("chrome/filter-chip", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === filterChipChromeDeriver)).toHaveLength(1);
    expect(filterChipChromeDeriver.family).toBe("filter-chip");
    expect(filterChipChromeDeriver.rank).toBe("derived");
  });
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(filterChipChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: filterChipChromeDeriver.produces,
