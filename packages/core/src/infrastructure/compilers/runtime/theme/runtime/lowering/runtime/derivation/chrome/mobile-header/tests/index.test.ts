/**
 * The `mobile-header` vocabulary at rest: every channel equals the one fallback its Modern
...
const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/skin/mobile-header/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/mobile-header", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === mobileHeaderChromeDeriver)).toHaveLength(1);
    expect(mobileHeaderChromeDeriver.family).toBe("mobile-header");
    expect(mobileHeaderChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = mobileHeaderChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...mobileHeaderChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace", () => {
    for (const channel of mobileHeaderChromeDeriver.produces) {
      expect(channel.startsWith("--ds-mobile-header-")).toBe(true);
    }
  });

  it("leaves the notch inset unproduced, because the emission door would drop it in silence", async () => {
    // Its only honest value is the safe-area inset, and `env` is not an admitted
    // value function: measured, the door admitted 15 of the deriver's 16 channels
    // and dropped this one whole. A deriver that stated it would claim a channel
    // the artifact never carries, so the skin keeps stating the inset as its own
    // double fallback and the pixels survive.
  ...
  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(mobileHeaderChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: mobileHeaderChromeDeriver.produces,
