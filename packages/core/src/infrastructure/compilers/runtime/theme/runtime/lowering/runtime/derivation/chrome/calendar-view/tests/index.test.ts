/**
 * The calendar-view family under the shared contract battery: generated from the
...
import { calendarViewChromeDeriver } from "..";
...
describeFamilyContract(calendarViewChromeDeriver, FIXTURES);

describe("chrome/calendar-view", () => {
  it("is registered once, at rank derived", () => {
    expect(
      FAMILY_DERIVERS.filter((deriver) => deriver === calendarViewChromeDeriver)
    ).toHaveLength(1);
    expect(calendarViewChromeDeriver.family).toBe("calendar-view");
    expect(calendarViewChromeDeriver.rank).toBe("derived");
  });
});
