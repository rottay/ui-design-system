/**
 * The `form-header` vocabulary at rest: every channel equals the one fallback its Modern
...
const SKIN = readFileSync(
  resolve(process.cwd(), "src/foundation/tokens/css/presentation/components/skin/form-header/index.css"),
  "utf8"
).replace(/\/\*[\s\S]*?\*\//g, "");
...
describe("chrome/form-header", () => {
  it("is registered once, at rank derived", () => {
    expect(FAMILY_DERIVERS.filter((deriver) => deriver === formHeaderChromeDeriver)).toHaveLength(1);
    expect(formHeaderChromeDeriver.family).toBe("form-header");
    expect(formHeaderChromeDeriver.rank).toBe("derived");
  });

  it("states every declared channel at the single resting value the skin reads it with", () => {
    const derived = formHeaderChromeDeriver.derive(context(), {});
    expect(Object.keys(derived).sort()).toEqual([...formHeaderChromeDeriver.produces].sort());
  ...
  it("names only its own family namespace, so the header contract's channels stay the contract's", () => {
    for (const channel of formHeaderChromeDeriver.produces) {
      expect(channel.startsWith("--ds-form-header-")).toBe(true);
    }
  });

  it("yields every channel to a vertical or tenant statement of it", () => {
    const channels = Object.keys(formHeaderChromeDeriver.derive(context(), {}));
    for (const rank of ["verticalOverride", "tenant"] as const) {
      const stated: FamilyDeriver = {
        family: `stated-${rank}`,
        rank,
        consumes: ["chrome.*"],
        produces: formHeaderChromeDeriver.produces,
