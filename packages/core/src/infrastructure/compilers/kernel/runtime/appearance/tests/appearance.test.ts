...
describe("appearanceGeneralToVariables", () => {
  ...
  it("emits light-dark() pairs for the derivations under auto with dark seeds", () => {
    ...
    expect(vars["--ds-color-text-on-primary"]).toBe(
  ...
  it.each([
    ...
    ({ light, dark, math, passThrough }) => {
      ...
      for (const channel of [
        ...
        if (math === "paired")
          expect(vars[channel], channel).toMatch(/^light-dark\(/);
        if (math === "scalar") {
  ...
  it("navigation.sidebarTone subtle produces sidebar vars with secondary bg", () => {
    ...
    expect(vars["--ds-sidebar-item-color-active"]).toBe(
...
describe("appearanceAdvancedToVariables", () => {
  ...
  it("chrome.controls.button color wins over text alias", () => {
    const vars = appearanceAdvancedToVariables({
      chrome: {
        controls: { buttonPrimary: { color: "#222222", text: "#111111" } },
      },
    });
  ...
  it("chrome advanced covers BrandChrome parity categories", () => {
    ...
    expect(vars["--ds-filter-pill-count-active-ring"]).toBe(
    ...
    expect(vars["--ds-badge-chip-radius"]).toBe(
      "calc(4px * var(--ds-radius-scale, 1))"
    );
    ...
    expect(vars["--ds-card-border-radius"]).toBe(
      "calc(8px * var(--ds-radius-scale, 1))"
    );
    expect(vars["--ds-metric-card-meter-fill-success"]).toBe(
      "linear-gradient(#0a0, #080)"
    );
  ...
  it.each([
    ...
    (_name, chrome) => {
      ...
      for (let index = 0; index < cap + 25; index++) {
        tokenOverrides[`--ds-example-${String(index).padStart(3, "0")}`] =
          "1px";
      }
      expect(() =>
        appearanceAdvancedToVariables({
...
describe("appearanceToVariables", () => {
  ...
  it("keeps dark ramps surface-aware while auto is deterministic light-first", () => {
    ...
    expect(dark["--ds-color-primary-50"]).not.toBe(
      light["--ds-color-primary-50"]
    );
    expect(dark["--ds-color-primary-900"]).not.toBe(
      light["--ds-color-primary-900"]
    );
  });
  ...
  it("keys concrete ramps to the final tenant surface with deterministic fallbacks", () => {
    ...
    expect(cream["--ds-color-primary-50"]).not.toBe(
      white["--ds-color-primary-50"]
    );
    expect(warmDark["--ds-color-primary-50"]).not.toBe(
      defaultDark["--ds-color-primary-50"]
    );
  });
  ...
  it("preserves v1 functional colors through deterministic OKLCH formulas", () => {
    const vars = appearanceToVariables({
      general: {
        palette: { primary: "hsl(170 78% 26%)", backgroundMode: "dark" },
      },
    });
  ...
  it("reapplies derived ramps from the final Advanced seed and never accepts authored ramp values", () => {
    ...
    expect(vars["--ds-color-primary-500"]).toBe(
      deriveAppearanceColorRamps({}, { "--ds-color-primary": "#2563EB" })[
...
describe("compileAppearanceVariables", () => {
  ...
  it("snaps an authored on-primary below the floor instead of publishing it", () => {
    ...
    expect(compiled.variables["--ds-color-text-on-primary"]).not.toBe(
      "#0C0C0E"
    );
    expect(compiled.adjustments).toEqual(
      expect.arrayContaining([
  ...
  it("keeps the derived on-primary when it already clears the floor", () => {
    ...
    expect(
      compiled.adjustments.filter(
        (a) => a.token === "--ds-color-text-on-primary"
      )
    ).toHaveLength(0);
  });
});
