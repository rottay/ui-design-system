...
describe('first-party artifact integrity', () => {
  ...
  it.each(TENANTS)('%s/index.css artifact exists', (tenant) => {
    expect(existsSync(resolve(CSS_SRC, `facade/artifacts/${tenant}/index.css`))).toBe(true);
  });

  it('rottay artifact is richest (400+ unique --ds-* vars)', () => {
    const css = readFileSync(resolve(CSS_SRC, 'facade/artifacts/rottay/index.css'), 'utf-8');
    expect(new Set(css.match(/--ds-[\w-]+/g)).size).toBeGreaterThan(400);
  });

  it('bithire artifact has substantial coverage (80+)', () => {
    const css = readFileSync(resolve(CSS_SRC, 'facade/artifacts/bithire/index.css'), 'utf-8');
    expect(new Set(css.match(/--ds-[\w-]+/g)).size).toBeGreaterThan(80);
  });

  it('evnto artifact has substantial coverage (60+)', () => {
    const css = readFileSync(resolve(CSS_SRC, 'facade/artifacts/evnto/index.css'), 'utf-8');
  ...
  it.each(TENANTS)('%s artifact uses canonical button color vars', (tenant) => {
    const css = readFileSync(resolve(CSS_SRC, `facade/artifacts/${tenant}/index.css`), 'utf-8');
  ...
  it('DB-owned tenants have no legacy CSS authority', () => {
    expect(existsSync(resolve(CSS_SRC, 'facade/legacy/themanagementmiami/index.css'))).toBe(false);
  });

  it('public entrypoint source files exist in facade/entrypoints/', () => {
    for (const f of ['facade/entrypoints/styles.css', 'facade/entrypoints/platform.css', 'facade/entrypoints/bithire.css', 'facade/entrypoints/evnto.css']) {
...
describe('H3 contract: rottay', () => {
  ...
  describe('rottay state semantics', () => {
    ...
    it('disabled: artifact dark block matches authored values', () => {
      const artifact = readFileSync(resolve(CSS_SRC, 'facade/artifacts/rottay/index.css'), 'utf-8');
...
describe('H3 contract: bithire', () => {
  ...
  describe('bithire surfaces in artifact (light + dark)', () => {
    const artifact = readFileSync(resolve(CSS_SRC, 'facade/artifacts/bithire/index.css'), 'utf-8');
...
describe('H3 contract: evnto', () => {
  ...
  describe('evnto artifact + generated output (I6 public path)', () => {
    const artifact = readFileSync(resolve(CSS_SRC, 'facade/artifacts/evnto/index.css'), 'utf-8');
    const darkSelector =
