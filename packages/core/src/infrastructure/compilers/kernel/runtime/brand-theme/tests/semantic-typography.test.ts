...
import { compileBrandTheme, compileTheme } from "../index";

/**
 * The two lowerings of one authored theme:
 *   leg A — the static BrandTheme transport, compiled directly.
 *   leg B — the ISO transport: BrandTheme -> Theme -> themeToBrandTheme -> compile.
...
type Compiled = ReturnType<typeof compileBrandTheme>;

function legA(brandTheme: FirstPartyBrandTheme): Compiled {
  return compileBrandTheme({ brandTheme, tenantSlug: brandTheme.id });
}

function legB(brandTheme: FirstPartyBrandTheme): Compiled {
  return compileTheme(brandThemeToTheme(brandTheme));
}
    ...
    .update(
      JSON.stringify(
        stable({
          ...
          engineBridge: compiled.engineBridge,
...
describe("present-with-undefined role keys", () => {
  ...
  it("keeps an authored labelStyle when the label role arrives as a skeleton", () => {
    ...
    const explicit = compileBrandTheme({
