...
import { brandThemeToChromeVariables, compileBrandTheme } from '../index';
import {
  isBundledTenant,
...
// `includeDarkSelector` itself; `compileBrandTheme` needs none of that -- it
...
const COMPILED_BY_TENANT = {
  bithire: compileBrandTheme({ brandTheme: bithireBrandTheme, tenantSlug: 'bithire' }),
  evnto: compileBrandTheme({ brandTheme: evntoBrandTheme, tenantSlug: 'evnto' }),
  rottay: compileBrandTheme({ brandTheme: rottayBrandTheme, tenantSlug: 'rottay' }),
};
...
// These vars are generated by compileBrandTheme() today.
...
// -- a conversion `compileBrandTheme` never re-implements. Its compiled
...
describe('shared pipeline: density baseline', () => {
  ...
  it('evnto generates density scale from brandTheme', () => {
    ...
    // through, the evnto VerticalPreset's own 1.125 -- compileBrandTheme
...
describe('shared pipeline: chrome vars NOW generated (G1)', () => {
  ...
  it('rottay generates table vars with correct values', () => {
    ...
    // `compileBrandTheme` does not autocorrect -- it compiles what the theme
