...
import { hexToRgb, isHexColor } from '@/infrastructure/compilers/kernel/foundation/css/color-math';
import { contrastRatio, rgbToHsl } from '@/foundation/kernel/accessibility/branding-contrast';
...
describe('BUNDLED_TENANT_SLUGS promise holds for every member', () => {
  ...
  it.each([...BUNDLED_TENANT_SLUGS])('%s has a bundled CSS artifact on disk', (slug) => {
    const artifactPath = resolve(process.cwd(), 'src/foundation/tokens/css/facade/artifacts', slug, 'index.css');
    expect(existsSync(artifactPath), `expected foundation/tokens/css/facade/artifacts/${slug}/index.css to exist`).toBe(true);
  });

  it('themanagementmiami legacy CSS cannot become a second runtime authority', () => {
    const legacyPath = resolve(process.cwd(), 'src/foundation/tokens/css/facade/legacy/themanagementmiami/index.css');
