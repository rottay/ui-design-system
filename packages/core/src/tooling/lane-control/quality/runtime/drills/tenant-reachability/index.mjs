 ...
 *
 * EVERY DRILL HERE SPAWNS THE GATE'S REAL CLI. It does not import it: `quality/`
 * sits below `public/` in the layer order, so a drill that imported its target
 * would be an upward dependency — and the structure audit is right to refuse
 * it. Exercising the CLI is also the more honest test, since that is what a
...
const GATE = `${ROOT}/packages/core/src/tooling/lane-control/public/tenant-reachability/index.mjs`;
