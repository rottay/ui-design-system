...
| `W4-build-free` | a verification needing the build; a work order that names no acceptance gate — the admissible set is READ from `scripts/ci/gates-manifest/index.mjs`, so a gate CI retires stops being quotable here in the same commit |
