...
const themeProviderSource = readSource('infrastructure/runtime/theming/composition/react/provider/index.tsx');
const bridgeSource = readSource('infrastructure/runtime/theming/presentation/adapters/react/css-variables-bridge/index.tsx');
const personalityPrimitivesSource = readSource('foundation/tokens/ts/runtime/personality/index.ts');
...
describe('Whitelabel Field Coverage', () => {
  ...
  describe('Token font family variables exist in tenant CSS', () => {
    ...
    for (const tenant of tenants) {
      const css = readSource(`foundation/tokens/css/facade/artifacts/${tenant}/index.css`);
  ...
  describe('Compiled BrandTheme CSS is variables-only before the governed extension', () => {
    const tenants = ['rottay', 'bithire', 'evnto'];

    for (const tenant of tenants) {
      it(`${tenant} CSS should not contain .ant- selectors outside comments`, () => {
        const css = readSource(`foundation/tokens/css/facade/artifacts/${tenant}/index.css`);
        const compiledTheme = css.split(DECLARED_EXTENSION_MARKER, 1)[0] ?? '';
        // Strip CSS comments before checking for .ant- selectors
        const withoutComments = compiledTheme.replace(/\/\*[\s\S]*?\*\//g, '');
