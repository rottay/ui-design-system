...
import { getChildKeys } from '../../../../types/components/tree';
  ...
  return (
    <li className={node.className} style={node.style}>
      ...
      >
        ...
        {/* Title */}
        <span className={`flex-1 select-none text-sm ${isSelected ? 'font-medium' : ''}`}>
          {node.title}
        </span>
      </div>
      ...
      {hasChildren && isExpanded && (
        <ul className={showLine ? 'border-l border-gray-200 ml-4' : ''}>
...
/**
 * Apollo Tree Component
 *
 * Pure Tailwind-styled tree display.
 */
export default function ApolloTree(props: TreeProps) {
  const {
    treeData = [],
    checkable = false,
  ...
  return (
    <ul className={`space-y-0.5 ${className}`} style={style}>
      {treeData.map((node) => (
        <TreeNode
          key={node.key}
          node={node}
