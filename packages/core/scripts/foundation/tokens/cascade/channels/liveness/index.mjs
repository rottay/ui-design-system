 ...
 * Usage
 * ============================================================================
 *   node scripts/foundation/tokens/cascade/channels/liveness/index.mjs                  # human report
 *   node scripts/foundation/tokens/cascade/channels/liveness/index.mjs --check           # fail-closed CI gate (requires R1 artifact)
 *   node scripts/foundation/tokens/cascade/channels/liveness/index.mjs --json             # full JSON report to stdout
 *   node scripts/foundation/tokens/cascade/channels/liveness/index.mjs --write [--round R1] [--artifact-path <p>]
 */
...
export const DEFAULT_FAMILY_INVENTORY = resolve(
  CORE_ROOT,
  'scripts/tooling/quality/evidence/programs/modern-rescue/family-inventory/index.json',
);
export const DEFAULT_EVIDENCE_CONTRACT = resolve(
  CORE_ROOT,
  'scripts/tooling/quality/evidence/programs/modern-rescue/evidence-contract/index.json',
);
export const DEFAULT_CI_GATES_MANIFEST = resolve(CORE_ROOT, 'scripts/tooling/quality/automation/gates/manifest/index.mjs');
...
export function buildArtifact(gateRun, { round = DEFAULT_ROUND, evidenceRoot = DEFAULT_EVIDENCE_ROOT } = {}) {
  ...
  return {
    schemaVersion: 2,
    generatedBy: 'scripts/foundation/tokens/cascade/channels/liveness/index.mjs --write',
