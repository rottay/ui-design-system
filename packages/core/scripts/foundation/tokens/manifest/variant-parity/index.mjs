 ...
 *
 * Usage:
 *   node scripts/foundation/tokens/manifest/variant-parity/index.mjs            escribe el artefacto
 *   node scripts/foundation/tokens/manifest/variant-parity/index.mjs --check    regenera en memoria,
...
export function buildDoc({ sources, provenance, enforceMetadata = true }) {
  ...
  return {
    $generatedBy: 'scripts/foundation/tokens/manifest/variant-parity/index.mjs',
    $regenerate: 'node scripts/foundation/tokens/manifest/variant-parity/index.mjs',
    $warning:
  ...
  for (const key of ['silentPairs', 'placeholderPairs', 'untaggedAuthoredLeaves']) {
    ...
    if (now > pinned) findings.push(`${key} GREW from ${pinned} to ${now}: aparecio divergencia o se perdio cobertura`);
      ...
      findings.push(
        ...
        + `lower \`${key}\` in scripts/foundation/tokens/manifest/variant-parity/baseline/index.json (decrease-only)`,
      );
    }
  }
  return findings;
...
if (invokedDirectly) {
  ...
  if (process.argv.includes('--check')) {
    ...
    if (current !== text) findings.unshift('generated/variant-parity/index.json desactualizado — corre `node scripts/foundation/tokens/manifest/variant-parity/index.mjs`');
