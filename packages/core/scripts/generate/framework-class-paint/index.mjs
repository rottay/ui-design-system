#!/usr/bin/env node
/**
 * Generate and verify `lib/report/index.json` -- the class vocabulary
 ...
 *
 * This is the same discipline `tests/index.test.mjs` applies to
 ...
 *
 * USAGE
 *   node scripts/generate/framework-class-paint/index.mjs            # verify (exit 1 on drift)
 *   node scripts/generate/framework-class-paint/index.mjs --check    # same, explicit
 *   node scripts/generate/framework-class-paint/index.mjs --write    # regenerate the manifest
 */
...
export const MANIFEST_PATH = join(here, 'report/index.json');
 ...
 *
 * Kept identical to `tests/index.test.mjs`'s pin on purpose --
...
function serialize(derived) {
  return `${JSON.stringify(
    {
      $comment:
        'GENERATED by scripts/generate/framework-class-paint/index.mjs from the installed daisyui package. Do not hand-edit: run `node scripts/generate/framework-class-paint/index.mjs --write`.',
...
export async function checkManifest(from = import.meta.url) {
  ...
  if (installed !== PINNED_DAISY_VERSION) {
    failures.push(
      `daisyui version moved: installed ${installed}, manifest pinned to ${PINNED_DAISY_VERSION}. ` +
        ...
        'node scripts/generate/framework-class-paint/index.mjs --write',
    );
    return failures;
  }
  ...
  if (serialize(derived) !== onDisk) {
    ...
    failures.push(
      'report/index.json no longer matches the installed daisyui package' +
        (added.length ? `\n  + ${added.join(' ')}` : '') +
        (removed.length ? `\n  - ${removed.join(' ')}` : '') +
        '\n  regenerate: node scripts/generate/framework-class-paint/index.mjs --write',
    );
  }
  return failures;
}
