 ...
 *
 *   - surface/report/index.json          (derived census; name-level truth)
 *   - TENANT_CAPABILITY_REGISTRY         (controls, tiers, evidence — TS AST)
 *   - contracts/css/hooks/index.json                (93 public hooks + declaredSlots
 ...
 *                                         derivation edges)
 *   - artifacts/generated/css/verticals/bithire/index.css via no-loss     (the RESOLVED default value per name;
...
import { parseRegistry, parseAllowlist } from '../surface/index.mjs';
...
const RESOLUTION_BUNDLE = 'artifacts/generated/css/verticals/bithire/index.css';
...
const RECONCILIATION_FAMILIES = Object.freeze([
  ['--ds-tall-card-*', 'RECONNECT_OR_PROMOTE'],
  ['--ds-workspace-card-*', 'RECONNECT_OR_PROMOTE'],
  ['--ds-compact-card-*', 'COMPONENT_PRIVATE'],
  ['--ds-collection-card-*', 'COMPONENT_PRIVATE'],
  ['--ds-signal-card-*', 'COMPONENT_PRIVATE'],
  ['--ds-metric-card-*', 'COMPONENT_PRIVATE'],
  ['--ds-listing-grid-*', 'COMPONENT_PRIVATE'],
  ['--ds-premium-card-*', 'COMPONENT_PRIVATE'],
  ['--ds-input-*', 'COMPONENT_PRIVATE'],
  ['--ds-button-*', 'COMPONENT_PRIVATE'],
  ['--ds-color-dark-*', 'DERIVE'],
  ['--ds-commercial-*', 'KEEP_INTERNAL'],
  ['--ds-action-dock-reserved-*', 'FRONTIER'],
  ['--ds-bottom-tab-bar-reserved-*', 'FRONTIER'],
  ['legacy-component-mirror', 'MIGRATE_USEFUL_THEN_RETIRE_MIRROR'],
  ['remainder', 'KEEP_INTERNAL'],
]);

const LEGACY_COMPONENT_MIRROR = /^--ds-(?:avatar|badge|radio|checkbox|select|slider|calendar|datepicker|inputnumber|steps|toggle|switch|autocomplete|anchor|backtop|tree|pagination|dropdown|drawer|notification|progress|list|image|spinner|tag|toast)-/;
...
| --- | --- | --- |
| name · class · tier · status · reads | \`surface/report/index.json\` | The census is the name-level authority; nothing here re-derives it. |
...
| default (bithire) | \`styles/${RESOLUTION_BUNDLE.split('/').pop()}\` resolved through \`scripts/check/tokens/cascade/preservation\` | DEFAULT state only (dark mode, density and every \`@media\` context are excluded on purpose), BitHire only, truncated to 40 characters. Another vertical can resolve differently — the density trap is real. |
...
scripts/generate/tokens/customization/catalog/index.mjs --check\`). The only hand-written files are the
two guides.
...
- Application: **${counts.publicHooks} public hooks** vs **${counts.unadjudicatedReads} fenced unadjudicated reads** (drainage program D2 — not a menu). Denominator: the \`contracts/css/hooks/index.json\` population, which is a third scope again — it is neither a subset nor a superset of the TS-classification reads on the line above, so the two read counts must not be compared directly.
...
`;
  ...
  views['catalog.md'] = `${GEN_HEADER('surface/report/index.json + hooks manifest + authored CSS + the resolved bithire bundle')}# Token Catalog — Operational Overview
...
\`packages/core/scripts/generate/tokens/customization/surface/report/index.json\`; these pages add the
...
`;
  ...
  const indexPage = (groups, dirPrefix, title, note) => {
    ...
    return `${GEN_HEADER('surface/report/index.json')}# ${title}
...
\`contracts/css/hooks/index.json → declaredSlots\`; apps assign them ONLY under an
...
D3 one authority per concept (this table shrinks) · D4 visual review
...
function buildReconciliation(report) {
  const assigned = new Set();
  const familyAdjudication = RECONCILIATION_FAMILIES.map(([family, disposition]) => {
    let names;
    if (family === 'remainder') {
      names = report.deadWriters.filter((name) => !assigned.has(name));
    } else if (family === 'legacy-component-mirror') {
      names = report.deadWriters.filter(
        (name) => !assigned.has(name) && LEGACY_COMPONENT_MIRROR.test(name)
      );
    } else {
      const prefix = family.slice(0, -1);
      names = report.deadWriters.filter(
        (name) => !assigned.has(name) && name.startsWith(prefix)
      );
    }
    for (const name of names) assigned.add(name);
    return { family, disposition, count: names.length };
  });
  if (assigned.size !== report.deadWriters.length) {
    throw new Error(
      `reconciliation coverage mismatch: ${assigned.size}/${report.deadWriters.length} dead writers classified`
    );
  }
  return {
    schemaVersion: 1,
    generatedBy: 'scripts/generate/tokens/customization/catalog/index.mjs --write',
    basedOnReportDigest: createHash('sha256')
      .update(readFileSync(REPORT_PATH))
      .digest('hex'),
    normative: {
      writers: report.counts.writers,
      deadWriters: report.counts.deadWriters,
      definition: 'writer names and productive reads are derived by the customization surface census',
    },
    familyAdjudication,
    preservationManifest: '../preservation/manifest/index.json',
  };
}
...
function runChecks(inputs, edges, built, context, { drill, inRepoOnly = false, deferredCrossRepo = [] } = {}) {
  ...
  if (existsSync(RECONCILIATION_PATH)) {
    const recon = JSON.parse(readFileSync(RECONCILIATION_PATH, 'utf8'));
    const reportDigest = createHash('sha256')
      .update(readFileSync(REPORT_PATH))
      .digest('hex');
    if (drill === 'recon-digest' || recon.basedOnReportDigest !== reportDigest) {
      failures.push(
        'reconciliation digest mismatch: catalog/reconciliation/index.json was not derived from the CURRENT census report — regenerate both'
      );
    }
    ...
    failures.push('missing catalog/reconciliation/index.json (versioned projection)');
  }
