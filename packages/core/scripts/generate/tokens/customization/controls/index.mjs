 ...
 *   × surface/report/index.json (tier lists, Expert allowlist size +
 *   domains, per-channel live read counts)
 * and emits `docs/generated/customization-controls/index.md` — Standard and Pro tables with the
...
function controlRow(cap, report) {
  ...
  return [
    ...
    domain,
    cap.defaultBehavior ?? '—',
    isData ? 'data → compiler → families' : `${cap.derivedChannels.length} ch`,
    cap.brandThemePath ? `\`${cap.brandThemePath}\`` : '—',
    cap.documentPath ? `\`${cap.documentPath}\`` : '—',
    isData ? 'via compiler (evidence →)' : String(total),
...
export async function build() {
  ...
  const header = '| id | control | domain/type | default | channel | static path (BrandTheme) | DB path (TenantThemeDocument) | live reads | proven consumer |';
  const sep = '|---|---|---|---|---|---|---|---|---|';
...
## STANDARD — ${byTier('standard').length} controls (few dials, broad surface)

${table(byTier('standard'))}

## PRO — ${byTier('pro').length} controls (families and profiles)

${table(byTier('pro'))}

## EXPERT — closed allowlist

- **${expert.tenantRawAllowlist} tokens** allowed for raw override (source: ${expert.tenantRawAllowlistSource}).
- Domains: ${domains}.
- Overlap with live writers: ${expert.rawOverlapWithWriters}.
- Contract: \`TENANT_THEME_OVERRIDE_TOKENS\` (bounded); anything outside the list is rejected.

## INTERNAL (not product) — ${byTier('internal').length}
...
export async function check({ drill } = {}) {
  ...
  if (storedDigest !== digest) {
    failures.push(`controls view STALE vs contracts/census — regenerate with --write (observed=${storedDigest ?? 'absent'}, expected=${digest})`);
  }
  if (drill === 'stale' && DRILL_STALE_CANDIDATE !== digest) {
    failures.push(`controls view STALE vs contracts/census (drill) — injected candidate (observed=${DRILL_STALE_CANDIDATE}, expected=${digest})`);
  }
...
async function runWrite() {
  ...
  console.log(`controls-catalog: ${active.filter((c) => c.tier === 'standard').length} standard + ${active.filter((c) => c.tier === 'pro').length} pro + Expert allowlist escritos en docs/generated/customization-controls/index.md`);
  return 0;
}
