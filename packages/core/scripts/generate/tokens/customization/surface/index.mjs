 ...
 *
 *   - contracts/css/hooks/index.json            (foundation/component/tenant-channel
 ...
 * exact stale-number independent code audit caught. The runtime list from the built bundle is
...
export function buildDeadBaselineDoc({ live, previous, reason, widen = false }) {
  ...
  /* NO SE RE-ANCLA SOBRE UN ARCHIVO QUE NO SE ENTIENDE. Defecto reproducido, y
   ...
   *
   * Es el vector que cerro el microfix de independent code audit en DRILL-77, aplicado aca: la
    ...
    if (wantAll || check === 'freshness') {
      if (!existsSync(REPORT_PATH)) failures.push('freshness: report missing — run --write');
        ...
        if (drill === 'freshness' || stored.meta.inputsDigest !== report.meta.inputsDigest) {
          failures.push('freshness: surface/report/index.json is STALE relative to its inputs — regenerate with --write');
        }
      }
    }
