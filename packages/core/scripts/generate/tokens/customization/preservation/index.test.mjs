...
const run = (...a) => {
  const r = spawnSync(process.execPath, [join(ROOT, 'scripts/generate/tokens/customization/preservation/index.mjs'), ...a], { cwd: ROOT, encoding: 'utf8' });
...
test('drill: a protected premium dead writer marked RETIRE is forbidden', () => {
  const { status, out } = run('--drill=protected-premium-retire');
  assert.notEqual(status, 0);
  assert.match(out, /FORBIDDEN: protected premium/);
});
test('drill: an implemented entry with placeholder evidence is rejected', () => {
