/** Preserves premium customization while classifying every dead writer. */
...
/** Premium dead-writer families: RECONNECT/PROMOTE, never auto-RETIRE. */
export const PREMIUM_DEAD_PREFIXES = Object.freeze([
...
const PROVENANCE_RULES = [
  { id: 'A-protected-premium', test: (n) => PREMIUM_DEAD_PREFIXES.some((p) => n.startsWith(p)), decision: 'RECONNECT_OR_PROMOTE', owner: 'design-system/premium-customization' },
...
export function buildManifest() {
    ...
    return {
      ...
      preservationEvidence:
        entry.status === 'retired'
          ...
          : 'PLANNED batch contract: BitHire computed default preserved byte-identically; upstream-moves-downstream drill defined per implementation batch',
  ...
  if (drill === 'protected-premium-retire') dead.push({ name: '--ds-signal-card-drill', provenance: 'A-protected-premium', decision: 'RETIRE_WITH_DEATH_PROOF', owner: 'drill' });
  ...
  return {
    schemaVersion: 1,
    generatedBy: 'scripts/generate/tokens/customization/preservation/index.mjs --write',
    ...
    laws: [
      ...
      'protected premium provenance defaults to RECONNECT/PROMOTE — RETIRE is forbidden there',
...
export function checkManifest(manifest) {
  const failures = [];
  // Implemented entries require concrete evidence.
  ...
  for (const entry of manifest.deadWriters.entries) {
    ...
    if (entry.provenance === 'A-protected-premium' && /RETIRE/.test(entry.decision)) {
      failures.push(`FORBIDDEN: protected premium dead writer marked RETIRE: ${entry.name}`);
    }
  }
  if (existsSync(OUT)) {
