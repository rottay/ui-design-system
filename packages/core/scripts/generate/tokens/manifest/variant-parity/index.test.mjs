...
/**
 * F4A-close, Lote C (independent audit arbitraje final, C-1b): `baseline` retirado del
...
/**
 * P2-1 (F4A-close, hallazgo independent audit postaudit real-keypath-parity): `nextIsText`
...
test('drill ratchet: sube -> FAIL; baja -> FAIL con instruccion; igual -> PASS (silentPairs + placeholderPairs)', () => {
  ...
  assert.ok(bajo.some((f) => /lower `silentPairs` in scripts\/generate\/tokens\/manifest\/variant-parity\/baseline\/index\.json/.test(f)));
...
test('drill ratchet: divergentSlots is informative and is not evaluated', () => {
  const d = doc({ rottay: SIN_TAGS, bithire: SIN_TAGS, evnto: SIN_TAGS });
...
test('integration: paint denominator is derived from the live source leaves', () => {
  const real = build();
  assert.equal(
    real.metadataExclusion.paintDenominator.value,
    Object.values(real.matrix.paintLeaves).reduce((sum, count) => sum + count, 0),
  );
  assert.equal(real.metadataExclusion.count, METADATA_EXCLUSION.length);
