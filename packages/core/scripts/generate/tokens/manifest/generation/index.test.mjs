...
import { computeSourceDigest, sightedApprovers } from '../../../../check/evidence/framework/receipts/index.mjs';
...
import {
  ...
  validateCascadeRoot,
  validateCell,
  validateControlCalibrationEvidence,
...
const MANIFEST_ROOT = join(PACKAGE_ROOT, 'governance/manifest');
const PROGRAM_ROOT = join(PACKAGE_ROOT, 'scripts/check/modern-rescue');
...
test('the segmented manifest is structurally complete without treating UNKNOWN as progress', () => {
  ...
  // file that had copied the number. program/index.json is the one authority; this
...
test('frontier, internal and proposed controls never create R0-R6 family cells', () => {
  ...
  const dataTable = JSON.parse(
    readFileSync(join(MANIFEST_ROOT, 'families/pattern/data/pattern-data-table/index.json'), 'utf8'),
  );
  ...
  for (const entry of view.controls) {
    for (const row of entry.families) {
      const family = JSON.parse(
        readFileSync(join(MANIFEST_ROOT, `families/${row.familyId}/index.json`), 'utf8'),
      );
...
test('bootstrap refuses to overwrite the existing manifest', () => {
  const result = spawnSync(
    process.execPath,
    ['packages/core/scripts/generate/tokens/manifest/generation/index.mjs', '--bootstrap'],
...
const REAL_BINDING = 'packages/core/scripts/check/modern-rescue/README.md';
 ...
 * (`orchestration/index.json#doubleAccept.coordinator` and
 * `quality-rubric/index.json#eligibility.finalSightedAuthority`) and fails closed when
...
test('a source binding resolves by content, so a path never proves a symbol', () => {
  ...
  assert.equal(
    resolveSourceBinding('packages/core/src/components/primitives', { repositoryRoot: REPOSITORY_ROOT }),
    null,
  ...
  assert.match(
    resolveSourceBinding('packages/core/src/components/primitives#Flex', { repositoryRoot: REPOSITORY_ROOT }),
    /must name a file/,
  );
  assert.match(
...
test('SOURCE_BOUND refuses an unresolvable binding and names it', () => {
  const { errors } = gradeCell({
    ...
    sourceBindings: [REAL_BINDING, 'packages/core/src/components/primitives/layout/Flex/contracts/index.ts#NOT_A_REAL_EXPORT'],
  });
...
test('a new family-shaped source directory cannot sit outside the inventory', () => {
  ...
  assertNames(errors, 'source family directory is claimed by no family-inventory/index.json row', 'ui/primitives/display/Sparkle');
...
test('a forwarding compatibility surface is not a family, but the same shape that implements is', () => {
  ...
  assertNames(
    errors,
    'source family directory is claimed by no family-inventory/index.json row: ui/primitives/display/ForkedAvatar',
  );
...
test('control calibration evidence is content-validated without making empty receipt arrays mandatory', () => {
  const valid = evidenceFixture({ measured: { evidenceKind: 'data-field-delta' } });
  const gradeCalibration = (calibration, fixture = valid) =>
    validateControlCalibrationEvidence(calibration, {
      label: 'control.json: calibration',
      repositoryRoot: fixture.root,
      contracts: fixture.contracts,
      now: new Date(),
    });

  // IMPLEMENTED and SOURCE_BOUND controls may honestly have no receipt claim yet.
  assert.deepEqual(
    gradeCalibration({
      assessmentState: 'IMPLEMENTED',
  ...
  assert.deepEqual(
    gradeCalibration({
      ...
      resolution: { dataFieldDeltaEvidenceIds: valid.evidenceIds },
    }),
    [],
  );
  ...
  const stale = evidenceFixture({ stale: { sourceDigest: '0'.repeat(64) } });
  assertNames(
    gradeCalibration({ staticDbParityEvidenceIds: stale.evidenceIds }, stale),
    'staticDbParityEvidenceIds',
  ...
  const tampered = evidenceFixture({ tampered: { artifactSha256: 'f'.repeat(64) } });
  assertNames(
    gradeCalibration({ exactRestoreEvidenceIds: tampered.evidenceIds }, tampered),
    'exactRestoreEvidenceIds',
    'does not match declared',
  );

  const pathOnly = `${valid.contracts.evidence.root}/captures/path-only.json`;
  writeFileSync(join(valid.root, pathOnly), '{"pass":true}\n');
  const pathErrors = gradeCalibration(
    { resolution: { dataFieldDeltaEvidenceIds: [pathOnly] } },
    valid,
  );
  assert.ok(pathErrors.length > 0, 'an existing JSON path must not vacuously count as a receipt');
  assertNames(pathErrors, 'resolution.dataFieldDeltaEvidenceIds', 'missing required receipt field');
});
 ...
 * `governance/manifest/cascade/roots/*.json` the way the program-check drills do: the
...
const CASCADE_FIXTURE_SITE =
  'packages/core/scripts/libraries/manifest/rules/index.mjs';

const CASCADE_FIXTURE_CONTEXT = Object.freeze({
  label: 'fixture/cascade/roots/fixture.axis/index.json',
  repositoryRoot: REPOSITORY_ROOT,
...
const VARIANTS_REASON =
  'CERO variantes por diseno: el eje es un escape hatch, no un vocabulario cerrado. Declarado en fuente, no tapado: packages/core/scripts/libraries/manifest/rules/index.mjs:1.';

test('rootChannel.authority es obligatorio y de vocabulario cerrado', () => {
  const missing = cascadeRootFixture({ variants: [NAMED_VARIANT] });
  delete missing.rootChannel.authority;
  assert.ok(validateCascadeRoot(missing, CASCADE_FIXTURE_CONTEXT).some((e) => e.includes('rootChannel.authority')));

  const unknown = cascadeRootFixture({ variants: [NAMED_VARIANT] });
  unknown.rootChannel.authority = 'inferida';
  assert.ok(validateCascadeRoot(unknown, CASCADE_FIXTURE_CONTEXT).some((e) => e.includes('rootChannel.authority')));
  assert.deepEqual(validateCascadeRoot(cascadeRootFixture({ variants: [NAMED_VARIANT] }), CASCADE_FIXTURE_CONTEXT), []);
});

test('tenant-seed admite sólo las dos seeds exactas bajo el contrato color-set', () => {
  const context = {
    ...CASCADE_FIXTURE_CONTEXT,
    activeControlIds: ['palette.status-seeds'],
    controlTier: 'standard',
    controlDomain: { kind: 'color-set', enumValues: [], bounds: null },
  };
  const seed = cascadeRootFixture({
    rootId: 'palette.status-seeds',
    variantDisposition: 'continuous',
    variants: [NAMED_VARIANT],
    rootChannel: {
      channel: '--ds-color-success',
      authority: 'tenant-seed',
      emission: [{ kind: 'compiler-emit', site: CASCADE_FIXTURE_SITE }],
    },
  });
  assert.deepEqual(validateCascadeRoot(seed, context), []);

  const lookalike = { ...seed, rootId: 'fixture.axis' };
  const errors = validateCascadeRoot(lookalike, { ...context, activeControlIds: ['fixture.axis'] });
  assert.ok(errors.some((e) => e.includes('restricted to the exact palette seed root channels')));

  const wrongDomain = validateCascadeRoot(seed, { ...context, controlDomain: { kind: 'token-map', enumValues: [], bounds: null } });
  assert.ok(wrongDomain.some((e) => e.includes('continuous color-set')));
});
 ...
 * the vocabulary tests do — `governance/manifest/cascade/roots/*.json` is a live,
...
const DERIVATIONS_REASON =
  'CERO derivaciones por diseno: el eje emite su canal cabeza y nadie deriva de el. Declarado en fuente, no tapado: packages/core/scripts/libraries/manifest/rules/index.mjs:1.';
...
// off governance/manifest/cascade by path: a drill can declare `density.mode` a DATA
