 ...
 *
 *   node --test scripts/generate/tokens/manifest/fanout/index.test.mjs
 */
...
test('ORACULO --ds-badge-sm-padding-x: solo estampado inline en el tsx modern, sin skin CSS', () => {
  ...
  const hit = stamps.find(
    (r) => r.file === 'src/components/primitives/display/badge/engines/modern/index.tsx' && r.line === 52,
  );
...
test('ORACULO --ds-input-success-message-margin-top: cero lectores → paints=false', () => {
  ...
  assert.equal(row.paintsInModern, false);
  assert.ok(
    row.declaredIn.includes('src/foundation/tokens/css/presentation/components/input/index.css'),
...
test('classifyPlane mapea cada raiz a su plano', () => {
  const cases = [
    ['src/foundation/tokens/css/runtime/engines/modern/skin/button/index.css', 'modern-skin-css'],
    ...
    ['src/foundation/tokens/css/runtime/engines/classic/theme/index.css', 'classic-css'],
    ['src/foundation/tokens/css/runtime/engines/rustic/theme.css', 'rustic-css'],
    ['src/components/primitives/display/Badge/engines/modern/index.tsx', 'tsx-inline-stamp'],
    ...
    ['src/graphics/icons/glyphs/runtime/factory/index.tsx', 'graphics-tsx'],
...
test('runtime-css excluye lo que cuelga de engines/, que ya tiene su propio plano', () => {
  assert.equal(
    classifyPlane('src/foundation/tokens/css/runtime/engines/modern/skin/button/index.css'),
    'modern-skin-css',
  );
...
test('classifyPlane devuelve null (unclassified) para sitios fuera del vocabulario', () => {
  // Documentation is outside every source paint plane.
  assert.equal(classifyPlane('docs/guides/i18n/examples/index.md'), null);
  // .css fuera del arbol de tokens
  assert.equal(classifyPlane('src/components/primitives/display/Badge/badge.css'), null);
});
...
test('isModernPaintReader exige engines/modern para el estampado inline', () => {
  assert.equal(
    isModernPaintReader('tsx-inline-stamp', 'src/components/primitives/display/Badge/engines/modern/index.tsx'),
    true,
  );
  assert.equal(
    isModernPaintReader('tsx-inline-stamp', 'src/components/primitives/display/Badge/engines/rustic/index.tsx'),
    false,
  );
...
test('los artifacts generados y el material de test/fixture/story quedan fuera', () => {
  assert.equal(isExcludedPath('src/foundation/tokens/css/facade/artifacts/rottay.css'), true);
  assert.equal(isExcludedPath('tests/fixtures/tenants/rottay.css'), true);
  assert.equal(isExcludedPath('src/components/primitives/display/Badge/tests/badge.test.tsx'), true);
  assert.equal(isExcludedPath('src/components/primitives/display/Badge/Badge.stories.tsx'), true);
  assert.equal(isExcludedPath('src/components/primitives/display/Badge/engines/modern/index.tsx'), false);
