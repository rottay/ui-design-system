...
function fakeCanon(overrides = {}) {
  return {
    ...
    owners: overrides.owners ?? [
      { id: 'primitive/display/badge', owner: 'src/components/primitives/display/Badge' },
    ],
...
test('ownerFamily resuelve por el prefijo sourceOwner mas largo', () => {
  const owners = [
    { id: 'chart/basic/area-chart', owner: 'src/components/patterns/visualization/charts/families/area-chart' },
    { id: 'pattern/visualization/charts', owner: 'src/components/patterns/visualization' },
  ].sort((a, b) => b.owner.length - a.owner.length);
  assert.equal(
    ownerFamily(owners, 'src/components/patterns/visualization/charts/families/area-chart/index.tsx'),
    'chart/basic/area-chart',
    'gana el prefijo mas largo, no el primero',
  );
  assert.equal(ownerFamily(owners, 'src/components/patterns/visualization/otro/index.tsx'), 'pattern/visualization/charts');
  assert.equal(ownerFamily(owners, 'src/entrypoints/public/index.ts'), null);
});

test('un prefijo solo cuenta en frontera de carpeta', () => {
  const owners = [{ id: 'x', owner: 'src/components/primitives/display/Badge' }];
  assert.equal(ownerFamily(owners, 'src/components/primitives/display/BadgeGroup/index.tsx'), null);
  assert.equal(ownerFamily(owners, 'src/components/primitives/display/Badge/index.tsx'), 'x');
});
...
test('resolveSite 1: una ruta bajo un sourceOwner gana sin mirar nada mas', () => {
  const site = { file: 'src/components/primitives/display/Badge/engines/modern/index.tsx', line: 52, prop: 'paddingInline' };
  ...
  const facts = {
    channels: [
      channel('--ds-a', [
        ...
        reader('tsx-inline-stamp', 'src/components/primitives/display/Badge/index.tsx', 5, 'md'),
      ]),
    ],
  };
...
test('buildGraph: una clave de tabla de lookup se descarta y queda auditable', () => {
  // src/components/primitives/feedback/Modal/engines/rustic/index.tsx: RADIUS_STYLES
  const facts = {
    channels: [
      channel('--ds-modal-radius-md', [
        reader('tsx-inline-stamp', 'src/components/primitives/feedback/Modal/engines/rustic/index.tsx', 140, 'md'),
      ]),
  ...
  const facts = {
    channels: [
      channel('--ds-badge-sm-padding-x', [
        reader('tsx-inline-stamp', 'src/components/primitives/display/Badge/engines/modern/index.tsx', 52, 'paddingInline'),
      ]),
...
function tempRootsDir(files) {
  ...
  for (const [id, body] of Object.entries(files)) {
    const target = path.join(dir, pathForManifestId(id), 'index.json');
...
function manifestFile(name, body) {
  ...
  const relativePath = `${pathForManifestId(body.rootId)}/index.json`;
  return { name: relativePath, rel: `${CASCADE_ROOTS_REL}/${relativePath}`, text, doc: JSON.parse(text) };
}
...
test('citeLiteral resuelve rutas con y sin el prefijo packages/core', () => {
  const conPrefijo = citeLiteral('packages/core/scripts/generate/tokens/manifest/fanout/index.mjs', 'EXCLUDED_PREFIXES = [');
  const sinPrefijo = citeLiteral('scripts/generate/tokens/manifest/fanout/index.mjs', 'EXCLUDED_PREFIXES = [');
...
test('classifyManifestRoot: una cabeza --ds-* presente en los hechos es medible', () => {
  const out = classifyManifestRoot(
    manifestFile('shape.radius-scale.json', {
      rootId: 'shape.radius-scale',
      rootChannel: { channel: '--ds-radius-scale', authority: 'floor-arbitrated', emission: [] },
    }),
    new Set(['--ds-radius-scale']),
  );
  assert.equal(out.measurable, true);
  assert.equal(out.rootChannel, '--ds-radius-scale');
  assert.equal(out.rootAuthority, 'floor-arbitrated');
  assert.equal(out.reasonCode, null);
  assert.equal(out.notMeasurableReason, null);
  assert.match(out.declaredAt, new RegExp(`^${CASCADE_ROOTS_REL}/shape/radius-scale/index\\.json:\\d+$`));
});
...
test('caso borde 1 — rootChannel null: no revienta y declara su motivo', () => {
  ...
  assert.match(out.notMeasurableReason, new RegExp(`${CASCADE_ROOTS_REL}/responsive/posture/index\\.json:\\d+`), 'lleva cita archivo:linea');
...
test('caso borde 2 — cabeza que es ATRIBUTO, no custom property', () => {
  ...
  assert.match(out.notMeasurableReason, new RegExp(`${CASCADE_ROOTS_REL}/chrome/anatomy/index\\.json:\\d+`));
});
...
test('un --ds-* declarado pero ausente de los hechos no se descarta en silencio', () => {
  ...
  assert.match(out.notMeasurableReason, /no existe como canal en generated\/fanout-facts\/index\.json/);
});

test('loadManifestRoots lee el directorio completo, ordenado y sin escribir nada', () => {
  const dir = tempRootsDir({
    'b.eje': { rootId: 'b.eje', rootChannel: { channel: '--ds-b', emission: [] } },
    'a.eje': { rootId: 'a.eje', rootChannel: { channel: null, emission: [] } },
  });
  try {
    ...
    assert.deepEqual(
      readManifestRecords(dir, 'rootId').map(({ id }) => id),
      ['a.eje', 'b.eje'],
      'el directorio queda intacto',
    );
  } finally {
...
test('mergeRoots: la union no duplica y la procedencia es exacta', () => {
  ...
  const declared = [
    { rootId: 'shape.radius-scale', rootChannel: '--ds-radius-scale', rootAuthority: 'floor-arbitrated', measurable: true },
    { rootId: 'palette.seeds', rootChannel: '--ds-color-primary', rootAuthority: 'tenant-seed', measurable: true },
  ...
  assert.equal(provenance.get('--ds-color-primary').rootAuthority, 'tenant-seed');
...
test('todos los manifiestos de cascada se clasifican, sin excepcion', () => {
  const onDisk = readManifestRecords(CASCADE_ROOTS_DIR, 'rootId').map(({ id }) => id).sort();
  assert.equal(declaredRoots.length, onDisk.length, 'ni uno se pierde por el camino');
  assert.deepEqual(declaredRoots.map((root) => root.rootId).sort(), onDisk, 'cada rootId tiene un unico path jerarquico');
...
test('los casos borde reales estan declarados, no silenciados', () => {
  ...
  assert.equal(anatomy.rootAuthority, 'floor-arbitrated');
  ...
  assert.equal(doc.roots.find((r) => r.root === '--ds-color-primary').rootAuthority, 'tenant-seed');
  assert.equal(doc.roots.find((r) => r.root === '--ds-color-success').rootAuthority, 'tenant-seed');
