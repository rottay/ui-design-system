 ...
 * el resto.
 *
 * Este script NO decide nada: proyecta `generated/fanout-facts/index.json` sobre el
 * canon de familias.
 *
...
export const FACTS_GENERATOR_REL = 'scripts/generate/tokens/manifest/fanout/index.mjs';
...
export function loadCanon(programRoot = PROGRAM_ROOT, packageRoot = PACKAGE_ROOT) {
  const inventory = JSON.parse(readFileSync(path.join(programRoot, 'family-inventory/index.json'), 'utf8'));
...
export function classifyManifestRoot(file, knownChannels, packageRoot = PACKAGE_ROOT) {
  ...
  const base = {
    rootId,
    rootChannel: raw,
    rootAuthority: typeof rootChannel?.authority === 'string' ? rootChannel.authority : null,
    declaredAt,
    file: file.rel,
  };
  ...
  if (!knownChannels.has(raw)) {
    ...
    return {
      ...
      notMeasurableReason: [
        `La cabeza ${raw} (${declaredAt}) no existe como canal en generated/fanout-facts/index.json.`,
...
export function loadManifestRoots(knownChannels, dir = CASCADE_ROOTS_DIR, packageRoot = PACKAGE_ROOT, relBase = CASCADE_ROOTS_REL) {
  return readManifestRecords(dir, 'rootId').map(({ document, pathname, relativePath }) => {
    const text = readFileSync(pathname, 'utf8');
    return classifyManifestRoot(
      { name: relativePath, rel: `${relBase}/${relativePath}`, text, doc: document },
      knownChannels,
      packageRoot,
    );
  });
  ...
  return {
    ...
    ...(provenance
      ? {
        ...
        rootAuthority: provenance.rootAuthority,
      }
      : {}),
    summary: {
...
export function mergeRoots(facts, extraRoots = [], manifestRoots = []) {
  ...
  const authoritiesByChannel = new Map();
  for (const d of manifestRoots) {
    ...
    if (!authoritiesByChannel.has(d.rootChannel)) authoritiesByChannel.set(d.rootChannel, new Set());
    if (d.rootAuthority) authoritiesByChannel.get(d.rootChannel).add(d.rootAuthority);
  }
  ...
  for (const r of roots) {
    ...
    const rootAuthorities = [...(authoritiesByChannel.get(r) ?? [])].sort();
    provenance.set(r, {
      discoveredBy: inPattern && inManifest ? 'both' : inPattern ? 'pattern' : inManifest ? 'manifest' : 'explicit',
      rootId: rootIds[0] ?? null,
      rootIds,
      rootAuthority: rootAuthorities.length === 1 ? rootAuthorities[0] : null,
    });
  }
...
export function buildChecklists(
  ...
  const notMeasurable = declared
    ...
    .map(({ rootId, rootChannel, rootAuthority, declaredAt, reasonCode, notMeasurableReason, evidence }) => ({
      rootId,
      rootChannel,
      rootAuthority,
      declaredAt,
  ...
  return {
    ...
    source: 'artifacts/generated/manifest/cascade/fanout/index.json',
    familyCanon: 'scripts/check/modern-rescue/family-inventory/index.json',
    corpusProvenance: {
      revertWave: [
