 ...
 *
 * Run: `node scripts/check/architecture/audits/integration/index.mjs`
 * Hook: `"lint:integration": "node scripts/check/architecture/audits/integration/index.mjs"`
 */
...
if (emittedVars.size > 0) {
  // Search every product and token-CSS consumer.
  ...
  for (const [varName, emitterPaths] of emittedVars) {
    ...
    if (!allConsumerContent.includes(varName)) {
      violations.push({
        ...
        message: `Premium chrome var "${varName}" is emitted but has no non-test consumer in engines/components/token CSS.`,
      });
    }
  }
}
...
const componentTokenFiles = walkFiles(COMPONENT_TOKEN_DIR, /\.css$/).filter(
  (file) => file !== join(COMPONENT_TOKEN_DIR, 'index.css')
);
if (componentTokenFiles.length === 0) {
  violations.push({
    rule: 'token-consumption-scan-empty',
    path: relPath(COMPONENT_TOKEN_DIR),
    message: 'Token-consumption audit found no component contracts.',
  });
}

let matchedModernEngines = 0;

for (const tokenFile of componentTokenFiles) {
  const tokenContent = readSafe(tokenFile);
  if (!tokenContent) continue;

  const baseName = basename(dirname(tokenFile));
...
if (componentTokenFiles.length > 0 && matchedModernEngines === 0) {
  violations.push({
    rule: 'token-consumption-engine-scan-empty',
    path: relPath(COMPONENT_TOKEN_DIR),
    message: 'Token-consumption audit resolved no component contract to a Modern engine.',
  });
}
...
for (const exemptedName of PRE_EXISTING_PERSONALITY_ORPHANS) {
  if (!emittedPersonalityVars.has(exemptedName)) {
    violations.push({
      ...
      path: relPath(PERSONALITY_CANONICAL_PATH),
      message: `PRE_EXISTING_PERSONALITY_ORPHANS names "${exemptedName}", which resolvePartialPersonalityCssVariables() no longer emits. Remove it from the exemption list in scripts/check/architecture/audits/integration/index.mjs.`,
    });
  }
}
