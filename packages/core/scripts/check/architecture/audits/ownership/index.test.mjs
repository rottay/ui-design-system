...
test('Collapse owns its public token hook without an infrastructure-to-UI re-export', () => {
  ...
  const collapseOwner = parse('components/primitives/layout/collapse/index.ts');
  const layoutOwner = parse('components/primitives/layout/index.ts');
  ...
  assert(
    moduleSpecifiers(hookFacade).every((specifier) => !specifier.includes('/components/')),
    'infrastructure hook facade must not re-export UI owners',
  );

  assertStarExport('components/primitives/index.ts', './layout');
  assertStarExport('components/index.ts', './primitives');
  assertStarExport('index.ts', './components');
});

test('onboarding stays a WizardSurface configuration, not a second authored surface owner', () => {
  const experience = parse('components/surfaces/presentation/pages/experience/index.ts');
  ...
  assertStarExport('components/surfaces/presentation/pages/forms/index.ts', './wizard');
  assertStarExport('components/surfaces/presentation/pages/index.ts', './forms');

  assert.equal(
    existsSync(resolve(SOURCE_ROOT, 'components/surfaces/presentation/pages/forms/wizard/onboarding')),
    false,
    'onboarding must not exist as an authored owner below the wizard surface',
  );

  const forms = parse('components/surfaces/presentation/pages/forms/index.ts');
  assert(
...
test('retired collection-workspace editorial kit barrel stays deleted', () => {
  assert.equal(
    existsSync(resolve(SOURCE_ROOT, 'components/structures/_kits/collection-workspace/index.ts')),
    false,
  );
});
