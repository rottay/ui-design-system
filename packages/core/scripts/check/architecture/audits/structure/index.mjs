 ...
 *
 * Usage:
 *   node scripts/check/architecture/audits/structure/index.mjs --check
 *   node scripts/check/architecture/audits/structure/index.mjs --report --details=30
 *   node scripts/check/architecture/audits/structure/index.mjs --write-baseline
 */
...
const DEFAULT_BASELINE_PATH = resolve(SCRIPT_DIRECTORY, 'baseline/index.json');
 ...
 * the direct children of the physical `components/` macro root. A higher UI layer may
...
export const SCOPED_OWNER_RANKS = Object.freeze({
  ...
  // engine-adapter is the posture/projection contract over that product, intent
  // narrows one ingress onto a patch, resolved adds the envelope the merge
  // destroys, and tenant-theme owns the persisted transport.
  ...
  // `tenant-theme` is LAST because a compiled artifact is the terminal product
  // of this chain, not an early stage of it: one row persists the ledger, the
  // lowered variables, the scope it is nested under and the engine projection a
  // runtime reads, so the transport is the widest consumer here rather than a
  // peer of the stages it stores.
  ...
  'graphics/icons/glyphs': Object.freeze({
    foundation: 0,
    runtime: 1,
    presentation: 2,
  }),
  'graphics/icons/semantic': Object.freeze({
    sources: 0,
    generated: 0,
    foundation: 1,
    runtime: 2,
  ...
  'graphics/icons/semantic/foundation': Object.freeze({
    registry: 0,
    contracts: 1,
    policy: 1,
    provenance: 1,
  ...
  'entrypoints/eslint': Object.freeze({
    contracts: 0,
    rules: 1,
    plugin: 2,
  }),
  ...
  'components/structures': Object.freeze({
    shell: 0,
    headers: 1,
  }),
  ...
  'components/structures/shell': Object.freeze({
    'surface-chrome': 0,
    navigation: 1,
  }),
  ...
  'components/structures/workspace': Object.freeze({
  ...
  'components/structures/feedback/surface-lifecycle': Object.freeze({
    states: 0,
    'use-surface-state': 1,
  }),
});
 ...
 * names are intentionally scoped to `components/<tier>/<category>/<Component>` instead
...
function readPublicEntrypointManifest(packageRoot) {
  const manifestPath = resolve(packageRoot, 'contracts/package/entrypoints/index.json');
...
function resolveSourceModule({ importer, specifier, sourceRoot, sourcePaths }) {
  ...
  if (physicalSpecifier.startsWith('.')) {
    ...
    unresolved = resolve(sourceRoot, 'components', physicalSpecifier.slice('@ui/'.length));
...
function resolveLocalLayer(file, commonDirectory) {
  ...
  for (const segment of pathSegments(relativeDirectory)) {
    ...
    // `runtime/` still belongs to the outer `components/surfaces` layer when it
    // consumes `components/patterns`).
...
export function auditCoreStructure({
  ...
  Object.entries(architectureTiers).forEach(([tier, domains], rank) => {
    ...
    // hierarchy (`components/primitives/`) without creating an anonymous root.
  ...
  // prevents (`components/structures/record/record/**`); non-adjacent layer names such
  ...
  // beside them. Keep ambient shims with the contracts they extend so the
  // source root contains only discoverable public entrypoints.
  for (const file of sourceFiles.filter(({ isDeclaration, relativePath }) => (
    ...
    findings.push(finding(
      ...
      file.relativePath,
      file.relativePath,
      'Root declaration file is loose; move it under foundation/contracts/ambient.',
      { destination: 'foundation/contracts/ambient' },
    ));
  }

  // The entrypoint support root is deliberately not another architecture
  // tier. Every subtree starts at a registered package boundary; code below
  // that boundary follows the same folder/index and test-container laws as
  // the rest of the source tree.
  const registeredEntrypointOwners = sorted([...rootEntrypoints]
    .filter((entrypoint) => entrypoint.startsWith('entrypoints/'))
    .map((entrypoint) => toPosix(dirname(entrypoint))));
  for (const file of files.filter(({ relativePath }) => (
    pathSegments(relativePath)[0] === 'entrypoints'
  ))) {
    const owner = registeredEntrypointOwners.find((candidate) => (
      file.relativePath === `${candidate}/index.ts`
      || file.relativePath === `${candidate}/index.tsx`
    ...
    if (!owner) {
      findings.push(finding(
        ...
        file.relativePath,
        file.relativePath,
        'Entrypoint support source is outside every registered package boundary.',
      ));
      continue;
    }
    if (file.isSource && !file.isIndex && !file.ignoredKind) {
      findings.push(finding(
        'invalid-entrypoint-support-module',
        file.relativePath,
        file.relativePath,
        'Entrypoint implementation sources must use folder/index.ts(x).',
      ));
    }
  }

  const owningTestContainer = (filePath) => {
    let current = dirname(filePath);
    while (current !== absoluteSourceRoot && current.startsWith(`${absoluteSourceRoot}${sep}`)) {
      if (TEST_CONTAINERS.has(basename(current))) return current;
      current = dirname(current);
    }
    return null;
  };

  // Rule 1: test files must be in a named test container. A semantic
  // folder/index test may live below that container; the container remains
  // the ownership boundary while the child directory names the behavior.
  for (const file of sourceFiles.filter(({ isTest }) => isTest)) {
    if (!owningTestContainer(file.path)) {
      findings.push(finding(
  ...
  const testContainerDirectories = new Set(
    sourceFiles
      .filter(({ ignoredKind }) => ignoredKind === 'test' || ignoredKind === 'test-support')
      .map(({ path }) => owningTestContainer(path))
      .filter(Boolean),
  );
  ...
  for (const [directory, children] of [...directChildDirectories]
    ...
    if (directorySegments.length === 2
      && directorySegments[0] === 'components'
    ...
    const branchRanks = (
      directory === 'foundation/contracts'
      || directory === 'graphics/icons/glyphs'
      || directory === 'graphics/icons/semantic'
      || directory === 'graphics/icons/semantic/foundation'
    )
      ? SCOPED_OWNER_RANKS[directory]
      : (isUiCapabilityOwner(directory)
    ...
    for (const specifier of moduleSpecifiers(sourceFile.source, sourceFile.path)) {
      ...
      if (sourceUiLayer && targetUiLayer && sourceUiLayer.rank !== targetUiLayer.rank) {
        ...
        findings.push(finding(
          ...
          `${sourceFile.relativePath}->${targetFile.relativePath}`,
          sourceFile.relativePath,
          `UI layer "${sourceUiLayer.name}" depends upward on "${targetUiLayer.name}" under components.`,
          {
            commonOwner: 'components',
...
export function createStructureBaseline(result) {
  ...
  return {
    ...
    policy: {
      ...
      localLayers: 'foundation/kernel/contracts/policy/quality/spec/validation < runtime < composition/react < presentation < facade/public; components/primitives < components/patterns < components/structures < components/surfaces; inside one UI capability foundation/contracts < runtime < engines < presentation < compound',
...
function selectCliMode(argv) {
  if (argv.includes('--write-baseline')) return 'write-baseline';
