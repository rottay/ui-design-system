...
test('rejects a deep import of an alias that only the parent barrel publishes', () => {
  const root = fixture({
    'src/components/primitives/Typography/compound/Text/index.tsx': 'export const TypographyText = 1;\n',
    'src/components/primitives/Typography/index.tsx': `
import { TypographyText } from './compound/Text';
export { TypographyText as Text };
`,
    'src/components/patterns/thing/index.tsx': `
...
test('accepts the same import once the compound module publishes the alias', () => {
  const root = fixture({
    'src/components/primitives/Typography/compound/Text/index.tsx': `
...
test('rejects a re-export of a binding the source module does not publish', () => {
  const root = fixture({
    'src/components/primitives/Box/index.ts': 'export const Box = 1;\n',
    'src/entrypoints/public/box/index.ts': "export { Container } from '../../../components/primitives/Box';\n",
  });
...
test('follows export-star chains before declaring a binding absent', () => {
  const root = fixture({
    'src/components/primitives/Box/impl/index.ts': 'export const Box = 1;\n',
    'src/components/primitives/Box/index.ts': "export * from './impl';\n",
    'src/components/patterns/thing/index.ts': "import { Box } from '../../primitives/Box';\nexport const Thing = Box;\n",
  });
  assert.equal(run(root).checkedEdges, 1);
});
...
test('skips importers of a module whose export surface cannot be resolved', () => {
  const root = fixture({
    'src/components/primitives/Box/index.ts': "export * from 'react';\n",
    'src/components/patterns/thing/index.ts': "import { useState } from '../../primitives/Box';\nexport const Thing = useState;\n",
  });
...
test('ignores type-only imports and type-only specifiers', () => {
  const root = fixture({
    ...
    'src/components/patterns/thing/index.ts': `
...
test('resolves a default import through both export-default and export-assignment forms', () => {
  const root = fixture({
    'src/components/primitives/Box/index.tsx': 'const Box = 1;\nexport default Box;\n',
    'src/components/primitives/Card/index.tsx': 'const Card = 1;\nexport { Card as default };\n',
    'src/components/patterns/thing/index.ts': `
...
test('resolves destructured const exports', () => {
  const root = fixture({
    'src/components/primitives/Box/index.ts': 'export const { Box, Card } = { Box: 1, Card: 2 };\n',
    'src/components/patterns/thing/index.ts': "import { Card } from '../../primitives/Box';\nexport const Thing = Card;\n",
  });
  assert.doesNotThrow(() => run(root));
});
