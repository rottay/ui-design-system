#!/usr/bin/env node
/**
 * Visual worklist gate — the binding worklist may never point a writer at CSS that
 * does not ship.
 *
...
import { collectReachable } from '../../../engine/css/paint/layers/index.mjs';
...
function reachableSet() {
  ...
  const shipperOk = tails.has('presentation/components/patterns/index.css');
 ...
 *            independent code audit audit found. Presence is decided by own-key, not by
 ...
 * CI, which was the final fail-open found during the independent code audit audit. */
...
export function checkWorklist({ rows, reachable, drillCase, roots = siblingRoots() }) {
  ...
  if (ca) {
    ...
    if (drillCase === 'prose-instead-of-structure') { ca.renderProof.consumerEvidence = 'app-bithire/src/components/surfaces/index.tsx (renders it, trust me)'; }
  ...
  for (const row of list) {
    ...
    if (rp.verdict === 'CONSUMER_APPLIED') {
      ...
      // path returned 0 failures — reproduced by independent code audit). The evidence is now
...
function main() {
  if (!existsSync(WORKLIST)) {
    console.error('visual-worklist FAIL — index.json ausente');
    process.exit(1);
  }
  const doc = JSON.parse(readFileSync(WORKLIST, 'utf8'));

  if (write) {
    if (drill || typeof doc.authority?.rows !== 'object' || doc.authority.rows === null) {
      console.error('customization-worklist WRITE FAIL — authority.rows válido y ausencia de drill son obligatorios');
      process.exit(1);
    }
    doc.authority.rows.canonicalRowsSha256 = canonicalRowsSha256(doc);
  }
  ...
  if (authorityFailures.length > 0) {
    for (const f of authorityFailures.slice(0, 12)) console.error(`customization-worklist AUTHORITY FAIL — ${f}`);
    console.error(`customization-worklist: ${authorityFailures.length} violación(es) de autoridad/metadata`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`customization-worklist FAIL — ${error.message}`);
    process.exit(1);
  }
  ...
  if (drill) {
    if (failures.length === 0) {
      console.error(`customization-worklist DRILL FAIL — "${drill}" no produjo violaciones (gate vacuo)`);
      process.exit(1);
    }
    console.log(`customization-worklist drill "${drill}" OK — ${failures.length} violación(es): ${failures.join(" | ").slice(0, 400)}`);
    return;
  }
  if (failures.length > 0) {
    for (const f of failures.slice(0, 12)) console.error(`customization-worklist FAIL — ${f}`);
    console.error(`customization-worklist: ${failures.length} violación(es) en ${rows.length} filas`);
    process.exit(1);
  }
  if (write) writeFileSync(WORKLIST, `${JSON.stringify(doc, null, 2)}\n`, 'utf8');
  console.log(`customization-worklist OK — ${rows.length} filas: autoridad/metadata válidas, digest de filas re-firmado, owners shipping-reachable, cero tombstone en filas, renderProof = nodo`);
}
