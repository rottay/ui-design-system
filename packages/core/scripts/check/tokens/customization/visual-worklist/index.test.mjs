/**
 * Visual-worklist drills, grouped for CI.
...
test('upstream vinculante: borrar o agregar una adjudicación relevante es rojo', () => {
  ...
  added.entries['--ds-synthetic-pending-part'] = { decision: 'PENDING_PART_BLOCKED' };
  assert.ok(matches(
    red(loadDoc(), { loadUpstream: () => added }),
    /A_pendingPartBlocked:.*faltan.*--ds-synthetic-pending-part/,
  ));
});
...
test('env presente pero inutilizable: rojo con causa y JAMÁS fallback', () => {
  const tempDir = mkdtempSync(join(tmpdir(), 'visual-worklist-'));
