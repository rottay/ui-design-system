...
export async function buildMembership({
  ...
  /* PISO DE ACEPTACION DE R2 (mejora independent audit): filas del inventario que ganan
  ...
  return {
    generated: true,
    generator: 'scripts/check/tokens/cascade/roots/membership/index.mjs',
    schemaVersion: 1,
    law: {
...
async function main(argv) {
  ...
  if (flags.length > 1 || !['--check', '--write'].includes(mode)) {
    console.error('uso: node scripts/check/tokens/cascade/roots/membership/index.mjs [--check|--write]');
    process.exit(2);
  }
