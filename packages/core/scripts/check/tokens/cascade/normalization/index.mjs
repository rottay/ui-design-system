...
export function readTree(coreRoot = CORE_ROOT) {
  ...
  const authorable = controlRecords.filter(
    (record) => record.ingress.staticThemePath || record.ingress.dbTenantThemePath,
  );
...
export function evaluate(result, baseline) {
  ...
  if (!baseline) return { failures: [...failures, '[baseline] no existe baseline/index.json'], drift: [] };
  const drift = [];
...
async function main(argv) {
  ...
  if (!['--check', '--write-baseline'].includes(mode)) {
    console.error('uso: node scripts/check/tokens/cascade/normalization/index.mjs [--check|--write-baseline --reason "..."]');
    process.exit(2);
  }
