...
test('sobre el arbol real: las leyes duras se sostienen y el ancla coincide', async () => {
  ...
  const baseline = JSON.parse(
    (await import('node:fs')).readFileSync(new URL('./baseline/index.json', import.meta.url), 'utf8'),
  );
...
test('el ancla registra QUE CLASE de movimiento la produjo', async () => {
  ...
  const baseline = JSON.parse(readFileSync(new URL('./baseline/index.json', import.meta.url), 'utf8'));
