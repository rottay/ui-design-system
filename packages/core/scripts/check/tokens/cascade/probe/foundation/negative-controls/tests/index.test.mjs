 ...
 * (`foundation/roster/fixtures/index.json`): Card, Button, Input, Skeleton and the
...
const INLINE_STYLE_SCAN_ROOTS = Object.freeze([
  'src/components/primitives/layout/flex/engines/modern',
  'src/components/primitives/layout/grid/engines/modern',
  'src/components/primitives/layout/stack/engines/modern',
  'src/components/primitives/layout/space/engines/modern',
  'src/components/primitives/inputs/button/engines/modern',
  'src/components/primitives/inputs/input/engines/modern',
  'src/components/primitives/display/card/engines/modern',
  'src/components/primitives/feedback/skeleton/engines/modern',
]);
...
test('every spacing.rhythm family declares ONLY did-not-move negative controls', () => {
  for (const family of LAYOUT_FAMILIES) {
    const manifest = readManifest(
      resolve(MANIFEST_ROOT, `families/primitive/layout/${family}/index.json`),
    );
...
test('negative drill: dropping one binding fails closed rather than checking nothing', () => {
  const manifest = readManifest(resolve(MANIFEST_ROOT, 'families/primitive/layout/flex/index.json'));
  const declared = declaredPhrasesFor({
    controlManifest: CONTROL_MANIFEST,
    familyManifest: manifest,
    controlId: 'spacing.rhythm',
...
test('active evidence ids resolve to receipts, while historical receipts never become source bindings', () => {
  ...
  if (ids.length === 0) {
    assert.match(
      control.calibration.nextAction,
      /RERUN_CAUSAL_PROOF_AFTER_TREE_FREEZE/,
      'empty active evidence is legal only while the canonical-path refactor awaits a fresh causal run',
    );
  }
  ...
  for (const family of LAYOUT_FAMILIES) {
    ...
    const cell = manifest.themeControls.find((entry) => entry.controlId === 'spacing.rhythm');
    assert.equal(cell.evidenceIds.length, ids.length === 0 ? 0 : 2, `${family} evidence state must match the control`);
  ...
  while (pending.length > 0) {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      assert.deepEqual(hashedEvidence, [], `${relative(evidenceDir, absolute)} hashes its own evidence root`);
    }
  }
