 ...
 *
 * Run: node --test scripts/check/tokens/cascade/probe/quality/drills/tests/index.test.mjs
 *
...
test('formula drill: the fresh composition agrees with the repository staleness gate', async () => {
  ...
  assert.equal(
    result.agrees,
    true,
    'this harness recomposes the bundles with a formula transcribed from scripts/build/verticals/css-build/index.mjs. ' +
...
test('bundle: the generated artifact is byte-identical to dist/, so it is not a second opinion', async () => {
  const shipped = await resolveBundle({ vertical: 'rottay', mode: 'dist' });
  assert.equal(shipped.provenance.distMatchesGenerated, true);
  assert.equal(
...
test('tenant-less scope: the shipped bundle modes refuse rather than silently recompose', async () => {
  for (const mode of ['dist', 'generated']) {
  ...
  try {
    for (const vertical of ['rottay', 'bithire', 'evnto']) {
      const committedPath = resolvePath(
        CORE_ROOT,
        `artifacts/quality/programs/modern-rescue/cascade-proofs/controls/responsive-posture/computed-static-db/${vertical}.json`,
      );
...
test('C5 drill W-A: ONE resolver owns the calibration surface, and the harness has no second reader', async () => {
  ...
  const sources = [
    'scripts/check/tokens/cascade/probe/runtime/ingress/index.mjs',
    'scripts/check/tokens/cascade/probe/composition/run/index.mjs',
  ];
  let directReads = 0;
