 ...
 *
 * Run: node --test scripts/check/tokens/cascade/probe/runtime/ingress/tests/index.test.mjs
 *
...
test('positive control: lowerStop reshapes the document into the REAL compiler argument shape', () => {
  ...
  // see the FlatTheme fragment wrapped under `brandTheme`, alongside the
...
test('counterfactual: writing the stop NAME into the bounded field is a FALSE NEGATIVE, not a smaller number', async () => {
  ...
  const properties = readFileSync(
    resolve(CORE_ROOT, 'src/foundation/tokens/css/foundation/base/properties/index.css'),
    'utf8',
  );
  assert.match(
    properties,
...
test('negative drill: a domain kind this harness cannot write fails closed', () => {
  ...
  // terminal message still enumerated four (independent audit's registered P2), and
 ...
 *
 * Before H-1 it compiled a ONE-FIELD FlatTheme. `compileTheme` read that
...
const discriminate = async (manifest, armId, vertical, overrides = {}) => {
  ...
  return assertStopDiscrimination({
    ...
    /* R-2: the same wiring the CLI does, from the same published FlatTheme.
...
// artifacts/quality/programs/modern-rescue/archive/cascade-proofs/unreceipted/typography-scale-static-db-residuals/residual-isolation.MEASURED-NOT-RECEIPTED.json;
...
const STATIC_PROVENANCE_FIXTURE = Object.freeze({
  ...
  input: {
    ...
    stopId: 'fixture',
    baseline: { source: 'dist/index.js#fixtureFlatTheme', digest: 'a'.repeat(64) },
  },
});
...
/**
 * The DB artifact is a delta over the vertical's shipped baseline. A channel
 * absent from that delta inherits the baseline; treating absence as
 * `undefined` compares transport shapes instead of effective CSS values.
 */
async function compiledBaseline(vertical) {
  const baselines = await loadStaticBaselines();
  const { compile: lowerFlatTheme } = await loadFlatThemeLowering({ coreRoot: CORE_ROOT });
  const compiled = lowerFlatTheme({
  ...
  return {
    ...
    modes: Object.fromEntries(compiled.modeBlocks.map((block) => [block.mode, block.cssVariables])),
  };
}
...
test('H-3(a) drill 3 [needs dist]: the two arms carry the SAME dark values', async () => {
  ...
  const channels = [...new Set([...Object.keys(d), ...Object.keys(s)])].sort();
  for (const channel of channels) {
    const dbEffective = Object.hasOwn(d, channel) ? d[channel] : baseline.modes.dark?.[channel];
    assert.equal(dbEffective, s[channel], `${channel} must agree across arms in dark`);
  }
...
test('H-3(a) drill 6: the inline path is NOT retired', async () => {
  ...
  const source = readFileSync(
    resolve(CORE_ROOT, 'scripts/check/tokens/cascade/probe/runtime/measure/index.mjs'),
    'utf8',
  );
...
test('H-3(a) drill 10 [needs dist]: no probe artifact carries a prefers-color-scheme rule TODAY', async () => {
  /* independent audit note 1. `renderArtifactCss` adds an `@media (prefers-color-scheme: dark)`
...
test('B-2 drill 2 [needs dist]: a BASELINE compile is untouched — production invariance', async () => {
  ...
  for (const spec of FIRST_PARTY_ARTIFACT_SPECS) {
    ...
    const bare = lowerFlatTheme({ brandTheme, vertical: spec.slug, tenantSlug: spec.slug });
    const explicitlyAbsent = lowerFlatTheme({
      brandTheme,
      vertical: spec.slug,
    ...
    const withTenant = lowerFlatTheme({
      brandTheme,
...
test('B-2 drill 3 [needs dist]: bithire\'s button follows the seed on BOTH arms', async () => {
  ...
  // and still pass.
  const dbEffective = Object.hasOwn(dbLowered.variables, channel)
    ? dbLowered.variables[channel]
    : baseline.base[channel];
  assert.equal(staticLowered.variables[channel], dbEffective);
...
test('B-2 drill 6: the CLOSED consulted vocabulary is what bounds the blast radius', () => {
  ...
  assert.deepEqual(intersections, [
    ...
    'palette.status-seeds -> palette.successColor',
    'palette.status-seeds -> palette.warningColor',
    'palette.status-seeds -> palette.errorColor',
    'palette.status-seeds -> palette.infoColor',
  ]);
});
...
test('B-2 drill 7 [needs dist]: W-B — an IDENTITY stop declares NO authorship', async () => {
  ...
  for (const [vertical, theme] of Object.entries(baselines)) {
    ...
    const baseline = lowerFlatTheme({ brandTheme: theme, vertical, tenantSlug: vertical });
    ...
    const asLowered = lowerFlatTheme({
  ...
  const armSource = readFileSync(resolve(CORE_ROOT, 'scripts/check/tokens/cascade/probe/runtime/ingress/index.mjs'), 'utf8');
 ...
 * in R-1, reproduced by independent audit, and frozen here as a fixture rather than
...
test('R-2 drill 10: the hardening did not change WHAT is excluded, only how it is admitted', async () => {
   ...
   *
   * What it proves: every excluded STOP in the six scenarios was already
   * excluded before the typed hardening. Diagnostic prose is deliberately not
...
test('PACKET-K drill 7 [needs dist]: two entries discriminate through the ABSENCE sentinel', async () => {
   ...
   *
   * P4 (independent audit): the literal is asserted here against the SOURCE, because a
  ...
  const source = readFileSync(resolve(CORE_ROOT, 'scripts/check/tokens/cascade/probe/runtime/ingress/index.mjs'), 'utf8');
