...
test('con membresia, la fila toma la raiz y la via que la membresia ya resolvio', async () => {
  ...
  assert.equal(doc.provenance.membershipSource, 'artifacts/generated/manifest/cascade/membership/index.json');
...
test('el modo sin membresia se pide por nombre: leerla por accidente es imposible', async () => {
  ...
  assert.throws(() => readMembership('/no/existe/root-membership/index.json'), /no existe/);
  const doc = await buildSynthetic();
...
test('LEDGER — la unidad de rowsWithoutRootAttribution es la que su razon pinea', async () => {
  ...
  const doc = JSON.parse(readFileSync(new URL('../../../../../artifacts/generated/manifest/cascade/slots/index.json', import.meta.url), 'utf8'));
