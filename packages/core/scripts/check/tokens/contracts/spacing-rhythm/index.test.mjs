...
function writeInventory(rows) {
  ...
  return write(root, 'family-inventory/index.json', JSON.stringify({ rows }));
}
...
test('clause (e) -- a component nested in ITSELF is still its own radius', () => {
  ...
  // CORRECTED (defect 4, post independent audit): a class named INSIDE the subject's
...
test('Classic, Rustic, generated, dist, artifact, test and fixture CSS are outside the corpus', () => {
  ...
  const inside = [
    ...
    'runtime/engines/modern/skin/card/index.css',
    ...
    // file lives under `src/components/` at all any more (the skin tree owns every
    ...
    'presentation/components/skin/mono-stat/index.css',
  ...
  const outside = [
    'runtime/engines/classic/theme/index.css',
    ...
    'tests/fixtures/tenants/rottay/index.css',
...
// The four independently reproduced bypasses, each with its own drill.
// ---------------------------------------------------------------------------
...
test('bypass 1 -- a subject re-spelled with :is() must still be caught as a repeated ancestor class', () => {
...
test('bypass 2 -- a class hidden inside :where() must still be caught', () => {
  assert.equal(subjectIsComponentRoot('.ds-semantic-surface :where(.ds-semantic-surface)'), true);
});

test('bypass 3 -- [class~="ds-card"] must be caught as equivalent to .ds-card, either side', () => {
...
test('bypass 4 -- a * subject decorated with :not([hidden]) carries no identity', () => {
