...
function collectStampedParts(componentsRoot = COMPONENTS) {
  const stamped = new Set();
  const walk = (dir) => {
...
function collectReferencedParts({ sourceRoot = SRC, skinDirs = SKIN_DIRS } = {}) {
  const refs = new Map();
    ...
    while (pending.length > 0) {
      ...
      for (const entry of readdirSync(current, { withFileTypes: true })) {
        ...
        for (const m of css.matchAll(/\[data-part=["']([a-z0-9-]+)["']\]/gi)) {
          ...
          refs.get(key).add(relative(sourceRoot, path).replaceAll('\\', '/'));
        }
      }
    }
  }
...
export function findDeadParts({ componentsRoot = COMPONENTS, sourceRoot = SRC, skinDirs = SKIN_DIRS } = {}) {
  const stamped = collectStampedParts(componentsRoot);
  const refs = collectReferencedParts({ sourceRoot, skinDirs });
  const dead = [...refs.keys()].filter((k) => !stamped.has(k)).sort();

  return { dead, refs, stamped };
}

export function countDeadParts({ verbose = false, ...roots } = {}) {
  const { dead, refs, stamped } = findDeadParts(roots);
