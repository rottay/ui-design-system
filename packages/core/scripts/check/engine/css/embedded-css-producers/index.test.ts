...
import { projectResponsiveGrid } from "../../../../../src/components/primitives/layout/grid/runtime/responsive";
...
const gridResponsiveCss = readFileSync(
  resolve(
    packageRoot,
    "src/foundation/tokens/css/foundation/responsive/grid/index.css"
  ),
  "utf8"
);
    ...
    function visit(node: ts.Node): void {
      if (
        ...
        if (ts.isStringLiteralLike(node.initializer))
          properties.push(node.initializer.text);
      }
...
describe("embedded CSS data producers", () => {
  it("executes the complete production responsive property map without paint declarations", () => {
    ...
    expect(propertyMap.length).toBeGreaterThanOrEqual(
      RESPONSIVE_PROPERTY_SITE_FLOOR
    );
    expect(
      propertyMap.every((property) => !isEmbeddedCssPaintProperty(property))
    ).toBe(true);
  ...
  it("executes the Grid projection and proves its CSS owner is layout-only", () => {
    const projection = projectResponsiveGrid(
    ...
    expect(projection).toMatchObject({
      columns: { xs: "2", md: "3" },
      rows: { xs: "auto", lg: "1fr 1fr" },
      columnBreakpoints: "xs md",
      rowBreakpoints: "xs lg",
    });
    const emitted = declarationProperties(gridResponsiveCss);
  ...
  it("executes the first-party static compiler and proves its generated block is custom-property-only", () => {
    ...
    const spec = FIRST_PARTY_ARTIFACT_SPECS.find(
      (entry) => entry.slug === "bithire"
    );
    if (!spec)
    ...
    for (const variables of generatedBlocks) {
      ...
      expect(declared.every((property) => property.startsWith("--"))).toBe(
        true
      );
      expect(
        declared.every((property) => !isEmbeddedCssPaintProperty(property))
      ).toBe(true);
    }
  });
