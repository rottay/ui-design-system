...
const sourceRoots = [
  ...
  'src/components/structures/dashboard',
  'src/components/structures/headers',
  'src/components/patterns/customization/tenant-preview',
...
const personalityPath = join(coreRoot, 'src/foundation/tokens/css/runtime/personality/index.css');
const compatibilitySkinPath = join(
  coreRoot,
  'src/foundation/tokens/css/presentation/components/skin/surface-accent-bar/index.css'
);
...
test('surface and engine skins do not paint named or tone-only edge rails', () => {
  ...
  const chartSkin = withoutComments(
    readFileSync(
      join(coreRoot, 'src/foundation/tokens/css/presentation/components/skin/chart-foundation/index.css'),
      'utf8'
    )
  );
  ...
  for (const path of [
    'src/foundation/tokens/css/presentation/components/skin/notification-surface/index.css',
    'src/foundation/tokens/css/runtime/engines/rustic/skin/alert/index.css',
    'src/foundation/tokens/css/runtime/engines/rustic/skin/callout/index.css',
    'src/foundation/tokens/css/runtime/engines/rustic/skin/notification/index.css',
  ]) {
    assert.doesNotMatch(
...
test('legacy compatibility is inert while functional affordances remain available without edge rails', () => {
  ...
  const workspaceSkin = withoutComments(
    readFileSync(
      join(coreRoot, 'src/foundation/tokens/css/presentation/components/skin/collection-workspace/index.css'),
      'utf8'
    )
  );
  const tableInteractions = withoutComments(
    readFileSync(
      join(coreRoot, 'src/foundation/tokens/css/presentation/components/skin/data-table-interactions/index.css'),
      'utf8'
    )
  );
