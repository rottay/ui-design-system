...
//      `runtime/engines/modern/framework-token-projection/index.css` writes exactly
...
// `daisy-painted-classes.mjs` and `tests/index.test.mjs`) by
...
//
// Usage:
//   node scripts/check/engine/lifecycle/framework-bundle/index.mjs --check          (default)
//   node scripts/check/engine/lifecycle/framework-bundle/index.mjs --check \
 ...
 * `tests/index.test.mjs`, which asserts the same rule over the
    ...
    if (root === 'daisyui') {
      findings.push(
        `${label}:${lineOf(css, match.index)}: DaisyUI cascade layer shipped (@layer ${match[1]}). ` +
          ...
          'src/foundation/tokens/css/runtime/engines/modern/compiled/index.css',
      );
      continue;
    }
