 ...
 *
 * Usage:
 *   node scripts/check/engine/tokens/audit/index.mjs            # print the current counts
 *   node scripts/check/engine/tokens/audit/index.mjs --check    # exit 1 if any counter rose above baseline
 *   node scripts/check/engine/tokens/audit/index.mjs --update-baseline   # tighten existing ceilings only
 *   node scripts/check/engine/tokens/audit/index.mjs --coverage # write the token-coverage report (informational)
 *   node scripts/check/engine/tokens/audit/index.mjs --current-json # emit current counters for reviewed tooling
 *   node scripts/check/engine/tokens/audit/index.mjs --check --quiet     # concise CI output
 */
...
import {
  ...
  modernEngineFiles,
} from '../../../../libraries/engine/corpus/index.mjs';
import {
  collectDaisyClassConsumers,
  countDaisyClassConsumers,
} from '../../../../libraries/engine/consumers/daisy-classes/index.mjs';
import { countPremiumEffectConsumers } from '../../../../libraries/engine/consumers/effects/index.mjs';
import { countMotionRecipeConsumers } from '../../../../libraries/engine/consumers/motion-recipes/index.mjs';
import { countSkinExemptionBreaches } from '../../../../libraries/engine/skins/exemptions/index.mjs';
import { countRuntimeSvgPaintByFile } from '../../../../libraries/paint/svg/index.mjs';
import { countEmbeddedCssPaintByFile, countEmbeddedCssPaintInFile } from '../../../../libraries/paint/embedded/index.mjs';
import { stripScriptComments } from '../../../../libraries/comments/index.mjs';
import { ARC09_INLINE_PAINT_FILES, collectFleetInlinePaintSourceFiles } from '../../../../libraries/paint/fleet/index.mjs';
import { collectSourceFiles as collectRuntimeSvgSourceFiles } from '../../css/paint/runtime-svg/index.mjs';
import { countDeadParts } from '../../skins/dead-parts/index.mjs';
import {
 ...
 * foundation/themes/default/index.css's literal radius scale; every other consumer
 * (including the component-specific --ds-radius-button/-card/... aliases in
 * foundation/base/borders/index.css) must reference it via var(), never redeclare the
 ...
 * so the consumer set is now src/components + src/graphics/motion + source skin files.
...
/**
 * Count DISTINCT src/components components consuming the motion recipe canon
...
const rusticThemeCssPath = join(tokensCssDir, 'runtime/engines/rustic/theme/index.css');
 ...
 * `foundation/themes/default/index.css` as short aliases onto the pre-existing
...
function renderCoverageMarkdown(coverage) {
  ...
  lines.push('modern-scoped counters in `node scripts/check/engine/tokens/audit/index.mjs --check` (motion/color/etc).');
...
const tokenAuditDir = join(
  repoRoot,
  'packages/core/artifacts/quality/audits/engine/tokens',
);

/** Write the coverage projection beneath the engine-token audit owner. */
function writeCoverageArtifacts(coverage) {
  const coverageDir = join(tokenAuditDir, 'coverage');
  const jsonPath = join(coverageDir, 'data/index.json');
  const mdPath = join(coverageDir, 'overview/index.md');
...
const FIXED_WIDTH_EXEMPTIONS = new Set([
  ...
  'src/components/patterns/data/list-toolbar/engines/modern/index.tsx:300',
  // Hover overlay: portaled, viewport-anchored, 288 < 360.
  'src/components/primitives/overlay/HoverCard/engines/modern/index.tsx:288',
  // Calendar panel: portaled, viewport-anchored, 288 < 360.
  'src/components/primitives/inputs/DatePicker/engines/modern/index.tsx:288',
]);
...
const counters = {
  ...
  // WO-CRA-15 (motion recipe canon adoption): distinct src/components components whose
  ...
  // TypeScript-AST counter -- see libraries/engine/consumers/daisy-classes for why a
  ...
  // the ratchet and makes the zero target impossible. Each is
  ...
  // skin ownership migration (P-79): the THIRD way a skin silently paints nothing. It parses
...
if (mode === 'apca-report') {
  const outDir = join(tokenAuditDir, 'contrast');
  const jsonPath = join(outDir, 'data/index.json');
  const mdPath = join(outDir, 'overview/index.md');
  mkdirSync(dirname(jsonPath), { recursive: true });
  mkdirSync(dirname(mdPath), { recursive: true });
  writeFileSync(
    jsonPath,
