...
import { isEngineSourceFile, modernEngineFiles } from '../../../../../../libraries/engine/corpus/index.mjs';
import { packageRoot as findPackageRoot } from '../../../../../../libraries/repo-root/index.mjs';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const packageDir = findPackageRoot(scriptDir);
const uiDir = join(packageDir, 'src/components');
...
// ---------------------------------------------------------------------------
// 4. The independent code audit K2/K3 catch: classes held in a map still count
// ---------------------------------------------------------------------------
...
test('the audited tree has exactly the consumers the baseline pins', () => {
  const baseline = JSON.parse(
    readFileSync(join(packageDir, 'scripts/check/engine/tokens/audit/baseline/index.json'), 'utf8'),
  );
  const current = spawnSync(
    process.execPath,
    [join(packageDir, 'scripts/check/engine/tokens/audit/index.mjs'), '--current-json', '--quiet'],
...
const FORMER_CONSUMERS = [
  ...
  'primitives/overlay/alert-dialog/engines/modern/index.tsx',
];
...
test("ConfirmDialog's modern engine is not a consumer either", () => {
  const file = join(packageDir, 'src/components/primitives/overlay/confirm-dialog/engines/modern/index.tsx');
 ...
 * Measured over `src/components`, that pass reported 18 consumers where 3 exist.
 *
...
test('a tsconfig `paths` alias counts (159 of the audited imports are `@/...`)', () => {
  assert.deepEqual(
    classesInProject({
      ...
      'src/components/index.tsx': `import { BAR } from '@/tokens/classes';
 ...
 * legitimate walk under `src/components` is 23, so this bound was being hit by ordinary
