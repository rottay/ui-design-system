...
export const NAMED_EXCEPTIONS = Object.freeze({
  'components/primitives/runtime/overlay/portal-scope/index.tsx':
    "readLocaleContext(anchor) snapshots the anchor's `dir` AND `lang` AND its data-{ds-root,vertical,tenant,theme,engine,density} scope, and a MutationObserver over the anchor's ancestor chain keeps that snapshot live. The question is what this anchor's ancestry declares, so a portal rendered outside the subtree can reproduce it; the locale cannot answer it, and swapping direction alone would make it the one field of the snapshot that stops following the anchor while every sibling field still does. It is the ONE implementation of that read: the anchor's COMPUTED direction is the authority (it is what the anchor paints, and CSS `direction` below the nearest `[dir]` ancestor overrides that attribute), and the `[dir]` ancestor is the fallback for a server render, a detached node, an empty computed direction, or a runtime that does not resolve `dir` into computed style at all. Whether the runtime resolves it is probed ONCE against a real `dir=\"rtl\"` element rather than assumed; that probe lives inside a throwaway SHADOW ROOT, because Chromium applies `dir` as a presentational hint at the START of the author origin, so a document-scope reset such as `* { direction: ltr }` outranks it and would silently downgrade the whole package to the fallback -- a document-scope selector does not match into a shadow tree, measured `rtl` there even against `*, :host, ::slotted(*) { direction: ltr !important }`. `direction: revert` on the probe does NOT work and was measured: reverting the author origin discards the presentational hint itself and the probe answers `ltr` on a clean page. The probe's answer is cached per MODULE, not per document, so a second document (an iframe or a popout) answers from its own import.",
});
