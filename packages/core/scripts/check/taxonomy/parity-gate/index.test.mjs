...
test('a planted ghost sourceOwner fails (relocation rejects it as missing)', () => {
  ...
  assert.throws(
    () => audit(fixture),
    (error) => {
      assert.equal(error.name, 'RelocationError');
      assert.ok(
        error.rejections.some((r) => r.id === 'primitive/display/avatar' && r.reason === 'missing'),
        'the RelocationError must name the offending row as missing',
      );
      return true;
    },
  );
...
test('a sourceOwner outside the UI tree fails (relocation rejects it as missing)', () => {
  ...
  assert.throws(
    ...
    (error) => {
      assert.equal(error.name, 'RelocationError');
      assert.ok(error.rejections.some((r) => r.id === 'primitive/display/avatar'));
      return true;
    },
  );
});
...
test('a manifest cell with a double .json extension fails and names the file', () => {
  const fixture = buildCleanFixture();
  fs.writeFileSync(
    path.join(fixture.manifestRoot, 'families/primitive/display/ghost.json.json'),
    JSON.stringify({ familyId: 'primitive/display/ghost' }),
  );
  const result = audit(fixture);
  const found = result.violations.filter(
    (violation) => violation.kind === 'manifest-parity' && /malformed/.test(violation.detail),
...
test('a missing program root fails loudly and names the path', () => {
  const fixture = buildCleanFixture();
  const missingRoot = path.join(fixture.root, 'no-such-program');
  assert.throws(
    () =>
      auditTaxonomyParity({
        repositoryRoot: fixture.root,
...
test('a missing manifest root fails loudly and names the path (the silent skip is gone)', () => {
  const fixture = buildCleanFixture();
  const missingRoot = path.join(fixture.root, 'no-such-manifest');
  assert.throws(
    () =>
      auditTaxonomyParity({
        repositoryRoot: fixture.root,
        programRoot: fixture.programRoot,
        manifestRoot: missingRoot,
        showroomRegistryRoot: fixture.registryRoot,
        registries: [{ file: 'primitives.ts', layers: ['primitive'] }],
        rootEntryFile: path.join(fixture.root, 'packages/core/src/index.ts'),
        relocationOverrides: [],
...
test('a row whose owner exists under neither prefix and has no override rejects the relocation, naming the row', () => {
  ...
  assert.throws(
    ...
    (error) => {
      assert.equal(error.name, 'RelocationError');
      assert.ok(
        error.rejections.some((r) => r.id === 'primitive/display/avatar'),
        'the RelocationError must name the offending row',
      );
      return true;
    },
  );
