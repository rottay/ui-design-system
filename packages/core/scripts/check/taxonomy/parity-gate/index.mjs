 ...
 * in `scripts/check/modern-rescue/check/index.mjs`
 ...
 * `program/index.json` -> `denominators.visibleFamilies`.
 *
 ...
 *            `libraries/taxonomy/roots`, which knows nothing about the
 ...
 * `scripts/generate/tokens/manifest/generation/index.mjs --check` as its own blocking entry, positioned ahead
 ...
 *
 * Usage: node scripts/check/taxonomy/parity-gate/index.mjs [--json]
 */
...
const GOVERNED_PUBLIC_SUPPORT = {
  ...
  //
  // Their rows in `family-inventory/index.json` own them now. Re-adding any of them
...
function collectFamilyManifestFiles(root) {
  if (!fs.existsSync(root)) {
    throw new Error(`taxonomy-parity: manifest families root does not exist: ${root}`);
  }
  const found = [];
  const walk = (dir) => {
...
export function auditTaxonomyParity({
  ...
  // Injected only by the duplicate-cell drill: two distinct manifest cells
  // can never derive the same flat id from a real directory walk (two
  ...
  const relativeOwnerOf = (row) =>
    path
      .relative(repositoryRoot, relocated.get(row.id))
      .replaceAll('\\', '/')
      .replace(/\/$/, '');
  const stripTrailingSep = (absDir) =>
    absDir.endsWith(path.sep) ? absDir.slice(0, -path.sep.length) : absDir;
  ...
  for (const row of rows) {
    ...
    const owner = relativeOwnerOf(row);
  ...
  const deriveFamilyCellId = (file) => {
    const relative = path.relative(familiesRoot, file).replaceAll('\\', '/');
    if (!relative.endsWith('.json')) return { ok: false, reason: 'does not end in .json' };
    const stem = relative.slice(0, -'.json'.length);
  ...
  const familyFileIdOwners = new Map();
  for (const file of familyFiles) {
    const relativeFile = path.relative(repositoryRoot, file);
    const derivation = deriveFamilyCellId(file);
    if (!derivation.ok) {
      add('manifest-parity', `manifest cell ${relativeFile} is malformed -- ${derivation.reason}`);
      continue;
    }
    const { id } = derivation;
    if (familyFileIdOwners.has(id)) {
      add(
        'manifest-parity',
        `manifest cells ${familyFileIdOwners.get(id)} and ${relativeFile} both derive the id "${id}"`,
      );
      continue;
    }
    familyFileIdOwners.set(id, relativeFile);
    familyFileIds.add(id);
  }
  ...
  const ownersByLength = rows
    .map((row) => ({ id: row.id, owner: stripTrailingSep(relocated.get(row.id)) }))
    // Longest first: the most specific declared territory wins. Lane 2 removed
    // the only nested pair, and `taxonomy-owner-nesting-drill` keeps it that way.
  ...
  // land inside that row's own territory. `relocateSourceOwners` above already
  // guarantees every row has a resolved directory (or the gate would have
  // thrown), so there is no `row.sourceOwner` guard left to carry here.
  for (const row of rows) {
    if (!Array.isArray(row.components)) continue;
    const territory = stripTrailingSep(relocated.get(row.id));
  ...
  const uiRoot = path.join(repositoryRoot, 'packages/core/src/components') + path.sep;
