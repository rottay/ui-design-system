 ...
 * name, and a terminal file under `packages/core/src/components`. Every inventory row
 ...
 *
 * THE RESOLVER IS SHARED, NOT REIMPLEMENTED. `libraries/taxonomy/roots` is
 ...
 * @param {Array<{ id: string, dir: string }>} [input.owners] Pre-resolved owner
 *   directories. The real tree passes these because its inventory records a
 *   physical prefix the tree no longer uses; fixtures omit them and resolve
 *   `sourceOwner` directly, which is what they are testing.
 */
function projectPublicComponents({ resolver, rows, repoRoot, uiRoot, owners: resolvedOwners }) {
  const owners =
    resolvedOwners ??
    rows.map((row) => ({
      id: row.id,
...
const realProjection = (() => {
  ...
  return {
    ...
    ...projectPublicComponents({
      resolver,
      rows: inventory.rows,
      owners: [...ownerDirectories].map(([id, dir]) => ({ id, dir })),
      repoRoot: REPO_ROOT,
      uiRoot: path.join(CORE_ROOT, 'src', 'components'),
    }),
  };
})();
...
test('the projection sees a real population (a silent zero would pass everything)', () => {
  ...
  // A partial owner map would make containment answer "no" for the rows it
  // dropped, which reads as unowned components rather than as a broken map.
  assert.equal(
    realProjection.ownerDirectories.size,
    realProjection.inventory.rows.length,
    'every inventory row must have resolved to an owner directory',
  );
});
...
test('a row pointing at a directory that does not exist fails the run, not the row', () => {
  ...
  assert.throws(
    ...
    (error) =>
      error.name === 'RelocationError' &&
      error.rejections.some(
        (rejection) =>
          rejection.id === 'primitive/display/not-a-real-owner' && rejection.reason === 'missing',
      ),
  );
});
...
const UI_REL = 'packages/core/src/components';
...
function buildFixture(plant = {}) {
  ...
  write(
    ...
    `export { Alpha, useAlpha } from './components/primitives/display/Alpha';\n${plant.extraExports ?? ''}`,
  );
...
test('drill: a component-shaped public export with no row is reported unowned', () => {
  const fixture = buildFixture({
    extraExports: "export { Beta } from './components/primitives/display/Beta';\n",
    extraFiles: {
...
test('drill: a barrel export whose target declares nothing is reported dangling', () => {
  ...
  const fixture = buildFixture({
    extraExports: "export { Gamma } from './components/primitives/display/Alpha';\n",
  });
  try {
    const result = projectFixture(fixture);
    assert.deepEqual(
...
test('drill: an alias resolves to the same row as the component it re-exports', () => {
  const fixture = buildFixture({
    extraExports: "export { Alpha as AlphaCompat } from './components/primitives/display/Alpha';\n",
  });
  try {
    const result = projectFixture(fixture);
    assert.equal(result.considered, 2);
