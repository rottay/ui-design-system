...
import { REPOSITORY_ROOT, getFamily, getRound, loadProgramContracts } from '../contracts/index.mjs';
...
const SIGHTED_AUTHORITY_SOURCES = Object.freeze([
  Object.freeze({
    where: 'orchestration/index.json#doubleAccept.coordinator',
    read: (contracts) => contracts?.orchestration?.doubleAccept?.coordinator,
  }),
  Object.freeze({
    where: 'quality-rubric/index.json#eligibility.finalSightedAuthority',
...
const LEGACY_F4B_ARTIFACT_ROOT =
  'packages/core/test-artifacts/quality-evidence/wo-cra-23/F4B/';
const CURRENT_CONTROL_PROOF_ROOT =
  'packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls/';

export function resolveEvidenceArtifactPath(artifactPath, { root = REPOSITORY_ROOT } = {}) {
  const normalized = String(artifactPath ?? '').replace(/^\.\//, '');
  if (!normalized.startsWith(LEGACY_F4B_ARTIFACT_ROOT)) return normalized;
  const [control, ...rest] = normalized.slice(LEGACY_F4B_ARTIFACT_ROOT.length).split('/');
  const relocated = `${CURRENT_CONTROL_PROOF_ROOT}${control}/computed-static-db/${rest.join('/')}`;
  return fs.existsSync(path.join(root, relocated)) ? relocated : normalized;
}

/**
 * Validates one evidence receipt against evidence-contract/index.json.
...
export function validateReceipt(receipt, options = {}) {
  ...
  if (isNonEmptyString(artifactPath)) {
    const normalized = resolveEvidenceArtifactPath(artifactPath, { root });
  ...
  if (isNonEmptyString(receipt?.roundId) && !getRound(contracts, receipt.roundId)) {
    failures.push(`round ${receipt.roundId} does not exist in rounds/index.json`);
  }
  if (isNonEmptyString(receipt?.familyId)) {
    ...
    if (!isProgramScope && !getFamily(contracts, receipt.familyId)) {
      failures.push(`family ${receipt.familyId} does not exist in family-inventory/index.json`);
    }
  }
