...
import {
  ...
  summarizeZeroLocks,
} from '../../../../../libraries/engine/zero-locks/index.mjs';
...
const AUDIT = join(CORE_ROOT, 'scripts/check/engine/tokens/audit/index.mjs');
const CLAIM_FLOOR = join(HERE, 'public-claim-floor/index.json');
const REGISTRY = join(findRepoRoot(HERE), 'roadmap/registry.json');

test('documented component symbols resolve to declarative source owners', () => {
  assert.equal(
    componentDirectoryFromDoc('primitives/inputs/OTPInput'),
    join(CORE_ROOT, 'src/components/primitives/inputs/otp-input'),
  );
  assert.equal(
    componentDirectoryFromDoc('primitives/overlay/ConfirmDialog'),
    join(CORE_ROOT, 'src/components/primitives/overlay/confirm-dialog'),
  );
  assert.equal(componentDirectoryFromDoc('primitives/inputs/MissingInput'), null);
});
...
test('G1-G7 structured analyzers fail closed on opaque claims and noncanonical evidence', () => {
  const claimFacts = analyzeClaimSourceRecords([
    ...
    {
      path: '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/index.ts',
      kind: 'core',
  ...
  assert.deepEqual(claimFacts['surface-profile-overrides'].staticallyResolvedSurfaceHookCallFiles, [
    '/repo/src/components/surfaces/pages/demo.tsx',
  ]);
...
test('G1 typed claim references resolve named/default/namespace/reexports without scope-name false positives', () => {
  const hookPath = '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/index.ts';
  const records = [
    {
      path: hookPath,
      kind: 'core',
    ...
    {
      path: '/repo/src/components/structures/foundation/chrome/contracts/index.ts',
      kind: 'core',
    ...
    },
    {
      ...
      path: '/repo/src/components/surfaces/foundation/contracts/index.ts',
      kind: 'core',
    ...
    {
      path: '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/named.ts',
      kind: 'core',
      text: `export { useSurfaceProfileDefaultsWithOverrides as useProfile } from './index';`,
    },
    {
      path: '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/default.ts',
      kind: 'core',
      text: `export { useSurfaceProfileDefaultsWithOverrides as default } from './index';`,
    },
    {
      path: '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/star.ts',
      kind: 'core',
...
test('G1b unused underscore exclusion destructuring is containment, a read binding is consumption', () => {
  ...
  const containment = {
    path: '/repo/src/components/primitives/display/Card/engines/classic/index.tsx',
    kind: 'core',
...
export default Impl;`,
  ...
  const consumption = {
    path: '/repo/src/components/primitives/display/Card/engines/modern/index.tsx',
    kind: 'core',
    text: `import type { EngineAwareProps } from '../../../../../../foundation/contracts/runtime/engine/index';
...
test('G2 claim census keeps opaque transports and computed access out of direct evidence', () => {
  const records = [
    ...
    },
    {
      ...
      path: '/repo/src/components/surfaces/pages/impostor-types.tsx',
      kind: 'core',
    ...
    },
    {
      ...
      kind: 'showroom',
      text: `import type { VisualOptions } from '../../../src/components/surfaces/foundation/contracts'; export const read = (visual: VisualOptions) => visual.profileOverrides; const fake = { profileOverrides: true }; export const ignored = fake.profileOverrides;`,
    },
  ];
...
test('G7 data-part evidence accepts only canonical static sinks', () => {
  ...
  const fixtureModules = new Map([
    ['/repo/src/components/primitives', {
      path: '/repo/src/components/primitives/index.tsx',
      text: `export * from './layout/box';\nexport * from './layout/stack';\nexport * from './display/typography';`,
    }],
    ['/repo/src/components/primitives/layout/box', {
      path: '/repo/src/components/primitives/layout/box/index.tsx',
      text: forwardingModule('Box'),
    }],
    ['/repo/src/components/primitives/layout/stack', {
      path: '/repo/src/components/primitives/layout/stack/index.tsx',
      text: forwardingModule('Stack'),
    }],
    ['/repo/src/components/primitives/display/typography', {
      path: '/repo/src/components/primitives/display/typography/index.tsx',
  ...
  const result = collectDataPartStampsFromText(`
    ...
    import { Box as AliasBox } from '@/components/primitives/layout/box';
    import { Stack as AliasStack } from '@/components/primitives/layout/stack';
    import { Text as AliasText } from '@/components/primitives/display/typography';
  ...
  `, '/repo/src/components/structures/demo.tsx', { moduleReader });
...
test('G7/B-1 forwarder terminals: drop, override order, imperative stamps and module misses', () => {
  ...
  const familySlug = (name) => name.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase();
  const family = (name, text) => [`/repo/src/components/primitives/layout/${familySlug(name)}`, {
    path: `/repo/src/components/primitives/layout/${familySlug(name)}/index.tsx`,
    text,
  }];
  const probe = (name, moduleText, extra = []) => {
    ...
    return collectDataPartStampsFromText(
      `import { ${name} } from '@/components/primitives/layout/${familySlug(name)}';\n` +
  ...
  }], ['/repo/src/components/primitives/layout/impostor', {
    path: '/repo/src/components/primitives/layout/impostor/index.ts',
    text: `export function stampDataPart(node: Element, part: string): void { node.setAttribute('data-part', part); }`,
  }]]);
  ...
  `, '/repo/src/components/structures/helpers.tsx', { moduleReader: (base) => helperModules.get(base) ?? null });
...
const SURFACE_PROFILE_FIXTURE = Object.freeze({
  claimType: '/repo/src/components/structures/foundation/chrome/contracts/index.ts',
  hookOwner: '/repo/src/components/structures/foundation/chrome/runtime/profile-defaults/overrides/index.ts',
  facade: '/repo/src/components/surfaces/foundation/contracts/index.ts',
  barrel: '/repo/src/components/surfaces/index.ts',
  header: '/repo/src/components/structures/headers/header-surface/index.tsx',
  page: '/repo/src/components/surfaces/presentation/pages/data/list/index.tsx',
  sidebar: '/repo/src/components/structures/shell/navigation/sidebar-surface/index.tsx',
});
...
test('GAT07 surface-profile authority: relocating a consumer breaks the roster at an unchanged count', () => {
  ...
  const moved = '/repo/src/components/surfaces/presentation/pages/data/relocated/index.tsx';
...
test('GAT07 surface-profile authority: pre-relocation owner paths carry no authority', () => {
  const F = SURFACE_PROFILE_FIXTURE;
  // The whole corpus re-hosted under the retired `src/components/surfaces/runtime/**`
  // owner resolves to nothing: no declarations, no applied consumers.
  const census = surfaceProfileCensus([
    {
      path: '/repo/src/components/surfaces/runtime/profile-defaults/overrides/index.ts',
      kind: 'core',
      text: `export function useSurfaceProfileDefaultsWithOverrides(value?: unknown) { return value; }`,
    },
    {
      path: '/repo/src/components/surfaces/runtime/profile-defaults/contracts/index.ts',
      kind: 'core',
      text: `export interface SurfaceVisualOverrides { density?: string }
...
const SURFACE_PROFILE_FIXTURE_AUTHORITY = Object.freeze({
  declarationOwners: {
    ...
    'src/components/surfaces/foundation/contracts/index.ts': 1,
  },
