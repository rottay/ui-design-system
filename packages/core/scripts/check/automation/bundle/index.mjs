 ...
 * and compares against the performance budget defined in docs/quality/performance-budget/index.md.
 *
 * Usage:
 *   node scripts/check/automation/bundle/index.mjs            # build + analyze
 *   node scripts/check/automation/bundle/index.mjs --skip-build  # analyze existing dist/
 *   node scripts/check/automation/bundle/index.mjs --chart-renderers  # isolated named-export budgets only
 *   node scripts/check/automation/bundle/index.mjs --chart-access  # bounded data-access facade budgets only
 *   node scripts/check/automation/bundle/index.mjs --chart-spec  # server-safe chart contract purity/budget only
 *   node scripts/check/automation/bundle/index.mjs --effects  # EffectRegistry purity/budget only
 *   node scripts/check/automation/bundle/index.mjs --spatial  # Spatial spec/host purity and isolation only
 *   node scripts/check/automation/bundle/index.mjs --asset-retention  # semantic asset closure budgets only
 *
 * Exit codes:
...
// ---------------------------------------------------------------------------
// Configuration -- keep in sync with docs/quality/performance-budget/index.md
// ---------------------------------------------------------------------------
...
const BUDGET = {
  ...
  // Measured from the eight-renderer public facade on 2026-08-31:
  // 1,517/1,521 B. The raw delta is declarative preserveModules import paths, not
  // retained runtime. Ceilings are measured +10%, rounded up to 100 B.
  'dist/charts.js': 1_700,
  'dist/charts.cjs': 1_700,
  ...
  // Eight-renderer facade measurements: 1,611/1,615 B on 2026-08-31. The facade imports leaves directly; named gzip
  // and isolation costs remain governed independently below. Ceilings are
  // measured +10%, rounded up to 100 B.
  'dist/chart-renderers.js': 1_800,
  'dist/chart-renderers.cjs': 1_800,
 ...
 */
// The current 282-role preserveModules build measures 11,715,861 B (2026-08-31),
// above this retained ceiling. This is package
...
const COMPONENT_ENTRIES = {
  ...
  Card: 'src/components/primitives/display/card/index.tsx',
  Badge: 'src/components/primitives/display/badge/index.tsx',
  Table: 'src/components/primitives/display/table/index.tsx',
  Tabs: 'src/components/primitives/navigation/tabs/index.tsx',
  Modal: 'src/components/primitives/feedback/modal/index.tsx',
 ...
 *
 * Measured 2026-07-09 (WO-GAT-02) with `DS_KEEP_BUNDLE_ANALYSIS=1 node scripts/check/automation/bundle/index.mjs
...
const ASSET_RETENTION_BUDGET = Object.freeze({
  ...
  // Exact decrease-only gzip ceiling for the 104-role BitHire preset.
...
const ASSET_RETENTION_FIXTURES = Object.freeze({
  namedSemanticRole: Object.freeze({
    ...
    entryEsm: 'dist/graphics/icons/semantic/generated/roles/action-add.js',
    entryCjs: 'dist/graphics/icons/semantic/generated/roles/action-add.cjs',
 ...
 *
 * Measured on 2026-08-31 through the built public facade with React/D3
 * external. The eight public renderers remain isolated from their siblings;
 * each ceiling adds at least 10% headroom and rounds upward to the next 100 B.
 */
const CHART_RENDERER_BUDGET = {
  SvgBarRenderer: 13_900,
  SvgLineRenderer: 12_800,
  SvgHeatMapRenderer: 10_700,
  SvgPieRenderer: 9_700,
  SvgFunnelRenderer: 12_200,
  SvgGaugeRenderer: 12_500,
  SvgRadarRenderer: 12_400,
...
// Measured 26,967 B gzip for all eight public renderers on 2026-08-31. The
// combined fixture proves that shared interaction/insight infrastructure is
// deduplicated instead of paying isolated costs additively.
const CHART_RENDERER_SUITE_BUDGET = 29_700;

const CHART_RENDERER_ENTRY = join(ROOT, 'dist', 'chart-renderers.js');
const CHART_RENDERER_SOURCE_DIR = '/components/patterns/visualization/charts/runtime/chart-engine/presentation/react/renderers/';
const CHART_RENDERER_MODULES = Object.freeze({
  SvgBarRenderer: `${CHART_RENDERER_SOURCE_DIR}bar/index.js`,
  SvgFunnelRenderer: `${CHART_RENDERER_SOURCE_DIR}funnel/index.js`,
  SvgGaugeRenderer: `${CHART_RENDERER_SOURCE_DIR}gauge/index.js`,
  ...
  SvgRadarRenderer: `${CHART_RENDERER_SOURCE_DIR}radar/index.js`,
...
const CHART_GEOMETRY_MODULE = '/components/patterns/visualization/charts/runtime/chart-engine/foundation/renderers/geometry/index.js';
const CHART_RENDERER_BUILDERS = {
  SvgBarRenderer: 'buildSvgBarGeometry',
  SvgFunnelRenderer: 'buildSvgFunnelGeometry',
  SvgGaugeRenderer: 'buildSvgGaugeGeometry',
  SvgLineRenderer: 'buildSvgLineGeometry',
  SvgHeatMapRenderer: 'buildSvgHeatMapGeometry',
  SvgPieRenderer: 'buildSvgPieGeometry',
  SvgRadarRenderer: 'buildSvgRadarGeometry',
...
async function runChartRendererBudgets() {
  ...
  const supplierContract = JSON.parse(readFileSync(join(ROOT, 'contracts/runtime/suppliers/index.json'), 'utf8'));
...
async function runMotionFixtureBudgets() {
  ...
  if (!publicContract?.symbols?.MotionProvider?.includes('motion')) {
    console.error('\x1b[31mMotionProvider must declare the motion supplier in contracts/runtime/suppliers/index.json.\x1b[0m\n');
    return 1;
  }
...
const CHART_SPEC_ARTIFACT_FRAGMENT = '/components/patterns/visualization/charts/runtime/chart-engine/foundation/spec/';
...
const CHART_ACCESS_ARTIFACT_FRAGMENTS = Object.freeze([
  '/components/patterns/visualization/charts/runtime/chart-engine/foundation/access/',
  '/components/patterns/visualization/charts/runtime/chart-engine/presentation/react/access/',
]);
const CHART_ACCESS_COMPONENT_MODULE = '/components/patterns/visualization/charts/runtime/chart-engine/presentation/react/access/index.js';
...
if (failures > 0) {
  console.error(`\x1b[31m${failures} file(s) over budget. See docs/quality/performance-budget/index.md for limits.\x1b[0m\n`);
  process.exit(1);
} else {
