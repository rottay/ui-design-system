...
function withRepo({ rootScripts = {}, coreScripts = {}, workflow }) {
  ...
  const copied = join(core, 'check/automation/wiring/workflows/index.mjs');
  mkdirSync(dirname(copied), { recursive: true });
  writeFileSync(copied, readGate(), 'utf8');
  mkdirSync(join(core, 'libraries/repo-root'), { recursive: true });
  cpSync(
    resolve(HERE, '../../../../libraries/repo-root/index.mjs'),
    join(core, 'libraries/repo-root/index.mjs'),
  );
