#!/usr/bin/env node
// Test typecheck ratchet. `--check` rejects growth above the reviewed baseline;
// `--write` updates that baseline after the underlying errors are fixed.
//
// Usage (run from the package dir; each package points at its own baseline):
//   node scripts/check/automation/tests/typecheck/index.mjs --project tsconfig.tests.json --baseline scripts/check/automation/tests/typecheck/baseline/index.json
//   node scripts/check/automation/tests/typecheck/index.mjs --project tsconfig.tests.json --baseline scripts/check/automation/tests/typecheck/baseline/index.json --write
...
const baselinePath = resolve(
  cwd,
  arg('baseline', 'scripts/check/automation/tests/typecheck/baseline/index.json'),
);
