...
// `motion.character`, then `shape.nesting` and `shape.control-height`. The
// headline's new denominator follows NEW_DECISION_DENOMINATOR, so it decreases
// with them. The list is DERIVED from the catalog rather than restated here, so
// a row re-recorded without moving the denominator fails this test rather than
