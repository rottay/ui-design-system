...
// denominator follows NEW_DECISION_DENOMINATOR. WO-DER-03 palette half
// (2026-09-10) repeats it for `palette.neutral-temperature` and
// `palette.contrast-posture`: 8 -> 6.
test('the catalog states the kit census: 29 rows, 19 standard, 10 pro, 6 new', () => {
  assert.deepEqual(censusErrors(), []);
  assert.equal(DECISIONS.length, 29);
  assert.equal(NEW_DECISION_IDS.length, 6);
...
test('the headline is exactly the string the acceptance gate reads', () => {
  ...
  assert.equal(headline(summary), 'decisions lit = 7/22 (+0/6 new)');
});
...
test('both halves are published, and the gate substring survives', () => {
  ...
  assert.ok(lines[0].includes('decisions lit = 7/22 (+0/6 new)'));
...
test('REFUSES a run in which a decision recorded `new` moved', () => {
  ...
  assert.equal(headline(summary), 'decisions lit = 7/22 (+1/6 new)');
  assert.ok(
...
function publishedArtifact(overrides = {}) {
  return {
    headline: 'decisions lit = 7/22 (+0/6 new)',
