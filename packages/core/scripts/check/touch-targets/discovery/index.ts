 ...
 *                   pointer interaction, or keyboard interaction while it is
 *                   itself focusable — the classic unfloored fake button
 *
...
export const POINTER_INTERACTION_HANDLERS: readonly string[] = [
  'onClick',
  'onPointerDown',
  'onPointerUp',
];

export const KEYBOARD_INTERACTION_HANDLERS: readonly string[] = [
  'onKeyDown',
  'onKeyUp',
];
...
function splitSelectorOptions(value: string): string[] {
  ...
  if (buffer.trim()) options.push(buffer.trim());
  return options;
}

/** Expand subject-bearing selector lists before normalization. */
export function expandSelectorBranches(selector: string): readonly string[] {
  const starts = [':is(', ':where(']
    .map((name) => ({ name, index: selector.indexOf(name) }))
    .filter(({ index }) => index >= 0)
    .sort((a, b) => a.index - b.index);
  const match = starts[0];
  if (!match) return [selector];

  const contentStart = match.index + match.name.length;
  let depth = 1;
  let quote: string | null = null;
  ...
  const options = splitSelectorOptions(selector.slice(contentStart, end - 1));
  if (options.length === 0) return [selector];
  return [
    ...new Set(
      options.flatMap((option) =>
        expandSelectorBranches(joinFunctionalBranch(prefix, option, suffix))
      )
    ),
  ];
}

function joinFunctionalBranch(
  prefix: string,
  option: string,
  suffix: string
): string {
  const type = option.match(/^([a-zA-Z][a-zA-Z0-9-]*)(.*)$/s);
  const compound = prefix.match(/([^\s>+~]+)$/)?.[1];
  if (!type || !compound || compoundElementType(compound)) {
    return `${prefix}${option}${suffix}`;
  }
  return `${prefix.slice(0, -compound.length)}${type[1]}${compound}${type[2]}${suffix}`;
}
...
export function classifySelector(
  ...
  const rawCompounds = splitCompounds(raw);
  const rawSubject = rawCompounds[rawCompounds.length - 1] ?? raw;
  if (declaresCursorPointer || /:focus-visible/.test(rawSubject)) return 'css-pointer';
  return null;
}
  ...
  for (const file of files) {
    ...
    root.walkRules((rule) => {
      ...
      for (const raw of rule.selectors ?? []) {
        for (const branch of expandSelectorBranches(raw)) {
          const base = normalizeSelector(branch);
          const targetBase = stripPseudoElements(base);
          const interaction = targetBase
            ? classifySelector(targetBase, branch, cursorPointer)
            : null;
 ...
 * `facade/entrypoints/base/index.css` that floors every `button`, `a`, `select`,
...
function expressionCanBeTabFocusable(
  ...
  if (ts.isConditionalExpression(expression)) {
    return (
      expressionCanBeTabFocusable(ts, expression.whenTrue) ||
      expressionCanBeTabFocusable(ts, expression.whenFalse)
    );
  }
  if (
...
function tabIndexCanBeFocusable(
  ts: typeof import('typescript'),
  attribute: import('typescript').JsxAttribute
): boolean {
  const initializer = attribute.initializer;
  if (!initializer) return true;
  if (ts.isStringLiteral(initializer)) return Number(initializer.text) >= 0;
  return (
    ts.isJsxExpression(initializer) &&
    expressionCanBeTabFocusable(ts, initializer.expression)
  );
}
    ...
    const visit = (node: import('typescript').Node): void => {
      if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
        ...
        for (const property of node.attributes.properties) {
          ...
          if (name === 'tabIndex') tabFocusable = tabIndexCanBeFocusable(ts, property);
        }
        ...
        const pointerHandled =
          intrinsic &&
          POINTER_INTERACTION_HANDLERS.some((handler) => attributes.has(handler));
        const keyboardHandled = KEYBOARD_INTERACTION_HANDLERS.some((handler) =>
          attributes.has(handler)
        );
        const focusable = intrinsic && tabFocusable;

        let interaction: TsxInteraction | null = null;
        if (native) interaction = 'tsx-native';
        else if (roleInteractive) interaction = 'tsx-role';
        else if (pointerHandled || (focusable && keyboardHandled)) {
          interaction = 'tsx-handler';
        }
...
export function resolveSkinFilesForTsx(
  ...
  const compactOwner = owner.replace(/-/g, '');
  const exact = [
    `${owner}/index.css`,
    `${compactOwner}/index.css`,
    `pattern-${owner}/index.css`,
    `ds-${owner}/index.css`,
    `${owner}.css`,
    `${compactOwner}.css`,
    `pattern-${owner}.css`,
  ...
  const affix = corpus.filter((name) => {
    const stem = name.endsWith('/index.css')
      ? name.slice(0, -'/index.css'.length)
      : name.replace(/\.css$/, '');
    return stem.endsWith(`-${owner}`) || stem.startsWith(`${owner}-`);
  });
