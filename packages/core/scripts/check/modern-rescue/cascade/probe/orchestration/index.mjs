#!/usr/bin/env node
/**
 * index.mjs — verification for BYTE-EQUIVALENT rewirings.
 *
 ...
 *
 * USAGE
 *   node index.mjs --channel --ds-button-md-padding-x [--channel ...]
 *   node index.mjs --family button
 *   node index.mjs --changed            # every channel edited vs baseline
 *   node index.mjs --changed --leg2     # add the Chromium leg
 *
 * OPTIONS
...
import { parseStylesheet } from "../css-parsing/index.mjs";
import { DEFAULT_DIALS, analyse, loadSide } from "../symbolic-analysis/index.mjs";
import {
  buildFixture,
  findChromium,
  inlineSheet,
...
export function resolveEntry(entry, { dir = ENTRYPOINT_DIR } = {}) {
  ...
  for (const candidate of [join(dir, entry, 'index.css'), join(dir, `${entry}.css`)]) {
    try {
...
export function familyChannels(family, dir = COMPONENTS_DIR) {
  const abs = join(dir, family, 'index.css');
...
export function entrypointsReaching(rels, { dir = ENTRYPOINT_DIR } = {}) {
    ...
    entries = readdirSync(dir, { withFileTypes: true })
      .flatMap((entry) => {
        if (entry.isFile() && entry.name.endsWith('.css')) {
          return [{ name: entry.name.replace(/\.css$/, ''), path: join(dir, entry.name) }];
        }
        const index = join(dir, entry.name, 'index.css');
        return entry.isDirectory() && existsSync(index) ? [{ name: entry.name, path: index }] : [];
      })
  ...
  for (const entry of entries) {
    const sheet = buildSheet(entry.path, reader);
    const inSheet = new Set(sheet.files.map((f) => f.rel));
    for (const rel of wanted) if (inSheet.has(rel)) out.get(rel).push(entry.name);
  }
  return out;
}
...
const invokedDirectly =
  process.argv[1] && process.argv[1].endsWith("index.mjs");
if (invokedDirectly) {
  try {
