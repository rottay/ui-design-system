 ...
 *
 * `src/components/primitives/runtime/overlay/portal` is the ONLY module allowed to
 ...
 *
 * The allowlist (`allowlist/index.json`) is an EXACT ledger,
 ...
 *
 * Usage:
 *   node scripts/check/boundaries/surfaces/portals/index.mjs           # print the census
 *   node scripts/check/boundaries/surfaces/portals/index.mjs --check   # exit 1 on any violation
 */
...
const ALLOWLIST_PATH = join(HERE, 'allowlist/index.json');

/** The one module authorized to call `createPortal` directly. */
export const SUBSTRATE_MODULE = 'src/components/primitives/runtime/overlay/portal/index.tsx';
  ...
  for (const { file, sites } of census) {
    ...
    if (!entry) {
      violations.push({
        ...
        message: `direct createPortal outside the substrate at line(s) ${sites.map((s) => s.line).join(', ')} -- render through <Portal> (src/components/primitives/runtime/overlay/portal) instead`,
      });
      continue;
    }
    seen.add(file);
