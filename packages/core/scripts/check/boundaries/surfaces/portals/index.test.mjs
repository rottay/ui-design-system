...
test('catches a SIMULATED NEW BYPASS in a fresh file', () => {
  const census = [
    ...
    { file: 'src/components/primitives/overlay/Brandnew/engines/modern/index.tsx', sites: [{ line: 42, text: 'createPortal' }] },
  ];
  const violations = evaluatePortalCensus(census, EMPTY_ALLOWLIST);
  assert.equal(violations.length, 1);
  assert.equal(violations[0].kind, 'unauthorized');
  assert.equal(violations[0].file, 'src/components/primitives/overlay/Brandnew/engines/modern/index.tsx');
...
test('catches a SIMULATED NEW BYPASS added inside an allowlisted file', () => {
  ...
  const allowlist = { entries: [{ file: 'src/components/primitives/overlay/Sheet/engines/rustic/index.tsx', sites: 1, reason: 'frozen' }] };
  const census = [
    { file: SUBSTRATE_MODULE, sites: [{ line: 127, text: 'createPortal' }] },
    {
      file: 'src/components/primitives/overlay/Sheet/engines/rustic/index.tsx',
...
test('a ledger entry cannot outlive its sites (stale entries fail)', () => {
  const allowlist = { entries: [{ file: 'src/components/primitives/overlay/Gone/index.tsx', sites: 2, reason: 'frozen' }] };
  const census = [{ file: SUBSTRATE_MODULE, sites: [{ line: 127, text: 'createPortal' }] }];
  const violations = evaluatePortalCensus(census, allowlist);
  assert.equal(violations.length, 1);
...
test('a partially migrated allowlisted file must lower its count', () => {
  const allowlist = { entries: [{ file: 'src/components/x/index.tsx', sites: 3, reason: 'frozen' }] };
  const census = [
    { file: SUBSTRATE_MODULE, sites: [{ line: 127, text: 'createPortal' }] },
    { file: 'src/components/x/index.tsx', sites: [{ line: 10, text: 'createPortal' }] },
  ];
  const violations = evaluatePortalCensus(census, allowlist);
  assert.equal(violations.length, 1);
  assert.equal(violations[0].kind, 'stale');
