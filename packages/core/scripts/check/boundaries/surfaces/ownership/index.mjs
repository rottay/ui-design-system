 ...
 * leakage in productive TS/TSX sources under `src/components/{patterns,surfaces}`:
 *
 ...
 *
 * Usage:
 *   node scripts/check/boundaries/surfaces/ownership/index.mjs --check
 *   node scripts/check/boundaries/surfaces/ownership/index.mjs --report --details=100
 *   node scripts/check/boundaries/surfaces/ownership/index.mjs --write-baseline
 *   node scripts/check/boundaries/surfaces/ownership/index.mjs --write-baseline --allow-increase
 */
...
const DEFAULT_BASELINE_PATH = resolve(SCRIPT_DIR, 'baseline/index.json');
const DEFAULT_ALLOWLIST_PATH = resolve(SCRIPT_DIR, 'allowlist/index.json');
...
export function collectOwnershipFiles(sourceRoot) {
  return [
    ...walkProductiveSources(resolve(sourceRoot, 'components/patterns')),
    ...walkProductiveSources(resolve(sourceRoot, 'components/surfaces')),
...
function ownedInlineSvgPath(path) {
  ...
  return path.includes('/components/patterns/visualization/charts/');
}
...
export function analyzeOwnershipSource(source, path = 'src/components/patterns/example/index.tsx') {
...
export function buildBaseline(findings) {
  ...
  return {
    schemaVersion: 1,
    scope: ['src/components/patterns', 'src/components/surfaces'],
...
export function validateBaseline(baseline) {
  ...
  for (const [index, entry] of baseline.entries.entries()) {
    ...
    if (typeof entry.path !== 'string' || !entry.path.startsWith('src/components/')) issues.push(`${prefix}.path is invalid`);
