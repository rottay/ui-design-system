...
const uiRoot = path.join(coreRoot, 'src/components');
...
function resolveSpecifier(fromRelativePath, specifier, files) {
  ...
  if (specifier.startsWith('.')) {
    ...
    if (base.startsWith('components/')) base = `ui/${base.slice('components/'.length)}`;
  } else {
    return null;
  }
...
test('the drill fails on a planted cross-family reach into a private runtime', () => {
  ...
  const aliased = new Map([
    [
      ...
      [
        `import { useBoardState } from "@/components/patterns/data/widget-board/runtime/state";`,
