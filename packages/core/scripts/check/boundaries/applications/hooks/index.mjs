 ...
 *
 * WHAT THIS ENFORCES (independent audit 2026-07-26, independent code audit finding C3)
 * ------------------------------------------------------------------
 ...
 *
 * The allowlist is DERIVED, never hand-listed — see scripts/libraries/hooks/index.mjs.
 */
 ...
 */
export const DEFAULT_MANIFEST_PATH = resolve(CORE_ROOT, 'contracts/css/hooks/index.json');
  ...
  return `${JSON.stringify(
    {
      ...
      note:
        ...
        'contract (audit 2026-07-26, independent code audit C3), fenced from growing — not an approval. ' +
...
export function checkManifestFreshness({ manifest, manifestPath, packageJson }) {
  ...
  const manifestExport = exportsMap[MANIFEST_EXPORT_SUBPATH];
  const manifestFile = (
    typeof manifestExport === 'string' ? manifestExport : manifestExport?.default
  )?.replace(/^\.\//u, '') ?? manifestPath.split(sep).pop();
  if (manifestExport !== undefined && !files.includes(manifestFile)) {
    problems.push({
