...
// ---------------------------------------------------------------------------
// The VALUE constraint on a public hook (audit 2026-07-26, independent code audit C3 point 3)
//
...
test('RED drill: a new unadjudicated read grows the published decrease-only baseline', () => {
  ...
  const publishedPath = write(
    root,
    'contracts/css/hooks/index.json',
...
// ---------------------------------------------------------------------------
// The PUBLISHED contract (independent code audit C6.6 — "exported, consumed")
// ---------------------------------------------------------------------------

function packageJsonFixture(overrides = {}) {
  return {
    exports: { '.': {}, './hooks-manifest': { default: './contracts/css/hooks/index.json' } },
    files: ['dist/**/*.js', 'contracts/css/hooks/index.json'],
    ...overrides,
  };
}
...
test('RED: manifest-stale — a committed artifact that drifts from source fails', () => {
  const root = tempDirectory('application-hook-contract');
  const manifest = deriveHookManifest({ coreRoot: CORE_ROOT, postcss });
...
test('RED: manifest-missing — nothing published at all fails', () => {
  ...
  const freshness = checkManifestFreshness({
    manifest,
    manifestPath: join(root, 'contracts/css/hooks/index.json'),
    packageJson: packageJsonFixture(),
  });
  assert.equal(freshness.ok, false);
...
test('the published declaration pins the same schema version as the JSON artifact', () => {
  ...
  const declarationPath = join(dirname(DEFAULT_MANIFEST_PATH), 'index.d.ts');
  ...
  assert.equal(
    ...
    'contracts/css/hooks/index.d.ts and contracts/css/hooks/index.json expose different schema versions',
  );
});
