 ...
 * `baseline/index.json`, `baseline/index.json`: la
 ...
 *
 * Defecto real encontrado por independent code audit en el preaudit de este lote, y medido: la
...
function resolveSourceModule(root, importer, specifier) {
  ...
  else if (specifier.startsWith('@ui/')) base = path.resolve(root, 'src/components', specifier.slice(4));
...
function bannedBarrels(root) {
  return new Set([
    ...
    path.resolve(root, 'src/components/index.ts'),
    ...[...UI_OWNERS].map((owner) => path.resolve(root, `src/components/${owner}/index.ts`)),
  ]);
}

export function runPublicEntrypointGate({ root = DEFAULT_ROOT, silent = false } = {}) {
  const manifestPath = path.join(root, 'contracts/package/entrypoints/index.json');
  ...
  for (const [subpath, entry] of Object.entries(manifest.entries ?? {})) {
    try {
      ...
      for (const exported of actualExports) {
        if (UI_OWNERS.has(entry.owner) && !exported.source.startsWith(`src/components/${entry.owner}/`)) {
          throw new Error(`${subpath} symbol ${exported.name} is outside owner ${entry.owner}`);
        }
        if (entry.owner === 'runtime' && exported.source.startsWith('src/components/')) {
      ...
      if (!viteSource.includes("readFileSync(resolve(__dirname, 'contracts/package/entrypoints/index.json')")
  ...
  for (const subpath of Object.keys(packageJson.exports ?? {})) {
    if ([...OWNERS].some((owner) => subpath.startsWith(`./${owner}/`)) && !governedSubpaths.has(subpath)) {
      failures.push(`${subpath} is public but absent from contracts/package/entrypoints/index.json`);
    }
  }
  if (JSON.stringify(packageJson.exports?.['./public-entrypoints-manifest'])
    !== JSON.stringify({ default: './contracts/package/entrypoints/index.json' })) {
    failures.push('./public-entrypoints-manifest export is missing or incorrect');
  }
  if (!(packageJson.files ?? []).includes('contracts/package/entrypoints/index.json')) {
    failures.push('contracts/package/entrypoints/index.json is absent from package files');
  }
...
export function writeCeilingsBaseline({ root = DEFAULT_ROOT, reason, widen = false } = {}) {
   ...
   *
   * Y "SIN ANCLA" NO ES SOLO "SIN ENTRADA". Defecto de independent audit, un paso mas alla
