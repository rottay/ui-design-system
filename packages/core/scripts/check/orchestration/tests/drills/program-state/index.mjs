...
const STATE_FILE = 'packages/core/scripts/check/modern-rescue/README.md';
const INTENT_FILE = 'packages/core/scripts/check/orchestration/public/program-state/examples/index.json';
...
const CARRIED = [
  ...
  'packages/core/governance/manifest/index.json',
...
function seedRepo(dir) {
  ...
  const inventory = JSON.parse(
    readFileSync(join(dir, 'packages/core/scripts/check/modern-rescue/family-inventory/index.json'), 'utf8'),
  );
...
export function runDrills() {
  ...
  return withTempDir('program-state', (dir) => {
    ...
    const laneBounds = (root) => {
      ...
      const plan = readPlan(join(root, 'packages/core/scripts/check/orchestration/plans/examples/index.json'));
      return plan.lanes
        .map((lane) => resolveLane(lane, context))
        .map((lane) => `${lane.id} | ${lane.writeRoot} | excl[${lane.writeExcludes.join(',')}] | files=${lane.files.length}`);
    };

    const ledgerPath = join(dir, 'packages/core/artifacts/quality/programs/modern-rescue/archive/ledgers/family-source-visitation/252-family-era/index.json');
