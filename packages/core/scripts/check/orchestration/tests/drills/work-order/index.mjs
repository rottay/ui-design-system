...
import { loadContext } from '../../../plans/index.mjs';
  ...
  return withTempDir('work-order', (dir) => {
    ...
    suite.expectRefusal({
      ...
      result: check('ghost-prepass', (wo) => {
        wo.model.prePassEvidence = 'packages/core/artifacts/quality/programs/modern-rescue/no-such-pre-pass.md';
      }),
    });
    ...
    suite.expectRefusal({
      ...
      result: check('build-bound', (wo) => {
        ...
        wo.verificationCommands = [
          'pnpm -C packages/core build && node packages/core/scripts/check/engine/tokens/audit/index.mjs --check',
        ];
      }),
    });
    ...
    const UNREGISTERED_GATE = 'packages/core/scripts/package/public-api/barrel/index.mjs';
    ...
    suite.expectFact({
      ...
      details: [
        `${UNREGISTERED_GATE} is on disk and reachable by hand (\`pnpm public-barrel:check\`)`,
    ...
    // nonexistent path.
    const TRAVERSAL_PATHSPEC = 'packages/core/src/components/primitives/display/avatar/../../inputs/button';
    suite.expectRefusal({
    ...
    suite.expectFact({
      ...
      ok:
        execFileSync('node', ['-e', `process.stdout.write(require('node:path').posix.normalize(${JSON.stringify(TRAVERSAL_PATHSPEC)}))`], {
          encoding: 'utf8',
        }) === 'packages/core/src/components/primitives/inputs/button',
      details: [
        `${TRAVERSAL_PATHSPEC}`,
        '  resolves to packages/core/src/components/primitives/inputs/button — a different family, owned by a different lane',
      ],
    });
    ...
    suite.expectRefusal({
      ...
      result: check('traversal-writeset', (wo) => {
        wo.writeSet = ['packages/core/src/components/primitives/display/avatar/../../inputs/button/**'];
      }),
    });
    ...
    suite.expectRefusal({
      ...
      result: check('swallowing-pathspec', (wo) => {
        ...
        wo.writeSet = ['packages/core/src/components/structures/shell/**'];
        wo.writeExcludes = ['packages/core/src/components/structures/shell/bottom-tab-bar'];
        wo.commitPathspecs = ['packages/core/src/components/structures/shell'];
      }),
    });
    ...
    const FILTERED_GRANT = check('filtered-grant-pathspec', (wo) => {
      wo.writeSet = ['packages/core/src/components/primitives/display/avatar/**/*.test.tsx'];
    ...
    suite.expectFact({
      ...
      ok:
        ...
        filteredWitnesses.every((file) => file.startsWith('packages/core/src/components/primitives/display/avatar/')) &&
    ...
    suite.expectPass({
      ...
      result: check('filtered-grant-control', (wo) => {
        wo.writeSet = ['packages/core/src/components/primitives/display/avatar/**/*.test.tsx'];
        wo.commitPathspecs = [
          'packages/core/src/components/primitives/display/avatar/tests/Avatar.test.tsx',
          'packages/core/src/components/primitives/display/avatar/tests/Avatar.compounds.test.tsx',
        ];
      }),
    });
    ...
    const DERIVED_OUTER = 'packages/core/scripts/check/orchestration/tests';
    const DERIVED_INNER = 'packages/core/scripts/check/orchestration/tests/harness';
    ...
    // inside `scripts/maintain/codemods`. A prefix reading of "does an exclusion fall
    ...
    const ANCHORED_ABOVE = check('exclusion-anchored-above-pathspec', (wo) => {
      ...
      wo.editClass = 'headers-only';
      wo.substitutionTable = [];
      wo.writeSet = ['packages/core/scripts/maintain/codemods/**'];
      wo.writeExcludes = [];
      wo.commitPathspecs = ['packages/core/scripts/maintain/codemods'];
    });
    suite.expectRefusal({
    ...
    suite.expectPass({
      ...
      result: check('exclusion-anchored-above-control', (wo) => {
        ...
        wo.writeSet = ['packages/core/scripts/maintain/codemods/**'];
        wo.writeExcludes = [];
        wo.commitPathspecs = ['packages/core/scripts/maintain/codemods/sizetype-to-size/index.mjs'];
      }),
    });
    ...
    suite.expectRefusal({
      ...
      result: check('no-findings-dir', (wo) => {
        wo.findingsFile = 'packages/core/artifacts/quality/quality-evidence/no-such-directory/findings.md';
      }),
    });
    ...
    suite.expectRefusal({
      ...
      result: check('escapes-bound', (wo) => {
        wo.writeSet = ['packages/core/src/components/primitives/display/avatar/**', 'packages/core/src/foundation/tokens/css/**'];
      }),
    });
    ...
    suite.expectRefusal({
      ...
      result: check('family-declares-root', (wo) => {
        ...
        wo.writeRoot = 'packages/core/src/components/structures/workspace/connected-command-palette';
      }),
    });
    suite.expectRefusal({
    ...
    suite.expectRefusal({
      ...
      result: check('integrator-outside-domains', (wo) => {
        delete wo.row;
        wo.laneRole = 'integrator';
        wo.writeRoot = 'packages/core/src/components/primitives/display/avatar';
        wo.claimsSharedFiles = ['packages/core/src/foundation/tokens/css/foundation/themes/default/index.css'];
      }),
    });
    suite.expectRefusal({
      ...
      result: check('claimless-integrator', (wo) => {
        ...
        wo.writeSet = ['packages/core/src/foundation/tokens/css/foundation/themes/default/index.css'];
        wo.commitPathspecs = ['packages/core/src/foundation/tokens/css/foundation/themes/default/index.css'];
      }),
    });
    suite.expectPass({
    ...
    suite.expectRefusal({
      ...
      result: check('meaningless-claim', (wo) => {
        wo.claimsSharedFiles = ['packages/core/src/components/primitives/display/avatar/index.tsx'];
      }),
    });
    suite.expectRefusal({
    ...
    suite.expectRefusal({
      ...
      result: check('silent-coverage', (wo) => {
        ...
        wo.writeSet = ['packages/core/src/foundation/tokens/css/foundation/base/**'];
        wo.claimsSharedFiles = ['packages/core/src/foundation/tokens/css/foundation/base/spacing/index.css'];
        wo.commitPathspecs = ['packages/core/src/foundation/tokens/css/foundation/base'];
      }),
    });
    ...
    unbounded.writeSet = ['packages/core/src/components/primitives/inputs/button/**'];
