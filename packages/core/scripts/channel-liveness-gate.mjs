 ...
 * Usage
 * ============================================================================
 *   node scripts/channel-liveness-gate.mjs                  # human report
 *   node scripts/channel-liveness-gate.mjs --check           # fail-closed CI gate (requires R1 artifact)
 *   node scripts/channel-liveness-gate.mjs --json             # full JSON report to stdout
 *   node scripts/channel-liveness-gate.mjs --write [--round R1] [--artifact-path <p>]
 */
...
export function buildArtifact(gateRun, { round = DEFAULT_ROUND, evidenceRoot = DEFAULT_EVIDENCE_ROOT } = {}) {
  ...
  return {
    schemaVersion: 2,
    generatedBy: 'scripts/channel-liveness-gate.mjs --write',
