 ...
 * SAME guarded bundle as artifacts/generated/css/engines/modern/index.css (engine + reduced-motion guard).
...
function writeGeneratedStyle(relPath, content) {
  const output = resolve(root, relPath);
...
const GUARD_HEADER =
  "/* === Global reduced-motion guard (sourced from runtime/personality/index.css) === */";
...
// The engine-only artifacts/generated/css/engines/modern/index.css bundle must carry the same global
// reduced-motion guard the tenant bundles inherit from base.css. base.css pulls
// runtime/personality/index.css (layer rottay-personality); its tail section holds the
...
const personalityCss = readFile(resolve(srcCss, "runtime/personality/index.css"));
const reducedMotionGuardStart = personalityCss.indexOf(
  "@media (prefers-reduced-motion: reduce)"
);
if (reducedMotionGuardStart === -1) {
  console.error(
    "  ERROR: reduced-motion guard not found in runtime/personality/index.css"
  );
  process.exit(1);
}
...
if (
  ...
  console.error(
    "  ERROR: reduced-motion guard in runtime/personality/index.css is missing its wildcard or runtime seam"
  );
  process.exit(1);
}
const modernBundle = [
...
const baseCssPath = resolve(srcCss, "facade/entrypoints/base/index.css");
...
if (check) {
  compareToDisk("artifacts/generated/css/all-verticals/index.css", stylesBundle);
  compareToDisk("artifacts/generated/css/engines/modern/index.css", modernBundle);
  ...
  writeGeneratedStyle("artifacts/generated/css/all-verticals/index.css", stylesBundle);
  writeGeneratedStyle("artifacts/generated/css/engines/modern/index.css", modernBundle);
