...
const skins = [
  {
    path: 'presentation/components/skin/chart-foundation/index.css',
    anchors: [
      '.ds-chart-scaffold',
      '.ds-chart-frame',
  ...
  { path: 'presentation/components/skin/chart-area/index.css', anchors: ['.ds-chart-area'] },
  { path: 'presentation/components/skin/chart-bar/index.css', anchors: ['.ds-chart-bar'] },
  { path: 'presentation/components/skin/chart-bullet/index.css', anchors: ['.ds-chart-bullet'] },
  { path: 'presentation/components/skin/chart-calendar-heatmap/index.css', anchors: ['.ds-chart-calendar-heatmap'] },
  { path: 'presentation/components/skin/chart-gantt/index.css', anchors: ['.ds-chart-gantt'] },
  { path: 'presentation/components/skin/chart-heatmap/index.css', anchors: ['.ds-chart-heatmap'] },
  { path: 'presentation/components/skin/chart-line/index.css', anchors: ['.ds-chart-line'] },
  { path: 'presentation/components/skin/chart-pie/index.css', anchors: ['.ds-chart-pie'] },
  { path: 'presentation/components/skin/chart-radar/index.css', anchors: ['.ds-chart-radar'] },
  { path: 'presentation/components/skin/chart-treemap/index.css', anchors: ['.ds-chart-treemap'] },
  { path: 'presentation/components/skin/chart-waterfall/index.css', anchors: ['.ds-chart-waterfall'] },
  {
    path: 'presentation/components/skin/chart-c/index.css',
    anchors: [
      '.ds-chart-histogram',
      '.ds-chart-scatter',
      '.ds-chart-gauge',
  ...
  { path: 'runtime/engines/modern/skin/pattern-calendar-view/index.css', anchors: ['.ds-pattern-calendar-view.ds-engine-modern'] },
  { path: 'runtime/engines/rustic/skin/pattern-calendar-view/index.css', anchors: ['.ds-pattern-calendar-view.ds-engine-rustic'] },
  { path: 'runtime/engines/modern/skin/pattern-kanban-board/index.css', anchors: ['.ds-pattern-kanban-board.ds-engine-modern'] },
  { path: 'runtime/engines/rustic/skin/pattern-kanban-board/index.css', anchors: ['.ds-pattern-kanban-board.ds-engine-rustic'] },
  { path: 'runtime/engines/modern/skin/pattern-map-view/index.css', anchors: ['.ds-pattern-map-view.ds-engine-modern'] },
  { path: 'runtime/engines/rustic/skin/pattern-map-view/index.css', anchors: ['.ds-pattern-map-view.ds-engine-rustic'] },
  { path: 'runtime/engines/modern/skin/pattern-timeline/index.css', anchors: ['.ds-pattern-timeline.ds-engine-modern'] },
  { path: 'runtime/engines/rustic/skin/pattern-timeline/index.css', anchors: ['.ds-pattern-timeline.ds-engine-rustic'] },
  { path: 'runtime/engines/modern/skin/pattern-tree-view/index.css', anchors: ['.ds-pattern-tree-view.ds-engine-modern'] },
  { path: 'runtime/engines/rustic/skin/pattern-tree-view/index.css', anchors: ['.ds-pattern-tree-view.ds-engine-rustic'] },
];
...
function splitTopLevelSelectorList(rawSelector) {
  const fail = (reason) =>
    new Error(`visualization skin cannot own an unparsable selector (${reason}): ${JSON.stringify(rawSelector)}`);
...
// See src/foundation/tokens/tests/reduced-motion-guard.test.ts. Nothing else
...
test('visualization skin certifies a roster of exactly 23 uniquely anchored skins', () => {
  assert.equal(skins.length, 23, 'visualization skin must certify exactly 23 skins');
  assert.equal(
    new Set(skins.map(({ path }) => path)).size,
    skins.length,
    'visualization skin skin paths must be unique',
  );
...
for (const skin of skins) {
  test(`visualization skin skin is scope-anchored and free of escalation: ${skin.path}`, () => {
    const absolutePath = join(cssRoot, skin.path);
    assert.ok(existsSync(absolutePath), `missing visualization skin skin: ${skin.path}`);
...
test('visualization skin scope predicate owns on a lexical boundary and rejects near misses', () => {
...
test('visualization skin per-skin scan judges the raw parsed selector, never a JS-trimmed arm', () => {
...
test('visualization skin selector splitter splits only top-level commas and fails closed', () => {
  const nbsp = NBSP;
  ...
  for (const broken of [
    ...
    assert.throws(
      ...
      /visualization skin cannot own an unparsable selector/,
      `must fail closed: ${broken}`,
    );
  }
});

test('visualization skin escalation exemption is confined to the three reduced-motion longhands', () => {
