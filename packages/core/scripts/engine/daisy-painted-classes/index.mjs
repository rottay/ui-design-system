 ...
 *
 * USAGE
 *   node scripts/engine/daisy-painted-classes/index.mjs            # verify (exit 1 on drift)
 *   node scripts/engine/daisy-painted-classes/index.mjs --check    # same, explicit
 *   node scripts/engine/daisy-painted-classes/index.mjs --write    # regenerate the manifest
 */
...
export const MANIFEST_PATH = join(here, 'daisy-painted-classes.json');
...
function serialize(derived) {
  return `${JSON.stringify(
    {
      $comment:
        'GENERATED by scripts/engine/daisy-painted-classes/index.mjs from the installed daisyui package. Do not hand-edit: run `node scripts/engine/daisy-painted-classes/index.mjs --write`.',
...
export async function checkManifest(from = import.meta.url) {
  ...
  if (installed !== PINNED_DAISY_VERSION) {
    failures.push(
      `daisyui version moved: installed ${installed}, manifest pinned to ${PINNED_DAISY_VERSION}. ` +
        ...
        'node scripts/engine/daisy-painted-classes/index.mjs --write',
    );
    return failures;
  }
  ...
  if (serialize(derived) !== onDisk) {
    ...
    failures.push(
      'daisy-painted-classes.json no longer matches the installed daisyui package' +
        (added.length ? `\n  + ${added.join(' ')}` : '') +
        (removed.length ? `\n  - ${removed.join(' ')}` : '') +
        '\n  regenerate: node scripts/engine/daisy-painted-classes/index.mjs --write',
    );
  }
  return failures;
}
