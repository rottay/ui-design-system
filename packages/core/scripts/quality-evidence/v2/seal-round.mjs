...
import { checkInventoryCorrespondence } from './inventory-correspondence.mjs';
...
export function sealRound(roundId, { root = REPOSITORY_ROOT, contracts = loadProgramContracts() } = {}) {
  ...
  if (!round) throw new Error(`round ${roundId} is not declared in rounds.json`);
  ...
  const summary = {
    schemaVersion: 2,
    generatedBy: 'packages/core/scripts/quality-evidence/v2/seal-round.mjs',
    roundId,
