...
export function checkInventoryCorrespondence(options = {}) {
  ...
  if (staleGeneratedRows.length > 0) {
    const sample = staleGeneratedRows
      ...
      .join('; ');
    blockers.push(
      `${staleGeneratedRows.length} inventory rows carry generated fields that disagree with source: ${sample}${staleGeneratedRows.length > 5 ? '; ...' : ''}. Regenerate with \`node scripts/quality-evidence/v2/cli.mjs inventory --write\`.`,
    );
  }
