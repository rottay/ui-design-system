/**
 * cascade-probe.test.mjs — run with `node --test`.
 *
...
import {
  ...
  stripComments,
  varReferences,
} from "./probe/css-parse.mjs";
import { describeType, evaluate, expandVars, sameValue } from "./probe/value-eval.mjs";
import {
  analyse,
  evalIn,
  measureResponse,
...
test("entrypoint reach is measured on the real import graph, not assumed", () => {
  // The exact fact behind the seven false FAILs: styles.css imports the
  // presentation components, base.css does not.
  const rel = "packages/core/src/foundation/tokens/css/presentation/components/collapse.css";
  const reach = entrypointsReaching([rel]);
  assert.ok(reach.get(rel).includes("styles"), "styles.css imports presentation/components/collapse.css");
  assert.equal(reach.get(rel).includes("base"), false, "base.css does not — that is the whole coverage hole");
});

test("--entry accepts every entrypoint the probe can recommend", () => {
  // Naming `--entry styles` in an OUT-OF-GRAPH message is only honest if it runs.
  assert.match(resolveEntry("styles"), /facade\/entrypoints\/styles\.css$/);
  assert.match(resolveEntry("base"), /facade\/entrypoints\/base\.css$/);
