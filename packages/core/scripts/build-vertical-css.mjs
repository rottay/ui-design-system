...
function springOverrideBlock(name) {
  ...
  const easing = springLinearEasing(motion.springTension, motion.springFriction);
  return [
...
const stylesBundle = [
  ...
  allTenantsCss,
  ...(allSpringBlocks ? ['', `/* === Precomputed spring easings (per-tenant, useSpring) === */`, allSpringBlocks] : []),
].join('\n');
