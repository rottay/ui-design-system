...
`governance/manifest/index.json` is the generated pivot. It points to:

- `governance/manifest/controls/<control-id>.json`, which exclusively owns public domain,
  tier, static/DB ingress, declared outputs and calibration;
- `governance/manifest/recipes/<group-id>.json`, which exclusively owns a finite recipe or
  anatomy vocabulary and its invariants; and
- `governance/manifest/families/<canonical-family-id>.json`, which exclusively owns
...
<!-- lane-control:program-state v1 — DO NOT EDIT BY HAND. Rewrite it with:
     ...
     head=956b3593c written=2026-08-31T01:05:33.452Z intent=7486f16c01a83c1a render=4dc274ea9069b797 -->

*Everything in this section is intent. Anything derivable is derived by command, not typed here.*

**Current wave:** R1 agresivo de impacto real. Button y Segmented ya cerraron la primera gramática; no se continúa primitiva por primitiva. Kimi K3 integra y abre en paralelo data/workflow, shell/navigation y forms/surfaces con múltiples Opus sobre worktrees disjuntos, mientras Sonnet prepara mapas mecánicos y la siguiente ola. Un único integrador toca compiler, contratos, raíces, recipes, manifest y generados. Los temas Rottay, BitHire y Evnto deben tener la misma estructura y diferir sólo en su identidad; los valores downstream derivan en cascada. Fable audita cada lote y Codex cada cierre de slice o cambio de arquitectura. El objetivo inmediato es una entrega visible y premium, no más infraestructura sin producto.

**Blocked on:** No hay bloqueo de producto ni decisión pendiente del owner para iniciar los slices. El WIP preservado de appearance-posture y sus tests debe reconciliarse primero como lote separado: acortar comentarios narrativos, comprobar el comportamiento, integrar sólo si Fable y Codex lo aceptan, y no mezclarlo con los nuevos slices. Deuda conocida que no autoriza espera ni otra plataforma de scripts: receipts históricos stale, rojos preexistentes clasificados y mejoras futuras de instrumentos ya declaradas. Nunca push; docs/reauditoria-cloud/ queda siempre fuera de staging.
...
|---|---|---|
| `inventory.families` | 255 | packages/core/scripts/tooling/quality/evidence/programs/modern-rescue/family-inventory/index.json rows.length |
...
`packages/core/scripts/foundation/tokens/manifest/generation/index.mjs`. Today it synchronizes registry,
...
| RETIRED | former creative-advisor inbox | removed; accepted findings belong in their canonical contract |
...
`scripts/foundation/tokens/manifest/fanout` as the single CSS dependency census after migrating any
