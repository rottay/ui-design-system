...
import { auditGraphicsPackaging } from '../../../../../entrypoints/packaging/graphics/licenses/index.mjs';
import {
  auditPublicDeclarationClosures,
} from '../../../../../entrypoints/packaging/public-api/declarations/index.mjs';
...
export function auditNoLucideBoundary({ repoRoot, paths } = {}) {
  ...
  const exactPaths = [
    ...
    resolve(coreRoot, 'contracts/dependencies/runtime-suppliers/index.json'),
  ...
  for (const path of [...new Set(candidates.map((candidate) => resolve(candidate)))].sort()) {
    ...
    let scanned = source;
    if (display === 'packages/core/contracts/dependencies/runtime-suppliers/index.json') {
