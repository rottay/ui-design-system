...
import { createRootPublicResolver } from '../../../../libraries/taxonomy/root-public-resolver/index.mjs';
...
export const RESOLUTION_LAW =
  'Resolved by packages/core/scripts/tooling/quality/evidence/framework/inventory-correspondence/index.mjs; regenerate with `node scripts/tooling/quality/evidence/framework/cli/index.mjs inventory --write` rather than hand-editing a row.';
...
function PROGRAM_ROOT_FILE(root) {
  return path.join(root, CORE_ROOT, 'scripts/tooling/quality/evidence/programs/modern-rescue');
}
...
export function checkInventoryCorrespondence(options = {}) {
  ...
  if (staleGeneratedRows.length > 0) {
    const sample = staleGeneratedRows
      ...
      .join('; ');
    blockers.push(
      `${staleGeneratedRows.length} inventory rows carry generated fields that disagree with source: ${sample}${staleGeneratedRows.length > 5 ? '; ...' : ''}. Regenerate with \`node scripts/tooling/quality/evidence/framework/cli/index.mjs inventory --write\`.`,
    );
  }
