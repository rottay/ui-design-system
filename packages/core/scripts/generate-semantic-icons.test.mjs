...
test('authored corpus is the exact 120 foundation plus 6 BitHire first batch', async () => {
  const corpus = validateCorpusManifest(await readJson(CORPUS_PATH));
  assert.equal(corpus.entries.length, 126);
  assert.equal(corpus.entries.filter(({ pack }) => pack === 'foundation').length, 120);
  assert.equal(corpus.entries.filter(({ pack }) => pack === 'bithire').length, 6);
  assert.equal(corpus.entries.filter(({ status }) => status === 'stable').length, 50);
  assert.equal(corpus.entries.filter(({ status }) => status === 'candidate').length, 76);
  assert.deepEqual(
    corpus.entries.filter(({ status }) => status === 'stable').map(({ id }) => id),
    EXPECTED_COMPAT_V3_IDS,
  );
  assert.equal(new Set(corpus.entries.map(({ id }) => id)).size, 126);
  assert.equal(new Set(corpus.entries.map(({ componentName }) => componentName)).size, 126);
...
test('Phosphor 2.1.10 adapter is ordered, exhaustive, and locally resolvable', async () => {
  ...
  assert.deepEqual(result, { checked: 126, packageVersion: '2.1.10' });
  assert.deepEqual(adapter.entries.map(({ id }) => id), corpus.entries.map(({ id }) => id));
  assert.equal(new Set(adapter.entries.map(({ module }) => module)).size, 126);
  assert.equal(new Set(adapter.entries.map(({ exportName }) => exportName)).size, 126);
});
...
test('generation is deterministic and emits bounded per-role and pack outputs', async () => {
  ...
  assert.equal(first.size, 132);
  assert.equal([...first.keys()].filter((path) => path.startsWith('roles/')).length, 126);
  assert.match(
    first.get('roles/action-add.ts'),
    /import \{ PlusIcon as SsrGlyph \} from "@phosphor-icons\/react\/dist\/ssr\/Plus";/u,
  );
  assert.match(first.get('roles/action-add.ts'), /createSemanticIcon\(SsrGlyph/u);
  ...
  assert.match(first.get('packs/foundation.tsx'), /export const FoundationIcon = forwardRef/u);
  ...
  assert.match(first.get('packs/bithire.tsx'), /export const BithireIcon = forwardRef/u);
...
test('generated public source and declarations expose only supplier-free component types', async () => {
  ...
  for (const [path, source] of files) {
    ...
    assert.match(source, /createSemanticIcon\(SsrGlyph/u);
  ...
  try {
    ...
    assert.ok(declarationFiles.length >= 131);
