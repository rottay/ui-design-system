...
test('ESCUDO — el arbol REAL: status-seeds sigue frontier y escuda sus 4 prefijos', async () => {
  ...
  assert.deepEqual(shield.shielded.map((f) => f.capability), ['palette.status-seeds']);
  assert.equal(shield.prefixes.length, 4);
  assert.equal(shield.covers('--ds-tint-error-16'), true);
  // Y la prosa publicada sale del MISMO objeto que el escudo: no pueden discrepar.
  assert.deepEqual(shield.families.map((f) => f.capability), ['palette.status-seeds']);
});

test('DEAD/FABLE — la puerta NO re-ancla sobre un ancla ilegible, ni con --widen', async () => {
  const { buildDeadBaselineDoc } = await import('./index.mjs');
  /* CASO DE FABLE, y medido mas ancho de lo reportado. Con `previous === null`
...
test('DEAD/FABLE — un bootstrap NO puede entrar por la ruta del decrece-solo', async () => {
  const { buildDeadBaselineDoc } = await import('./index.mjs');
