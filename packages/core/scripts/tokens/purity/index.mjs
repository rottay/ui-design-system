...
export const OUT_PATH = join(CORE_ROOT, 'manifest/generated/purity.json');
...
export async function buildPurity({ coreRoot = CORE_ROOT, arm = null, inventory = null, catalog = null } = {}) {
  ...
  const rows = (inventory ?? JSON.parse(readFileSync(join(coreRoot, 'manifest/generated/slot-inventory.json'), 'utf8'))).rows;
  const cat = catalog ?? JSON.parse(readFileSync(join(coreRoot, 'manifest/cascade/root-catalog.json'), 'utf8'));
  ...
  return {
    generated: true,
    generator: 'scripts/tokens/purity/index.mjs',
    schemaVersion: 1,
    law: {
...
async function main(argv) {
  ...
  if (flags.length > 1 || !['--check', '--write'].includes(mode)) {
    console.error('uso: node scripts/tokens/purity/index.mjs [--check|--write]');
    process.exit(2);
  }
