 ...
 * (`manifest/variant-parity/index.mjs` ya lo parsea); el catalogo de cascada
 * clasifica RAICES (`manifest/cascade/root-catalog.json`); y entre las dos vive
...
import {
  ...
  analyzeSource,
  readSources,
} from '../../../manifest/variant-parity/index.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));
export const CORE_ROOT = findPackageRoot(HERE);
/**
 * VIVE EN manifest/generated/, NO EN UN OWNER PROPIO.
 *
 * El brief pedia `manifest/normalization/slot-inventory.json`, y la ley de
 * estructura (scripts-tree-gate, regla M1/R3) no lo admite: la raiz de
 * `manifest/` solo acepta los cinco data-owners declarados y directorios de
 * capacidad con `index.mjs` adentro, asi que un owner nuevo con dos JSON y
 * ningun productor es un `R3-ownerless-dir`. La salida es un artefacto
 * generado de manifest y su casa declarada ya existe: `manifest/generated/`,
 * donde viven fanout-facts, mirror-parity, root-checklists y variant-parity,
 * que son exactamente la misma especie. El ancla, por la misma ley, vive junto
 * a su productor y lleva su prefijo.
 */
export const OUT_PATH = join(CORE_ROOT, 'manifest/generated/slot-inventory.json');
export const BASELINE_PATH = join(HERE, 'slot-inventory.baseline.json');
...
export async function buildInventory({
  ...
  return {
    generated: true,
    generator: 'scripts/tokens/slot-inventory/index.mjs',
    schemaVersion: 1,
    law: {
    ...
    stats: {
       ...
       * manifest/variant-parity/index.mjs (METADATA_EXCLUSION). Las dos cifras
...
async function main(argv) {
  ...
  if (flags.length > 1 || !['--check', '--write'].includes(mode)) {
    console.error('uso: node scripts/tokens/slot-inventory/index.mjs [--check|--write]');
    process.exit(2);
  }
