...
const baseline = JSON.parse(
  readFileSync(join(scriptsDir, "engine-token-audit.baseline.json"), "utf8")
);
const exemptions = JSON.parse(
  readFileSync(
    resolve(packageRoot, "../..", "roadmap/skin-exemptions.json"),
    "utf8"
  )
);
...
test("engine audit wires full runtime/fleet censuses and rejects vanished keys", () => {
  const censusRun = spawnSync(
    process.execPath,
    [join(scriptsDir, "runtime-svg-paint-census.mjs"), "--json"],
  ...
  const embeddedRun = spawnSync(
    process.execPath,
    [join(scriptsDir, "embedded-css-paint-census.mjs"), "--json"],
    { cwd: packageRoot, encoding: "utf8", maxBuffer: 4 * 1024 * 1024 }
  );
  assert.equal(
    embeddedRun.status,
  ...
  assert.equal(embedded.total, 202);
  assert.equal(embedded.classifiedPaint, 202);
  ...
  assert.equal(
    Object.values(embedded.files).filter(({ count }) => count > 0).length,
    24
  );

  const run = spawnSync(
    process.execPath,
    [join(scriptsDir, "engine-token-audit.mjs"), "--check"],
    { cwd: packageRoot, encoding: "utf8", maxBuffer: 4 * 1024 * 1024 }
  );
  assert.equal(
    run.status,
  ...
  const baselineRuntimeKeys = Object.keys(baseline).filter((key) =>
    key.startsWith("runtimeSvgPaint.")
  );
  ...
  const perFileKeys = baselineRuntimeKeys.filter(
    (key) => !runtimeAggregateKeys.has(key)
  );
  const baselineFleetKeys = Object.keys(baseline).filter((key) =>
    key.startsWith("fleet.inlinePaint.")
  );
  const fleetAggregateKeys = new Set([
    "fleet.inlinePaint.filesScanned",
    "fleet.inlinePaint.total",
  ]);
  const fleetPerFileKeys = baselineFleetKeys.filter(
    (key) => !fleetAggregateKeys.has(key)
  );
  const baselineEmbeddedKeys = Object.keys(baseline).filter((key) =>
    key.startsWith("embeddedCssPaint.")
  );
  ...
  const embeddedPerFileKeys = baselineEmbeddedKeys.filter(
    (key) => !embeddedAggregateKeys.has(key)
  );
  ...
  assert.equal(
    baseline[
      "runtimeSvgPaint.patterns/communication/presence/index.tsx"
    ],
    1
  );
  assert.equal(
    baseline[
      "runtimeSvgPaint.surfaces/pages/experience/oauth-transition/provider-icons/index.tsx"
    ],
    9
  );
  const runtimeExemptions =
    exemptions["SKIN-EXEMPT-RUNTIME-VALUE"].files;
  assert.equal(
    runtimeExemptions["patterns/communication/presence/index.tsx"]
      .runtimeSvgFloor,
    1
  );
  assert.equal(
    runtimeExemptions[
      "patterns/visualization/charts/tooltip/crosshair.ts"
    ].runtimeSvgFloor,
    1
  );
  assert.equal(
    runtimeExemptions[
      "surfaces/pages/experience/oauth-transition/provider-icons/index.tsx"
    ].runtimeSvgFloor,
    9
  );
  assert.equal(perFileKeys.length, baseline["runtimeSvgPaint.filesScanned"]);
  assert.deepEqual(
    collectMissingPrefixedCounters(
      counters,
      baseline,
      "runtimeSvgPaint."
    ),
  ...
  assert.equal(baseline["embeddedCssPaint.total"], 202);
  assert.equal(baseline["embeddedCssPaint.classifiedPaint"], 202);
  ...
  assert.equal(
    baseline[
      "embeddedCssPaint.patterns/data/table-checkbox-styles/index.tsx"
    ],
    7
  );
  assert.equal(
    exemptions["SKIN-EXEMPT-EMBEDDED-CSS-CONTRACT"].files[
      "patterns/data/table-checkbox-styles/index.tsx"
    ].embeddedCssFloor,
    7
  );
  assert.equal(
    embeddedPerFileKeys.length,
    baseline["embeddedCssPaint.filesScanned"]
  );
  assert.deepEqual(
    collectMissingPrefixedCounters(
      counters,
  ...
  assert.equal(baseline["fleet.inlinePaint.total"], 732);
  assert.equal(
    fleetPerFileKeys.length,
    baseline["fleet.inlinePaint.filesScanned"]
  );
  assert.equal(
  ...
  assert.equal(
    baseline[
      "fleet.inlinePaint.surfaces/foundation/common/story-helpers.tsx"
    ],
    6
  );
  assert.equal(
    baseline[
      "fleet.inlinePaint.surfaces/foundation/common/test-utils.tsx"
    ],
    1
  );
  assert.deepEqual(
  ...
  for (const relativePath of ARC09_INLINE_PAINT_FILES) {
    assert.ok(
      `arc09.inlinePaint.${relativePath}` in counters,
      `${relativePath} must remain covered by ARC-09`
    );
    assert.ok(
      !(`fleet.inlinePaint.${relativePath}` in counters),
      `${relativePath} must not be duplicated in fleet`
    );
  }
  ...
  const embeddedRepresentative = embeddedPerFileKeys.find(
    (key) => baseline[key] > 0
  );
  assert.ok(
    embeddedRepresentative,
    "the baseline must contain a non-zero embedded CSS file counter"
  );
  const withOneEmbeddedFileRemoved = { ...counters };
  delete withOneEmbeddedFileRemoved[embeddedRepresentative];
  assert.deepEqual(
    collectMissingPrefixedCounters(
      withOneEmbeddedFileRemoved,
      baseline,
      "embeddedCssPaint."
    ),
    [embeddedRepresentative]
  ...
  assert.deepEqual(
    collectMissingPrefixedCounters({}, baseline, "runtimeSvgPaint."),
    baselineRuntimeKeys
  );
  assert.deepEqual(
    collectMissingPrefixedCounters({}, baseline, "fleet.inlinePaint."),
    baselineFleetKeys
  );
  assert.deepEqual(
    collectMissingPrefixedCounters({}, baseline, "embeddedCssPaint."),
    baselineEmbeddedKeys
  );
});

test("embedded CSS reconciliation preserves the exhaustive +63 correction", () => {
  ...
  assert.equal(
    exhaustivePreOAuthInventory - oauthMigration - loadingOverlayMigration,
    202
  );
