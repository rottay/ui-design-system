 ...
 * customization-model/index.json#controlImpactContract (`applicableFamilyFields`,
 * `internalChannelFields`, `familyDispositionVocabulary`) and from
 * customization-model/index.json#cardinalityLaw / #orthogonality.duplicateControlRule.
...
import { sightedApprovers, validateReceipt } from '../../../check/evidence/framework/receipts/index.mjs';
 ...
 * schema.json#vocabulary.domainKinds; index.mjs enforces both sides.
 */
/**
...
export const ROOT_CHANNEL_AUTHORITIES = Object.freeze(['tenant-seed', 'floor-arbitrated']);

const TENANT_SEED_ROOTS = Object.freeze({
  'palette.seeds': '--ds-color-primary',
  'palette.status-seeds': '--ds-color-success',
});
...
export function validateCascadeRoot(doc, {
  ...
  if (!ROOT_CHANNEL_AUTHORITIES.includes(rc?.authority)) {
    errors.push(`${label}: rootChannel.authority must be tenant-seed or floor-arbitrated`);
  } else if (rc.authority === 'tenant-seed') {
    const expectedChannel = TENANT_SEED_ROOTS[doc.rootId];
    if (!expectedChannel || rc.channel !== expectedChannel) {
      errors.push(`${label}: tenant-seed authority is restricted to the exact palette seed root channels`);
    }
    const closedColorSet =
      controlTier === 'standard' &&
      doc.tier === 'standard' &&
      doc.variantDisposition === 'continuous' &&
      controlDomain?.kind === 'color-set' &&
      Array.isArray(controlDomain.enumValues) &&
      controlDomain.enumValues.length === 0 &&
      controlDomain.bounds === null;
    if (!closedColorSet) {
      errors.push(`${label}: tenant-seed requires a Standard continuous color-set control with open color values`);
    }
    if (!Array.isArray(rc.emission) || !rc.emission.some((e) => e?.kind === 'compiler-emit')) {
      errors.push(`${label}: tenant-seed requires a compiler-emit rootChannel.emission`);
    }
  }
  /**
...
/**
 * customization-model/index.json#controlImpactContract.forbiddenControlSegmentFields, verbatim.
 *
...
/**
 * customization-model/index.json#controlImpactContract.internalChannelFields, verbatim.
...
/**
 * customization-model/index.json#controlImpactContract.applicableFamilyFields, verbatim.
...
/** customization-model/index.json#orthogonality.duplicateControlRule */
 ...
 * evidence-contract/index.json and validates its CONTENT with the existing v2 receipt validator
...
export function resolveEvidenceReceipt(evidenceId, { repositoryRoot, contracts, now }) {
  ...
  if (!isNonEmptyString(evidenceRoot)) {
    return { receipt: null, failures: ['evidence-contract/index.json declares no evidence root'] };
  }
...
/** Validates every receipt-valued field authored anywhere under a control calibration. */
export function validateControlCalibrationEvidence(
  calibration,
  { label, repositoryRoot, contracts, now },
) {
  const errors = [];
  const visit = (value, path) => {
    ...
    for (const [key, field] of Object.entries(value)) {
      ...
      if (!key.endsWith('EvidenceIds')) {
        visit(field, fieldPath);
        continue;
      }
      const where = `${label}: ${fieldPath}`;
      if (!Array.isArray(field)) {
        errors.push(`${where} must be an array of evidence receipt ids`);
        continue;
      }
      if (field.length === 0) continue;
      const resolved = resolveEvidenceIds(field, {
        label: where,
        familyId: null,
        repositoryRoot,
        contracts,
...
/**
 * Cross-file channel laws over the WHOLE manifest, from customization-model/index.json#cardinalityLaw:
...
// ---------------------------------------------------------------------------
// Duplicate-control orthogonality (customization-model/index.json#orthogonality)
// ---------------------------------------------------------------------------
 ...
 * depth pattern are derived from family-inventory/index.json itself.
 *
...
export function checkSourceInventoryCorrespondence({ repositoryRoot, inventory }) {
  ...
  for (const candidate of candidates.sort()) {
    ...
    if ((claimedBy.get(candidate) ?? []).length === 0) {
      errors.push(`source family directory is claimed by no family-inventory/index.json row: ${candidate}`);
    }
  }
