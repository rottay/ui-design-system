...
function scaffold({ version = '2.19.34', withDist = true } = {}) {
  ...
  write('scripts/libraries/build/input-hash/index.mjs', '// fingerprint producer\n');
  write('scripts/build/stamp/index.mjs', '// stamp producer\n');
  write('src/index.ts', 'export const answer = 42;\n');
  write('src/components/button/index.tsx', 'export const Button = () => null;\n');
...
test('the build-input hash is stable across repeated computation', () => {
  ...
  try {
    ...
    assert.equal(a.fileCount, 2, 'only src/index.ts and src/components/button/index.tsx are shippable inputs');
  } finally {
    f.cleanup();
  }
});
