...
// size into a committed baseline (baseline/index.json) and enforces:
//
//   1. Additions: a packed path that is neither in the baseline nor in the
//      explicit-review approved-additions/index.json fails. New published
...
//
// Usage:
//   node scripts/package/artifacts/inventory/index.mjs --check   (default)
//   node scripts/package/artifacts/inventory/index.mjs --write    (re-seed baseline; refuses a dirty pack)
...
const BASELINE_PATH = resolve(scriptDir, 'baseline/index.json');
const ADDITIONS_PATH = resolve(scriptDir, 'approved-additions/index.json');
const LUCIDE_ALLOWLIST_PATH = resolve(scriptDir, 'third-party-icon-allowlist/index.json');
const FIXTURES_DIR_REL = 'tests/fixtures/brand-themes';
const SEED_HINT =
  ...
  if (baseline) {
    ...
    for (const { path } of pack.files) {
      if (!baselinePaths.has(path) && !additionsSet.has(path)) {
        failures.push(
          `unexpected new packed entry (not in baseline, not in approved-additions/index.json): ${path}`,
        );
      }
    }
