...
import { auditGraphicsPackaging } from './index.mjs';
import { packageRoot as findPackageRoot } from '../../../libraries/repo-root/index.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));
const coreRoot = findPackageRoot(HERE);
const canonicalManifestRoot = resolve(coreRoot, 'governance/graphics');
...
function withManifestFixture(run) {
  ...
  const manifestPath = resolve(manifestRoot, 'sources/index.json');
  try {
...
test('archived license bytes and the packaged notice are immutable evidence', () => {
  withManifestFixture(({ manifestPath, manifestRoot, fixtureRoot }) => {
    const licensePath = resolve(
      manifestRoot,
      'licenses/phosphor-icons-react/index.md',
    );
