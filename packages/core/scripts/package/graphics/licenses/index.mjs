...
const DEFAULT_MANIFEST_PATH = resolve(
  DEFAULT_CORE_ROOT,
  'governance/graphics/sources/index.json',
);
...
export function auditGraphicsPackaging(options = {}) {
  ...
  const cloudAdapterPath = resolve(
    options.cloudAdapterPath
      ?? resolve(
        graphicsRoot,
        'marks/runtime/adapters/thesvg-react/cloud-service/index.tsx',
      ),
  );
  const errors = [];

  if (!existsSync(manifestPath)) {
    throw new Error(`graphics packaging manifest is missing: ${manifestPath}`);
  }
  ...
  if (!packageFiles.includes('governance/graphics/**')) {
    errors.push('package files must include governance/graphics/**');
  }
  const scripts = packageManifest.scripts ?? {};
  if (!/package\/graphics\/licenses\/index\.mjs/u.test(scripts['graphics-licenses:check'] ?? '')) {
    errors.push('scripts.graphics-licenses:check must invoke the graphics packaging license gate');
  }
  for (const lifecycle of ['prepack', 'prebuild', 'lint']) {
    if (!/graphics-licenses:check/u.test(scripts[lifecycle] ?? '')) {
      errors.push(`scripts.${lifecycle} must invoke graphics-licenses:check`);
    }
  }
  ...
  const archivedLicenses = existsSync(licenseDirectory)
    ? readdirSync(licenseDirectory, { withFileTypes: true })
        .filter((entry) => entry.isDirectory())
        .flatMap((entry) =>
          readdirSync(resolve(licenseDirectory, entry.name), { withFileTypes: true })
            .filter((file) => file.isFile())
            .map((file) => `licenses/${entry.name}/${file.name}`),
        )
    : [];
  ...
  const markCatalogPath = resolve(
    graphicsRoot,
    'marks/foundation/catalog/index.ts',
  );
  const markProvenancePath = resolve(
    graphicsRoot,
    'marks/runtime/provenance/index.ts',
  );
  ...
  const brandAdapterPath = resolve(
    graphicsRoot,
    'marks/runtime/adapters/thesvg-react/brand/index.tsx',
  );
  ...
  if (errors.length > 0) {
    throw new Error(`graphics packaging/license gate failed:\n- ${errors.join('\n- ')}`);
  }
