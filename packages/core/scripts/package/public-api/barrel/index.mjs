...
function fail(message) {
  throw new Error(`public-barrel gate: ${message}`);
}
...
if (!readFileSync(rottayCss, 'utf8').includes('.ds-collection-shell__static-particle-field')) {
  fail('dist/rottay.css does not contain the static fallback selector');
}
...
const virtualEntries = new Map([
  [
    ...
    `import { CollectionWorkspace } from ${JSON.stringify(rootEntry)};\n` +
      'globalThis.__publicBarrelCollectionWorkspace = CollectionWorkspace;\n',
  ],
  [
    PARTICLE_API_ENTRY,
    `import { ParticleField } from ${JSON.stringify(rootEntry)};\n` +
      'globalThis.__publicBarrelParticleField = ParticleField;\n',
  ],
]);
