...
const UI_ROOT = join(SRC_ROOT, 'components');
...
describe('source governance', () => {
  ...
  it('ships dual-scoped artifacts compiled from one authored source', () => {
    const tenantCssRoot = join(SRC_ROOT, 'foundation', 'tokens', 'css', 'facade', 'artifacts');
    const tenantFiles = collectFirstPartyArtifactFiles(tenantCssRoot);
    const matches: string[] = [];
...
describe('stale paths', () => {
  ...
  it('should not have src/testing/ at top-level (tests live outside product source)', () => {
...
describe('folder structure', () => {
  ...
  it('should classify internal owners by architectural weight', () => {
    ...
    expect(existsSync(join(SRC_ROOT, 'tooling'))).toBe(false);
  });
...
describe('package exports', () => {
  ...
  it('vitest setupFiles should point to tests/setup/index.ts', () => {
    const vitestConfig = readFileSync(
      join(process.cwd(), 'vitest.config.ts'),
      'utf-8'
    );
    expect(vitestConfig).toContain('tests/setup/index.ts');
...
describe('import boundaries', () => {
  it('infrastructure/runtime/ code should not import test support', () => {
    ...
    for (const file of runtimeFiles) {
      ...
      if (
        ...
        source.includes('packages/core/tests/')
      ) {
        matches.push(file);
      }
    }
