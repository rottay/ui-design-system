...
import {
  ...
  type DiagramEdge,
  type DiagramNode,
} from '../../../../src/components/patterns/visualization/ascii-diagram';
...
function skinCss(file: string): string {
  return readFileSync(join(SKIN_DIR, file.replace(/\.css$/, ""), "index.css"), "utf8");
}
...
const ASCII_DIAGRAM_SOURCE = readFileSync(
  join(SOURCE_ROOT, "components/patterns/visualization/ascii-diagram/index.tsx"),
  "utf8",
);
...
function collectSurfacePairCssFiles(): string[] {
  return readdirSync(SKIN_DIR, { withFileTypes: true })
    ...
    .map((entry) => join(SKIN_DIR, entry.name, "index.css"))
...
describe("P3 -- SectionFrame ordinal has an accessible text equivalent, without double-announcing", () => {
  ...
  it("stays server-safe: the family imports no React hook and declares no client boundary", () => {
    const source = readFileSync(
      join(SOURCE_ROOT, "components/structures/headers/section-frame/index.tsx"),
      "utf8",
    );
