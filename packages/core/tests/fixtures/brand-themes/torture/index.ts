...
export const tortureDarkBrandTheme: BrandTheme = {
  ...
  // This fixture IS its dark mode, so it says so and its plain palette holds
  // that mode's values. It used to declare `darkBackgroundColor` with no
  // `backgroundColor` and keep its dark seeds in `dark*Color` fields, which
  // made the emission incoherent with itself: the OKLCH ramps derived from
  // `#FF00AA` on `#050307`, while the flat `--ds-color-primary` literal came
  // from `primaryColor` and shipped `#CC0088`. One palette, one mode.
  ...
  // The clear-mode seeds this fixture used to carry in its plain fields. Kept
  // as a real mode overlay so the fixture still declares BOTH grounds and the
  // probe's channel coverage stays complete.
  modes: {
    light: {
      palette: {
        primaryColor: '#CC0088',
...
export const tortureLightBrandTheme: BrandTheme = {
  ...
  // The hostile dark half, so the fixture declares BOTH grounds and the probe
  // list stays fully covered. torture-light renders in clear mode, so these
  // values are never painted at rest -- they exist to be declared, and now
  // they compile into a real `[data-theme='dark']` block instead of sitting in
  // `dark*Color` fields that no consumer could reach.
  modes: {
    dark: {
      palette: {
        primaryColor: '#9B33FF',
