/**
 * Five-phase causal capture harness for the `palette.status-seeds` canary.
 * Existing receipts retain the legacy round identifier `F4C` as historical metadata.
 *
 ...
 *
 * ===========================================================================
 * DEFECTOS QUE ESTE DISENO EVITA (hallazgos de revisiones independientes).
 * Cada uno esta atado a un mecanismo concreto de este archivo.
 * ===========================================================================
 *
 *  1. FALLBACK SILENCIOSO DE `?only=`. `component-behaviors` sin un `only` valido
 *     renderiza Tabs (bithire/component-behaviors/page.tsx:104), `primitive-state-repairs` cae en
 *     image-fallback (:31), `pattern-behaviors` en live-feed (:29-31), y
 *     `structures` / `surfaces` devuelven 404 sin `only`
 *     (structures/page.tsx:15, surfaces/page.tsx:15). Una corrida por
 ...
 *     `stable-dom-part -> observable-property` (customization-model/index.json:67),
 ...
 * MODOS
 * ===========================================================================
 *   node causal-canary-capture.mjs <out-dir> --phase <A|B|C|D|E>
 *                               [--ground bithire|the-management] [--self-test]
 *   node causal-canary-capture.mjs --compare <dirA> <dirB> [--drill <id>]
 *   node causal-canary-capture.mjs <out-dir> --check
 *
...
/** Only the interactive case pages mount this node; causal captures require its absence. */
...
/** NADIE debe mover estas por un cambio de color: aislamiento intra-carrier. */
const HOLD_PROPS = [
  'fontFamily',
  'fontSize',
  'fontWeight',
...
const CAPTURES = [
  {
    ...
    only: 'alert',
    /* Fallback de component-behaviors sin `only` valido: 'tabs' (page.tsx:104), cuyo caso
       monta [data-testid="lab-tabs"] (sections/component-behaviors/index.tsx:767). */
    ...
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:1056`,
      },
    ],
    carriers: [
      {
        id: 'alert-error',
        /* tone="danger" (sections/component-behaviors/index.tsx:660) se estampa como
        ...
        count: { exact: 1 },
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:660`,
      },
    ],
  },
  {
    ...
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:975`,
      },
    ],
    carriers: [
      {
        ...
        count: { exact: 1 },
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:976-980`,
      },
    ],
  },
  {
    ...
    route: 'primitive-state-repairs',
    only: 'callout-dismiss-focus',
    /* Fallback de primitive-state-repairs: 'image-fallback' (page.tsx:31), testid
       lab-image-fallback (sections/primitive-state-repairs/index.tsx:607). */
    ...
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/primitive-state-repairs/index.tsx:693`,
      },
    ],
    carriers: [
      {
        ...
        count: { exact: 1 },
        source: `${SHOWROOM_SCENES}/sections/primitive-state-repairs/index.tsx:252`,
      },
      {
        id: 'callout-info',
        tone: 'info',
        ...
        count: { exact: 1 },
        source: `${SHOWROOM_SCENES}/sections/primitive-state-repairs/index.tsx:264`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    route: 'pattern-behaviors',
    only: 'cockpit-header-trail-and-posture',
    /* Fallback de pattern-behaviors: 'live-feed-shell-continuity' (page.tsx:29-31). */
    ...
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/pattern-behaviors/index.tsx:1484`,
      },
    ],
    carriers: [
      {
        ...
        count: { min: 1 },
        source: `${SHOWROOM_SCENES}/sections/pattern-behaviors/index.tsx:1416`,
      },
      {
        id: 'cockpit-warning',
        tone: 'warning',
        ...
        count: { min: 1 },
        source: `${SHOWROOM_SCENES}/sections/pattern-behaviors/index.tsx:1417`,
      },
    ],
  },
  {
    ...
    /* structures falla con 404 sin `only` valido (page.tsx:15): el status
       HTTP 200 ya es la prueba anti-fallback de esta ruta. */
    forbiddenSelectors: [],
    readiness: [
      {
        what: 'bloque ActivityCompact exportado (banda modern)',
        /* La escena structures renderiza TODAS las engines
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/structures/index.tsx:134`,
      },
    ],
    carriers: [
      {
  ...
  },
  {
    ...
    /* surfaces falla con 404 sin `only` valido (page.tsx:15). */
    forbiddenSelectors: [],
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/surfaces/config-b.tsx:137-139`,
      },
    ],
    carriers: [
      {
        ...
        count: { exact: 1 },
        source: `${SHOWROOM_SCENES}/sections/surfaces/config-b.tsx:142`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    /* DECLARADA Y DESACTIVADA. La matriz adjudicada tiene 8 filas y su unica
       ...
       sections/component-behaviors/index.tsx:1630-1633). No se activa por cuenta propia
    ...
    readiness: [
      {
        ...
        exact: 1,
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:1625`,
      },
    ],
    carriers: [
      {
        ...
        count: { min: 1 },
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:1632`,
      },
      {
        id: 'timeline-error',
        tone: 'error',
        ...
        count: { min: 1 },
        source: `${SHOWROOM_SCENES}/sections/component-behaviors/index.tsx:1630-1631`,
      },
    ],
  },
];
...
/** Perturbaciones nombradas del comparador: el drill negativo que exige evidence-contract/index.json:76. */
const DRILLS = {
...
async function gotoWithRetry(page, url) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      // structures); el retry no rescata un goto que muere a los 120s.
...
async function assertNoFallbackScene(page, capture, label) {
  ...
  if (judged > 0) {
    throw new Error(
      `JUDGE MODE para ${label} — ${JUDGE_MODE_SELECTOR} presente. El canary mide pintura real, no una transformada de juicio.`,
    );
  }
}
...
async function captureOne(browser, { ground, capture, width, outPath, channels, cssCache }) {
  ...
  try {
    ...
    return {
      ...
      carriers,
      /* Campos de contrato por fila (evidence-contract/index.json:59-75). */
 ...
 * independent audit, 2026-08-30). Used by `--check` and the comparator.
 */
...
async function compareRuns(receiptA, receiptB, { drill = null, dirA = null, dirB = null, getBrowser = null } = {}) {
  ...
  /* Una corrida degradada NO es comparable: cada campo del receipt debe
     recomputar certificable de forma independiente. El agregado `pass` no
     reemplaza la recomputacion por campos. */
...
async function runCapture(args) {
  ...
  if (!outDir) throw new Error('uso: causal-canary-capture.mjs <output-dir> --phase <A|B|C|D|E> [--ground g] [--self-test]');
  ...
  const receipt = {
    ...
    commandOrTool: `node packages/showroom/scripts/causal-canary-capture.mjs ${process.argv.slice(2).join(' ')}`,
    toolVersion: {
      node: process.version,
      playwright: playwright?.version ?? null,
      playwrightResolvedFrom: playwright?.resolvedFrom ?? null,
    },
    producer: `causal-canary-capture.mjs@${selfSha.slice(0, 12)}`,
    ...
    negativeDrill: {
      available: DRILLS,
      howToRun: 'node causal-canary-capture.mjs --compare <dirA> <dirB> --drill <id> (exit 0 solo si el comparador falla)',
    },
    tierCoverage: {
...
async function main() {
  ...
  if (args.flags.check) {
    const outDir = args.positional[0];
    if (!outDir) throw new Error('uso: causal-canary-capture.mjs <output-dir> --check');
    runCheck(outDir);
    return;
  }
