...
const CONTAINER_SELECTOR = '[data-testid="probe-media-states"]';
...
// ---------------------------------------------------------------------------
// 4 rest shots -- both fixtures, both engines, full probe-media-states grid.
// ---------------------------------------------------------------------------

for (const fixture of FIXTURES) {
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / media-states / ${engine} @ w1280`, async ({ page }) => {
      test.setTimeout(60_000);

      const container = await openProbe(page, fixture.id, engine);
      await expect(container).toHaveScreenshot(`${fixture.id}-media-states-${engine}.png`);
    });
  }
}
...
for (const engine of ENGINES) {
  test(`rottay (dark) / media-states / ${engine}: Card hoverable`, async ({ page }) => {
    test.setTimeout(60_000);
    ...
    const cardRoot = container
      .locator(`[data-testid='probe-media-states-card'] [data-variant='outlined'][${hoverAttr}='true']`)
      .first();
    await cardRoot.hover();
    await waitForSettled(page, cardRoot);

    await expect(cardRoot).toHaveScreenshot(`rottay-media-states-card-${engine}-hovered.png`);
  });
}

for (const engine of ENGINES) {
  test(`rottay (dark) / media-states / ${engine}: Tag closable`, async ({ page }) => {
    test.setTimeout(60_000);

    const container = await openProbe(page, 'rottay', engine);
    const tagRow = container.locator("[data-testid='probe-media-states-tag']").first();
    const close = tagRow.locator("[data-part='close']").first();
    await close.hover();
    await waitForSettled(page, close);

    await expect(tagRow).toHaveScreenshot(`rottay-media-states-tag-${engine}-close-hovered.png`);
  });
}

for (const engine of ENGINES) {
  test(`rottay (dark) / media-states / ${engine}: Image preview`, async ({ page }) => {
    test.setTimeout(60_000);

    const container = await openProbe(page, 'rottay', engine);
    const zoomableImage = container.locator("[data-testid='probe-media-states-image'] [data-zoomable='true']").first();
    await zoomableImage.hover();
    await waitForSettled(page, zoomableImage);

    await expect(zoomableImage).toHaveScreenshot(`rottay-media-states-image-${engine}-preview-hovered.png`);
  });
}
