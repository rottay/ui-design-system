...
const CONTAINER_SELECTOR = '[data-testid="probe-data-display-states"]';
...
async function openProbe(page: Page, fixture: Fixture, engine: Engine): Promise<Locator> {
  ...
  await page.goto(
    `/probe/whitelabel-torture?fixture=${fixture}&engine=${engine}&w=1280&slug=button&dataDisplayStates=1`,
...
// ---------------------------------------------------------------------------
// 4 rest shots -- both fixtures, both engines, full probe-data-display-states grid.
// ---------------------------------------------------------------------------

for (const fixture of FIXTURES) {
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / data-display-states / ${engine} @ w1280`, async ({ page }) => {
      test.setTimeout(60_000);

      const container = await openProbe(page, fixture.id, engine);
      await expect(container).toHaveScreenshot(`${fixture.id}-data-display-states-${engine}.png`);
    });
  }
}
...
for (const engine of ENGINES) {
  test(`rottay (dark) / data-display-states / ${engine}: Tree node hovered`, async ({ page }) => {
    test.setTimeout(60_000);

    const container = await openProbe(page, 'rottay', engine);
    const treeRoot = container.locator('[data-testid="probe-data-display-states-tree"]').first();
    const row = treeRoot.locator("[data-part='row']:not([data-disabled])").first();
    await row.hover();
    await waitForSettled(page, "[data-testid='probe-data-display-states-tree'] [data-part='row']:not([data-disabled])");

    await expect(treeRoot).toHaveScreenshot(`rottay-data-display-states-tree-${engine}-hovered.png`);
  });
}
...
for (const engine of ENGINES) {
  test(`rottay (dark) / data-display-states / ${engine}: List item hovered`, async ({ page }) => {
    test.setTimeout(60_000);

    const container = await openProbe(page, 'rottay', engine);
    const listRoot = container.locator('[data-testid="probe-data-display-states-list"]').first();
    const item = listRoot.locator("[data-part='item']").first();
    await item.hover();
    await page.waitForTimeout(300);

    await expect(listRoot).toHaveScreenshot(`rottay-data-display-states-list-${engine}-hovered.png`);
  });
}
...
for (const engine of ENGINES) {
  test(`rottay (dark) / data-display-states / ${engine}: Tooltip open`, async ({ page }) => {
    test.setTimeout(60_000);

    const container = await openProbe(page, 'rottay', engine);
    const trigger = container.locator('[data-testid="probe-data-display-states-tooltip-trigger"]');
    await trigger.hover();
    ...
    await expect(bubble).toHaveScreenshot(`rottay-data-display-states-tooltip-${engine}-open.png`);
  });
}
