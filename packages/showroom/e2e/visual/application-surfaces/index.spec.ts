...
// ---------------------------------------------------------------------------
// WO-SKIN-06 checkpoint application surfaces -- category-A patterns/misc visual evidence.
//
// The fixture is gated by ?applicationSurfaces=1 and keeps each component/state family in
...
const ROOT_SELECTOR = '[data-testid="probe-application-surfaces"]';
...
async function waitForSettled(page: Page, locator: Locator): Promise<void> {
  ...
  await page.waitForFunction(
    (el) => {
      ...
      const w = window as unknown as { __applicationSurfaceSettle?: { key: string; hits: number } };
      if (!w.__applicationSurfaceSettle || w.__applicationSurfaceSettle.key !== key) {
        w.__applicationSurfaceSettle = { key, hits: 1 };
        return false;
      }
      w.__applicationSurfaceSettle.hits += 1;
      return w.__applicationSurfaceSettle.hits >= 3;
    },
    handle,
...
async function openProbe(page: Page, fixture: Fixture, engine: Engine): Promise<Locator> {
  ...
  await page.goto(
    `/probe/whitelabel-torture?fixture=${fixture}&engine=${engine}&w=1280&slug=button&applicationSurfaces=1`,
  ...
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / misc H2 / ${engine}: isolated rest bands`, async ({ page }) => {
      ...
      for (const bandName of REST_BANDS) {
        const band = root.locator(`[data-testid="probe-application-surfaces-${bandName}"]`);
        await waitForSettled(page, band);
        await expect(band).toHaveScreenshot(`${fixture.id}-application-surfaces-${engine}-${bandName}.png`, {
...
for (const engine of ENGINES) {
  test(`rottay (dark) / misc H2 / ${engine}: explicit high-risk state bands`, async ({ page }) => {
    ...
    const list = root.locator('[data-testid="probe-application-surfaces-file-list"]');
    const grid = root.locator('[data-testid="probe-application-surfaces-file-grid"]');
    ...
    const fullProfiles = root.locator('[data-testid="probe-application-surfaces-user-full"]');
    ...
    for (const cycle of ['monthly', 'yearly']) {
      const pricing = root.locator(`[data-testid="probe-application-surfaces-pricing-${cycle}"]`);
    ...
    for (const bandName of HIGH_RISK_BANDS) {
      const band = root.locator(`[data-testid="probe-application-surfaces-${bandName}"]`);
      await waitForSettled(page, band);
      await expect(band).toHaveScreenshot(`rottay-application-surfaces-${engine}-${bandName}-state.png`, {
        maxDiffPixelRatio: 0.0005,
      });
    }
  });
...
test('rottay (dark) / misc H2: token-inspector active with color/text rows, unpinned then pinned', async ({ page }) => {
  ...
  const target = root.locator('[data-testid="probe-application-surfaces-token-target"]');
  ...
  await expect(panel).toHaveScreenshot('rottay-application-surfaces-token-inspector-unpinned.png', {
    maxDiffPixelRatio: 0.0005,
  });
  ...
  await waitForSettled(page, panel);
  await expect(panel).toHaveScreenshot('rottay-application-surfaces-token-inspector-pinned.png', {
    maxDiffPixelRatio: 0.0005,
  });
