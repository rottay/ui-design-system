...
const ROOT_SELECTOR = '[data-testid="probe-visualizations"]';
  ...
  await page.waitForFunction(
    (element) => {
      const signature = [...(element as Element).querySelectorAll('svg *')]
        ...
        .join('\n');
      const state = window as unknown as { __visualizationSettle?: { signature: string; hits: number } };
      if (!state.__visualizationSettle || state.__visualizationSettle.signature !== signature) {
        state.__visualizationSettle = { signature, hits: 1 };
        return false;
      }
      state.__visualizationSettle.hits += 1;
      return state.__visualizationSettle.hits >= 4;
    },
    handle,
...
for (const fixture of FIXTURES) {
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / visualizations visualization patterns / ${engine}`, async ({ page }) => {
      test.setTimeout(120_000);
      const root = await openProbe(page, fixture.id, engine, fixture.ground);
      const band = root.locator('[data-testid="probe-visualizations-patterns"]');
    ...
    test(`${fixture.id} (${fixture.ground}) / visualizations chart catalog / ${engine}`, async ({ page }) => {
      test.setTimeout(120_000);
      const root = await openProbe(page, fixture.id, engine, fixture.ground);
      ...
      await expect(band).toHaveScreenshot(`${fixture.id}-visualizations-${engine}-charts-prestep.png`, {
        maxDiffPixelRatio: 0.0005,
      });
    });
  }
...
for (const fixture of FIXTURES) {
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / visualizations mobile composition / ${engine}`, async ({ page }) => {
      test.setTimeout(120_000);
      const root = await openProbe(page, fixture.id, engine, fixture.ground, '360');
      const patterns = root.locator('[data-testid="probe-visualizations-patterns"]');
      const charts = root.locator('[data-testid="probe-visualizations-charts"]');
      await expect(patterns).toBeVisible();
      await expect(charts.locator('[data-chart-fixture]')).toHaveCount(19);
      await expect(patterns).toHaveScreenshot(`${fixture.id}-visualizations-${engine}-patterns-mobile-prestep.png`, {
        maxDiffPixelRatio: 0.0005,
      });
      await expect(charts).toHaveScreenshot(`${fixture.id}-visualizations-${engine}-charts-mobile-prestep.png`, {
        maxDiffPixelRatio: 0.0005,
      });
    });
  }
...
test('visualizations interactive tooltip, crosshair, sankey hover and brush selection', async ({ page }) => {
  test.setTimeout(120_000);
  ...
  await expect(barCell).toHaveScreenshot('rottay-visualizations-modern-bar-hover-prestep.png');
  ...
  await expect(sankeyCell).toHaveScreenshot('rottay-visualizations-modern-sankey-hover-prestep.png');
  ...
  await expect(brushCell).toHaveScreenshot('rottay-visualizations-modern-brush-selected-prestep.png');
});

test('visualizations rustic Kanban and Timeline interactive paint states', async ({ page }) => {
  test.setTimeout(120_000);
  ...
  await expect(kanban).toHaveScreenshot('rottay-visualizations-rustic-kanban-card-hover-prestep.png');

  const addItem = kanban.locator('[data-part="add-item"]').first();
  await addItem.hover();
  await expect(kanban).toHaveScreenshot('rottay-visualizations-rustic-kanban-add-hover-prestep.png');
  ...
  await page.evaluate(
    ({ cardSelector, targetSelector }) => {
      ...
      if (!source || !target) throw new Error('visualizations drag fixture did not render');
      const dataTransfer = new DataTransfer();
      source.dispatchEvent(new DragEvent('dragstart', { bubbles: true, cancelable: true, dataTransfer }));
      target.dispatchEvent(new DragEvent('dragover', { bubbles: true, cancelable: true, dataTransfer }));
    },
    {
      cardSelector: '[data-testid="probe-visualizations-patterns"] .ds-pattern-kanban-board [data-part="card"]',
      targetSelector:
        '[data-testid="probe-visualizations-patterns"] .ds-pattern-kanban-board [data-part="column-body"][data-empty="true"]',
    }
  );
  await expect(dropTarget).toHaveAttribute('data-dropping', 'true');
  await expect(kanban).toHaveScreenshot('rottay-visualizations-rustic-kanban-drop-target-prestep.png');
  ...
  await expect(timeline).toHaveScreenshot('rottay-visualizations-rustic-timeline-hover-prestep.png');
});
