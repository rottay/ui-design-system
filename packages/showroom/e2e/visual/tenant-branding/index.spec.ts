...
const ROOT_SELECTOR = '[data-testid="probe-tenant-branding"]';
  ...
  await page.waitForFunction(
    (element) => {
      ...
      const state = window as unknown as {
        __tenantBrandingSettle?: { key: string; hits: number };
      };
      if (!state.__tenantBrandingSettle || state.__tenantBrandingSettle.key !== key) {
        state.__tenantBrandingSettle = { key, hits: 1 };
        return false;
      }
      state.__tenantBrandingSettle.hits += 1;
      return state.__tenantBrandingSettle.hits >= 3;
    },
    handle,
...
for (const fixture of FIXTURES) {
  for (const engine of ENGINES) {
    test(`${fixture.id} (${fixture.ground}) / tenant branding tenant-preview / ${engine}`, async ({ page }) => {
      test.setTimeout(90_000);
      const root = await openProbe(page, fixture.id, engine, fixture.ground);
      const band = root.locator('[data-testid="probe-tenant-branding-tenant-preview"]');
      ...
      await expect(band).toHaveScreenshot(`${fixture.id}-tenant-branding-${engine}-tenant-preview-prestep.png`, {
        maxDiffPixelRatio: 0.0005,
      });
    });
  }

  test(`${fixture.id} (${fixture.ground}) / tenant branding branding sandbox full state`, async ({ page }) => {
    test.setTimeout(90_000);
    const root = await openProbe(page, fixture.id, 'modern', fixture.ground);
    const band = root.locator('[data-testid="probe-tenant-branding-branding-sandbox"]');
    ...
    await waitForSettled(page, band);
    await expect(band).toHaveScreenshot(`${fixture.id}-tenant-branding-branding-sandbox-prestep.png`, {
      maxDiffPixelRatio: 0.0005,
    });
  });

  test(`${fixture.id} (${fixture.ground}) / tenant branding Brand Studio both scoped grounds`, async ({ page }) => {
    test.setTimeout(90_000);
    const root = await openProbe(page, fixture.id, 'modern', fixture.ground);
    const band = root.locator('[data-testid="probe-tenant-branding-brand-studio"]');
    ...
    await waitForSettled(page, band);
    await expect(band).toHaveScreenshot(`${fixture.id}-tenant-branding-brand-studio-prestep.png`, {
      maxDiffPixelRatio: 0.0005,
    });
  });
}
