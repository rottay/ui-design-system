...
const PROBE_SECTIONS = ['r6-surfaces', 'compositions', 'monochrome'];
...
test('the R6 fixture modules are inside the scanned tree', () => {
  const files = probeSourceFiles().map((file) =>
    path.relative(SHOWROOM_ROOT, file),
  );
  assert.ok(files.length > 0, 'no R6 probe sources found');
  const sections = 'src/app/probe/ds-reference/sections/r6-surfaces';
...
test('no R6 probe fixture reads the clock or entropy', () => {
  const offenders = [];
...
test('every date the R6 probe constructs is an explicit literal instant', () => {
  const offenders = [];
  for (const file of probeSourceFiles()) {
