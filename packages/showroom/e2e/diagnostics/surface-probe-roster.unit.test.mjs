...
const SURFACE_FIXTURES = path.join(SHOWROOM_ROOT, 'src/app/probe/ds-reference/sections/surfaces');
...
function slugArray(file, exportName) {
  const source = readFileSync(path.join(SURFACE_FIXTURES, file), 'utf8');
...
const ROSTER = slugArray('cases.ts', 'SURFACE_CASES');
const MODULES = [
...
test('the dispatcher actually renders every fixture module', () => {
  const dispatcher = readFileSync(path.join(SURFACE_FIXTURES, 'index.tsx'), 'utf8');
