...
//      horizontally. One violation per (set/slug/tenant) cell.
//   2. TOUCH TARGETS. Under coarse-pointer emulation every interactive part
//      reaches at least 44x44 CSS px. One violation per (slug, selector, size).
//
...
//
// Touch targets are measured under the rottay palette only: a palette changes
// colors, never box sizes, so sweeping both would double the runtime to
// re-measure identical geometry.
// ---------------------------------------------------------------------------

const FLAGSHIP_SLUGS = ['button', 'input', 'select', 'card', 'badge', 'table', 'tabs', 'modal'] as const;
const RESPONSIVE_SLUGS = ['checkbox', 'switch', 'dropdown', 'stats-grid', 'data-table', 'form-builder'] as const;
...
const INTERACTIVE_SELECTOR =
  'button, [role="tab"], [role="switch"], [role="checkbox"], input:not([type=hidden]), select, a[href]';
...
test.describe('responsive conformance', () => {
  test('no capture cell scrolls the document horizontally at 360px', async ({ page }) => {
      ...
      const measurement = await page.evaluate(() => {
        ...
        document.querySelectorAll('*').forEach((element) => {
          ...
          if (rect.width > 0 && rect.right > viewportWidth + 1) {
            ...
            spilled.add(`${element.tagName.toLowerCase()}${cls ? `.${cls}` : ''}`);
          }
        });
        return { overflow: root.scrollWidth - root.clientWidth, spilled: [...spilled].slice(0, 4) };
      });

      if (measurement.overflow > 1) {
        const key = `${cell.set}/${cell.slug}/${cell.tenant}`;
        observed.push(key);
        detail[key] = measurement.spilled;
      }
    }
    ...
    expect(
      ...
      'new horizontal overflow at 360px. Make the width intrinsic; do not widen the baseline.',
    ).toEqual([]);
  });
    ...
    try {
      for (const set of ['flagship', 'responsive'] as const) {
        ...
        for (const slug of slugs) {
          await gotoCell(page, set, slug, 'rottay');
          ...
          for (const entry of undersized) observed.push(`${slug}/${entry}`);
        }
      }
    } finally {
      await context.close();
