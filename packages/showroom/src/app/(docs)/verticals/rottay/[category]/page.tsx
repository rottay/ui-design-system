...
const PLATFORM_DEMOS: Record<string, CategoryData> = {
  identity: {
    label: 'Identity',
    summary:
      'Rottay identity surfaces are about trust at scale: fast lookup, stable user context, and clear control over sessions and profile state.',
  ...
  tenancy: {
    ...
    operators: ['Rottay ops', 'Customer success', 'Billing admins'],
...
function PlatformCategoryContent({ category }: { category: string }) {
  ...
  if (!data) {
    return (
      <Stack spacing="lg" fullWidth>
        <Link href="/verticals/rottay" style={{ textDecoration: 'none' }}>
          <Flex align="center" gap={4}>
            <ChevronLeftIcon size={16} />
            <Text size="sm" color="primary">Back to Rottay</Text>
          </Flex>
        </Link>
        <Card
          style={{
        ...
        >
          <Stack spacing="md">
            ...
            <Box>
              ...
              >
                The category &quot;{category}&quot; does not exist in the Rottay vertical.
              </Text>
            </Box>
            <Box
              style={{
  ...
  return (
    <VerticalCategoryAppendix
      backHref="/verticals/rottay"
      backLabel="Back to Rottay"
      label={data.label}
      headline={`${data.label} in Rottay is about controlled clarity, not ornamental UI.`}
      summary={`${data.summary} The workflow stays Rottay-specific, but the live rendering should still come from the active docs runtime rather than a local preset.`}
