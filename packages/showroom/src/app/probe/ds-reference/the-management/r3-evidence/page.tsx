import { R3EvidenceScene, type R3EvidenceCase } from '../../sections/r3-evidence';
import { JudgeOverlay } from '../../judge';

const CASES: R3EvidenceCase[] = [
...
export default async function R3EvidencePage({
  searchParams,
}: {
  ...
  const only = (CASES as string[]).includes(raw ?? '')
    ? (raw as R3EvidenceCase)
    : 'live-feed-shell-continuity';
  return (
    <>
      <JudgeOverlay searchParams={searchParams} />
      <R3EvidenceScene only={only} />
    </>
  );
}
