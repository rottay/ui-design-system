...
export type PrimitiveAnatomyCase =
  | 'table-anatomy'
  | 'tree-anatomy'
  | 'carousel-anatomy'
...
export function PrimitiveAnatomyScene({ only }: { only: PrimitiveAnatomyCase }) {
  return (
    <SceneFrame title={`PRIMITIVE STATE REPAIRS B - ${only.toUpperCase()}`}>
      {only === 'table-anatomy' && (
