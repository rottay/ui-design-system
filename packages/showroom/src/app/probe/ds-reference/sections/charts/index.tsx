...
/* Chart probe: one family per case, deterministic fixtures, so every
...
import type { ChartCase } from './cases';
...
function Chart({ only }: { only: ChartCase }): ReactNode {
  switch (only) {
    case 'area-chart':
...
export function ChartScene({ only }: { only: ChartCase }) {
  return (
