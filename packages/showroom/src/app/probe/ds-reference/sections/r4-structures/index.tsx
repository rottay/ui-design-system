...
/* R4 structure probe: each family renders under three engine scopes with
...
import type { R4Case } from './cases';

const ENGINES = ['modern', 'classic', 'rustic'] as const;

function Family({ only }: { only: R4Case }): ReactNode {
  switch (only) {
...
export function R4StructureScene({ only }: { only: R4Case }) {
  return (
