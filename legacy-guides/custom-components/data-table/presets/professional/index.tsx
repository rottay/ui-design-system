...
/**
 * We normalize table row keys to strings inside this preset.
 *
 * The public API still accepts React `Key`, but the internal grid logic only
 * needs a serializable identifier. Constraining the local state avoids the
 * `symbol` branch from React.Key leaking into selection arrays and breaking
 * state updates.
 */
type RowKey = string;
...
export const ProfessionalDataTable = createPreset<ProfessionalDataTableProps & Record<string, unknown>>({
  ...
  render: ({ primitives, props, tokens, engine }: PresetContext<ProfessionalDataTableProps>) => {
    ...
    const defaultExpandedRowKeys = Array.isArray(rawDefaultExpandedRowKeys)
      ? rawDefaultExpandedRowKeys.map((key) => String(key))
      : [];
    ...
    // Internal State
    // ========================================================================
    const [expandedKeys, setExpandedKeys] = useState<Set<RowKey>>(new Set(defaultExpandedRowKeys));
    const [sortState, setSortState] = useState<{ key: string; direction: 'asc' | 'desc' } | null>(null);
    const [searchQuery, setSearchQuery] = useState('');
    const [internalFilter, setInternalFilter] = useState('all');
    const [internalSelectedKeys, setInternalSelectedKeys] = useState<RowKey[]>([]);
    ...
    const selectedKeys = controlledSelectedKeys
      ? controlledSelectedKeys.map((key) => String(key))
      : internalSelectedKeys;

    // ========================================================================
    // Helpers
    // ========================================================================
    const getRowKey = useCallback((record: Record<string, unknown>, index: number): RowKey => {
    ...
    const handleSelectionChange = useCallback((keys: RowKey[], rows: unknown[]) => {
    ...
    const toggleExpand = useCallback((key: RowKey, record: unknown) => {
    ...
    const toggleSelectRow = useCallback((key: RowKey, record: unknown) => {
