...
export const stackedPreset = createPreset<ToastManagerProps>((context: PresetContext<ToastManagerProps>) => {
    ...
    <Box className={className} style={{ ...getPositionStyles(), ...style }}>
      {toasts.map((toast) => (
        <Box
          key={toast.id}
          style={{
            ...createSurfaceStyle(tokens, { elevation: 'lg' }),
            borderRadius: tokens.borderRadius.lg,
            padding: tokens.spacing[4],
            display: 'flex',
            gap: tokens.spacing[3],
            position: 'relative',
            paddingLeft: `calc(${tokens.spacing[4]} + 4px)`,
            borderLeft: `4px solid ${getTypeColor(toast.type)}`,
          ...
          <Box style={{ flex: 1, minWidth: 0 }}>
            <Box
              style={{
                ...
                marginBottom: toast.description ? tokens.spacing[1] : 0,
              }}
            >
              {toast.title}
            </Box>
