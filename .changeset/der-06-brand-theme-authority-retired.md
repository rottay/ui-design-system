...
signature .#BrandSwitchChrome — loses labelColor with its only producer; the fourteen readerless --ds-switch-* channels (label-color, label-gap, {sm,md,lg}-{width,height,thumb-size}, radius, transition, thumb-offset) are removed; the seven Classic-read channels stay declared (D6-2d-resto)
signature .#BrandSemanticControlChrome — loses inkMuted, surfaceRaised and brandTintHover with their producers; the --ds-control-ink-muted, --ds-control-surface-raised and --ds-control-brand-tint-hover channels are removed (D6-2d-resto)
signature .#BrandSurfaceChrome — loses shadowHover with its producer; the --ds-surface-shadow-hover channel is removed (D6-2d-resto)
signature .#BrandPalette — loses textPageColor, interactiveBorderColor and interactiveBgMutedColor with their producers; the --ds-color-text-page, --ds-color-interactive-border and --ds-color-interactive-bg-muted channels are removed (D6-2d-resto)
...
signature .#BrandPalette — its primary seed now derives --ds-focus-ring-color through the ramps owner: the nearest ramp stop that clears the 3:1 ring floor against the surface's own ground, a compliant seed resolving to itself (D6-FAM-01); the foundation sheet's declaration remains the answer for a seedless theme
```
