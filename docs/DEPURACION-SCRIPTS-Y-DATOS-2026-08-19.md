...
[`DIAGNOSTICO-Y-PLAN-2026-08-19.md`](DIAGNOSTICO-Y-PLAN-2026-08-19.md); la ley
objetivo está en [`ARCHITECTURE.md`](ARCHITECTURE.md) §1.9–1.10 y §2.9).

Territorio: `packages/core/scripts/` (633 archivos), `packages/core/provenance/`,
`packages/core/tokens/`, los archivos sueltos de `packages/core/`, y el
`manifest/` de Modern Rescue.
...
GAT/CRA). Cae solo `skin-orphan-scope-audit.baseline.json` (con su script).
...
sección v2 es doc vigente), `scripts/quality-evidence-gate.test.mjs` (cae en
...
## 4. `provenance/` — SE QUEDA íntegro (10/10)
...
- `graphics/pack-allowlist.json` + licencias phosphor/thesvg: los exige
  ...
  `provenance/graphics/**`) — son la obligación MIT de redistribución y la
  fuente machine-audited de `THIRD_PARTY_NOTICES.md`.

## 5. El `manifest/` de Modern Rescue — sí es útil; no se mueve ahora; hay un agujero de frescura
...
| `cascade/roots/` (20) + `root-catalog.json` (63 raíces) | **Autorado**: la mecánica de cascada, cruzada fail-closed contra controls por `program-check.mjs:311-328` | PERMANENTE |
...
| `generator.mjs`, `rules.mjs`, `program-check.mjs` | Maquinaria y ley del manifiesto | PERMANENTE (gradúan con él) |
...
diseño: `packages/core/tokens/controls/README.md` (generado, con dos gates
...
family-inventory) **gradúa a `packages/core/manifest/`** y lo efímero (rounds,
...
- `probe/cascade-probe.mjs` + su test (monolito duplicado de la sonda raíz; ya
  adjudicado — NO tocar los otros 5 de `probe/`, son dependencia viva).
- `manifest/cascade/backlog/producers.draft.json` + la sección que lo genera.
- `KIMI-ANNOTATIONS/` (vacío; el protocolo queda como puntero en el contrato).
- `phase-a/ledger-schema.json` (360 KB; la fase A nunca corrió, cero
...
1. `CANARY-MANIFEST.json` — 0 referencias en todo el repo (su función viva la
   cubre `tenant-theme-canary-fixtures.json`, publicado y con gates).
...
5. `packages/core/ARCHITECTURE.md` — se pliega en `docs/ARCHITECTURE.md`
   (anexos de mecánica runtime → `packages/core/docs/`); actualizar el link en
   `README.md:94`.
...
React 18|19 es correcto), `PERFORMANCE_BUDGET.md:56` (presupuesto de
...
toolchain configs, `GETTING_STARTED.md`, `ENGINE_SPLITTING.md`,
