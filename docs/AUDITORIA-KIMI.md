...
ya estaban en el repo — [`MAPA-DEL-REPO.md`](MAPA-DEL-REPO.md),
[`QUE-SE-QUEDA-Y-QUE-SE-BORRA.md`](QUE-SE-QUEDA-Y-QUE-SE-BORRA.md) y
[`VERIFICACION-FABLE.md`](VERIFICACION-FABLE.md) — y sobre el árbol real.
...
| 4 — Monolito de la sonda | DE ACUERDO | `wc -l` exacto: monolito 2729, su test 692, sonda raíz 776. La raíz importa los módulos de `probe/` en `cascade-probe.mjs:75-77,84` (4 directos; `value-eval.mjs` llega vía `leg1-symbolic.mjs:51` — la advertencia del doc se sostiene). El monolito solo importa builtins (`node:fs/path/url`, líneas 117-119). Nadie importa `probe/cascade-probe.mjs` fuera de su propio test, y ningún glob de `test:scripts` alcanza ese test. Dato lateral: la sonda raíz tampoco tiene invocador automatizado (ni gates ni npm scripts la llaman) — no cambia el veredicto, pero "conservarla" es decisión de criterio, no de consumo |
...
- **LOTE 1:** la ruta viva `infrastructure/runtime/foundation/graphics/continuous-runtime-governor/` (5 archivos, citada por `cra-15-runtime-hardening-gate.mjs:47,166`) — solo se borra la ruta homónima sin `foundation/`. Y `KIMI-ANNOTATIONS/inbox/` requiere orden explícita del owner/DT (es el buzón declarado del programa vigente, `tenant-art-direction.json:363`).
- **LOTE 4:** los 5 módulos de `probe/` (el documento ya lo advierte y la advertencia es correcta: `cascade-probe.mjs:75-77,84` de la raíz).
...
`find packages/core/scripts -name '*.test.*'` → 98 archivos. Los pasos cubren: raiz plana (85), `v2/*.test.mjs` (1), 2 explicitos de modern-rescue, `../../scripts/provenance/effect-registry-audit/index.test.mjs`, y vitest con include `scripts/**/*.vitest.test.ts` (`vitest.scripts.config.ts:12`, 1 archivo). **No alcanzados (9)**:

| Test | Por que | Otra via |
|---|---|---|
| `scripts/lib/first-party-roster-source.test.mjs` | subdirectorio, glob plano | solo `gates:ci` (`ci-gates.manifest.mjs:111`) |
| `scripts/lib/modern-framework-layer.test.mjs` | idem | **ninguna** — no lo ejecuta ningun comando |
| `scripts/lib/owner-nesting.test.mjs` | idem | solo `gates:ci` (`:308-311`) |
| `scripts/lib/root-public-resolver.test.mjs` | idem | solo `gates:ci` (`:316-320`) |
| `quality-evidence/programs/modern-rescue/cascade-probe.test.mjs` | ni glob v2 ni explicitos | **ninguna** |
| `.../modern-rescue/manifest/fanout-facts.test.mjs` | idem | **ninguna** |
| `.../modern-rescue/manifest/mirror-parity.test.mjs` | idem | **ninguna** |
| `.../modern-rescue/manifest/root-checklist.test.mjs` | idem | **ninguna** |
| `.../modern-rescue/probe/cascade-probe.test.mjs` | idem; docblock dice ejecucion manual | **ninguna** |

El propio manifest admite el agujero (`ci-gates.manifest.mjs:44-53`) pero su remediacion solo anadio 2 de los 7 tests de modern-rescue. Existen DOS suites `cascade-probe.test.mjs` distintas, ninguna ejecutada.
...
**Duplicacion real detectada en `quality-evidence/` (ver seccion 5): `probe/cascade-probe.mjs` monolitico vs la pata modular — esa si es segunda copia.**
...
| `manifest/families/**` | 255 | Un JSON por familia canonica | README:146-149 |
| `manifest/cascade/{roots,extracted,materialized,backlog}` | 64 | roots autorado; resto generado por `cascade-*.mjs` | `"generator":` en cada JSON |
...
| `probe/cascade-probe.mjs` (95 KB, autocontenido, solo builtins, lineas 117-119) vs orquestador raiz `cascade-probe.mjs` (31 KB) + `probe/leg1-symbolic.mjs` + `css-model.mjs` | **DUPLICACION REAL: segunda implementacion de la misma sonda simbolica** | orquestador importa los modulos (`cascade-probe.mjs:68-84`); el monolito reexporta su propio `loadSheet/evaluate/restEquivalence/derivative` (`probe/cascade-probe.mjs:891-2161`). Solo lo consume su propio test; **ninguna de las dos suites corre en CI ni npm**. Cual es la viva: **NO VERIFICADO** |
| `manifest/controls/*.json` vs `cascade/roots/*.json` (mismos nombres de archivo, 20 vs 20) | Complementarios | diff de `spacing.rhythm.json`: controls posee calibracion, roots posee prescripcion de cascada |

### 6. Archivos de un solo uso ya consumido

| Archivo | Estado | Evidencia | Sigue invocado |
|---|---|---|---|
| `codemod-motion-tokens.mjs` | Aplicado: `engine-token-audit.baseline.json` → `"motion.cubicBezierLiterals": 0`; grep de `cubic-bezier(` sin `var(` en engines/modern → 0 | **No** (0 refs en package.json, workflows, manifest, tests) |
...
Método: lectura completa de `docs/VERIFICACION-FABLE.md` (356 líneas) y `docs/QUE-SE-QUEDA-Y-QUE-SE-BORRA.md` (793 líneas), más verificación propia con `Read`, `Grep`, `cmp`, `comm`, `find` y `git ls-files` contra el árbol actual.
...
|---|---|---|---|
| a | `modern-rescue/probe/` NO se puede borrar entero: la sonda raíz importa sus módulos | `cascade-probe.mjs:75-84` importa `./probe/css-model.mjs`, `css-parse.mjs`, `leg1-symbolic.mjs`, `leg2-chromium.mjs` (leído directo). `value-eval.mjs` entra por vía indirecta: `probe/leg1-symbolic.mjs:51` | **SE SOSTIENE**, con precisión: `QUE-SE-QUEDA` dice que la raíz importa "5 de sus 7 archivos" en sus líneas 76-84; son **4 directos** (75-84) y `value-eval` es transitivo. El veredicto (borrar solo `probe/cascade-probe.mjs` + su test) es correcto. |
...
| e | `scripts/roadmap/commercial-status/index.mjs` es copia declarada, no reimplementación | `head -12` muestra "Copied from ... scripts/roadmap-status.mjs (2026-07-07) ... Byte-identical EXCEPT for exactly TWO functional divergences"; `comm -12` sobre líneas únicas no vacías → **191**; `wc -l` → 3177 / 304 | **SE SOSTIENE** |
| f | `platform.css` y `rottay.css` byte-idénticos | `cmp` → sin diferencias; ambos 126218 líneas / 5342714 bytes | **SE SOSTIENE** |
| g | test-artifacts raíz: 7577 archivos, no 1858 | `find test-artifacts -type f` → **7577**; `git ls-files` → 203; `packages/core/test-artifacts` → 589/589 | **SE SOSTIENE** |
...
|---|---|---|
| `tooling/lane-control/` y `tooling/resolution-probe/` | 0 hits en `package.json` (verificado: no hay script npm que los corra) | `core-structure-audit.mjs:187-193,353-375` les fija estructura y ranks por escrito, y `cascade-extract.mjs:138,502` declara a `lane-control/runtime/tenant-reach` *"el enumerador autoritativo del programa"* |
| la cadena `cascade-extract` → `cascade-materialize` → `cascade-backlog` | ningún pipeline las invoca (0 hits en package.json/CI, confirmado por Fable L408-410) | producen `manifest/cascade/*.json` **versionados en git** (`git ls-files` los lista: `backlog/chrome.anatomy.json`, etc.). Son el generador de evidencia comiteada; borrarlos deja esos JSON sin productor |
...
4. `audit-presets.mjs` y `audit-report.json` en la **raíz**: muertos por el mismo motivo que el LOTE 2 (escanean `./packages/core/src/composition/components/custom`, ruta inexistente — `test -d` → NO-EXISTE) y **ni siquiera aparecen en el mapa** (`grep "audit-" docs/MAPA-DEL-REPO.md` → 0 hits para ambos).
