...
| **K5b** (resto de las 40: 3 de raíz + ~12 graduadas + ~6 baseline + 10 con relación escrita + 3 seeds evnto) | K4 cerrado **+ K-1, K-3, K-6 adjudicadas por escrito** (y K-4 si fue (B)) | `bithire/index.ts`, `evnto/index.ts`, y la entrada "Baja #14" de `baseline/index.json` **después** de regenerar y verificar | `untaggedAuthoredLeaves` **-> 0**; negativo obligatorio; relación-escrita aplicada a **todas** las coincidencias medidas | Un `baseline` sin razón falsable -> PARAR. Régimen aplicado **selectivamente** -> PARAR (F-D: sería maquillaje) |
