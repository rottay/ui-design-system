...
| 5 | **`checkpoint/index.json.blockedOn`** repite exactamente los 3 primeros ítems como condición de cierre | `checkpoint/index.json:4` | checkpoint (autoridad machine-rendered) |
...
F4A-15 = K5  [CHROME.table — 40 hojas]
   ...
   │    write-set: bithire/index.ts, evnto/index.ts, baseline/index.json ("Baja #14")
   │
...
Fuente primaria: `packages/core/manifest/generated/variant-parity.json` (generado por `manifest/variant-parity/index.mjs`, última corrida F4A-13 sobre este HEAD — leí el JSON, no corrí el generador). Cruzado contra `packages/core/manifest/variant-parity/baseline/index.json` y `docs/prompt-codex-continue.md:46-54`.
...
|---|---|---|
| `canonicalFamilies` | **255** (105 primitive + 57 pattern + 18 chart + 39 structure + 36 surface) | `denominators` + `family-inventory/index.json.counts` (cruzado, coincide) |
...
| **`realKeypathParity`** | **NO EXISTE** en el árbol | **generador de F4A-close pendiente** | `grep -rl "realKeypathParity"` sólo encuentra la MENCIÓN en prosa dentro de `baseline/index.json` (`reading.divergentSlots`); no hay script, ni test, ni entrada en `gates-manifest/index.mjs`. Escribirlo (script + test + wiring en `scripts/ci/gates-manifest/index.mjs`) es trabajo de implementación de F4A-close, no algo que exista para correr. |
...
|---|---|---|---|
| **Ratchet a cero** | `variant-parity.json` (generado), `baseline/index.json` | `packages/core/manifest/variant-parity/baseline/index.json` (edición a mano, "Baja #N", ley decrease-only ya establecida — mismo patrón que las 13 bajas anteriores) | patrón F4A-2..13, ley en `baseline/index.json:3` |
