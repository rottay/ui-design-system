...
- **`baseline/index.json` como destino de "Baja #14"**: LEGÍTIMO — es la autoridad manual decrease-only con 13 Bajas precedentes escritas a mano por el DT tras regenerar (no es un `manifest/generated/*`); el protocolo "después de regenerar y verificar" del paquete es el correcto. K5c no se mezcla con K5a/K5b en ninguna fila ✓.
