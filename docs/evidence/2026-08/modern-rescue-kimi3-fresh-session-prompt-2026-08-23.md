...
4. **C1 — drift de autoridad DT**: la sucesión owner 2026-08-23 (Codex → Kimi K3 como DT) no está asentada en NINGUNA autoridad. Parchear con procedimiento T-1 (program-check + program-check.test antes y después): `AGENTS.md:36,40-56`, `CLAUDE.md:14-16`, `agent-orchestration.json` (coordinator.model + 3er record de sucesión), `program-check.mjs` (`LIVE_COORDINATOR`, cadena 2→3 records), README del programa (:194,254,308,352-384), `tenant-art-direction.json:7,9,367`, `rounds.json` (`codex-audit`/`Codex GO`/`IMPLEMENTED_PENDING_CODEX_AUDIT` → ley neutra del asiento DT), `checkpoint.intent.json` (lane authority + currentWave stale: F4A ya cerró), y el fence `composition/receipt/tests/index.test.mjs:197` (comparación rota `SIGHTED_APPROVER`). Lista exacta en ROADMAP_PATCH_REQUIRED §6 de la re-auditoría.
