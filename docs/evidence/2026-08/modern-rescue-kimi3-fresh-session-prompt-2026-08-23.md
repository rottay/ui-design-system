...
Sos el DT entrante del programa Modern Rescue (WO-CRA-23) en el repo `/Users/daniel/Developer/Rottay/ui-design-system`. Este prompt es autocontenido: no asumas contexto de sesiones previas.
...
- Autoridad canónica: `docs/ROADMAP-EJECUCION-2026-08-19.md`. Constitución: `packages/core/scripts/quality-evidence/programs/modern-rescue/` (README.md, program/index.json, customization-model/index.json, orchestration/index.json) + `AGENTS.md` raíz. Verificación: `node packages/core/scripts/quality-evidence/programs/modern-rescue/program-check.mjs` (hoy **BLOCKED**, ver abajo).
...
- F0/F0.5/F1 100%, F2 seguro 100%, F4A 14/14 cerrado (doblemente auditado, evidencia durable en `docs/f4a/` + 5 docs de cierre sólo en /tmp — persistirlos es C5).
...
3. **C4 — regularizar el desbloqueo de F4B**: los 14 commits que drenaron `unknownProvenance` 2.024→0 (`8d2985638`…`6cfdcc1a9`, 08-22) no tienen asiento en roadmap, ni ruling de apertura de Lote B, ni la doble postaudit que la condición vinculante (roadmap:1142-1146) exigía; tocaron 2 archivos de `src/` fuera del write-set declarado. Producir la doble postaudit retroactiva y asentar, o declarar la brecha y su remedación.
4. **C1 — drift de autoridad DT**: la sucesión owner 2026-08-23 (Codex → Kimi K3 como DT) no está asentada en NINGUNA autoridad. Parchear con procedimiento T-1 (program-check + program-check.test antes y después): `AGENTS.md:36,40-56`, `CLAUDE.md:14-16`, `orchestration/index.json` (coordinator.model + 3er record de sucesión), `program-check.mjs` (`LIVE_COORDINATOR`, cadena 2→3 records), README del programa (:194,254,308,352-384), `art-direction/index.json:7,9,367`, `rounds/index.json` (`codex-audit`/`Codex GO`/`IMPLEMENTED_PENDING_CODEX_AUDIT` → ley neutra del asiento DT), `checkpoint/index.json` (lane authority + currentWave stale: F4A ya cerró), y el fence `composition/receipt/tests/index.test.mjs:197` (comparación rota `SIGHTED_APPROVER`). Lista exacta en ROADMAP_PATCH_REQUIRED §6 de la re-auditoría.
5. **C5 — persistir evidencia de cierre**: los 5 docs F4A-close y los 4 de experience.profile viven sólo en /tmp con SHA asentado en roadmap; copiarlos a `docs/f4a/` (o equivalente) con los SHA ya registrados.
...
- Vos: DT. Decidís y coordinás; no escribís source delegado ni delegás authority docs a workers. No reabras F4A/PRE_F4B ni auditorías generales sin regresión concreta.
