...
`git log 652cf285e..HEAD -- customization-model.json` = vacío (cero toques en
...
### 7. Paso C — baseline 29→4 y M1 sobre manifest/, verificados
...
hallazgo F4). M1 cubre `packages/core/manifest/` con la división correcta
...
`manifest/cascade/roots/density.mode.json` (no admitida por sus enumValues) →
...
`scripts/ci/runner/index.test.mjs:21` resuelve `run-ci-gates.mjs` en su
...
(a) 5 archivos citan `manifest/generator.mjs:171-206` como instrucción
...
(b) `manifest/rules/index.mjs:231` dice «solo tres raices llevan
...
- Los 3 artefactos de `manifest/generated/` siguen stale por adjudicación
