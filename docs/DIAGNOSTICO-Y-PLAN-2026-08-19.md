...
Estado de la cascada (verificado hoy contra `manifest/` del programa):
...
- Borrados de riesgo cero ya adjudicados (lotes 1, 2, 3 con su paso de barrel +
  ...
  `probe/cascade-probe.mjs` + su test, los 2 agentes `.claude/` que describen
...
- `packages/core/ARCHITECTURE.md` se pliega: sus anexos mecánicos (engine
  factory, merge chains, modelo de surfaces, capas CSS) se mudan a
  `packages/core/docs/` y el archivo desaparece.
...
único); lápidas `collapse-paint.css` y `patterns-paint.css`; barriles
...
- Los 8 componentes que una lectura de solo-liveness marcó huérfanos
  ...
  probes del showroom.
- `scripts/codemods/*.mjs` (se corren a mano en los consumidores, por diseño).
...
> `docs/DIAGNOSTICO-Y-PLAN-2026-08-19.md` (este plan). Verifícalos contra el
