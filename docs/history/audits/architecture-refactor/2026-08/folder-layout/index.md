...
- Eslabones que PUEDEN pudrirse: enumerados en H4/H5; los ya declarados
  (trío de governance/manifest/projections → F2, evidencia browser cra-15 → F1, receipts
...
### 5. Manifest graduado (`packages/core/governance/manifest/`) — refleja el censo final
...
- 255 archivos en `families/` y 255 familias en `index.json` (20 controles ×
  ...
  (`packages/core/scripts/foundation/tokens/manifest/generation.mjs`); 0 referencias al path viejo dentro
  de `index.json`.
...
- `family-inventory/index.json` NO migró (correcto: quedó en el programa; el lector
...
- **phase-a no reescrito**: defendible y verificado — cero consumidores vivos
  de `phase-a` fuera de sí mismo (grep sobre `scripts/`, `governance/manifest/`, `src/`,
...
- **3 artefactos stale a F2**: defendible — no están cableados a CI (rot
  declarado) pero sus tests SÍ corren en la suite (`governance/manifest/**` entró al
...
- `quality-evidence/` en todo el rango del Paso B (7346f6028..HEAD),
  ...
  contratos de G2 (`program/index.json`, `rounds/index.json`, `orchestration/index.json`,
  autorización expresa en el commit) y `program-check.*`/`program/index.json` de la
...
`scripts/check/tokens/customization/visual-worklist/` — exactamente la clase que G2 movió
...
- `packages/core/governance/manifest/controls/spacing/rhythm/index.json:122,127` — campos
  `evidence` citan `packages/core/scripts/spacing-rhythm-contract-gate.mjs`
  (hoy `scripts/check/tokens/contracts/spacing-rhythm/index.mjs`). Está en el
  árbol GRADUADO, no en phase-a.
- `roadmap/registry.json` (~10 campos `measure`, líneas 16-79) — «node
  scripts/check/engine-token-audit.mjs (…)» plano.
- `quality-evidence/programs/modern-rescue/probe/index.mjs:14` — cita
...
### H8 — INFO: el doble hueco de `governance/manifest/` es consistente y está declarado
...
(:82), y el `scripts-tree-gate` tampoco cubre `packages/core/governance/manifest/`
...
1. **¿Violaciones de §1.2/§2.9 que los gates no vean?** Sí, tres clases,
   ...
   de R5 (H1, con 3 casos vivos en lib), el árbol `governance/manifest/` sin ley (H8,
...
- No audité `docs/history/inventories/repository-map/2026-08/index.md` ni la totalidad de las citas F9 en docs;
