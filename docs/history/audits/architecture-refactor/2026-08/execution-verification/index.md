...
Ejecución del Anexo de [`docs/history/programs/architecture-refactor/2026-08/diagnosis/index.md`](/docs/history/programs/architecture-refactor/2026-08/diagnosis/index.md):
...
grep -n 'commercial' vite.config.ts contracts/package-api/public-entrypoints/index.json → (vacío)
...
| `quality-evidence` v1 | **PARCIAL** | Correcto que está vivo vía `quality-evidence:check` y `quality-evidence-gate.test.mjs`. Falta un consumidor: `packages/core/artifacts/quality/programs/modern-rescue/checkpoints/truth-instrumentation-baseline/2026-08-05/manifest.json` referencia `quality-evidence/cli.mjs` — evidencia trackeada que quedaría apuntando a la nada |
...
| «7 scripts vivos de la raíz» | `scripts/check/dependencies/index.mjs` + su test, `scripts/check/effects/index.mjs` + su test, `scripts/maintain/roadmap/status/index.mjs` + su test, `scripts/maintain/roadmap/commercial-status/index.mjs` = **7**, todos de julio. CONFIRMADO |
| Monolito `probe/index.mjs` + test | Hay **dos** `index.mjs`. El vivo es el de la raíz del programa (31.712 B), que importa `./probe/index.mjs` y `./probe/index.mjs`. El monolito es `probe/index.mjs` (**94.953 B**), con **0 importadores**; su test (28.154 B) sólo lo menciona en un comentario de uso. CONFIRMADO |
| «5 módulos vivos de la sonda» (§4) | `index.mjs`, `index.mjs`, `index.mjs`, `index.mjs`, `index.mjs` = **5**. CONFIRMADO |
...
packages/core/scripts/check/evidence` → **0 archivos**. Las 802 se drenaron.
...
- «Adjudicación de exposición por raíz … falta la adjudicación»: **REFUTADO.**
  `governance/manifest/cascade/catalog/index.json` ya trae `exposure` en las **63/63**
...
- Migración `targetBinding → internalChannels`: **CONFIRMADO exacto.** 255
  ...
  **1462** (28,7 % ≈ el 29 % heredado); restantes **3638**, la cifra exacta del
  ...
  → vacío), mientras `internalChannels` lo leen `index.mjs`,
  `scripts/foundation/tokens/manifest/generation.mjs` y `scripts/foundation/tokens/scripts/foundation/tokens/manifest/rules.mjs`. Las 3638 celdas no están
...
- `daisy.classConsumers: 0` en `engine-baseline/index.json`.
  **CONFIRMADO.**
...
| 4 | 20 diales (STANDARD 13 / PRO 7 / EXPERT allowlist / INTERNAL 1) | PARCIAL | 20 controles: **13 `tier: standard` + 7 `tier: pro`**. EXPERT existe como capa (`customization-model/index.json.expert`: allowlist de **294**, máx. 200 overrides/documento) pero no como tier de control. **«INTERNAL 1» no existe**: ningún control tiene `tier: internal` |
...
| 9 | 19,3 % de intersección entre los tres temas | CONFIRMADO (dos métodos) | Grep propio sobre `facade/artifacts/*/index.css`: rottay **1237**, bithire **1282**, evnto **430**; unión **1908**, intersección **369** → **19,34 %**. Idéntico a `governance/manifest/projections/mirror-parity.json` |
...
scripts/check/dependencies/index.mjs:3210        exige el alias dist/platform.css
packages/core/docs/quality/performance-budget/index.md:56                     presupuesto para dist/platform.css
