...
Segunda ronda. La primera ([`REAUDITORIA-FABLE-2026-08-19.md`](/docs/history/audits/architecture-refactor/2026-08/execution-verification/index.md))
auditó el diagnóstico; esta audita **la ejecución antes de que ocurra**: el
[`ROADMAP-EJECUCION-2026-08-19.md`](/docs/history/programs/architecture-refactor/2026-08/execution/index.md) contra el
árbol real, con la ley de [`ARCHITECTURE.md`](ARCHITECTURE.md) y la evidencia de
zona de [`DEPURACION-SCRIPTS-Y-DATOS-2026-08-19.md`](/docs/history/programs/architecture-refactor/2026-08/scripts-and-data/index.md)
y [`CONFORMIDAD-SRC-2026-08-19.md`](/docs/history/audits/architecture-refactor/2026-08/source-conformance/index.md).
...
2. **El helper es inaccesible para 11 de los 146 archivos** (7 de `scripts/`
   ...
   bajo `packages/core/scripts/libraries/`; el árbol §2.1 del `scripts/` de raíz no
...
- **Precondición «commitear el trabajo en vuelo» — YA SATISFECHA.**
  ...
  (34 archivos, 20 bajo `governance/manifest/` — el plan decía 39-40).
...
- **«44 archivos referenciantes (21 externos, 23 internos)» — PARCIAL: hoy son
  ...
  **38** = 15 internos (bajo `governance/manifest/`) + 3 internos-programa + **20
  externos**. Los «23 internos» solo cierran sumando referencias por join
  relativo sin el literal (`index.mjs` tiene **9** usos de
  `join(PROGRAM_DIR,'governance/manifest/…')` invisibles al grep). El «27 fuera del
...
- **Cirugía de `..` (generator 7→3, fanout/checklist/parity 5→1) —
  CONFIRMADO**, con un coste extra: los cuatro viven dentro de `governance/manifest/` y se
...
- **«Regeneración de index.json + generated/* + checkpoint» — PARCIAL.**
  `governance/manifest/index.json` y `governance/manifest/projections/` (3 JSON, 22 MB) existen y sí
  ...
  **`checkpoint/index.json` no tiene productor** (lo lee `index.mjs`;
  ningún script lo genera) y vive en la raíz del programa, fuera de
  `governance/manifest/` — «regenerar el checkpoint» no es ejecutable tal como está
  escrito.
- **«git grep "modern-rescue/manifest" → 0» — INALCANZABLE como está
  ...
  aun así es **insuficiente**, porque las 9 referencias de `index.mjs`
...
- **«Los 4 tests que quedan fuera del glob `scripts/**`» — CONFIRMADO
  ...
  (`.test.mjs` bajo `governance/manifest/`). `generator.test.mjs` está además cableado por
...
- **Discrepancia de alcance no adjudicada:** DEPURACION §5 gradúa la
  «constitución» completa (incluye `index.mjs`,
  `customization-model/index.json`, `family-inventory/index.json`, que viven **fuera** de
  `governance/manifest/`); el roadmap solo mueve `governance/manifest/`. Con el roadmap,
  program-check queda en el programa apuntando a `packages/core/governance/manifest/` —
...
- **JSON publicados a `contracts/` — cierto pero con coste cross-repo no
  ...
  los bytes de `src/tooling/supplier-honesty/public/cli/index.mjs`, que se distribuye a las apps
...
| Monolito `probe/index.mjs` + test | **CONFIRMADO** | 0 importadores; su test solo importa el monolito + builtins; los otros 5 módulos de `probe/` están vivos (los importa el probe raíz `:75-84` y su test `:26`) |
...
- **Los 2 enums vacíos — CONFIRMADO exacto**, con un hallazgo que cambia el
  ...
  `governance/manifest/cascade/roots/*.json` campo `variants` (`chrome.anatomy` 14
...
- **El contador del ratchet NO existe hoy** — es obra nueva. Ningún script
  ...
  paréntesis). Ancla natural: clave nueva en `engine-baseline/index.json`
...
- **Lo que ningún frente lista del lado core**: cada retirado tiene **skin CSS
  ...
  (`governance/manifest/families/...json`) — la ley del CLAUDE.md exige retiro atómico en
...
- **Gates de frontera que F6 no nombra:** `pack-inventory.baseline.json`
  ...
  propio `contracts/package-api/public-entrypoints/index.json` es artefacto a reescribir
...
- **«Extirpar platform completo» — la lista del plan es corta.** Referencias
  ...
  `packages/core/docs/history/programs/premium-styling/` (7 líneas que declaran `./styles/platform`
...
- Barril raíz: 15 `export *` + nombrados en `src/index.ts` (333 líneas); el
  gate que lo vigila es `cra14:check` (no existe `scripts/entrypoints/packaging/`).
...
- **`codemods/` no existe donde §1 lo exige** (raíz del repo). Existe
  `packages/core/scripts/tooling/maintenance/codemods/` con `sizetype-to-size` y
...
9. **checkpoint/index.json no tiene productor** y vive fuera de `governance/manifest/`
   (§2.4).
