...
> **Instantánea histórica, no navegación vigente.** Este inventario describe el
> árbol del commit y la fecha indicados abajo. La arquitectura actual y sus
> nombres canónicos viven en `docs/ARCHITECTURE.md` y en
> `packages/core/docs/index.md`; no use los paths de esta instantánea como
...
| `components/` | 41 | 9 | 0 |
| `graphics/` + `entrypoints/` + soporte reclasificado | 22 | 0 | 0 |
...
| `packages/core/artifacts/quality/` | 589 | 589 | 49 MB |
...
`infrastructure/`, 422 en `graphics/`, `entrypoints/` y soporte después
reclasificado, 295 en `components/`, 96 en la raíz y el showroom.
...
3. **Dos sondas de cascada con el mismo nombre.** `modern-rescue/index.mjs`
   (776 líneas, en inglés) y `modern-rescue/probe/index.mjs` (2.729 líneas,
...
9. **Dos motores de roadmap.** `scripts/maintain/roadmap/status/index.mjs` (3.177 líneas) y
   `scripts/maintain/roadmap/commercial-status/index.mjs` (304). No se importan entre sí, pero sí comparten
...
10. **Arquitectura local unificada.** `docs/ARCHITECTURE.md` es la única
    autoridad de este repositorio. El antiguo `packages/core/ARCHITECTURE.md`
    fue absorbido y sus anexos operativos viven, clasificados, bajo
    `packages/core/docs/architecture/`. La documentación de ecosistema del repo
    hermano sigue siendo complementaria, no un segundo canon local.
11. **`styles/platform.css` y `styles/rottay.css` son byte-idénticos.** `platform`
    ya no existe como vertical del roster, y el gate `retired-identity`
    lo contradice.
...
- `graphics/motion/.../animation-lease/` — 1.
...
- `scripts/maintain/roadmap/status/index.mjs` — (1 archivo, 3.177 líneas) motor de estado del roadmap principal; único camino legal para cambiar el estado de un WO (`pnpm roadmap:status` / `roadmap:check`). VIVO.
- `scripts/maintain/roadmap/status/index.test.mjs` — (1 archivo, 82 KB) tests del motor anterior, `pnpm roadmap:test`. VIVO.
- `scripts/maintain/roadmap/commercial-status/index.mjs` — (1 archivo, 304 líneas) el mismo trabajo que `roadmap/status/index.mjs` pero para el programa comercial aislado; `pnpm roadmap:commercial`. **[DUPLICA]** a `scripts/maintain/roadmap/status/index.mjs`: son dos implementaciones separadas del mismo modelo de estado (registry.json + STATUS.md generado). No se importan entre sí, pero no es reimplementación: la cabecera del comercial declara "Copied from scripts/roadmap-status.mjs (2026-07-07) ... Byte-identical EXCEPT for exactly TWO functional divergences" y `comm -12` da 191 líneas idénticas. Es una copia divergida. La duplicación es una decisión explícita del dueño (aislamiento 2026-07-07), no un accidente, pero es duplicación de maquinaria igual.
- `scripts/check/dependencies/index.mjs` + `index.test.mjs` — (2 archivos, 259 KB) el gate de honestidad de dependencias; `pnpm dependency:honesty`, referenciado por `.github/workflows/ci.yml`. VIVO y es el archivo más grande de la carpeta.
- `scripts/check/effects/index.mjs` + `index.test.mjs` — (2 archivos) audita la procedencia de los efectos contra `packages/core/governance/effects`; `pnpm effects:provenance`. VIVO. Importa `CI_GATES` desde `packages/core/scripts/check/automation/ci-gates.manifest/index.mjs`.
- `scripts/add-accent-bars.mjs`, `add-hover-transforms.mjs`, `add-style-memo.mjs`, `adopt-card-style.mjs`, `adopt-helpers.mjs`, `audit-helper-gaps.mjs`, `fix-focus-rings.mjs`, `fix-fontsize.mjs`, `fix-glass-adoption.mjs`, `fix-hardcoded-borders.mjs`, `fix-hover-transforms-v2.mjs`, `fix-null-array-guards.mjs`, `fix-rgba-overlays.mjs`, `fix-shadow-helpers.mjs`, `fix-transition-tokens.mjs`, `fix-usememo-v2.mjs`, `helper-gaps-report.json` — (17 archivos, 17 en git, ~110 KB) **[SIN CONSUMIDOR]** codemods de una sola pasada de las fases P0..P3E. Los 16 scripts escriben sobre `packages/core/src/components/custom/`, un subárbol inexistente dentro de la raíz vigente `components/`. Ninguno aparece en `package.json`, en `.github/workflows/ci.yml` ni en ningún `.md`; la única referencia cruzada es entre ellos mismos. Su último commit es del 2026-02-08 ("complete engine-awareness improvements across all 337 presets"). `helper-gaps-report.json` es la salida congelada de `audit-helper-gaps.mjs` y apunta a los mismos paths inexistentes.

Sobre la comparación pedida `scripts/` vs `packages/core/scripts/` (632 archivos): **NO hay duplicación por nombre** — `comm -12` sobre las dos listas devuelve cero coincidencias. La división real es: la raíz guarda lo que cruza paquetes (roadmap, dependency-honesty, effect-registry, que necesitan ver `packages/core` y `packages/showroom` a la vez) y `packages/core/scripts/` guarda los ~600 gates internos del paquete core. La única grieta es que `packages/core/scripts/` contiene también gates que se invocan desde la raíz (`cra12:check` llama a `packages/core/scripts/check/evidence/certification/contracts/motion/index.mjs` directamente por ruta), así que la frontera "cross-package vs interno" no se respeta al 100%.
...
`docs/ARCHITECTURE.md` ya no está huérfano: lo enlazan el README raíz,
`CLAUDE.md`, el README del paquete y el hub `packages/core/docs/index.md`. La
documentación del paquete quedó clasificada en arquitectura operativa, guías,
calidad, referencias, generados e historia; ya no compite con el canon.
...
- `roadmap/README.md` — (31 KB) la ley del proceso: orden de arranque, protocolo de handoff, "diecisiete reglas aprendidas a golpes", y la regla de que el estado solo cambia vía `scripts/maintain/roadmap/status/index.mjs`.
...
Al buscar quién escribe en `test-artifacts/`, la ruta más citada del repo (247 menciones, en `packages/core/scripts/check/modern-rescue/` y en `channel-liveness-gate.mjs`) es **`test-artifacts/quality-evidence/`, que no existe en disco**. Lo mismo con `test-artifacts/advisory/` (2 menciones). Son destinos del programa Modern Rescue que se crean al vuelo cuando se corre el gate; hoy el árbol está vacío de ellos, es decir que **el volcado que la ley del programa activo considera canónico no está presente**, mientras sí quedan 479 MB de volcados de programas cerrados en julio.
...
**[DUPLICA]**
- `scripts/maintain/roadmap/commercial-status/index.mjs` vs `scripts/maintain/roadmap/status/index.mjs` (copia declarada y divergida, 191 líneas idénticas; 304 vs 3.177 líneas).
  ...
  `foundation/contracts/` — (1 archivo) barril público de tipos de sistema: reúne en un solo import los contratos de motor, tema, tenant, tokens, extensiones, perfiles de producto, verticales, errores y patrones; deliberadamente no reexporta props de primitivas (esas viven junto a su componente).
    ...
    `foundation/contracts/runtime/` — (0 archivos) nivel de agrupación sin código propio; separa los contratos que describen comportamiento en ejecución (motor, movimiento, efectos, errores, datos de patrones) de los atómicos de `kernel/`.
      `foundation/contracts/runtime/components/` — (1 archivo, 3 líneas) barril que expone los contratos de datos de los componentes de tarea sin arrastrar ninguna implementación renderizada.
        `foundation/contracts/runtime/components/patterns/` — (1 archivo) reúne los cinco grupos de formas de datos que comparten los patrones de `components/patterns` y las superficies que los componen, de modo que la capa de contratos nunca importe código que pinta.
  ...
  `foundation/tokens/` — (6 archivos) raíz del sistema de tokens: su `index.ts` reexporta el fachada TS y documenta que hay dos formatos paralelos del mismo juego de variables `--ds-*` (los CSS que definen valores y los objetos TS que dan handles tipados); junto a él viven cuatro ledgers JSON de adjudicación (`prototype-ledger.json` + su schema, `premium-dead-adjudication.json`, `reads-adjudication.json`, `residual-adjudication.json`) donde se registra, token por token, si un canal tiene escritor/lector real, si se promueve, se reconecta o se propone retirar.
    ...
    `foundation/tokens/css/` — (0) nivel de agrupación que separa el formato CSS (las variables reales que pinta el navegador) del espejo TypeScript de `ts/`; se ordena internamente por rol de dependencia: `foundation/` (valores base) → `runtime/` (motores y personalidad) → `presentation/` (canales por componente y skins) → `facade/` (los entrypoints públicos y los artifacts compilados).
      ...
      `foundation/tokens/css/runtime/` — (1) `personality.css`, la única proyección de los tokens de personalidad que publica `SystemCssVariablesBridge` hacia los canales canónicos de componente (sombras, scrollbar, `::selection`, blur de overlays) cruzando los tres contratos de DOM de motor; se importa en la capa `rottay-personality`.
        `foundation/tokens/css/runtime/bridges/` — el puente `collapse/index.css` traduce los `--ds-collapse-*` del wrapper `.ds-collapse` a las clases internas `.ant-collapse-*` del motor classic, en la capa `rottay-components`. El duplicado de pintura sin consumidores fue retirado.
  ...
  `infrastructure/compilers/` — (1 archivo) Barril raíz de toda la maquinaria que convierte la configuración visual de una marca o de un inquilino en variables CSS y tokens; solo reexporta lo que `facade/` declara público, y lo importan `src/index.ts` y dos gates de `scripts/`.
    ...
    `infrastructure/runtime/` — (1 archivo) agrupa todo el motor interno del design system que corre en tiempo de ejecución (arranque, engines, theming, tenant, personalidad, features, responsive, motion, i18n, gráficos, adaptadores y hooks de aplicación); su `index.ts` reexporta esos módulos para que el proveedor los orqueste, y no está pensado para que las apps lo importen directo.
      `infrastructure/runtime/adapters/` — (0) carpeta contenedora de los puntos de inyección con los que la app le presta al design system piezas que el design system decide no tener (por ejemplo su router), sin archivos propios.
        `infrastructure/runtime/adapters/presentation/` — (0) nivel intermedio que indica que estos adaptadores son de capa de presentación (React), sin archivos propios.
          `infrastructure/runtime/adapters/presentation/react/` — (0) nivel intermedio que agrupa los tres adaptadores concretos de React, sin archivos propios.
            ...
            `infrastructure/runtime/adapters/presentation/react/focus-mode/` — (1) contexto React donde la app publica un booleano de "modo concentración"; el hook `useDsFocusMode` deja que un componente del design system se esconda cuando la app activa ese modo, y lo usa `components/structures/dashboard/data-terminal-card`.
      ...
      `infrastructure/runtime/application/` — (0) carpeta contenedora de los hooks de React de nivel "aplicación": utilidades genéricas y sin semántica de negocio (datos, formularios, atajos, estado, búsqueda, accesibilidad, IA, voz) que el paquete exporta para que las apps las cableen; sin archivos propios.
        `infrastructure/runtime/application/accessibility/` — (1) hooks de accesibilidad: `useKeyboardNavigation` (mover el foco con flechas en listas, menús y grillas), `useRovingTabindex` (el patrón WAI-ARIA de un solo tab-stop por grupo) y `useAriaAnnounce` (anunciar cambios al lector de pantalla con una región aria-live). Lo consume, entre otros, `components/patterns/data/gallery-view`.
          ...
          `infrastructure/runtime/application/automation/voice/` — (0) nivel contenedor del dictado por voz; sin archivos propios.
            `infrastructure/runtime/application/automation/voice/composition/` — (0) nivel intermedio que marca que lo de abajo es código de composición React; sin archivos propios.
              `infrastructure/runtime/application/automation/voice/composition/react/` — (0) nivel intermedio de React para el dictado por voz; sin archivos propios.
                `infrastructure/runtime/application/automation/voice/composition/react/input/` — (1) el hook `useVoiceInput`, que envuelve la Web Speech API del navegador: pide permiso al micrófono, expone estados (no soportado, escuchando, transcribiendo, error) y devuelve la transcripción; lo usan `components/primitives/inputs/VoiceInputButton` y `components/structures/workspace/search-command-bar`.
        `infrastructure/runtime/application/commands/` — (1) barril público del registro de comandos: reexporta el proveedor y los hooks de `runtime/registry`. Lo importan `components/structures/workspace/search-command-bar` y `connected-command-palette`.
        ...
        `infrastructure/runtime/application/forms/` — (1) barril de los cuatro hooks de formulario (autoguardado, borrador, diferencias y guardia de cambios sin guardar); lo importan las páginas de formulario de `components/surfaces/presentation/pages/forms/`.
        ...
        `infrastructure/runtime/application/state/` — (1) barril de los tres hooks de estado avanzado (deshacer/rehacer, preferencias de disposición y sincronización entre pestañas); fuera de la carpeta lo importan la fachada pública y `components/patterns/navigation/command-palette`.
      ...
      `infrastructure/runtime/foundation/` — (0 archivos) carpeta agrupadora sin archivos propios: junta las piezas de runtime de más abajo (las que no dependen de nadie dentro de runtime) para que los runtimes y fachadas de arriba puedan usarlas sin depender entre sí.
        ...
        `infrastructure/runtime/foundation/recipes/` — (0) carpeta agrupadora sin archivos propios: reúne el sistema de "recetas", que traduce elecciones semánticas (variante, tamaño, forma) en los nombres de clase estables sobre los que pinta la piel visual.
          ...
          `infrastructure/runtime/foundation/recipes/governance/manifest/` — (1) arma, leyendo las definiciones vivas, un catálogo legible por máquina de lo que una aplicación puede personalizar: slots por familia, ejes con sus valores admitidos, perfiles disponibles y prefijos de variables CSS.
            `infrastructure/runtime/foundation/recipes/governance/manifest/tests/` — (1) pruebas de que el catálogo se deriva de las definiciones reales y no de una lista escrita a mano.
...
## Mapa de `packages/core/src/components/`

Repo: `/Users/daniel/Developer/Rottay/ui-design-system`
Fecha: 2026-08-18. Solo lectura, nada del repo fue modificado.

Totales medidos: **1737 carpetas**, **2268 archivos** bajo `components/`.
...
`patterns/commercial/` **ya no existe** en el árbol. El `CLAUDE.md` la declara como deuda de migración con 11 owners, pero hoy no hay ninguna carpeta llamada `commercial` bajo `packages/core/src` y `grep -ra "patterns/commercial"` sobre `components/` no devuelve nada. Los componentes de vocabulario monocromo/marketing que vivían ahí quedaron reclasificados por lo que hacen y hoy están repartidos así (todos existen y todos tienen consumidor):
...
`components/index.ts` — (1 archivo) barril que junta los cuatro tiers. El comentario aclara que `@rottay/design-system/components/...` no es subpath público: la carpeta describe la arquitectura de fuente, no la API publicada.

---

### `components/primitives/` — (1313 archivos) las piezas sueltas de UI: cada una es una hoja con tres pinturas por engine. Es el tier más grande del sistema.

`components/primitives/display/` — (288) mostrar contenido ya resuelto: texto, imagen, datos, contenedores de lectura. Componentes: Avatar, Badge, Calendar, Callout, Card, Carousel, CodeBlock, CropMarks, Descriptions, Empty, Image, Kbd, List, MarkdownView, QRCode, Statistic, Table, Tag, TextureBackdrop, Timeline, Tooltip, Tree, Typewriter, Typography.
...
`components/primitives/inputs/` — (342) capturar datos del usuario. Componentes: AutoComplete, Button, Cascader, Checkbox, ColorPicker, DatePicker, Form, FormField, Input, InputNumber, Mentions, OTPInput, PasswordInput, Radio, Select, Slider, Switch, TagInput, Textarea, TimePicker, Toggle, Transfer, TreeSelect, Upload, VoiceInputButton.
...
`components/primitives/layout/` — (166) repartir el espacio en la pantalla. Componentes: AsciiFrame, AspectRatio, Box, Collapse, Container, Divider, Flex, Grid, InvertSection, Layout, ScrollArea, SemanticSurface, Space, Splitter, Stack.
...
`components/primitives/navigation/` — (163) moverse dentro del producto. Componentes: Affix, Anchor, BackTop, Breadcrumb, FloatButton, Link (publicado como NavLink), Menu, Pagination, Segmented, Stepper, Steps.
...
`components/primitives/feedback/` — (182) decirle al usuario qué pasó o que espere. Componentes: Alert, Drawer, Message, Modal, Notification, Progress, Rate, Result, Skeleton, Spinner, Toast.
...
`components/primitives/overlay/` — (126) capas flotantes por encima de la página. Componentes: AlertDialog, ConfirmDialog, ContextMenu, Dropdown, HoverCard, Popconfirm, Popover, Sheet, Tour, Watermark.
...
`components/primitives/foundation/` — (23) piezas mínimas compartidas por varias familias, no catálogo público de página.
...
`components/primitives/runtime/` — (20) los motores de comportamiento compartidos. No pintan nada.
...
`components/primitives/facade/` — (1) fachada interna del tier: cuando un primitive necesita componer a otro de OTRA categoría, lo pide acá y recibe el componente ya conmutado por engine. Explícitamente no es un segundo barril y nadie fuera de `components/primitives` debe importarlo.

`components/primitives/tests/` — (1) prueba transversal del tier.

---

### `components/patterns/` — (664 archivos) widgets de tarea reutilizables: resuelven un trabajo completo (una tabla operativa, un formulario por esquema, un tablero) componiendo primitives.

`components/patterns/data/` — (152) mostrar y operar colecciones de datos.
...
`components/patterns/forms/` — (52) armar y filtrar formularios.
...
`components/patterns/visualization/` — (186) dibujar datos.
...
`components/patterns/communication/` — (43) conversación y avisos.
...
`components/patterns/workflow/` — (34) colas de trabajo y aprobación.
...
`components/patterns/navigation/` — (48) navegar y cambiar de contexto.
...
`components/patterns/shell/` — (31) marcos de página. Es donde más se pisa con `structures/shell/`.
...
`components/patterns/customization/` — (38) que el tenant vea y ajuste su marca.
...
`components/patterns/feedback/` — (15) qué mostrar cuando no hay nada o algo va lento.
...
`components/patterns/commerce/` — (9) `pricing-table/` (8): retícula de comparación de planes con filas de features y plan destacado.

`components/patterns/identity/` — (10) `profile/user-profile-card/` (8): tarjeta de perfil de usuario. El nivel `profile/` solo agrupa.

`components/patterns/foundation/` — (10) apoyo puro del tier, no renderiza producto.
...
`components/patterns/runtime/` — (27) estado y comportamiento compartidos entre patterns. Nada pinta.
...
`components/patterns/tooling/` — (2) `storybook/catalog/core/` y `/advanced`: dos archivos de historias que catalogan patterns. Solo herramienta.

`components/patterns/facade/` — (1) igual que la de primitives: cuando un pattern compone a otro de otro grupo, lo pide acá y recibe el componente ya conmutado por engine. Grupo con grupo nunca se importan directo.

`components/patterns/tests/` — (5) contratos transversales del tier.

---

### `components/structures/` — (162 archivos) el chrome de la página: lo que rodea a un pattern. Casi todo es "engine-free" (compone primitives ya conmutados, no tiene tres pinturas propias); las excepciones que sí traen `engines/` son stats-header, mobile-header, bottom-tab-bar y action-dock.

`components/structures/headers/` — (31) el encabezado de cada tipo de página. Siete owners:
...
`components/structures/workspace/` — (39) los controles que rodean a una tabla o colección.
...
`components/structures/record/` — (12) la anatomía de una ficha de registro.
...
`components/structures/dashboard/` — (31) el chrome de una pantalla de métricas.
...
`components/structures/feedback/` — (12) el estado de la pantalla entera.
...
`components/structures/shell/` — (26) los marcos de aplicación.
...
`components/structures/foundation/chrome/` — (9) el apoyo compartido entre shells y surfaces. Bajó desde `surfaces/runtime` porque el chrome no puede depender de las surfaces.
...
### `components/surfaces/` — (128 archivos) recetas de pantalla completa: objetos de configuración que describen una página entera. Se ordena en tres capas de dependencia.

`components/surfaces/foundation/` — (11) los contratos estables.
...
`components/surfaces/runtime/` — (7) el comportamiento que consume esos contratos.
...
`components/surfaces/presentation/pages/` — (107) las páginas completas, en seis grupos.
...
`components/surfaces/tests/` — (2) contratos transversales del tier.
...
`graphics/` — (0 archivos, total=417) raíz de los activos visuales no-componente:
iconos, marcas de terceros, pictogramas y el sistema de movimiento. No tiene
barrel propio; cada familia expone el suyo.

##### graphics/marks/ (total=13)

`graphics/marks/` — (1) familia de logotipos de terceros (OpenAI, GitHub,
...
legal.
  `graphics/marks/foundation/` — (0) capa de contratos, sin lógica.
    `graphics/marks/foundation/catalog/` — (1) define el corpus cerrado y
  ...
  `graphics/marks/presentation/` — (0) agrupador de los dos componentes React.
    `graphics/marks/presentation/brand-mark/` — (1) el componente
    ...
    `graphics/marks/presentation/cloud-service-mark/` — (1) igual pero para
    marcas de servicios cloud (AWS y sus servicios), con variante óptica propia.
  `graphics/marks/runtime/` — (0) agrupador de la maquinaria de resolución.
    `graphics/marks/runtime/adapters/` — (0) frontera con el proveedor.
      `graphics/marks/runtime/adapters/thesvg-react/` — (1) único punto del
    ...
    `graphics/marks/runtime/provenance/` — (1) registro auditable: qué
    paquete, qué versión, qué licencia y el aviso de marcas registradas.
    `graphics/marks/runtime/resolution/` — (1) barrel de las tres reglas de
  ...
  `graphics/marks/tests/` — (1) `marks.test.tsx`, prueba de la familia completa.

##### graphics/icons/ (total=332 — la familia más grande de graphics)

`graphics/icons/` — (1) barrel público del sistema de iconos.

  `graphics/icons/glyphs/` — capability de glifos con contratos, factory y catálogo.
    `.../glyphs/foundation/` — (1) tokens canónicos de tamaño.
      `.../foundation/contracts/` — (1) contratos compartidos de SVG, tamaño, rol,
      tono, estado, mirroring y procedencia.
        `.../contracts/base/` — (1) `BaseIcon`, wrapper SVG 24x24 accesible.
    `.../glyphs/runtime/factory/` — (1) factory del catálogo de glifos.
      `.../factory/phosphor-compat/` — (1) adaptador interno de glifos SSR.
        `.../phosphor-compat/tests/` — (1) prueba focal del adaptador.
      `.../factory/tests/` — (1) prueba focal de `createIcon`.
    `.../glyphs/presentation/catalog/` — (1) catálogo por categoría: action,
    animated, communication, compatibility, content, data, layout, media,
    navigation, status y user.
      `.../catalog/status/alert-icon/` — (1) `AlertIcon`, SVG manual estable.

  `graphics/icons/semantic/` — dueño único del sistema gobernado.
    `.../semantic/foundation/contracts/` — (1) contrato discriminado `IconProps`.
    `.../semantic/foundation/registry/` — (1) `ICON_NAMES`, `ICON_CORPUS` y guards.
    `.../semantic/foundation/policy/` — (1) política de pesos y tonos.
      `.../policy/tests/` — (1) prueba focal de la política.
    `.../semantic/foundation/provenance/` — (1) procedencia fija del corpus.
    `.../semantic/sources/adapters/` — (2) mapeo Phosphor 2.1.10 y JSON Schema.
    `.../semantic/sources/corpus/` — (2) manifiesto gobernado y JSON Schema.
    `.../semantic/sources/presets/bithire/` — (1) inventario de entrada del preset.
    `.../semantic/runtime/create-icon/` — (1) factory de roles semánticos.
      `.../create-icon/tests/` — (1) prueba de comportamiento generado.
    `.../semantic/presentation/icon/` — (1) fachada pública `Icon`.
      `.../icon/tests/` — (1) prueba de corpus, SSR y accesibilidad.
    `.../semantic/generated/` — (1) salida única del generador.
      `.../generated/corpus/` — (1) nombres y metadatos derivados.
      `.../generated/phosphor-adapter/` — (1) procedencia y tabla del adaptador.
      `.../generated/facade-map/` — (1) mapa exhaustivo usado por `Icon`.
      `.../generated/packs/` — (6) cinco packs tree-shakeables y barrel.
      `.../generated/presets/bithire/` — (1) fachada acotada del preset.
      `.../generated/roles/` — (**282 archivos**) un componente por rol gobernado.
  ...
  `graphics/motion/react/` — (1) todo lo que depende de React.
    `graphics/motion/react/presentation/` — (0) componentes.
      ...
      `.../presentation/primitives/` — (1) barrel de primitivas de transición.
        ...
        `CopyToCheck` y `HamburgerToX` desde `graphics/icons/glyphs/presentation/catalog/
...
`entrypoints/public/**` se generan desde `contracts/package/entrypoints/index.json`
...
##### entrypoints/eslint/ (total=16)

`entrypoints/eslint/` — (1) → **`./eslint`** (publicado, `dist/eslint.js`).
Dueño Node-only completo del plugin: contratos mínimos de Rule API, objeto plugin,
presets y las seis reglas. Los tests focales viven junto a sus reglas.
...
fachada `Icon` + **todo el catálogo histórico** (`export * from '../../graphics/icons'`).
  ...
  `dist/graphics/icons/semantic/generated/roles/*`). Ese subpath **no
...
archivo, cada uno generado/validado contra `contracts/package/entrypoints/index.json`
(que además fija presupuestos: `maxDirectSources`, `maxReachableModules`,
`maxSourceBytes`). Casi todos son una sola línea `export { X } from "../../components/..."`.
...
#### Soporte clasificado fuera de `src/`

La medición del 2026-08-18 agrupaba soporte bajo una raíz de fuente adicional.
La topología vigente lo clasifica por responsabilidad:

- declaraciones ambientales: `packages/core/src/foundation/contracts/ambient/stylesheets/`;
- implementación de ESLint: `packages/core/src/entrypoints/eslint/`;
- fixtures, helpers, setup y suites transversales: `packages/core/tests/`;
- coordinación de carriles: `packages/core/scripts/check/orchestration/`;
- sonda de resolución de cascada: `packages/core/scripts/check/tokens/cascade/probe/`;
- autoridad de objetivos táctiles: `packages/core/scripts/check/touch-targets/`.

No existe una sexta raíz de soporte bajo `packages/core/src/`. El inventario
fechado conserva su propósito descriptivo; estas rutas son sus anclas vigentes.

### Resumen de la reclasificación

Las capacidades de soporte no forman familias de componentes ni subpaths
públicos por sí mismas. Cada una vive ahora con su dueño de fuente, test o
comando, y las cinco raíces físicas de `src/` quedan limitadas a `foundation/`,
`infrastructure/`, `graphics/`, `components/` y `entrypoints/`.
...
La topología vigente tiene siete raíces de autoridad versionadas: `src/`,
`scripts/`, `tests/`, `contracts/`, `governance/`, `artifacts/` y `docs/`.
`dist/` es salida descartable de build.
...
### 0. Autoridades de invocación vigentes

`package.json`, el manifiesto de gates en
`scripts/check/automation/gates/manifest/index.mjs` y el workflow de CI son las
autoridades activas. Los tests viven junto a cada capacidad y se descubren de
forma recursiva.

### 1. `packages/core/scripts/`

La topología vigente tiene exactamente seis raíces orientadas por intención:

- `scripts/build/`: compila y ensambla salidas ejecutables o distribuibles.
- `scripts/check/`: verifica arquitectura, comportamiento, contratos y evidencia.
- `scripts/generate/`: proyecta fuente canónica en autoridades generadas.
- `scripts/libraries/`: hechos y primitivas compartidos por varios comandos.
- `scripts/maintain/`: mantenimiento y codemods acotados.
- `scripts/package/`: prueba y ensambla la frontera publicable.

Toda capacidad usa un dueño `intent/subdomain/capability/index.*`; ninguna raíz
basada en capas de fuente forma parte del roster. Los conteos y agrupaciones
funcionales medidos el 2026-08-18 son evidencia fechada anterior a esta
reclasificación y no definen la topología vigente.

### 2. `packages/core/styles/` — (6 archivos)

**Son ARTEFACTOS GENERADOS, no fuente.** Los seis los escribe
`scripts/build/verticals/css-build/index.mjs`; `node scripts/build/verticals/css-build/index.mjs --check` (parte de
...
- `styles/platform.css` — (126.218 líneas) **[DUPLICA]** de `styles/rottay.css`: byte-idénticos
  ...
  y hay un gate dedicado (`retired-identity`) que persigue justamente ese residuo.
...
- `styles/modern.css` — (954 líneas) bundle SOLO-motor: Tailwind v4.2.4 + DaisyUI envueltos en
  `@layer rottay-framework` por `scripts/libraries/engine/modern-framework-layer/index.mjs`, más el guard de
...
- `docs/generated/customization-controls/index.md` — (1) el catálogo de la API de producto de customización, en tablas
  por tier (STANDARD 13 controles, más los tiers pro y expert). Generado por
  `scripts/generate/tokens/customization/controls/index.mjs --write` (`pnpm tokens:catalog:write` — ojo: el alias se llama
  `tokens:catalog:*` pero escribe acá), lleva su propio `digest:` y el encabezado dice "NO editar a
  mano". `scripts/generate/tokens/customization/controls/index.mjs --check` es gate bloqueante en CI, así que un README
  editado a mano rompe el build.

---

### 4. `packages/core/governance/{effects,graphics}/` — (10 archivos)

**Fuente escrita a mano** (licencias de terceros + allowlists). Dos subárboles por clase de activo.

- `governance/effects/overview/index.md` — (1) documenta la procedencia de los efectos premium portados.
- `governance/effects/sources/index.json` — (1) el registro máquina de qué efecto vino de qué proyecto y
  bajo qué licencia. Lo lee `scripts/check/effects/index.mjs` de la RAÍZ DEL MONOREPO
  (`../../scripts/`), no un script de este paquete; corre en CI como gate `effects:provenance`.
- `governance/effects/licenses/` — (5) los textos de licencia originales, cada proveedor bajo su propio `index`.
- `governance/graphics/licenses/` — (2) licencias de los suppliers de glifos y marcas, cada proveedor bajo su propio `index.md`.
- `governance/graphics/sources/index.json` — (1) el registro de proveedores, clases y paquetes gráficos admitidos; lo leen
  `scripts/package/graphics/licenses/index.mjs` (gate bloqueante `cra17:licenses`) y
  `scripts/check/evidence/certification/packaging-integrity/index.mjs`. `governance/graphics/**` SÍ se publica (`files` lo incluye).

---

### 5. `packages/core/src/entrypoints/suppliers/` — (1 archivo)

- `cli/index.mjs` — (1) la fuente del gate canónico del lado de la APP, no del DS. Vite lo compila a
  `dist/entrypoints/suppliers/cli/index.js`, que es la única ruta publicada por `bin` y `exports`, para que `app-platform`, `app-bithire`
  ...
  anterior al contrato. Lo alimenta `contracts/runtime/suppliers/index.json` (generado por
  `scripts/package/contracts/supplier/index.mjs`), y `scripts/package/artifacts/inventory/tests/index.test.mjs` lo drillea.
  Las copias comiteadas en los repos de app se verifican contra esta fuente canónica.

---

### 6. `packages/core/docs/` — (181 archivos, clasificados por responsabilidad)

Documentación local del paquete, con `index.md` como entrada única. No define
una arquitectura paralela: `docs/ARCHITECTURE.md` es la autoridad canónica y
los documentos del paquete son guías operativas, referencias o snapshots.

- `docs/architecture/` — (10) anexos activos de autoridad de tenant, resolución
  de engines, cascada de customización, owners de componentes/surfaces,
  separación de bundles y referencia del tier structures.
- `docs/guides/` — (2) instalación y flujo de consumo.
- `docs/quality/` — (2) contratos de calidad activos, hoy el presupuesto de
  performance y su índice.
- `docs/reference/` — (11) referencias de componentes, contratos, hooks, i18n,
  theming y tokens; todos los owners usan `folder/index`.
- `docs/generated/` — (3) salidas no editables a mano: taxonomía de componentes
  y catálogo de controles de customización, más su índice.
- `docs/history/audits/` — (89) auditorías cerradas agrupadas por tema y fecha:
  utilización, integración, customización Modern, premium y reset de calidad.
- `docs/history/programs/` — (62) planes, prompts y handoffs superseded agrupados
  por programa: arquitectura de aplicaciones, plataforma de diseño, estilo
  premium, refactor de la suite y refactor del árbol fuente.

`packages/core/ARCHITECTURE.md`, los tres Markdown sueltos y las carpetas con
nombres de ejecución dejaron de ser peers del paquete. La mecánica vigente se
integró a `docs/architecture/`; la taxonomía quedó en `docs/generated/`; los
snapshots se preservaron byte a byte bajo `docs/history/`.
...
1. El CLI de evidencia v1 registrado por la medición frente a
   `scripts/check/evidence/framework/cli/index.mjs` (v2) — **confirmado en la
   fecha de medición**. La implementación v1 y su drill fueron retirados durante
   la reclasificación; sólo la ruta v2 sigue siendo un ancla vigente.
2. `.../modern-rescue/index.mjs` (776 líneas, inglés) vs
   `.../modern-rescue/probe/index.mjs` (2729 líneas, español) — dos implementaciones
...
4. `scripts/check/evidence/certification/packaging-integrity/index.mjs` importa y reejecuta `cra-17-packaging-license-gate.mjs` +
...
7. Los codemods convergen bajo `scripts/maintain/`: `codemods/` para apps consumidoras y
   `engine/` o `public-entrypoint-imports/` para la fuente del DS.
...
- `scripts/generate/surface-capabilities/index.mjs`
- `scripts/codemod-motion-tokens.mjs`
- `scripts/codemod-motion-durations.mjs`
- `scripts/maintain/codemods/sizetype-to-size/index.mjs` y `scripts/maintain/codemods/variant-tone-split/index.mjs`
  (por diseño: se corren a mano en el repo de la app)
- `.../modern-rescue/index.mjs`, `cascade-materialize.mjs`, `cascade-backlog.mjs`,
  `index.mjs` y `probe/index.mjs` — el subsistema de cascada completo. Los otros
  cinco archivos de `probe/` SÍ tienen consumidor: los importa `index.mjs` de la raíz
- Los tests que la medición marcó fuera de los globs fueron reubicados o
  retirados durante la reclasificación. Los tests vigentes se descubren de
  forma recursiva junto a cada capacidad.
...
- `scripts/check/tokens/cascade/purity/color-mix/index.mjs`
- `scripts/check/engine/lifecycle/framework-bundle/index.mjs`
- `scripts/check/tokens/contracts/chart-series/index.mjs`
- `scripts/check/evidence/certification/packaging-integrity/index.mjs`
- `scripts/embedded-css-paint-census.mjs` (solo lo spawnea un test de integración)

#### Otros

- `package.json` referencia `scripts/check/effects/index.mjs` y su test, pero ese script NO existe
...
- `dist/` está desactualizado respecto a `src/`: `scripts/build/verticals/css-build/index.mjs` importa
