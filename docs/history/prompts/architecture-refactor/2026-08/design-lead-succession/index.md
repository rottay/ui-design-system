...
equivalente operativo de `docs/history/prompts/architecture-refactor/2026-08/design-lead-session/index.md` para
esta transferencia.
...
`program/index.json` → `customization-model/index.json` → `orchestration/index.json` → y el
roadmap `docs/history/programs/architecture-refactor/2026-08/execution/index.md` (la escribanía durable).
...
- **Receipts F4B:** cualquier edición de fuentes/artefactos los stalea (ligan
  ...
  `scripts/check/evidence/framework/cli/index.mjs receipts` → 133/134 con el
...
- **Cadena derivada completa medida** (el defecto C5 era no correrla entera):
  ...
  actualizar `documentation-seal/index.json` al SHA nuevo antes
  de `gat07:write`).
- **Leyes C5:** toda afirmación de consumidor/lector exige grep citado sobre
  `src/` + `scripts/` + `governance/manifest/`; batería de cierre = gates completo.
...
7. Observación Fable pendiente de forma: nombrar la "reversión de remoción
   commiteada" como vía propia en la ley de `baseline/index.json` en
   su próxima edición.
