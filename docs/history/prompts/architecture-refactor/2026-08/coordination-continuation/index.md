...
> está en `docs/history/programs/architecture-refactor/2026-08/execution/index.md` §13 (mismo directorio); si algo
...
- **Reconciliación de identidad ✅** commit `85d0583b9` (incluye MI fix a
  `scripts/foundation/tokens/manifest/generation/index.test.mjs`: el drill producer=approver usa la
...
EXISTE: `scripts/foundation/tokens/scripts/foundation/tokens/manifest/variant-parity/` (productor hermano, solo importa de
mirror-parity; parser léxico puro, nunca dist) + gate blocking **#88** +
artefacto `generated/variant-parity.json` + baseline autorado
`baseline/index.json` (**divergentSlots 2268 ·
...
2. **Cadena de regeneración** (F4A-6 la fijó COMPLETA — dos artefactos stale
   ...
   **reads-ledger** (`reads-adjudication.json`: su `basedOnManifestDigest` debe
   ser el sha256 del `contracts/css/hooks/index.json` vigente; si el lote corrió
   `hooks:generate`, re-anclarlo) → **gat-07 SIEMPRE ÚLTIMO Y LO SELLÁS VOS**
   (`node scripts/check/evidence/certification/claims/exactness/index.mjs --write` luego
...
4. **Suite**: pierna 1 = `node --test "scripts/**/*.test.mjs" "governance/manifest/**/*.test.mjs"`
...
tail -80 docs/history/programs/architecture-refactor/2026-08/execution/index.md   # §13, la verdad viva
```
