...
[`REAUDITORIA-FABLE-2026-08-19.md`](/docs/history/audits/architecture-refactor/2026-08/execution-verification/index.md) (el
diagnóstico) y [`REAUDITORIA-FABLE-2-EJECUCION-2026-08-19.md`](/docs/history/audits/architecture-refactor/2026-08/execution-followup/index.md)
...
3. **Borrados de riesgo cero confirmado** (verificados 2-3 veces): 23 carpetas
   ...
   `art-direction/index.json:363`; documentar su recreación o dejarla); 17
   ...
   `probe/index.mjs` + test; 2 agentes `.claude/`; `audit-presets.mjs`
...
- **Graduación del manifest (lote atómico):** censo correcto: **38
  ...
  mano, no regeneración; (c) `checkpoint/index.json` **no tiene productor** —
...
- **Core:** los 4 JSON publicados a `contracts/` (remapear `exports`/`files`;
  `src/entrypoints/suppliers/cli/index.mjs` es la fuente canónica y Vite publica
  `dist/entrypoints/suppliers/cli/index.js` en la misma versión); `KIMI-*` al directorio del programa; censos vivos a su
...
- **Ratchet nuevo** (obra nueva; no existe contador que clasifique por nombre):
  ancla en `engine-baseline/index.json`, con dos reglas de construcción:
...
- Retirar 75 subpaths granulares (los 2 con importador showroom — ambos desde
  ...
  y reescritura explícita de `contracts/package/entrypoints/index.json`. La maquinaria
...
- **Extirpación platform completa:** lo del plan + `residual-adjudication.json`
  (src, dato vivo), `packages/core/docs/reference/tokens/index.md`,
  `packages/core/docs/history/programs/premium-styling/`,
...
transcripción); plegado de `packages/core/ARCHITECTURE.md` ya ejecutado
(autoridad única en `docs/ARCHITECTURE.md`, anexos operativos en
`packages/core/docs/architecture/`); correcciones de READMEs (263→282,
platform→rottay, badge TS);
...
`SIGHTED_ACCEPTED`. Esa verdad se lee de `governance/manifest/index.json`; **252 permanece
...
- F9 corre después de estabilizar F5/F6 y migrar F8. Si una absorción de F5
  ...
  atómico en `family-inventory`, `program/index.json`, segmentos del manifest,
...
3. `roadmap/registry.json` y `checkpoint/index.json` se actualizan cuando el
...
- `index.mjs`: `CONSTITUTION_READY` sobre el HEAD anclado.
...
1. `packages/core/governance/manifest/cascade/catalog/index.json`;
...
1. `index.test.mjs` muta/renombra el manifest real mientras el gate lo
   corre concurrente con `scripts/foundation/tokens/manifest/generation/index.test.mjs`: P1 de carrera y
...
2. **T-1a**: sandbox de `index.test.mjs`, clausura derivada y negativas
...
#### T-1a / sandbox de `index.test.mjs` — SELLADO
...
Resultado: `tests/reanchor/index.test.mjs` copia el corpus
...
regex de continuación `nextIsText` en `scripts/foundation/tokens/scripts/foundation/tokens/manifest/variant-parity/index.mjs`
...
- **Lote B** — iconSize: catálogo + generator + `governance/manifest/index.json` +
  ...
  `input.css:38`, emitido por rottay y bithire); `governance/manifest/index.json`
  mueve sólo `inputsDigest`; los 275 JSON de `governance/manifest/controls/**` +
  `governance/manifest/families/**` byte-idénticos a HEAD; roster 198→198 entradas,
...
5. **Obligación del digest `basedOnReportDigest` — cerrada por el
   ...
   testeado (`tests/index.test.mjs:103`) y el productor de regeneración
...
- `GOVERNOR_CLASS.baseline: 'razon-falsable'` sigue en
  `scripts/foundation/tokens/scripts/foundation/tokens/manifest/variant-parity/index.mjs:149` — inerte: el mapa es
...
sin acoplamiento mecánico con `governance/manifest/cascade/catalog/index.json` (64
...
A1–A12, cero `src/**`, roots, catálogo, `governance/manifest/index.json`, materialized,
backlog, docs o GAT. **A12 (`scripts/check/automation/gates/manifest/index.mjs`) queda
byte-idéntico al prestate** (`3b7f906f…`): anexar los tres drills nuevos a su
`run[]` rompía `scripts/check/automation/runner/index.test.mjs:239`, que sella ese argv
...
| T-1a sandbox `index.test.mjs` | 100%; `SOURCE_READY` + postaudit Fable `ACCEPT` |
...
| Normalización profunda (puerta ejecutable pre-F4C; directiva owner 2026-08-27) | **COHORTE 1 CERRADA (2026-08-27, `b00e5700a`; cohorte 0 en `3ccd2f402`)** — membresía canal→raíz persistida (753/1.610 canales con raíz por 3 vías sin coincidencia textual) + tabla owner-tiers autorada por el DT (114) + inventario v2 con **R2 26→906** (piso 412) + `collapsesLegacy` fechado. Cero literales colapsados aún — el primer colapso (color/surfaces) espera el gate `normalization-contract-gate` (drill de duplicación-real) y el diseño de cohorte 2. **PUERTA EN PIE (2026-08-27, `1c5037903`):** gate blocking con las 5 leyes de la enmienda + 18 drills + baseline con deuda enumerada (sombras 152, grupos divergentes 214/1.495, unassigned 297 — decrease-only). **COHORTE 2A CERRADA (`45d39fc64`):** eje `paso` integrado (33 raíces nuevas, tabla owner-step-rules del DT) — R2 **1.005**, purity.pure **332**, r2Floor **593**, cero delta resuelto. **CORRECCIÓN PRE-2B CERRADA (`0c74b9412`):** instrumento cero-delta endurecido (conjunto unresolved al diff, fallback a spec CSS con ciclo-miembro correcto, atribución externo+causa; Fable ACCEPT). **E-2 APLICADA (`df121b4c4`):** +32KB a los 18 subpaths con los 3 themes, autorización owner con las 4 condiciones en `_budgetGovernance`. **2B SUB-LOTE 1 (`d8592aad5`)** — primer colapso real (21 slots). **OWNER-STEP-RULES LOTE 2 (`3cbd42d95`):** ley de espejo + convenciones BrandPalette, pure 315→333. **2B PLENA CERRADA (`628e228a3`):** 128 slots en 19 raíces, cero-delta 3×, contrafactual 8/8, **el primer grupo divergente cierra por colapso real** (239→238/1639→1635, baja legal); pure 333→205 consumida. **2B COLA CERRADA (`9f9dbc597`) — COHORTE 2B COMPLETA:** 3 slots finales (edit en línea con 3 aserciones), contadores quietos, pure **202**; la clase `pure` fuera de paleta queda **agotada salvo 5 bloqueadas-con-dueño** (4 esperan la cohorte paleta por E7 uniformidad — ley nueva codificada en el extractor v4 — y 1 es deuda del eje `seeds.json`). Próximo: cohorte PALETA — **P1 CERRADA (`e66eaf1ad`):** 27 hojas redundantes retiradas con ausencia declarada (`@absent`), **cero bytes de artefacto movidos** y pins de canal quietos; Fable ACCEPT tras condición cumplida; re-anclas hacia abajo con aritmética (divergentSlots 1.635→1.633; ledger slot-inventory 1.707→1.443 con unidad pineada — hallazgo: ese baseline no lo verifica ningún gate, deuda del frente). **LOTE 3 (alias-inertes) COMPLETO (`a8d9ead19` + `f72699056`):** ley nueva `@parent` (padre-por-diseño, 39 canales/72 reglas DT) con guard mecánico en el refinamiento; 3 raíces nuevas `tier.raised.fg.{primary,secondary,muted}` (46 filas salen de STOP, pure 157→166); re-ancla gobernada de `divergentGroups` 238→242 (re-atribución por clave más fina, slots bajan a 1.630). Siguen: **P0 (gobierno de las 5 raíces `ramp.seed.*` gap — espera decisión owner por el dial público nuevo `palette.status-seeds`)**, P2 derivación consciente del modo (consulta Codex preparada, paquete listo) |
...
`packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls/spacing-rhythm/computed-static-db/`
(8 artifacts + 8 receipts), los 8 válidos contra
`scripts/check/evidence/framework/receipts/index.mjs`.
...
`packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls/surfaces-effect-intensity/computed-static-db/`,
los 6 receipts válidos contra `scripts/check/evidence/framework/receipts/index.mjs`. Ningún
...
8. `program-check` pasa de **25 a 24** hallazgos: 0 nuevos, y se resuelve
   `governance/manifest/index.json is stale`. Advertencia honesta: regenerar ese índice
...
los dos temas: 6 artifacts + 6 receipts (12 archivos) en
`packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls/shape-radius-scale/computed-static-db/`.
...
productor canónico (`scripts/foundation/tokens/manifest/generation/index.mjs --sync`): **21 archivos**, 19
...
`governance/manifest/index.json` sólo sus digests derivados. **Cero deltas semánticos y cero
...
8. **Los 6 receipts de `surfaces.effect-intensity` se reemitieron.** El repin de la
   ...
   `governance/manifest/controls/surfaces/effect-intensity/index.json`, así que ninguna variante de
...
4 receipts en
`packages/core/artifacts/quality/programs/modern-rescue/cascade-proofs/controls/experience-profile/computed-static-db/`.
...
- **DT vivo: Codex.** No hubo sucesión DT el 2026-08-23. Existe un handoff
  ...
  autoridad (AGENTS.md, CLAUDE.md, orchestration/index.json, index.mjs,
...
- **Estado de gates en HEAD (medido, no heredado de memos):**
  `index.mjs` = `BLOCKED`, 24 fallos, todos celdas `spacing.rhythm` de
  `governance/manifest/families/primitive/layout/{flex,grid,space,stack}.json`
...
- **Deudas con dueño (C6):** `OPEN_VERTICAL_CASCADE_DEFECT`
  (`governance/manifest/controls/experience/profile/index.json:95`; bithire dark:
...
(`docs/history/prompts/architecture-refactor/2026-08/design-lead-session/index.md`, ejecutada por la sesión fresca
...
autoridades):** `AGENTS.md`, `CLAUDE.md`, `orchestration/index.json`,
`index.mjs`, `index.test.mjs`, `README.md` del programa
(sección Roles reescrita; bloque stampado re-renderizado por
`program-state --write`, nunca a mano), `art-direction/index.json`,
`rounds/index.json`, `program/index.json`, `quality-rubric/index.json`,
`visual-craft/index.json`, `checkpoint/index.json` (currentWave F4B,
...
3. **`phase-a/index.json` no se toca** (sellado; scope adjudicado
   `:!**/phase-a/**`).
4. **Las aceptaciones futuras nombran al rol, no al actor:** R1–R4 GO, R6
   ...
   (`DT`), coherente con `customization-model/index.json#r7Execution.entryLaw`.

**Pruebas (Node v22.17.0, antes → después):** `index.mjs` BLOCKED con
...
es el próximo packet). `index.test.mjs`: 46/48 → 46/48 con identidad
...
sandbox T-1a — `CLOSURE_MEMBERS` no incluye `packages/core/artifacts/quality`,
...
A11 preexistente (`index.test.mjs` N13/T-21, deuda PRE_F4B).
...
`scripts/foundation/tokens/scripts/foundation/tokens/manifest/rules/**`, `program-check*`, `v2/**`) y ninguna celda se reancló a
...
mitades — Fable muestreó 4/4); `governance/manifest/index.json` regenerado por el
...
**Verificación DT de primera mano:** `index.mjs` =
...
3. **H1 (follow-up atómico):** 4 canales `--_ds-` reales
   ...
   `governance/manifest/cascade/roots/spacing/rhythm/index.json` (fuera del write-set C2).
...
`packages/core/artifacts/quality/gates/gat-07/semantic-evidence.json` y
...
alcance): `index.mjs` (nuevo, 1665), `index.mjs`
(nuevo, 651), `index.mjs` (+577), `index.test.mjs`
(+3816), `index.mjs` (nuevo, 572),
`index.mjs` (nuevo, 3092),
`artifacts/generated/manifest/cascade/producers/index.json` (4156 líneas modificadas), y los
...
1. **T-11 anclado** (`index.test.mjs`): el test ahora pinea que
...
Verificación: `index.test.mjs` **209/209** (PRE 207/2; N13 y
...
30/30 VALID+FRESH (corrida propia del DT); `governance/manifest/index.json`
...
drift real);
`index.mjs` = **CONSTITUTION_READY** (corrida propia del DT);
...
cumplir": la ley superior sí contempla DATA; `scripts/foundation/tokens/scripts/foundation/tokens/manifest/rules` no. Ruling
...
**La enmienda (bloques A–G en `scripts/foundation/tokens/scripts/foundation/tokens/manifest/rules/` + mirror en
...
cada uno por su causa verdadera. `index.test.mjs` pasa de 46/48 a
...
1. **A11 (frescura de producers.json):** el inventario commiteado quedó
   ...
   **todas menos 492 hojas son POSICIONALES** (deriva de ordinal/línea tras
   ...
   `index.test.mjs` 209/209 (N13 y T-21 verdes — T-21 sólo
   dentro del packet).
2. **Test 1 (sandbox CLOSURE_MEMBERS):** el fix nombrado en T-1 (añadir
   `packages/core/artifacts/quality`) resultó necesario pero NO suficiente —
...
**Estado del tablero tras T-2:** `index.test.mjs` 48/48;
`index.mjs` CONSTITUTION_READY; receipts 39/39 VALID+FRESH;
...
`artifacts/quality/programs/modern-rescue/archive/cascade-proofs/unreceipted/typography-scale-static-db-residuals/isolation.MEASURED-NOT-RECEIPTED.json`):**
...
R8 — `index.mjs:1647` pinea el set a exactamente R0..R7 e
...
arreglá el fence SIGHTED_APPROVER: leelo contra orchestration/index.json").
Writer: Opus. Postaudit Fable: **ACCEPT**, memo
...
(`orchestration/index.json#doubleAccept.coordinator` y
`quality-rubric/index.json#eligibility.finalSightedAuthority`; lo pineado son
...
constante tenía TRES consumidores, no uno — `scripts/foundation/tokens/scripts/foundation/tokens/manifest/rules/index.mjs:1166`
...
`governance/manifest/controls/palette/seeds/index.json#knownDefects[0]` (writer),
`checkpoint/index.json#blockedOn` (DT), el asiento C6 de arriba (DT), y
...
`governance/manifest/index.json` ACEPTADA** — índice derivado de los manifests de
...
razón medida. Desviación `index.test.mjs` (CLOSURE_MEMBERS +=
...
muerde desde el primer byte). `contracts/runtime/suppliers/index.json` regenerado (sólo
...
(mecánico):** las 3 fallas de `index.test.mjs` eran el re-seat de
...
**El "hang" de index.test.mjs — DIAGNOSTICADO (Opus, 2026-08-25;
...
`spawnSync` y cuyo costo entero es `index.test.mjs` (217 tests,
...
(`index.test.mjs`): (1) `timeout: 900_000` + `maxBuffer: 64MB` en el
...
`src/` + `scripts/` + `governance/manifest/`. Corrección: packet C5 propio (preaudit
...
`ceilings/index.json` (ninguno movido; los 18 de la E-2
...
(`--ds-color-text-{primary,secondary,muted}`, reclamante base único `tier.base.fg[.*]`, regla de
generalidad intacta) y `derivationDebt`+`derivation`+`evidence` (la premisa del descuento se paga
al crear — el finding del exposure-gate lo exige). **La tabla no se tocó:** las 42 reglas ya
pedían esos pasos; al existir la raíz, dejaron de ser inertes. **Verificaciones exactas:**
head-not-emitted 384→**338** (46 filas salen de STOP) · `pure` 157→**166** (+9 colapsables) ·
value-shift +37 · `rowsRefinedByStep` 44→**65** (refinamiento real) · **`stepNote` inerte = 0**
(la condición del orden A→B, cumplida) · `withRoot`/`withoutRoot` quietos · counts exposure
28/28/8 quietos · `literalPinsOnDeclaredHead` 20 quieto · **ley dura: `resolved-map --check`
EXIT=0 sin re-pin, digest sin mover** (cambia atribución, no emisión). **El freno, con su medición
y su puerta:** `divergentGroups` subió 238→242 — el writer frenó SIN re-anclar (la ley: un
contador que sube es STOP y escala) y midió grupo por grupo: un grupo bithire de 24 slots/5
posiciones se partió en subgrupos más finos (3+13+7+1 por modo) y uno rottay 3→1+2 — **los grupos
suben porque la CLAVE es más fina; los slots divergentes BAJAN 51→48 (−3)** y 3 slots salieron (2 clases) de
la divergencia por completo. Cero duplicación nueva. Es exactamente el caso que la puerta
gobernada `--reattribution` anticipa en su docblock; la re-ancla la ejecutó el DT por esa puerta
con la razón completa del writer (writer propone / DT ejecuta). **La otra re-ancla (3 sitios):
`refinedRootsExcluded` 33→36** (catálogo `reading` + baseline `reading` + valor), con los 3 "33"
HISTÓRICOS del catálogo intactos (verificado: 0 en el diff), y el drill del exposure-gate ahora
LEE el baseline en vez de teclear la cifra (un solo número que mover — dos copias del mismo hecho
divergen en silencio). **Hallazgo de orden (asentado como ley de operación): re-ancla → cadena →
barrido** (el catálogo es insumo de la cadena; re-anclar después deja `governance/manifest/index.json`
rancio). **Corrección de MI operatoria, asentada:** el barrido final de 3A quedó INVALIDADO por
contaminación — lo lancé mientras el ítem B ya escribía en el worktree; sus 4 rojos eran el estado
intermedio de B, no una regresión (3A quedó evidenciada por los barridos del writer y de Fable,
101/0 ambos, antes de que B arrancara). **Regla desde ya: el barrido final de un lote cierra ANTES
de despachar el siguiente.** **Residuo honesto (deuda previa, no del lote):** 3 canales siguen en
`tier.raised.fg` sin adjudicar (`--ds-card-footer-border-color`, `--ds-card-header-border-color`,
`--ds-collection-card-status-color`) — del bucket de 268; tres reglas los cierran. **5 líneas de
clase C** al `exposureNote` de las 5 raíces `tier.*.border.primary` ("colapsada a propósito por
K2; leer @governor antes de reportar alias" — la lección vive donde un detector la lee).
**LOTE 3 COMPLETO: 62 alias-inertes resueltos** (39 padre-por-diseño + 21 vocabulario real + 2 que
eran trabajo ya hecho). Desviación declarada: el drill de 3A que congelaba cifras se re-expresó
como propiedad (ninguna regla apunta a raíz inexistente — más fuerte y no se rompe cuando el eje
crece). Barrido del writer 93/1 (el 1 = la re-ancla esperando mi ejecución); post-ejecución, el
mío. Métrica sin mover: **62% / ~43%** — se reevalúa al cierre de la cohorte paleta.
...
más que uno que aborta). Cero artefactos movidos (cadena con diff CERO en `governance/manifest/`,
...
de ausencia** (re-corrida a HEAD, fail-closed, 29/29 reproducidas) marca como redundantes —
borrarlas no cambia nada. **Resultado central, medido: `git status` sobre `styles/` y
`facade/artifacts/` = 0 archivos.** No es "bytes sin valores" como 2B: no cambió ni un byte —
una hoja redundante no aporta emisión. **Correcciones al plan original, ambas medidas y
adoptadas:** (i) **NO mueve pins de canal** (mi encabezado y el diseño §6.7 lo anticipaban al
revés): brand-themes no son plano productor y el cssString es byte-idéntico — el lote que
moverá pins es el que retire hojas *necesarias*; (ii) el censo `writers` queda en 4.670 (delta
0, no ≈−29 como preveía mi ley 5): una hoja redundante nunca es el único escritor de su canal —
la re-proyección de reconciliation es solo-puntero, correctamente, con la aserción de cifras
normativas intactas. **Población final 27/29:** las 2 de bithire base (`borderFocusColor`,
`linkColor`) quedaron fuera por **regla nueva del eje: una hoja cuya redundancia depende del
caso no es retirable** (autoran `#3a6fb0`, la derivación emite `#3A6FB0`; la prueba de ausencia
normalizaba caso y no lo veía — desde ya compara **caso-exacto**). Van a la deuda caso-hex.
**Forma del edit: ausencia declarada, no borrado desnudo** — cada hoja deja `@absent <ruta>` +
`@governor` agrupado al final de su objeto (5 objetos contenedores; los pasos 50/100 quedan:
ningún objeto vacío). El borrado desnudo subía `silentPairs` 2→29 (ley F4A: una ausencia es
escrita o no es); con `@absent` todos los contadores de variant-parity en ancla (2/3.947/3).
Los 2 residuales autorizados no hicieron falta: cero tags vecinos tocados (el +2 era artefacto
de la simulación). **Disposición:** el preservation-manifest no tiene sujeto — los 27 canales
no mueren (los sigue escribiendo la derivación) y no están entre los 266 dead-writers; la
disposición vive en el `@absent`, que sí es gate-verificado; no se creó registro paralelo sin
verificador (adjudicado). **Sonda por canal 27/27** (los canales siguen declarados con el mismo
valor — redundante con el diff cero de artefactos, corrida igual). **Re-anclas (writer propone,
DT ejecuta):** `divergentSlots` 1.635→1.633 (2 retiradas eran miembros de grupos divergentes de
paleta; `divergentGroups` 238 quieto) y baseline slot-inventory `rowsWithoutRootAttribution`
1.707→1.443 — **con la historia completa: deriva silenciosa 1.707→1.467 desde cohorte 1 (2A/2B
movieron filas y ningún gate lee esa cifra) + −24 de este lote**. **HALLAZGO con dueño (el
frente): el baseline de slot-inventory no lo verifica nadie** — `BASELINE_PATH` está definido en
el productor y no se usa en ninguna otra línea; 4 de sus 5 cifras sin verificador (solo un drill
acopla `unassignedRows`) — por eso derivó 240 en silencio, esta vez adentro de casa; deuda:
cablear el `--check` del productor contra su baseline (patrón ya existente) o retirar lo no
verificado. La razón del ancla nueva **pinea la unidad**:
`stats.byRootAttribution["no-persisted-membership"]` (convive con otra unidad — sin-rootId total
1.626 — y sin el pin la próxima deriva es inmedible), y la descomposición del −24 queda exacta:
**23 rottay + 1 bithire** (`onPrimaryColor`). purity.pure 202→**200** (cifra operativa −2: solo
3 de las 27 estaban en su denominador). **Ajuste estructural declarado (condición del ACCEPT de
Fable, cumplida en este commit):** en evnto, `linkHoverColor` y `linkVisitedColor` se
relocalizaron byte-idénticas por encima del bloque `@absent` (la agrupación al final del objeto
las pedía); el conteo de líneas correcto es −8 en evnto (2 hojas + 4 líneas internas de docblock
reusando el cascarón + 2 relocalizadas), no "10 (2+8)". Valores intactos, artefactos en cero —
benigno, pero la ley pedía la declaración y aquí queda.
Cero-delta `--against` EXIT=0 ×2 (verificado por DT y Fable). Contadores quietos salvo la baja
legal. Barrido pre-commit 100 PASS/1 FAIL (gat-07, re-stamp tras este asiento). **Próximo:** P0
(gobierno de las 5 raíces `ramp.seed.*` gap — **espera decisión owner por el dial público nuevo
`palette.status-seeds`**) y consulta Codex pre-P2 (paquete listo). Deudas vivas: caso-hex
(+2 de este lote), eje `seeds.json`, purity string-exacto, drill decrease-only 77 techos, 60
alias-inertes (lote 3), ledger slot-inventory sin gate. Métrica sin mover: **62% / ~43%** — se
reevalúa al cierre de la cohorte paleta.


**2B COLA CERRADA — LA CLASE `pure` FUERA DE PALETA QUEDA AGOTADA SALVO 5 BLOQUEADAS-CON-DUEÑO
(2026-08-28; commit `9f9dbc597`; writer Opus; postaudit Fable: **ACCEPT** con reproducción propia
— SequenceMatcher propio sobre el diff, `--against` y gate re-corridos, clave L4b del v4
replicada en JS independiente con los mismos 238/1635 y los mismos 4 grupos uniformes).** Último lote de la
cohorte 2B: **3 slots colapsados** literal→`var()` en bithire (`filled.bgFocus`→
`--ds-surface-control`, tenant-dial con contrafactual e=1; `clear.colorHover` y
`autofill.color`→`--ds-color-text-primary`), **+52 B** de fuente. Novedad de método: **edit en
línea** — las 3 líneas llevan otros literales que quedaron byte-intactos, con tres aserciones
mecánicas por slot (clave única en la línea con guard de borde, literal == `authoredValue`
caso-exacto, prefijo+sufijo byte-idénticos) y custodia probada por SequenceMatcher sobre el diff
(exactamente 1 fragmento alterado por línea). Cero-delta resuelto `--against` (valores +
unresolved) EXIT=0 verificado por el DT; contadores quietos (297/10/228/238/1635, cero
re-anclas); purity.pure **205→202**; pins congelados intactos. **Las 5 que no colapsaron quedan
bloqueadas con dueño escrito:** 4 por **E7 (uniformidad cross-cohorte)** — su grupo es uniforme
hoy y su hermano `PALETTE.*` espera la cohorte paleta; colapsar la mitad chrome lo partiría
(medido: +4/+8, la misma contrafactual 242/1643 ya replicada dos veces) — y 1 por **ley 3 sobre
la línea fuente** (`evnto:OVERLAY.chrome.table.headerColor` lee `seeds.json`; el inventario
registra el valor evaluado y la veía literal). **Leyes nuevas adoptadas (efectivas ya):** (i)
**E7** — un slot no es elegible si su grupo L4b es uniforme y algún miembro está excluido del
lote; ninguna exclusión puede volver a vivir en la memoria del writer: está codificada en el
**extractor v4** (instrumento DT, `/private/tmp/mr-2b-extract-v4.py`), que además verifica la ley
3 sobre la línea fuente (clave+literal en ventana desde el tag). Corrido al cierre: **0 elegibles
libres, E7=4, ley3-fuente=1** — la cola queda agotada con ese matiz. (ii) **El censo de
superficie corre ANTES de sus tres consumidores** en la cadena canónica (kimi-preservation →
controls-catalog → tokens:catalog:write); el orden documentado no lo decía y enrojecía el barrido
por causa ajena al lote — asentado. **Desviaciones del writer declaradas y aceptadas:** orden de
cadena corregido en caliente (primer barrido 9 rojos, todos staleo downstream; segundo 97/4 con
los 4 de contrato DT) y re-proyección de `customization-reconciliation.json` (solo el puntero
digest, cifras normativas intactas verificadas por aserción — writers 4.670, deadWriters 266).
**Nota de honestidad del ledger (matiz Fable):** el censo independiente del writer reportó 7
libres donde v4 reporta 0 — su E5 sin herencia intra-vertical deja pasar **SEIS** filas de la
clase A, no las 3 que él citó: los 3 conocidos (`tabs.borderActive`, `input.borderFocus`,
`sidebar.itemColorActive`) MÁS 3 nunca vistos (`collectionCard.selectedBorder`,
`table.resizeBgHover`, `sidebar.groupColor`) que un colapso con ese criterio habría pintado
distinto en dark sin que nadie lo viera hasta el `--against` (verificado por Fable fila por fila
contra el mapa resuelto: las 6 exclusiones son verdad medida). v4 los caza a los 6 ANTES de
tocar una línea — es la prueba empírica de que la E5 endurecida no era formalismo. La cifra
autoritativa es la del instrumento.
**Próximo:** cohorte PALETA — P0 (gobierno de las 5 raíces `ramp.seed.*` gap: dial público nuevo
`palette.status-seeds` + `neutral` en `palette.seeds`; **espera decisión del owner**) y P1
(retire×29, cero-delta por construcción, primer lote que mueve los pins de canal — declarado).
Consulta Codex pre-P2 preparada (toca el compilador: derivación de rampa consciente del modo).
Deudas vivas: eje `seeds.json` (tercera aparición de la clase), purity string-exacto, drill
decrease-only de los 77 techos, 60 alias-inertes (lote 3). Métrica sin mover: **62% / ~43%** — se
reevalúa al cierre de la cohorte paleta.


**2B PLENA CERRADA — 128 SLOTS COLAPSADOS EN 19 RAÍCES, Y EL PRIMER CONTADOR DEL FRENTE BAJA
POR COLAPSO REAL (2026-08-27; commit `628e228a3`; writer Opus; postaudit Fable: ACCEPT con
reproducción propia).** rottay 84 · bithire 40 · evnto 4; +2,5 KB en artefactos (dentro de la
re-ancla E-2). **Cero-delta resuelto con el instrumento endurecido, verificado 3×** (por raíz
durante la corrida, tras el build real, tras la cadena): `--against` EXIT=0 en valores Y en el
conjunto unresolved. Diff fuente verificado por el DT: 128+/128−, toda `+` es
`clave: "var(--ds-…)"`, toda `−` literal sin `var(`/`calc(`. Contrafactual tenant 8/8 raíces
dial con e=1 (probe real; el alcance del tenant se amplía a canales que el dial no alcanzaba —
visible solo bajo dial no-default, sancionado por la directiva). **El contador baja por
primera vez:** `rottay|tier.control.fg.disabled|fg|disabled|overlay` cerró entero (sus
miembros aterrizaron en la misma referencia; la auditora lo identificó miembro a miembro) —
`divergentGroups` 239→238, `divergentSlots` 1639→1635, re-ancla hacia abajo con la aritmética
Y la contrafactual escritas en la razón (con los 4 excluidos habría SUBIDO a 242/1643 — la
auditora lo replicó exacto). **CORRECCIÓN ESCRITA al asiento de step-rules lote 2:** mi frase
"en cohortes de colapso puro los contadores divergent* no se mueven por construcción" era
**falsa en ambas direcciones, medido**: (a) SUBE por partición cross-cohorte de un grupo
uniforme cuya mitad paleta queda fuera de 2B (los 4 excluidos: `select.dropdownBorderColor`,
`input.colorPlaceholder` overlay, `sidebar.text` overlay, `calendar.dayColorOther` — cierran
en la cohorte paleta, con ambas mitades); (b) BAJA cuando un grupo divergente entero aterriza
en la misma referencia. **La ley 6 ORIGINAL del brief se mantiene** (bajan o el asiento dice
por qué; subir está prohibido sin puerta) — lo que cae es mi reformulación. **Exclusiones por
ley:** 5 ley-3 (no-literales; la literalidad no es la comilla — endurecimiento del writer,
adoptado al extractor v3) + 4 uniformidad cross-eje. Pins congelados intactos
(4873/10312/4586). Censo de superficies regenerado (productor fuera de cadena; su diff es solo
el colapso: tsReadNames +1 y conteos `ts` por superficie ≈ sitios colapsados — el árbol no
tenía drift ajeno que absorber, así que la omisión temporal del censo no aplica acá).
Receipts re-firmados por el DT (134/134 quirúrgico). Desviaciones del writer declaradas y
ACEPTADAS por la auditora (memo consolidado con `--against` por raíz; instrumento intermedio
esbuild calibrado en reposo con verificación vinculante sobre `dist` real; principio sobre el
...
auditora con la lógica exacta del gate: ningún portador sin tocar, ningún tocado sin portar;
`./runtime/visual-authority` quedó correctamente fuera — alcanza un contrato de protocolo, no
un tema). **Bytes vivos (medidos hoy, gate real) → techo viejo (headroom) → techo nuevo:**
provider 2.009.590→2.009.590 (0)→2.041.590 · typography 1.714.069→1.714.069 (0)→1.746.069 ·
skeleton 1.713.316→1.713.316 (0)→1.745.316 · dashboard-header 2.310.235→2.310.235 (0)→2.342.235
· collection-workspace 3.560.350→3.560.350 (0)→3.592.350 · divider 1.599.457→1.600.000
(543)→1.632.000 · button 2.299.377→2.300.000 (623)→2.332.000 · charts 2.447.227→2.450.000
(2.773)→2.482.000 · data-table 2.646.498→2.650.000 (3.502)→2.682.000 · card
1.644.137→1.650.000 (5.863)→1.682.000 · column-menu 2.590.939→2.600.000 (9.061)→2.632.000 ·
tenant 1.080.982→1.100.000 (19.018)→1.132.000 · presence 1.830.285→1.850.000
(19.715)→1.882.000 · toast 1.779.889→1.800.000 (20.111)→1.832.000 · motion
1.723.816→1.750.000 (26.184)→1.782.000 · tag 1.619.800→1.650.000 (30.200)→1.682.000 · badge
1.618.732→1.650.000 (31.268)→1.682.000 · record-summary 1.870.963→2.486.403
(615.440)→2.518.403. **Las 4 condiciones del owner escritas en `_budgetGovernance` (raíz del
manifest) + `budgetNote` por entry:** (i) calculada desde bytes vivos; (ii) trazada a la
proyección +24.418 B → +32.000 (≈31% de aire sobre la proyección; la prosa histórica decía
"25%", la cifra exacta queda acá); (iii) expira en F6 (esos subpaths mueren); (iv)
decrease-only desde este commit, no habilita futuras re-anclas hacia arriba. **Deuda diferida
con dueño (el frente), observación Fable no bloqueante:** el decrease-only de este manifest es
textual, no mecánico — drill que pinee los 77 techos decrease-only con puerta nombrada
(patrón `--reattribution`), a tomar en la próxima pasada de endurecimiento. **Batería:** gate
real EXIT=0 (sus bytes vivos coinciden con la tabla), tests gate+migrate 10/10, diff-check
limpio. **2B queda desbloqueada en presupuesto.** Métrica sin mover: 62%/~43%.
...
root-membership(92) < slot-inventory(94) < normalization-contract(96)): **L1** el contrato tipado
de familia no puede relajarse en silencio (las 10 claves readonly sin `?`, `Record` no `Partial`
— reconciliación declarada: el SeedSlotValue por-hoja no existe, es de las cohortes de colapso);
**L2** prohibido nombrar autoridad INEXISTENTE (inversión ratificada: la forma literal de
`GOVERNOR_CLASS` reprobaría 115 governors legítimos que declaran "no hay dial"; el daño perseguido
es la mentira, no el silencio; 10 preexistentes enumerados por nombre; falso positivo del comodín
`--ds-stats-grid-*` evitado y drilleado — 24 hallazgos falsos); **L3** `unassigned` decrease-only
297 con drill de que ambos baselines llevan el MISMO número; **L4(a)** 152 pines-sombra
enumerados (la cohorte 0 solo pudo 20 sin membresía; 70% vía la tabla del lote 1); **L4(b)**
duplicación de decisión como **trinquete decrease-only 214 grupos/1.495 slots** con coordenada
refinada (tema, raíz, propiedad, estado, modo — la enunciada (tema,raíz,modo) medía 127/1.688 =
46% del corpus porque conflata decisiones que DEBEN diferir; un gate que exige cero en deuda
preexistente es un gate permanentemente rojo, que es lo mismo que no tener gate — ruling (d1)
ratificado con reproducción de ambas mediciones); **L5** allowlist COMPUESTO (67+8×20+9×7=290)
+ puerta DB 20/20 (`ingress.dbTenantThemePath` — el `documentPath` del brief no existe en
manifiestos; sustitución verificada, ruling (d3)). **18 drills**, con el trío obligatorio en forma
property-based: anti-coincidencia PASA / duplicación-real FALLA / D3 biyección de valores
invariante (+ su par: colapsar valores SÍ cambia el veredicto) — **la única comparación de valores
del frente es cardinalidad (`Set(...).size`), jamás contenido**. Baseline con `namedDebt`
enumerado y `--write-baseline` que niega re-ancla hacia arriba sin `--reason`. **Incidente de la
auditora, declarado y reparado antes del commit:** ensayó `--write-baseline` creyendo sandbox y el
CLI escribió el baseline real (sin afordancia de ruta); daño exacto = el campo `lastMove`;
reparación determinista re-corriendo con la razón original preservada en las reasons
por-contador; lección adoptada por el programa: **jamás ejecutar un modo de escritura de un CLI
sin leer antes su contrato de rutas** (me la anoto también a mí). Batería: barrido 97 PASS/0
FAIL, artifacts byte-idénticos, cero derivados (el gate LEE, no escribe), perímetro 4 archivos.
**Con esto, la condición de la directiva "puerta con implementación real antes de craft masivo"
queda satisfecha a nivel gate** — el craft masivo sigue pausado porque el frente de colapso es lo
que la puerta protege, y ese frente es el que corre.

**COHORTE 1 CERRADA (2026-08-27; commit `b00e5700a`; writer Opus en dos fases + tabla autorada
del DT; postaudit Fable: ACCEPT con 3 rulings favorables).** La membresía canal→raíz queda
**persistida y gobernada**: `scripts/check/tokens/cascade/roots/membership/` (productor + 19 drills) →
`governance/manifest/projections/root-membership.json` (1.610 canales, `--check` blocking, ciclo roto por
modo explícito `membership:'none'` con default fail-closed — ruling (b) ratificado). **Tres
vías, ninguna mira valores:** head-exact 36 + declared-fallback 295 + governed-owner-table 422 =
**753 canales con raíz**; cabezas disputadas 2/0 resueltas por la regla de generalidad
(`--ds-color-text-primary`→`tier.base.fg`, `--ds-color-border`→`tier.base.border`, las demás
raíces como `referencesHead`). **La tabla `governance/manifest/cascade/assignments/tiers/index.json` es AUTORADA por el
DT** (lote 1: 114 owners con razón estructural por fila — control 62 / page 20 / raised 18 /
overlay 12 / base 2; muestra de 6 auditada por Fable contra los heads del catálogo; las 2 filas
`inset`→base son la adjudicación más débil y su corrección pertenece al packet de fusión
8-roles-vs-6-tiers, no bloquea). Paletas/ramps, motion, expressive y badge/tag/avatar/alert
quedaron SIN adjudicar a propósito (gramática no-tier; "null legítimo, nunca presión de
cobertura"). **Admisión V3 por CONCORDANCIA** (desviación aceptada por el DT en fase A, ruling
(a) ratificado: 0 de los 857 residuales es por desacuerdo de tier). **Inventario v2: R2 26 →
906** (piso computado 412, superado 2,2×), filas con raíz 107 → 1.771, R5 2.607 → 1.785, R3
sigue en 0 (hallazgo: los temas autoran valores resueltos, no nombres de stop). **Re-ancla
única con razón:** solo `rowsWithoutRootAttribution` 3.378→1.707 (causa: membresía persistida);
los otros 4 contadores QUIETOS, verificados campo a campo. **Rename `collapses`→`collapsesLegacy`
con las prosas en el mismo edit** (0 restos en prosa viva; 3 menciones históricas en
`corrections`, correctas). **Hallazgo estructural (185 canales, 27 combinaciones
`tier.propiedad` inexistentes — shadow 78, padding, height, gap, ring):** la cascada de nivel
está definida solo para color/borde; **dirección adjudicada por el DT: domicilio en el EJE de la
propiedad** (shadow→`elevation.ladder` con `surfaces.elevation-posture` como ley de escalón;
padding/gap→`spacing.step`/`control.ratio.*`; height→`control.height`; ring→`state.delta.focus`),
**NO 27 raíces nuevas** — la matriz tier queda cromática; la mecánica entra al diseño de la
cohorte 2 con preaudit. **Política temporal aceptada (ruling (c) con condición):** la omisión de
`surface-census:generate` se declara por cohorte y EXPIRA cuando cierre la deuda arc-11 (pin de
revisiones o des-trackeo); si sobrevive al cierre, es divergencia silenciosa y defecto.
**Batería:** barrido **95 PASS / 0 FAIL** (97 gates / 95 blocking, drill primero); cadena 13/13
una vez; artifacts byte-idénticos; receipts 134/134 fresh (este write-set no los toca — Fable
verificó 0/134 con root-catalog en sourceFiles); diff --check verde. **Lo que la cohorte 1 NO
hizo, por ley: colapsar un solo literal.** **Próximo paso:** lote `normalization-contract-gate`
(5 leyes + el drill de duplicación-real, prerequisito del primer colapso — ahora posible: ya hay
con qué agrupar dos seeds por raíz-modo) y diseño de la cohorte 2 (color/surfaces, primer
colapso, sobre R2=906). Métrica sin mover (62%/~43%) — instrumentación; el porcentaje se mueve
con las cohortes de colapso.
...
(`scripts/check/tokens/cascade/slots/`, 620+288 líneas, 15 drills) que mide el canal de cada hoja por
...
perturbaciones) → `governance/manifest/projections/slot-inventory.json` (3.656 filas, digest, `--check`
...
JOIN + inventario enumerado `governance/manifest/normalization/slot-inventory.json` con `--check` fail-closed
...
(`computeSourceDigest` importado de `scripts/check/evidence/framework/receipts/index.mjs`, parche de texto
...
familias — `governance/manifest/controls/typography/families/index.json` — no en receipts). Lo que
...
CSS nueva stalea `contracts/css/hooks/index.json` — la cadena de cierre incluye `hooks:generate`
...
`checkpoint/index.json` y la sección generada del README decían "F4B 5/20"
...
en `scripts/maintain/roadmap/status/index.mjs` queda nombrada como deuda con ese dueño —
...
vigente en `governance/manifest/controls/typography/families/index.json`); barrido fail-continue
...
inmediato** (registry.json y checkpoint/index.json narran momentos previos a la
...
19 sube a `customization-model/index.json` + gate ejecutable dentro del Lote F′.
...
`index.mjs` quedó BLOCKED ("governance/manifest/index.json is stale").
Correctivo aplicado con el productor canónico
(`scripts/foundation/tokens/manifest/generation/index.mjs --write`): el único delta material es
`governance/manifest/index.json` (2 hunks: `inputsDigest` + digest de chrome.families);
0 receipts citan `governance/manifest/index.json` en sourceFiles (ninguno se re-stalea);
...
Commit correctivo separado: **`fd725246a`** (sólo `governance/manifest/index.json`). **Lección de
...
nacidos en /tmp en esta serie; cada colisión futura la cobra. `governance/manifest/
...
`contracts/css/hooks/index.json` entra al write-set como consecuencia derivada probada
...
bloqueante (Paso D), manifest graduado a `packages/core/governance/manifest/`, raíces de
...
por basename; raíz a mano sin helper en un test). **`contracts/css/hooks/index.json`
...
**Graduación del manifest ✅ `a739b988e`** — `packages/core/governance/manifest/`
...
de wiring-coverage y forma capability de governance/manifest/ (9 planos) anotados para
...
- **F1.2** — vocabulario cerrado **SIN lift** (corregido por el worker con
  ...
  `vocabularyDomicile` (customization-model/index.json) ya adjudicó el domicilio —
...
`scripts/foundation/tokens/manifest/generation/index.test.mjs` (el drill de conflicto producer=approver
...
su razón): `tests/reanchor/index.test.mjs:50` tiene la misma
...
`tests/reanchor/index.test.mjs` planta
...
`statusAuthority` en program/index.json) dice "Codex remains the final machine and
sighted auditor and local committer" y 252 familias (son 255);
`checkpoint/index.json` habla de spacing.rhythm y carriles retirados;
...
La cadena arrastró `contracts/css/hooks/index.json` (+3 líneas, el gate pidió
`hooks:generate`) y `governance/manifest/index.json` (`generator --sync`; 36/36 y
...
nuevas** (nota operativa mía: la pierna 1 es `node --test scripts/**+governance/manifest/**`
...
→ denominador 3690. Artefacto `governance/manifest/projections/variant-parity.json` (5
secciones, tagRegistry hoy vacío y correcto) + baseline AUTORADO
`baseline/index.json` (2268 / 3686, ley decrease-only escrita, se
...
El coordinador regeneró la vista (`docs/generated/customization-controls/index.md`, digest
...
`governance/manifest/projections/` regenerados y enchufados blocking (87 gates). Delta por
...
**Paso C3 ✅ `8ab678dc1` + `82b800f94`** — governance/manifest/ en forma capability (5
...
nombre histórico → forma corta; (e) forma capability de packages/core/governance/manifest/
...
`packages/core/docs/generated/component-taxonomy/index.md` y
