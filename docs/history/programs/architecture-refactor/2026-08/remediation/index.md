...
- [`docs/history/inventories/repository-map/2026-08/index.md`](/docs/history/inventories/repository-map/2026-08/index.md) — qué hace cada carpeta.
- [`docs/history/audits/architecture-refactor/2026-08/independent-verification/index.md`](/docs/history/audits/architecture-refactor/2026-08/independent-verification/index.md) — verificación afirmación por afirmación.
- [`docs/history/audits/architecture-refactor/2026-08/coordination/index.md`](/docs/history/audits/architecture-refactor/2026-08/coordination/index.md) — tercer lector independiente.
- [`docs/history/programs/architecture-refactor/2026-08/retention/index.md`](/docs/history/programs/architecture-refactor/2026-08/retention/index.md) — la adjudicación de esas tres lecturas.
...
| A2 | **Pocas raíces de cascada con vocabulario cerrado de variantes nombradas** | capa base del motor `modern` + gobierno en `governance/manifest/controls/` y `governance/manifest/cascade/` | las `--ds-*` raíz; el tenant las toca vía los 20 controles | **no en fuente**: hay catálogo textual de 63 raíces y 20 controles, no variables reales | el gobierno sí, la fuente no existe | no puede usarse: 2.229 de 3.548 canales de componente no alcanzan ninguna raíz |
...
| A13 | Marcas de empresa/nube | `graphics/marks/` | `./marks` | sí | sí | poco — 10 archivos |
...
| # | capacidad | dueño | camino | existe | ubicado | se usa |
|---|---|---|---|---|---|---|
| A17 | Enforcement mecánico en apps (no-raw-html, no-hardcoded-colors, no-db-in-components) | `entrypoints/eslint/` | `./eslint` | sí | sí | referenciado por configs de app (fuera de `src/`) |
...
Nota importante para el orden: `customization-model/index.json` ya declara
...
**9 archivos `.test.mjs` sin ningún invocador** — 4 de `scripts/libraries/`, la capa que
...
Cruce de los 12 lotes de [`QUE-SE-QUEDA-Y-QUE-SE-BORRA.md`](/docs/history/programs/architecture-refactor/2026-08/retention/index.md) contra el sistema
...
`probe/index.mjs` + su test; los otros 5 archivos de `probe/` **no**), 5
...
`scripts/maintain/roadmap/status/index.mjs`. Los criterios de aceptación son comandos de este
...
grep -rn "styles/platform.css" .../governance/manifest/families | wc -l       # hoy 802 -> 0
