...
rutas muertas; patrón añadido a las reglas duras del brief). Paradas correctas:
`platform-identity-zero-gate` (se atrapaba a sí mismo: su ruta nueva contiene
`verticals/platform`, el residuo que persigue — opción (a) aprobada, exención
estrecha del propio nombre, va en **lote C2** EN VUELO) y el sello
`Generated by build-vertical-css` (triplicado en 3 implementaciones de la
misma composición — va en **lote C3**, brief `/tmp/f05-pasoC3-brief.md`).
Re-sello red-inventory hecho por el coordinador con las 4 identities que el
worker computó (exactas, gate verde). Enmienda de punteros APROBADA y en el
brief: archivos de corpus hasheado (`src/foundation/tokens/css/**` fuera de
